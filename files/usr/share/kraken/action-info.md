# Kraken Actions
_This documentation is available online at https://github.com/linuxmint/kraken/wiki/Kraken-Actions_

### About
Kraken actions allow the user to add custom menu actions to apply to selected files. These can be one-shot commands or call their own scripts, and can be restricted to specific files by defining various conditions for the action.

Action files are just text files that end in `.kraken_action` and are stored in system and user data locations - `/usr/share/kraken/actions`, `~/.local/usr/share/kraken/actions` (all of your environment's `XDG_DATA_DIRS` are scanned, though typically only the aforementioned are used).
...
If you add this to a newly created file `~/.local/share/kraken/actions/my-test-action.kraken_action`, and right-click on a single file in Kraken, this will appear in the menu. You should get the system notification immediately after you activate it.

---

### Troubleshooting
For debugging actions, you can run Kraken as follows:
```bash
# First, kill any existing instances
kraken --quit
# Then relaunch with debugging enabled
NEMO_DEBUG=Actions kraken --debug
```
...
```bash
Kraken-DEBUG: kraken_action_update_display_state: ../libkraken-private/kraken-action.c:2011: Action 'new-sample' determined VISIBLE
Kraken-DEBUG: get_final_label: ../libkraken-private/kraken-action.c:1538: Action Label: Test action on file Documents
Kraken-DEBUG: get_final_tt: ../libkraken-private/kraken-action.c:1563: Action Tooltip: If you click this menu entry, /home/mtwebster/Documents will be acted upon!
Kraken-DEBUG: kraken_action_activate: ../libkraken-private/kraken-action.c:1462: Action Spawning: notify-send "Hey there" "This is the result of an action you ran on the file '/home/mtwebster/Documents' - well done"
```
...
**Comment** (optional): The tooltip to display in Kraken's bottom status bar. This can include selection tokens and can be localized (see below).

`
Comment=If you click this menu entry, %F will be acted upon!
`

**Icon-Name** (optional): The name of the icon to display in Kraken's context menu next to the action entry. This must be an icon that is part of the GtkIconTheme, or an absolute path.  Symbolic icons are supported.

`
Icon-Name=folder
`

If you have a custom icon, you can enclose the filename in `<...>` to have it loaded from the action's directory. For instance, with `<my-action-icon.png>` kraken will look for an icon by that name in the same directory as the action file. If your action is a Spice and has its own subfolder, that can be included as a relative path (`my-action@me/my-action-icon.png`).
...
- `desktop`: The selection must be the desktop (not the 'Desktop' folder in Kraken's bookmarks, but *the* desktop).
...
**Active** (optional): Whether or not the action is visible to Kraken. This really is never necessary, as actions can be enabled/disabled in preferences.
...
- `%X`: insert the XID for the KrakenWindow this action is being activated in.
