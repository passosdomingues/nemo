...
Kraken's file and content search utilizes a plugin-type system to allow content search for additional file types.
...
##### Example definition file (note: text/plain support is built-in to kraken, this is only to illustrate):

```
[Kraken Search Helper]
TryExec=cat
Exec=cat %s
MimeType=text/plain;
Priority=100
...
```
The `Kraken Search Helper` group name is mandatory.

- The filename must end in `.kraken_search_helper`.
- `TryExec` should be a list executables required. When the helpers are loaded, Kraken will check that the program(s) a) exist and
...
These definition files can be placed in `<datadir>/kraken/search-helpers` where `<datadir>` can be some directory in XDG_DATA_DIRS or under the user's data directory (`~/.local/share/namo/search-helpers`). The user directory is *always* processed last.

##### Debugging:
If something doesn't seem to be working, you can run kraken with debugging enabled:
```
NEMO_DEBUG=Search kraken --debug
```
