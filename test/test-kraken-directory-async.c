...
static void
files_added (KrakenDirectory *directory,
	     GList *added_files)
{
#if 0
	GList *list;
	...
	for (list = added_files; list != NULL; list = list->next) {
		KrakenFile *file = list->data;

		g_print (" - %s\n", kraken_file_get_uri (file));
	}
#endif
...
static void
files_changed (KrakenDirectory *directory,
	       GList *changed_files)
{
#if 0
	GList *list;
...
static gboolean
force_reload (KrakenDirectory *directory)
{
...
static void
done_loading (KrakenDirectory *directory)
{
	static int i = 0;
...
main (int argc, char **argv)
{
	...
	kraken_query_set_file_pattern (query, "richard hult");
	directory = kraken_directory_get_by_uri ("x-kraken-search://0/");
	kraken_search_directory_set_query (KRAKEN_SEARCH_DIRECTORY (directory), query);
	...
	kraken_directory_file_monitor_add (directory, client1, TRUE,
