/*
 *  kraken-column.c - Info columns exported by KrakenColumnProvider
 *                      objects.
 *
...
struct _KrakenColumn
{
    GObject parent_object;

    KrakenColumnPrivate *details;
};

G_DEFINE_TYPE_WITH_PRIVATE (KrakenColumn, kraken_column, G_TYPE_OBJECT)

/**
 * SECTION:kraken-column
 * @Title: KrakenColumn
 * @Short_description: A column used in Kraken's  list view.
 *
 * A column is linked to a particular file attribute to display in the view.
 * Many of these are built in to Kraken, but they can also be provided by
 * a #KrakenColumnProvider/#KrakenInfoProvider extension.
 **/
 ...
 *
 * Returns: (transfer full): a newly created #KrakenColumn
 */
KrakenColumn *
kraken_column_new (const char *name,
		     ...
		     const char *description)
{
	...
	column = g_object_new (KRAKEN_TYPE_COLUMN, 
			       "name", name,
			       "attribute", attribute,
			       "label", label,
 ...
 */
KrakenColumn *
kraken_column_new2 (const char         *name,
                  const char         *attribute,
                  const char         *label,
                  const char         *description,
                  gint                width_chars,
...
static void
kraken_column_get_property (GObject *object,
			      guint param_id,
			      GValue *value,
			      GParamSpec *pspec)
{
...
static void
kraken_column_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
kraken_column_finalize (GObject *object)
{
	KrakenColumn *column;
	...
	G_OBJECT_CLASS (kraken_column_parent_class)->finalize (object);
}

static void
kraken_column_init (KrakenColumn *column)
{
    column->details = G_TYPE_INSTANCE_GET_PRIVATE (column, KRAKEN_TYPE_COLUMN, KrakenColumnPrivate);

    column->details->xalign = 0.0;
}

static void
kraken_column_class_init (KrakenColumnClass *class)
{
	G_OBJECT_CLASS (class)->finalize = kraken_column_finalize;
	G_OBJECT_CLASS (class)->get_property = kraken_column_get_property;
	G_OBJECT_CLASS (class)->set_property = kraken_column_set_property;
