/*
 *  kraken-menu-item.c - Menu items exported by KrakenMenuProvider
 *                         objects.
 *
...
typedef struct {
    ...
    GtkWidget *widget_b;
    gboolean separator;
} KrakenMenuItemPrivate;

struct _KrakenMenuItem
{
    GObject parent_class;

    KrakenMenuItemPrivate *details;
};

G_DEFINE_TYPE_WITH_PRIVATE (KrakenMenuItem, kraken_menu_item, G_TYPE_OBJECT)

static guint signals[LAST_SIGNAL] = { 0 };

/**
 * kraken_menu_item_new:
 ...
 *
 * Returns: a newly create #KrakenMenuItem
 */
KrakenMenuItem *
kraken_menu_item_new (const char *name,
			const char *label,
			const char *tip,
			const char *icon)
{
	...
	item = g_object_new (KRAKEN_TYPE_MENU_ITEM, 
			     "name", name,
			     "label", label,
			     "tip", tip,
			     "icon", icon,
 ...
 *
 * Returns: a newly created #KrakenMenuItem that will pass widgets to an eventual KrakenWidgetAction
 */
KrakenMenuItem *
kraken_menu_item_new_widget (const char *name,
                           GtkWidget  *widget_a,
                           GtkWidget  *widget_b)
{
    KrakenMenuItem *item;
...
/**
 * kraken_menu_item_new_separator:
 * @name: the identifier for the menu item
 *
 * Creates a new menu item that represents a menu separator.
 * 
 * Returns: a newly created #KrakenMenuItem
 */
KrakenMenuItem *
kraken_menu_item_new_separator (const char *name)
{
    KrakenMenuItem *item;
...
/**
 * kraken_menu_item_activate:
 * @item: pointer to a #KrakenMenuItem
 *
 * emits the activate signal.
 */
void
kraken_menu_item_activate (KrakenMenuItem *item)
{
	g_signal_emit (item, signals[ACTIVATE], 0);
}

/**
 * kraken_menu_item_set_submenu:
 * @item: pointer to a #KrakenMenuItem
 * @menu: pointer to a #KrakenMenu to attach to the button
 *
 * Attachs a menu to the given #KrakenMenuItem.
 */
void
kraken_menu_item_set_submenu (KrakenMenuItem *item, KrakenMenu *menu)
{
	g_object_set (item, "menu", menu, NULL);
}

/**
 * kraken_menu_item_set_widget_a:
 * @item: pointer to a #KrakenMenuItem
 * @widget: pointer to a #GtkWidget use in place of text
 *
 * Sets the custom widget A for this #KrakenMenuItem
 */
void
kraken_menu_item_set_widget_a (KrakenMenuItem *item, GtkWidget *widget)
{
...
/**
 * kraken_menu_item_set_widget_b:
 * @item: pointer to a #KrakenMenuItem
 * @widget: pointer to a #GtkWidget use in place of text
 *
 * Sets the custom widget B for this #KrakenMenuItem
 */
void
kraken_menu_item_set_widget_b (KrakenMenuItem *item, GtkWidget *widget)
{
...
static void
kraken_menu_item_get_property (GObject *object,
				 guint param_id,
				 GValue *value,
				 GParamSpec *pspec)
{
...
static void
kraken_menu_item_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
kraken_menu_item_finalize (GObject *object)
{
	KrakenMenuItem *item;
	...
	G_OBJECT_CLASS (kraken_menu_item_parent_class)->finalize (object);
}

static void
kraken_menu_item_init (KrakenMenuItem *item)
{
    item->details = G_TYPE_INSTANCE_GET_PRIVATE (item, KRAKEN_TYPE_MENU_ITEM, KrakenMenuItemPrivate);
...
static void
kraken_menu_item_class_init (KrakenMenuItemClass *class)
{
	G_OBJECT_CLASS (class)->finalize = kraken_menu_item_finalize;
	G_OBJECT_CLASS (class)->get_property = kraken_menu_item_get_property;
	G_OBJECT_CLASS (class)->set_property = kraken_menu_item_set_property;
