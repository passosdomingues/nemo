/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-thumbnail-cache.h: Thumbnail code for icon factory.
...
#define DEBUG_FLAG KRAKEN_DEBUG_THUMBNAILS
...
#define KRAKEN_THUMBNAIL_FRAME_LEFT 3
#define KRAKEN_THUMBNAIL_FRAME_TOP 3
#define KRAKEN_THUMBNAIL_FRAME_RIGHT 3
#define KRAKEN_THUMBNAIL_FRAME_BOTTOM 3
...
/* How it works:
 * 
 * When kraken_create_thumbnail(), kraken_thumbnail_remove_from_queue or kraken_thumbnail_prioritize are called,
 * a new KrakenThumbnailInfo is made, with info->cmd_type set based on the method called.
 *
 ...
 * - kraken_create_thumbnail (THUMBNAIL_ADD): The info is looked up by uri in thumbnails_to_make_hash. If
 ...
 *
 * - kraken_thumbnail_remove_from_queue (THUMBNAIL_REMOVE): The info is looked up by uri in thumbnails_to_make_hash.
 ...
 *
 * - kraken_thumbnail_prioritize (THUMBNAIL_BUMP): The info is looked up by uri in thumbnails_to_make_hash. If found,
 ...
 * - KrakenThumbnailInfos are garbage-collected in the threadpool worker only.
 */
...
/* Causes the feeder_task to end. Only called when Kraken is shutting down. */
...
get_max_threads (void) {
    ...
    gint pref = g_settings_get_int (kraken_preferences, KRAKEN_PREFERENCES_MAX_THUMBNAIL_THREADS);
...
lifo_sorter (gconstpointer a,
             ...
             gpointer      data)
{
    gint64 ta = ((const KrakenThumbnailInfo *) a)->add_time;
    gint64 tb = ((const KrakenThumbnailInfo *) b)->add_time;
...
static void
free_thumbnail_info (KrakenThumbnailInfo *info)
{
...
static GdkPixbuf *
kraken_get_thumbnail_frame (void)
{
    static GdkPixbuf *thumbnail_frame = NULL;
    static gsize once_init = 0;

    if (g_once_init_enter (&once_init)) {
        GInputStream *stream = g_resources_open_stream ("/org/kraken/icons/thumbnail_frame.png", 0, NULL);
        if (stream != NULL) {
...
/* This is a one-shot idle callback called from the main loop to call
   ...
   We do this in an idle callback as I don't think kraken_file_changed() is
   thread-safe. */
static gboolean
thumbnail_thread_notify_file_changed (gpointer image_uri)
{
    ...
    file = kraken_file_get_by_uri ((char *) image_uri);

    DEBUG ("(Thumbnail Thread) Notifying file changed file: %p uri: %s", file, (char*) image_uri);

    if (file != NULL) {
        kraken_file_set_is_thumbnailing (file, FALSE);
...
static void
remove_from_hash_table (KrakenThumbnailInfo *info)
{
...
thumbnail_thread (gpointer data,
                  gpointer user_data)
{
    KrakenThumbnailInfo *info = (KrakenThumbnailInfo *) data;
    GdkPixbuf *pixbuf;
    time_t current_time;
    ...
    /* We need to call kraken_file_changed(), but I don't think that is
...
#endif
        KrakenThumbnailInfo *feeder_info = (KrakenThumbnailInfo *) data;
        KrakenThumbnailInfo *existing_info = NULL;
...
finalize_thumbnailer (void)
{
    ...
    info = g_new0 (KrakenThumbnailInfo, 1);
    ...
    while ((data = g_async_queue_try_pop (feeder_queue))) {
        ...
        KrakenThumbnailInfo *existing_info = (KrakenThumbnailInfo *) data;
...
/* Mainloop */
void
kraken_create_thumbnail (KrakenFile *file)
{
    time_t file_mtime = 0;
    static gsize once_init = 0;
    if (g_once_init_enter (&once_init)) {
    ...
    if (kraken_file_is_in_favorites (file)) {
        ...
        real_file = kraken_file_get_by_uri (uri);
        if (real_file != NULL) {
            kraken_create_thumbnail (real_file);
    ...
    gchar *file_uri = kraken_file_get_uri (file);

    /* Hopefully the KrakenFile will already have the image file mtime,
...
/* Mainloop */
void
kraken_thumbnail_remove_from_queue (const char *file_uri)
{
    if (feeder_queue == NULL)
        return;
...
/* Mainloop */
void
kraken_thumbnail_prioritize (const char *file_uri)
{
    if (feeder_queue == NULL)
        return;
...
gboolean
kraken_can_thumbnail_internally (KrakenFile *file)
{
...
gboolean
kraken_can_thumbnail (KrakenFile *file)
{
    ...
    mime_type = kraken_file_get_mime_type (file);
    mtime = kraken_file_get_mtime (file);
...
void
kraken_thumbnail_frame_image (GdkPixbuf **pixbuf)
{
    GdkPixbuf *pixbuf_with_frame, *frame;

    /* The pixbuf isn't already framed (i.e., it was not made by
     * an old Kraken), so we must embed it in a frame.
     */

    frame = kraken_get_thumbnail_frame ();
    if (frame == NULL) {
        return;
    }

    pixbuf_with_frame = eel_embed_image_in_frame (*pixbuf, frame,
                                                  KRAKEN_THUMBNAIL_FRAME_LEFT,
                                                  KRAKEN_THUMBNAIL_FRAME_TOP,
                                                  KRAKEN_THUMBNAIL_FRAME_RIGHT,
                                                  KRAKEN_THUMBNAIL_FRAME_BOTTOM);
    g_object_unref (*pixbuf);
    *pixbuf = pixbuf_with_frame;
}

void
kraken_thumbnail_pad_top_and_bottom (GdkPixbuf **pixbuf,
                                   gint        extra_height)
{
...
gboolean
kraken_thumbnail_factory_check_status (void)
{
