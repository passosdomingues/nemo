...
struct _KrakenProgressInfoClass
{
	GObjectClass parent_class;
};

G_DEFINE_TYPE (KrakenProgressInfo, kraken_progress_info, G_TYPE_OBJECT)

static void
kraken_progress_info_finalize (GObject *object)
{
	KrakenProgressInfo *info;
	
	info = KRAKEN_PROGRESS_INFO (object);
	...
	if (G_OBJECT_CLASS (kraken_progress_info_parent_class)->finalize) {
		(*G_OBJECT_CLASS (kraken_progress_info_parent_class)->finalize) (object);
	}
}

static void
kraken_progress_info_dispose (GObject *object)
{
	KrakenProgressInfo *info;
...
static void
kraken_progress_info_class_init (KrakenProgressInfoClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	
	gobject_class->finalize = kraken_progress_info_finalize;
	gobject_class->dispose = kraken_progress_info_dispose;
...
static void
kraken_progress_info_init (KrakenProgressInfo *info)
{
	...
	kraken_progress_info_manager_add_new_info (manager, info);
	g_object_unref (manager);
}

KrakenProgressInfo *
kraken_progress_info_new (void)
{
	KrakenProgressInfo *info;
	
	info = g_object_new (KRAKEN_TYPE_PROGRESS_INFO, NULL);
	
	return info;
}

char *
kraken_progress_info_get_status (KrakenProgressInfo *info)
{
	char *res;
...
char *
kraken_progress_info_get_details (KrakenProgressInfo *info)
{
	char *res;
...
char *
kraken_progress_info_get_initial_details (KrakenProgressInfo *info)
{
    char *res;
...
double
kraken_progress_info_get_progress (KrakenProgressInfo *info)
{
	double res;
...
void
kraken_progress_info_cancel (KrakenProgressInfo *info)
{
	g_mutex_lock (&info->info_lock);
...
GCancellable *
kraken_progress_info_get_cancellable (KrakenProgressInfo *info)
{
	GCancellable *c;
...
gboolean
kraken_progress_info_get_is_started (KrakenProgressInfo *info)
{
	gboolean res;
...
gboolean
kraken_progress_info_get_is_finished (KrakenProgressInfo *info)
{
	gboolean res;
...
gboolean
kraken_progress_info_get_is_paused (KrakenProgressInfo *info)
{
	gboolean res;
...
static void
queue_idle (KrakenProgressInfo *info, gboolean now)
{
...
void
kraken_progress_info_queue (KrakenProgressInfo *info)
{
    g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_pause (KrakenProgressInfo *info)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_resume (KrakenProgressInfo *info)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_start (KrakenProgressInfo *info)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_finish (KrakenProgressInfo *info)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_take_status (KrakenProgressInfo *info,
				    char *status)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_set_status (KrakenProgressInfo *info,
				   const char *status)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_take_details (KrakenProgressInfo *info,
				     char           *details)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_set_details (KrakenProgressInfo *info,
				    const char           *details)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_take_initial_details (KrakenProgressInfo *info,
                                                     char *initial_details)
{
    g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_pulse_progress (KrakenProgressInfo *info)
{
	g_mutex_lock (&info->info_lock);
...
void
kraken_progress_info_set_progress (KrakenProgressInfo *info,
				     double                current,
				     double                total)
{
...
gdouble
kraken_progress_info_get_elapsed_time (KrakenProgressInfo *info)
{
    gdouble elapsed;
