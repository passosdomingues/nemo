/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-vfs-file.c: Subclass of KrakenFile to help implement the
...
G_DEFINE_TYPE (KrakenVFSFile, kraken_vfs_file, KRAKEN_TYPE_FILE);

static void
vfs_file_monitor_add (KrakenFile *file,
...
static void
vfs_file_monitor_remove (KrakenFile *file,
			 gconstpointer client)
{
...
static void
vfs_file_call_when_ready (KrakenFile *file,
...
static void
vfs_file_cancel_call_when_ready (KrakenFile *file,
				 KrakenFileCallback callback,
				 gpointer callback_data)
{
...
static gboolean
vfs_file_check_if_ready (KrakenFile *file,
			 KrakenFileAttributes file_attributes)
{
				...
				gpointer callback_data)
{
	...
	if (new_info != NULL) {
		if (kraken_file_update_info (file, new_info)) {
...
static void
vfs_file_set_metadata (KrakenFile           *file,
		       const char             *key,
		       const char             *value)
{
	GFileInfo *info;
...
static void
vfs_file_set_metadata_as_list (KrakenFile           *file,
			       const char             *key,
			       char                  **value)
{
	GFile *location;
...
static gboolean
vfs_file_get_item_count (KrakenFile *file,
			 guint *count,
...
vfs_file_get_deep_counts (KrakenFile *file,
			  ...
			  goffset *total_size)
{
	...
	if (file->details->deep_counts_status != KRAKEN_REQUEST_NOT_STARTED) {
		if (directory_count != NULL) {
	...
	type = kraken_file_get_file_type (file);
	if (type == G_FILE_TYPE_UNKNOWN
	    || type == G_FILE_TYPE_DIRECTORY) {
		return KRAKEN_REQUEST_NOT_STARTED;
	}
...
static gboolean
vfs_file_get_date (KrakenFile *file,
...
static char *
vfs_file_get_where_string (KrakenFile *file)
{
    ...
    if (!kraken_file_is_in_recent (file) &&
        !kraken_file_is_in_favorites (file)) {
        location = kraken_file_ref (file);
    } else {
        activation_location = kraken_file_get_activation_location (file);
        location = kraken_file_get (activation_location);
        g_object_unref (activation_location);
    }

    where_string = kraken_file_get_parent_uri_for_display (location);
			 ...
			 gpointer callback_data)
{
	...
	mounted_on = g_file_mount_mountable_finish (G_FILE (source_object),
						    res, &error);
	kraken_file_operation_complete (op, mounted_on, error);
	if (mounted_on) {
...
static void
vfs_file_mount (KrakenFile                   *file,
			 ...
			 gpointer callback_data)
{
    ...
    kraken_file_operation_complete (op, G_FILE (source_object), error);
	if (error) {
		g_error_free (error);
	}
}
...
static void
vfs_file_unmount (KrakenFile                   *file,
		  GMountOperation                *mount_op,
		  GCancellable                   *cancellable,
		  KrakenFileOperationCallback   callback,
		  gpointer                        callback_data)
...
static void
vfs_file_eject (KrakenFile                   *file,
		GMountOperation                *mount_op,
		GCancellable                   *cancellable,
		KrakenFileOperationCallback   callback,
		gpointer                        callback_data)
...
static void
vfs_file_start (KrakenFile                   *file,
		GMountOperation                *mount_op,
		GCancellable                   *cancellable,
		KrakenFileOperationCallback   callback,
		gpointer                        callback_data)
...
static void
vfs_file_poll_for_media (KrakenFile *file)
{
	KrakenFileOperation *op;
	GFile *location;

	op = kraken_file_operation_new (file, NULL, NULL);
...
static void
kraken_vfs_file_init (KrakenVFSFile *file)
{
}

static void
kraken_vfs_file_class_init (KrakenVFSFileClass *klass)
{
