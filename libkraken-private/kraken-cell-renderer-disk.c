...
#include "kraken-cell-renderer-disk.h"
#include <math.h>

G_DEFINE_TYPE (KrakenCellRendererDisk, kraken_cell_renderer_disk,
	       GTK_TYPE_CELL_RENDERER_TEXT);


static void     kraken_cell_renderer_disk_get_property  (GObject                    *object,
                                                       guint                       param_id,
                                                       GValue                     *value,
                                                       GParamSpec                 *pspec);

static void     kraken_cell_renderer_disk_set_property  (GObject                    *object,
                                                       guint                       param_id,
                                                       const GValue               *value,
                                                       GParamSpec                 *pspec);

static void     kraken_cell_renderer_disk_finalize (GObject *gobject);

static void     kraken_cell_renderer_disk_render (GtkCellRenderer       *cell,
                                                cairo_t               *cr,
                                                GtkWidget             *widget,
...
static void
kraken_cell_renderer_disk_init (KrakenCellRendererDisk *cell)
{
	g_object_set (cell,
		      "disk-full-percent", 0,
...
static void
kraken_cell_renderer_disk_class_init (KrakenCellRendererDiskClass *klass)
{
	GtkCellRendererClass *cell_class = GTK_CELL_RENDERER_CLASS (klass);
    GObjectClass         *object_class = G_OBJECT_CLASS(klass);
    parent_class           = g_type_class_peek_parent (klass);
    object_class->finalize = kraken_cell_renderer_disk_finalize;

    object_class->get_property = kraken_cell_renderer_disk_get_property;
    object_class->set_property = kraken_cell_renderer_disk_set_property;
    cell_class->render = kraken_cell_renderer_disk_render;
...
GtkCellRenderer *
kraken_cell_renderer_disk_new (void)
{
    return g_object_new (KRAKEN_TYPE_CELL_RENDERER_DISK, NULL);
}

static void
kraken_cell_renderer_disk_finalize (GObject *object)
{
    G_OBJECT_CLASS (parent_class)->finalize (object);
}

static void
kraken_cell_renderer_disk_get_property (GObject    *object,
                                      guint       param_id,
                                      GValue     *value,
                                      GParamSpec *psec)
{
  KrakenCellRendererDisk  *celldisk = KRAKEN_CELL_RENDERER_DISK (object);
...
static void
kraken_cell_renderer_disk_set_property (GObject      *object,
                                      guint         param_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
...
static void
kraken_cell_renderer_disk_render (GtkCellRenderer       *cell,
                                cairo_t               *cr,
                                GtkWidget             *widget,
                                const GdkRectangle    *background_area,
                                const GdkRectangle    *cell_area,
                                GtkCellRendererState   flags)
{
    KrakenCellRendererDisk *cellprogress = KRAKEN_CELL_RENDERER_DISK (cell);
    gint                        x, y, w;
    gint                        xpad, ypad;
    gint                        full;
