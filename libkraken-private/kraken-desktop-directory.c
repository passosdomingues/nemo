/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-desktop-directory.c: Subclass of KrakenDirectory to implement
...
struct KrakenDesktopDirectoryDetails {
...
typedef struct {
	KrakenDesktopDirectory *desktop_dir;
...
G_DEFINE_TYPE (KrakenDesktopDirectory, kraken_desktop_directory,
	       KRAKEN_TYPE_DIRECTORY);

static gboolean
desktop_contains_file (KrakenDirectory *directory,
		       KrakenFile *file)
{
	KrakenDesktopDirectory *desktop;

	desktop = KRAKEN_DESKTOP_DIRECTORY (directory);

	if (kraken_directory_contains_file (desktop->details->real_directory, file)) {
		return TRUE;
	}
...
merged_callback_destroy (MergedCallback *merged_callback)
{
	...
	g_assert (KRAKEN_IS_DESKTOP_DIRECTORY (merged_callback->desktop_dir));
...
merged_callback_check_done (MergedCallback *merged_callback)
{
	...
	(* merged_callback->callback) (KRAKEN_DIRECTORY (merged_callback->desktop_dir),
...
static void
desktop_call_when_ready (KrakenDirectory *directory,
...
static void
desktop_cancel_callback (KrakenDirectory *directory,
			KrakenDirectoryCallback callback,
			gpointer callback_data)
{
	KrakenDesktopDirectory *desktop;
...
merged_monitor_destroy (MergedMonitor *monitor)
{
	...
	kraken_directory_file_monitor_remove (desktop->details->real_directory, monitor);

	kraken_directory_monitor_remove_internal (KRAKEN_DIRECTORY (desktop), NULL, monitor);
...
static void
desktop_monitor_add (KrakenDirectory *directory,
...
static void
desktop_monitor_remove (KrakenDirectory *directory,
		       gconstpointer client)
{
	KrakenDesktopDirectory *desktop;
	MergedMonitor *monitor;
...
static void
desktop_force_reload (KrakenDirectory *directory)
{
	KrakenDesktopDirectory *desktop;

	desktop = KRAKEN_DESKTOP_DIRECTORY (directory);

	kraken_directory_force_reload (desktop->details->real_directory);
...
static gboolean
desktop_are_all_files_seen (KrakenDirectory *directory)
{
	KrakenDesktopDirectory *desktop;

	desktop = KRAKEN_DESKTOP_DIRECTORY (directory);

	if (!kraken_directory_are_all_files_seen (desktop->details->real_directory)) {
		return FALSE;
	}
...
static gboolean
desktop_is_not_empty (KrakenDirectory *directory)
{
	KrakenDesktopDirectory *desktop;

	desktop = KRAKEN_DESKTOP_DIRECTORY (directory);

	if (kraken_directory_is_not_empty (desktop->details->real_directory)) {
		return TRUE;
	}
...
static GList *
desktop_get_file_list (KrakenDirectory *directory)
{
	GList *real_dir_file_list, *desktop_dir_file_list = NULL;
	
	real_dir_file_list = kraken_directory_get_file_list
				(KRAKEN_DESKTOP_DIRECTORY (directory)->details->real_directory);
	desktop_dir_file_list = KRAKEN_DIRECTORY_CLASS (kraken_desktop_directory_parent_class)->get_file_list (directory);

	return g_list_concat (real_dir_file_list, desktop_dir_file_list);
}

KrakenDirectory *
kraken_desktop_directory_get_real_directory (KrakenDesktopDirectory *desktop)
{
	kraken_directory_ref (desktop->details->real_directory);
...
desktop_finalize (GObject *object)
{
	...
	desktop = KRAKEN_DESKTOP_DIRECTORY (object);

	kraken_directory_unref (desktop->details->real_directory);
	...
	G_OBJECT_CLASS (kraken_desktop_directory_parent_class)->finalize (object);
}
...
done_loading_callback (KrakenDirectory *real_directory,
		       KrakenDesktopDirectory *desktop)
{
     kraken_directory_emit_done_loading (KRAKEN_DIRECTORY (desktop));
}
...
static void
update_desktop_directory (KrakenDesktopDirectory *desktop)
{
	char *desktop_path;
	char *desktop_uri;
	KrakenDirectory *real_directory;
	...
	real_directory = kraken_directory_get_by_uri (desktop_uri);
	g_free (desktop_uri);
	g_free (desktop_path);
...
{
	update_desktop_directory (KRAKEN_DESKTOP_DIRECTORY (data));
	kraken_directory_force_reload (KRAKEN_DIRECTORY (data));
}

static void
kraken_desktop_directory_init (KrakenDesktopDirectory *desktop)
{
	desktop->details = g_new0 (KrakenDesktopDirectoryDetails, 1);
	...
	update_desktop_directory (KRAKEN_DESKTOP_DIRECTORY (desktop));
...
static void
kraken_desktop_directory_class_init (KrakenDesktopDirectoryClass *class)
{
	 ...
	 * in KrakenDesktopDirectory->details->real_directory,
