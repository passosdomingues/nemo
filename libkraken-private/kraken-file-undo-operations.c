...
/* kraken-file-undo-operations.c - Manages undo/redo of file operations
 *
...
G_DEFINE_TYPE (KrakenFileUndoInfo, kraken_file_undo_info, G_TYPE_OBJECT)
...
struct _KrakenFileUndoInfoDetails {
	KrakenFileUndoOp op_type;
...
/* description helpers */
static void
kraken_file_undo_info_init (KrakenFileUndoInfo *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, KRAKEN_TYPE_FILE_UNDO_INFO,
						  KrakenFileUndoInfoDetails);
	self->priv->apply_async_result = NULL;
}

static void
kraken_file_undo_info_get_property (GObject *object,
				      guint property_id,
				      GValue *value,
				      GParamSpec *pspec)
{
	KrakenFileUndoInfo *self = KRAKEN_FILE_UNDO_INFO (object);
...
static void
kraken_file_undo_info_set_property (GObject *object,
				      guint property_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
...
static void
kraken_file_redo_info_warn_redo (KrakenFileUndoInfo *self,
...
static void
kraken_file_undo_info_warn_undo (KrakenFileUndoInfo *self,
				   GtkWindow *parent_window)
{
...
static void
kraken_file_undo_info_strings_func (KrakenFileUndoInfo *self,
				      gchar **undo_label,
				      gchar **undo_description,
				      gchar **redo_label,
				      gchar **redo_description)
...
static void
kraken_file_undo_info_finalize (GObject *obj)
{
	KrakenFileUndoInfo *self = KRAKEN_FILE_UNDO_INFO (obj);

	g_clear_object (&self->priv->apply_async_result);

	G_OBJECT_CLASS (kraken_file_undo_info_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_class_init (KrakenFileUndoInfoClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_finalize;
	oclass->get_property = kraken_file_undo_info_get_property;
	oclass->set_property = kraken_file_undo_info_set_property;

	klass->undo_func = kraken_file_undo_info_warn_undo;
	klass->redo_func = kraken_file_redo_info_warn_redo;
	klass->strings_func = kraken_file_undo_info_strings_func;

	properties[PROP_OP_TYPE] =
		g_param_spec_int ("op-type",
				  "Undo info op type",
				  "Type of undo operation",
				  0, KRAKEN_FILE_UNDO_OP_NUM_TYPES - 1, 0,
				  G_PARAM_READWRITE |
	...
	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoDetails));
	g_object_class_install_properties (oclass, N_PROPERTIES, properties);
}

static KrakenFileUndoOp
kraken_file_undo_info_get_op_type (KrakenFileUndoInfo *self)
{
	return self->priv->op_type;
}

static gint
kraken_file_undo_info_get_item_count (KrakenFileUndoInfo *self)
{
	return self->priv->count;
}

void
kraken_file_undo_info_apply_async (KrakenFileUndoInfo *self,
				     gboolean undo,
				     GtkWindow *parent_window,
				     ...
				     gpointer user_data)
{
	...
	self->priv->apply_async_result = 
		g_simple_async_result_new (G_OBJECT (self),
					   callback, user_data,
					   kraken_file_undo_info_apply_async);

	if (undo) {
		KRAKEN_FILE_UNDO_INFO_CLASS (G_OBJECT_GET_CLASS (self))->undo_func (self, parent_window);
	} else {
		KRAKEN_FILE_UNDO_INFO_CLASS (G_OBJECT_GET_CLASS (self))->redo_func (self, parent_window);
	}
}
...
gboolean
kraken_file_undo_info_apply_finish (KrakenFileUndoInfo *self,
				      GAsyncResult *res,
				      gboolean *user_cancel,
				      GError **error)
{
...
void
kraken_file_undo_info_get_strings (KrakenFileUndoInfo *self,
				     gchar **undo_label,
				     gchar **undo_description,
				     gchar **redo_label,
				     gchar **redo_description)
{
	return KRAKEN_FILE_UNDO_INFO_CLASS (G_OBJECT_GET_CLASS (self))->strings_func (self,
											undo_label, undo_description,
											redo_label, redo_description);
}

static void
file_undo_info_complete_apply (KrakenFileUndoInfo *self,
			       gboolean success,
			       gboolean user_cancel)
{
...
file_undo_info_transfer_callback (GHashTable * debuting_uris,
				  gboolean success,
                                  gpointer user_data)
{
	KrakenFileUndoInfo *self = user_data;
...
static void
file_undo_info_operation_callback (KrakenFile * file,
...
G_DEFINE_TYPE (KrakenFileUndoInfoExt, kraken_file_undo_info_ext, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoExtDetails {
	GFile *src_dir;
	GFile *dest_dir;
...
static char *
ext_get_first_target_short_name (KrakenFileUndoInfoExt *self)
{
	GFile *first_file;
	char *file_name = NULL;
...
static void
ext_strings_func (KrakenFileUndoInfo *info,
		  gchar **undo_label,
		  gchar **undo_description,
		  gchar **redo_label,
		  gchar **redo_description)
{
	KrakenFileUndoInfoExt *self = KRAKEN_FILE_UNDO_INFO_EXT (info);
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (info);
	gint count = kraken_file_undo_info_get_item_count (info);
	...
	if (op_type == KRAKEN_FILE_UNDO_OP_MOVE) {
		if (count > 1) {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_RESTORE_FROM_TRASH)  {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_COPY) {
		if (count > 1) {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_DUPLICATE) {
		if (count > 1) {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_CREATE_LINK) {
		if (count > 1) {
...
static void
ext_create_link_redo_func (KrakenFileUndoInfoExt *self,
			   GtkWindow *parent_window)
{
	kraken_file_operations_link (HEAD (self->priv->sources), NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_duplicate_redo_func (KrakenFileUndoInfoExt *self,
			 GtkWindow *parent_window)
{
	kraken_file_operations_duplicate (HEAD (self->priv->sources), NULL, parent_window,
					    file_undo_info_transfer_callback, self);
}

static void
ext_copy_redo_func (KrakenFileUndoInfoExt *self,
		    GtkWindow *parent_window)
{
	kraken_file_operations_copy (HEAD (self->priv->sources), NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_move_restore_redo_func (KrakenFileUndoInfoExt *self,
			    GtkWindow *parent_window)
{
	kraken_file_operations_move (HEAD (self->priv->sources), NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_redo_func (KrakenFileUndoInfo *info,
	       GtkWindow *parent_window)
{
	KrakenFileUndoInfoExt *self = KRAKEN_FILE_UNDO_INFO_EXT (info);
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (info);
	...
	if (op_type == KRAKEN_FILE_UNDO_OP_MOVE ||
	    op_type == KRAKEN_FILE_UNDO_OP_RESTORE_FROM_TRASH)  {
...
static void
ext_restore_undo_func (KrakenFileUndoInfoExt *self,
		       GtkWindow *parent_window)
{
	kraken_file_operations_trash_or_delete (HEAD (self->priv->destinations), parent_window,
						  file_undo_info_delete_callback, self);
}


static void
ext_move_undo_func (KrakenFileUndoInfoExt *self,
		    GtkWindow *parent_window)
{
	kraken_file_operations_move (HEAD (self->priv->destinations), NULL,
				       self->priv->src_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_copy_duplicate_undo_func (KrakenFileUndoInfoExt *self,
			      GtkWindow *parent_window)
{
	GList *files;
	...
	kraken_file_operations_delete (files, parent_window,
					 file_undo_info_delete_callback, self);
...
static void
ext_undo_func (KrakenFileUndoInfo *info,
	       GtkWindow *parent_window)
{
	KrakenFileUndoInfoExt *self = KRAKEN_FILE_UNDO_INFO_EXT (info);
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (info);

	if (op_type == KRAKEN_FILE_UNDO_OP_COPY ||
	    op_type == KRAKEN_FILE_UNDO_OP_DUPLICATE ||
	    op_type == KRAKEN_FILE_UNDO_OP_CREATE_LINK) {
		ext_copy_duplicate_undo_func (self, parent_window);
	} else if (op_type == KRAKEN_FILE_UNDO_OP_MOVE) {
...
static void
kraken_file_undo_info_ext_init (KrakenFileUndoInfoExt *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_ext_get_type (),
						  KrakenFileUndoInfoExtDetails);
}

static void
kraken_file_undo_info_ext_finalize (GObject *obj)
{
	KrakenFileUndoInfoExt *self = KRAKEN_FILE_UNDO_INFO_EXT (obj);
	...
	G_OBJECT_CLASS (kraken_file_undo_info_ext_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_ext_class_init (KrakenFileUndoInfoExtClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_ext_finalize;

	iclass->undo_func = ext_undo_func;
	iclass->redo_func = ext_redo_func;
	iclass->strings_func = ext_strings_func;

	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoExtDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_ext_new (KrakenFileUndoOp op_type,
				 gint item_count,
				 GFile *src_dir,
				 GFile *target_dir)
{
	KrakenFileUndoInfoExt *retval;

	retval = g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_EXT,
			       "op-type", op_type,
	...
	return KRAKEN_FILE_UNDO_INFO (retval);
}

void
kraken_file_undo_info_ext_add_origin_target_pair (KrakenFileUndoInfoExt *self,
						    GFile                   *origin,
						    GFile                   *target)
{
...
G_DEFINE_TYPE (KrakenFileUndoInfoCreate, kraken_file_undo_info_create, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoCreateDetails {
	char *template;
	GFile *target_file;
	gint length;
};
...
static void
create_strings_func (KrakenFileUndoInfo *info,
		     gchar **undo_label,
		     gchar **undo_description,
		     gchar **redo_label,
		     gchar **redo_description)
...
{
	KrakenFileUndoInfoCreate *self = KRAKEN_FILE_UNDO_INFO_CREATE (info);
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (info);
	char *name;
	...
	*undo_description = g_strdup_printf (_("Delete '%s'"), name);
	...
	if (op_type == KRAKEN_FILE_UNDO_OP_CREATE_EMPTY_FILE) {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_CREATE_FOLDER) {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_CREATE_FILE_FROM_TEMPLATE) {
...
static void
create_from_template_redo_func (KrakenFileUndoInfoCreate *self,
				GtkWindow *parent_window)
{
	GFile *parent;
	...
	kraken_file_operations_new_file_from_template (NULL, NULL,
							 parent_uri, new_name,
							 self->priv->template,
...
static void
create_folder_redo_func (KrakenFileUndoInfoCreate *self,
			 GtkWindow *parent_window)
{
	GFile *parent;
	gchar *parent_uri;
	...
	parent = g_file_get_parent (self->priv->target_file);
	parent_uri = g_file_get_uri (parent);
	kraken_file_operations_new_folder (NULL, NULL, parent_uri,
					     create_callback, self);
...
static void
create_empty_redo_func (KrakenFileUndoInfoCreate *self,
			GtkWindow *parent_window)

{
	...
	parent_uri = g_file_get_uri (parent);
	new_name = g_file_get_parse_name (self->priv->target_file);
	kraken_file_operations_new_file (NULL, NULL, parent_uri,
					   new_name,
					   self->priv->template,
					   self->priv->length,
					   create_callback, self);
...
static void
create_redo_func (KrakenFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	KrakenFileUndoInfoCreate *self = KRAKEN_FILE_UNDO_INFO_CREATE (info);
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (info);
...
static void
create_undo_func (KrakenFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	KrakenFileUndoInfoCreate *self = KRAKEN_FILE_UNDO_INFO_CREATE (info);
	GList *files = NULL;
...
static void
kraken_file_undo_info_create_init (KrakenFileUndoInfoCreate *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_create_get_type (),
						  KrakenFileUndoInfoCreateDetails);
}

static void
kraken_file_undo_info_create_finalize (GObject *obj)
{
	KrakenFileUndoInfoCreate *self = KRAKEN_FILE_UNDO_INFO_CREATE (obj);
	g_clear_object (&self->priv->target_file);
	g_free (self->priv->template);	

	G_OBJECT_CLASS (kraken_file_undo_info_create_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_create_class_init (KrakenFileUndoInfoCreateClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_create_finalize;

	iclass->undo_func = create_undo_func;
	iclass->redo_func = create_redo_func;
	iclass->strings_func = create_strings_func;

	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoCreateDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_create_new (KrakenFileUndoOp op_type)
{
	return g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_CREATE,
			     "op-type", op_type,
			     "item-count", 1,
			     NULL);
}
...
void
kraken_file_undo_info_create_set_data (KrakenFileUndoInfoCreate *self,
                                         GFile                      *file,
                                         const char                 *template,
					 gint                        length)
{
...
/* rename */
G_DEFINE_TYPE (KrakenFileUndoInfoRename, kraken_file_undo_info_rename, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoRenameDetails {
	GFile *old_file;
	GFile *new_file;
...
static void
rename_strings_func (KrakenFileUndoInfo *info,
		     gchar **undo_label,
		     gchar **undo_description,
		     gchar **redo_label,
		     gchar **redo_description)
{
	KrakenFileUndoInfoRename *self = KRAKEN_FILE_UNDO_INFO_RENAME (info);
...
static void
rename_redo_func (KrakenFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	KrakenFileUndoInfoRename *self = KRAKEN_FILE_UNDO_INFO_RENAME (info);
	KrakenFile *file;

	file = kraken_file_get (self->priv->old_file);
	kraken_file_rename (file, self->priv->new_display_name,
...
static void
rename_undo_func (KrakenFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	KrakenFileUndoInfoRename *self = KRAKEN_FILE_UNDO_INFO_RENAME (info);
	KrakenFile *file;

	file = kraken_file_get (self->priv->new_file);
	kraken_file_rename (file, self->priv->old_display_name,
			      file_undo_info_operation_callback, self);
...
static void
kraken_file_undo_info_rename_init (KrakenFileUndoInfoRename *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_rename_get_type (),
						  KrakenFileUndoInfoRenameDetails);
}

static void
kraken_file_undo_info_rename_finalize (GObject *obj)
{
	KrakenFileUndoInfoRename *self = KRAKEN_FILE_UNDO_INFO_RENAME (obj);
	...
	G_OBJECT_CLASS (kraken_file_undo_info_rename_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_rename_class_init (KrakenFileUndoInfoRenameClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_rename_finalize;

	iclass->undo_func = rename_undo_func;
	iclass->redo_func = rename_redo_func;
	iclass->strings_func = rename_strings_func;

	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoRenameDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_rename_new (void)
{
	return g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_RENAME,
			     "op-type", KRAKEN_FILE_UNDO_OP_RENAME,
			     "item-count", 1,
			     NULL);
}

void
kraken_file_undo_info_rename_set_data_pre (KrakenFileUndoInfoRename *self,
					     GFile                      *old_file,
					     gchar                      *old_display_name,
					     gchar                      *new_display_name)
{
...
void
kraken_file_undo_info_rename_set_data_post (KrakenFileUndoInfoRename *self,
					      GFile                      *new_file)
{
...
/* trash */
G_DEFINE_TYPE (KrakenFileUndoInfoTrash, kraken_file_undo_info_trash, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoTrashDetails {
	GHashTable *trashed;
};

static void
trash_strings_func (KrakenFileUndoInfo *info,
		    gchar **undo_label,
		    gchar **undo_description,
		    gchar **redo_label,
		    gchar **redo_description)
{
	KrakenFileUndoInfoTrash *self = KRAKEN_FILE_UNDO_INFO_TRASH (info);
...
trash_redo_func_callback (GHashTable *debuting_uris,
			  ...
			  gpointer user_data)
{
	KrakenFileUndoInfoTrash *self = user_data;
...
static void
trash_redo_func (KrakenFileUndoInfo *info,
		 GtkWindow *parent_window)
{
	KrakenFileUndoInfoTrash *self = KRAKEN_FILE_UNDO_INFO_TRASH (info);
	...
	if (g_hash_table_size (self->priv->trashed) > 0) {
		...
		kraken_file_operations_trash_or_delete (locations, parent_window,
...
static GHashTable *
trash_retrieve_files_to_restore_finish (KrakenFileUndoInfoTrash *self,
					GAsyncResult *res,
					GError **error)
{
...
{
	KrakenFileUndoInfoTrash *self = KRAKEN_FILE_UNDO_INFO_TRASH (object);
...
static void
trash_retrieve_files_to_restore_async (KrakenFileUndoInfoTrash *self,
				 GAsyncReadyCallback callback,
				 gpointer user_data)
{
...
trash_retrieve_files_ready (GObject *source,
			    ...
			    gpointer user_data)
{
	KrakenFileUndoInfoTrash *self = KRAKEN_FILE_UNDO_INFO_TRASH (source);
...
static void
trash_undo_func (KrakenFileUndoInfo *info,
		 GtkWindow *parent_window)
{
	KrakenFileUndoInfoTrash *self = KRAKEN_FILE_UNDO_INFO_TRASH (info);

	/* Internally managed op, pop flag. */
	kraken_file_undo_manager_pop_flag ();

	trash_retrieve_files_to_restore_async (self, trash_retrieve_files_ready, NULL);
}

static void
kraken_file_undo_info_trash_init (KrakenFileUndoInfoTrash *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_trash_get_type (),
						  KrakenFileUndoInfoTrashDetails);
	self->priv->trashed =
		g_hash_table_new_full (g_file_hash, (GEqualFunc) g_file_equal, 
				       g_object_unref, NULL);
}
...
static void
kraken_file_undo_info_trash_finalize (GObject *obj)
{
	KrakenFileUndoInfoTrash *self = KRAKEN_FILE_UNDO_INFO_TRASH (obj);
	g_hash_table_destroy (self->priv->trashed);

	G_OBJECT_CLASS (kraken_file_undo_info_trash_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_trash_class_init (KrakenFileUndoInfoTrashClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_trash_finalize;

	iclass->undo_func = trash_undo_func;
	iclass->redo_func = trash_redo_func;
	iclass->strings_func = trash_strings_func;

	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoTrashDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_trash_new (gint item_count)
{
	return g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_TRASH,
			     "op-type", KRAKEN_FILE_UNDO_OP_MOVE_TO_TRASH,
			     "item-count", item_count,
			     NULL);
}

void
kraken_file_undo_info_trash_add_file (KrakenFileUndoInfoTrash *self,
					GFile                     *file)
{
	GTimeVal current_time;
...
G_DEFINE_TYPE (KrakenFileUndoInfoRecPermissions, kraken_file_undo_info_rec_permissions, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoRecPermissionsDetails {
	GFile *dest_dir;
...
static void
rec_permissions_strings_func (KrakenFileUndoInfo *info,
			      gchar **undo_label,
			      gchar **undo_description,
			      gchar **redo_label,
			      gchar **redo_description)
{
	KrakenFileUndoInfoRecPermissions *self = KRAKEN_FILE_UNDO_INFO_REC_PERMISSIONS (info);
	char *name;
...
static void
rec_permissions_redo_func (KrakenFileUndoInfo *info,
			   GtkWindow *parent_window)
{
	KrakenFileUndoInfoRecPermissions *self = KRAKEN_FILE_UNDO_INFO_REC_PERMISSIONS (info);
	gchar *parent_uri;
	...
	kraken_file_set_permissions_recursive (parent_uri,
...
static void
rec_permissions_undo_func (KrakenFileUndoInfo *info,
			   GtkWindow *parent_window)
{
	KrakenFileUndoInfoRecPermissions *self = KRAKEN_FILE_UNDO_INFO_REC_PERMISSIONS (info);
...
static void
kraken_file_undo_info_rec_permissions_init (KrakenFileUndoInfoRecPermissions *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_rec_permissions_get_type (),
						  KrakenFileUndoInfoRecPermissionsDetails);
...
static void
kraken_file_undo_info_rec_permissions_finalize (GObject *obj)
{
	KrakenFileUndoInfoRecPermissions *self = KRAKEN_FILE_UNDO_INFO_REC_PERMISSIONS (obj);

	g_hash_table_destroy (self->priv->original_permissions);
	g_clear_object (&self->priv->dest_dir);

	G_OBJECT_CLASS (kraken_file_undo_info_rec_permissions_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_rec_permissions_class_init (KrakenFileUndoInfoRecPermissionsClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_rec_permissions_finalize;
	...
	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoRecPermissionsDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_rec_permissions_new (GFile   *dest,
					     guint32 file_permissions,
					     guint32 file_mask,
					     guint32 dir_permissions,
					     guint32 dir_mask)
{
	KrakenFileUndoInfoRecPermissions *retval;

	retval = g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_REC_PERMISSIONS,
			       "op-type", KRAKEN_FILE_UNDO_OP_RECURSIVE_SET_PERMISSIONS,
			       "item-count", 1,
			       NULL);
...
void
kraken_file_undo_info_rec_permissions_add_file (KrakenFileUndoInfoRecPermissions *self,
						  GFile                              *file,
						  guint32                             permission)
{
...
G_DEFINE_TYPE (KrakenFileUndoInfoPermissions, kraken_file_undo_info_permissions, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoPermissionsDetails {
	GFile *target_file;
...
static void
permissions_strings_func (KrakenFileUndoInfo *info,
			  gchar **undo_label,
			  gchar **undo_description,
			  gchar **redo_label,
			  gchar **redo_description)
{
	KrakenFileUndoInfoPermissions *self = KRAKEN_FILE_UNDO_INFO_PERMISSIONS (info);
	gchar *name;
...
static void
permissions_real_func (KrakenFileUndoInfoPermissions *self,
		       guint32 permissions)
{
	KrakenFile *file;

	file = kraken_file_get (self->priv->target_file);
	kraken_file_set_permissions (file, permissions,
				       file_undo_info_operation_callback, self);
...
static void
permissions_redo_func (KrakenFileUndoInfo *info, 
		       GtkWindow *parent_window)
{
	KrakenFileUndoInfoPermissions *self = KRAKEN_FILE_UNDO_INFO_PERMISSIONS (info);
...
static void
permissions_undo_func (KrakenFileUndoInfo *info, 
		       GtkWindow *parent_window)
{
	KrakenFileUndoInfoPermissions *self = KRAKEN_FILE_UNDO_INFO_PERMISSIONS (info);
...
static void
kraken_file_undo_info_permissions_init (KrakenFileUndoInfoPermissions *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_permissions_get_type (),
						  KrakenFileUndoInfoPermissionsDetails);
}

static void
kraken_file_undo_info_permissions_finalize (GObject *obj)
{
	KrakenFileUndoInfoPermissions *self = KRAKEN_FILE_UNDO_INFO_PERMISSIONS (obj);
	g_clear_object (&self->priv->target_file);

	G_OBJECT_CLASS (kraken_file_undo_info_permissions_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_permissions_class_init (KrakenFileUndoInfoPermissionsClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_permissions_finalize;

	iclass->undo_func = permissions_undo_func;
	iclass->redo_func = permissions_redo_func;
	iclass->strings_func = permissions_strings_func;

	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoPermissionsDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_permissions_new (GFile   *file,
					 guint32  current_permissions,
					 guint32  new_permissions)
{
	KrakenFileUndoInfoPermissions *retval;

	retval = g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_PERMISSIONS,
			     "op-type", KRAKEN_FILE_UNDO_OP_SET_PERMISSIONS,
			     "item-count", 1,
			     NULL);
...
G_DEFINE_TYPE (KrakenFileUndoInfoOwnership, kraken_file_undo_info_ownership, KRAKEN_TYPE_FILE_UNDO_INFO)

struct _KrakenFileUndoInfoOwnershipDetails {
	GFile *target_file;
...
static void
ownership_strings_func (KrakenFileUndoInfo *info,
			gchar **undo_label,
			gchar **undo_description,
			gchar **redo_label,
			gchar **redo_description)
{
	KrakenFileUndoInfoOwnership *self = KRAKEN_FILE_UNDO_INFO_OWNERSHIP (info);
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (info);
	gchar *name;

	name = g_file_get_parse_name (self->priv->target_file);

	if (op_type == KRAKEN_FILE_UNDO_OP_CHANGE_GROUP) {
	...
	} else if (op_type == KRAKEN_FILE_UNDO_OP_CHANGE_OWNER) {
...
static void
ownership_real_func (KrakenFileUndoInfoOwnership *self,
		     const gchar *ownership)
{
	KrakenFileUndoOp op_type = kraken_file_undo_info_get_op_type (KRAKEN_FILE_UNDO_INFO (self));
	KrakenFile *file;

	file = kraken_file_get (self->priv->target_file);

	if (op_type == KRAKEN_FILE_UNDO_OP_CHANGE_OWNER) {
		kraken_file_set_owner (file,
					 ownership,
					 file_undo_info_operation_callback, self);
	} else if (op_type == KRAKEN_FILE_UNDO_OP_CHANGE_GROUP) {
		kraken_file_set_group (file,
					 ownership,
					 file_undo_info_operation_callback, self);
	}
...
static void
ownership_redo_func (KrakenFileUndoInfo *info,
		     GtkWindow *parent_window)
{
	KrakenFileUndoInfoOwnership *self = KRAKEN_FILE_UNDO_INFO_OWNERSHIP (info);
...
static void
ownership_undo_func (KrakenFileUndoInfo *info,
		     GtkWindow *parent_window)
{
	KrakenFileUndoInfoOwnership *self = KRAKEN_FILE_UNDO_INFO_OWNERSHIP (info);
...
static void
kraken_file_undo_info_ownership_init (KrakenFileUndoInfoOwnership *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, kraken_file_undo_info_ownership_get_type (),
						  KrakenFileUndoInfoOwnershipDetails);
}

static void
kraken_file_undo_info_ownership_finalize (GObject *obj)
{
	KrakenFileUndoInfoOwnership *self = KRAKEN_FILE_UNDO_INFO_OWNERSHIP (obj);

	g_clear_object (&self->priv->target_file);
	g_free (self->priv->original_ownership);
	g_free (self->priv->new_ownership);

	G_OBJECT_CLASS (kraken_file_undo_info_ownership_parent_class)->finalize (obj);
}

static void
kraken_file_undo_info_ownership_class_init (KrakenFileUndoInfoOwnershipClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	KrakenFileUndoInfoClass *iclass = KRAKEN_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = kraken_file_undo_info_ownership_finalize;

	iclass->undo_func = ownership_undo_func;
	iclass->redo_func = ownership_redo_func;
	iclass->strings_func = ownership_strings_func;

	g_type_class_add_private (klass, sizeof (KrakenFileUndoInfoOwnershipDetails));
}

KrakenFileUndoInfo *
kraken_file_undo_info_ownership_new (KrakenFileUndoOp  op_type,
				       GFile              *file,
				       const char         *current_data,
				       const char         *new_data)
{
	KrakenFileUndoInfoOwnership *retval;

	retval = g_object_new (KRAKEN_TYPE_FILE_UNDO_INFO_OWNERSHIP,
			       "item-count", 1,
			       "op-type", op_type,
			       NULL);
