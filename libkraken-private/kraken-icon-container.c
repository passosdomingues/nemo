...
/* kraken-icon-container.c - Icon container widget.
...
#include <config.h>
#include <math.h>
#include "kraken-icon-container.h"
...
#include "kraken-selection-canvas-item.h"
#include "kraken-desktop-utils.h"
...
#define DEBUG_FLAG KRAKEN_DEBUG_ICON_CONTAINER
...
/* Copied from KrakenIconContainer */
#define KRAKEN_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT 5

/* Copied from KrakenFile */
...
} KrakenIconContainerAccessiblePrivate;

static GType         kraken_icon_container_accessible_get_type (void);

static void          preview_selected_items                         (KrakenIconContainer *container);
static void          activate_selected_items                        (KrakenIconContainer *container);
static void          activate_selected_items_alternate              (KrakenIconContainer *container,
								     KrakenIcon          *icon);
static void          compute_stretch                                (StretchState          *start,
								     StretchState          *current);
static KrakenIcon *get_first_selected_icon                        (KrakenIconContainer *container);
static KrakenIcon *get_nth_selected_icon                          (KrakenIconContainer *container,
								     int                    index);
static gboolean      has_multiple_selection                         (KrakenIconContainer *container);
static gboolean      all_selected                                   (KrakenIconContainer *container);
static gboolean      has_selection                                  (KrakenIconContainer *container);
static void          icon_destroy                                   (KrakenIconContainer *container,
								     KrakenIcon          *icon);

static gboolean      is_renaming                                    (KrakenIconContainer *container);
static gboolean      is_renaming_pending                            (KrakenIconContainer *container);
static void          process_pending_icon_to_rename                 (KrakenIconContainer *container);
...
static void          handle_vadjustment_changed                     (GtkAdjustment         *adjustment,
								     KrakenIconContainer *container);
static GList *       kraken_icon_container_get_selected_icons (KrakenIconContainer *container);
static void          queue_update_visible_icons                 (KrakenIconContainer *container, gint delay);
static void          reveal_icon                                    (KrakenIconContainer *container,
								     KrakenIcon *icon);

static void         text_ellipsis_limit_changed_container_callback  (gpointer callback_data);

static int compare_icons_horizontal (KrakenIconContainer *container,
				     KrakenIcon *icon_a,
				     KrakenIcon *icon_b);

static int compare_icons_vertical (KrakenIconContainer *container,
				   KrakenIcon *icon_a,
				   KrakenIcon *icon_b);

static void remove_search_entry_timeout (KrakenIconContainer *container);

static gboolean handle_icon_slow_two_click (KrakenIconContainer *container,
                                            KrakenIcon *icon,
                                            GdkEventButton *event);

static void schedule_align_icons (KrakenIconContainer *container);

static gpointer accessible_parent_class;

static GQuark accessible_private_data_quark = 0;

static const char *kraken_icon_container_accessible_action_names[] = {
	"activate",
	"menu",
	NULL
};

static const char *kraken_icon_container_accessible_action_descriptions[] = {
	"Activate selected items",
	"Popup context menu",
	NULL
};

G_DEFINE_TYPE (KrakenIconContainer, kraken_icon_container, EEL_TYPE_CANVAS);

/* The KrakenIconContainer signals.  */
enum {
	ACTIVATE,
	ACTIVATE_ALTERNATE,
	ACTIVATE_PREVIEWER,
...
static void
tooltip_prefs_changed_callback (KrakenIconContainer *container)
{
    container->details->show_desktop_tooltips = g_settings_get_boolean (kraken_preferences,
                                                                        KRAKEN_PREFERENCES_TOOLTIPS_DESKTOP);
    container->details->show_icon_view_tooltips = g_settings_get_boolean (kraken_preferences,
                                                                          KRAKEN_PREFERENCES_TOOLTIPS_ICON_VIEW);

    container->details->tooltip_flags = kraken_global_preferences_get_tooltip_flags ();

    kraken_icon_container_request_update_all (container);
}

/* Functions dealing with KrakenIcons.  */

static gboolean
clicked_on_text (KrakenIconContainer *container,
                          KrakenIcon *icon,
                    GdkEventButton *event)
{
    if (icon == NULL)
    ...
    icon_rect = kraken_icon_canvas_item_get_text_rectangle (icon->item, TRUE);
...
static gboolean
clicked_on_icon (KrakenIconContainer *container,
                          KrakenIcon *icon,
                    GdkEventButton *event)
{
    if (icon == NULL)
    ...
    icon_rect = kraken_icon_canvas_item_get_icon_rectangle (icon->item);
    eel_canvas_window_to_world (EEL_CANVAS (container), event->x, event->y, &eventX, &eventY);
...
gboolean
kraken_icon_container_icon_is_positioned (const KrakenIcon *icon)
{
	return icon->x != ICON_UNPOSITIONED_VALUE && icon->y != ICON_UNPOSITIONED_VALUE;
}

static void
icon_get_size (KrakenIconContainer *container,
	       KrakenIcon *icon,
	       guint *size)
{
	if (size != NULL) {
		*size = MAX (kraken_get_icon_size_for_zoom_level (container->details->zoom_level)
			       * icon->scale, KRAKEN_ICON_SIZE_SMALLEST);
	}
}
 ...
 */
static void
icon_set_size (KrakenIconContainer *container,
	       KrakenIcon *icon,
	       guint icon_size,
	       gboolean snap,
	       gboolean update_position)
{
	...
	scale = (double) icon_size /
		kraken_get_icon_size_for_zoom_level
		(container->details->zoom_level);
	kraken_icon_container_move_icon (container, icon,
					   icon->x, icon->y,
					   scale, FALSE,
...
static void
emit_stretch_started (KrakenIconContainer *container, KrakenIcon *icon)
{
...
static void
emit_stretch_ended (KrakenIconContainer *container, KrakenIcon *icon)
{
	g_signal_emit (container,
...
static void
icon_toggle_selected (KrakenIconContainer *container,
		      KrakenIcon *icon)
{
	...
	if (icon == container->details->stretch_icon) {
		...
		kraken_icon_canvas_item_set_show_stretch_handles (icon->item, FALSE);
		/* snap the icon if necessary */
		if (container->details->keep_aligned) {
			kraken_icon_container_move_icon (container,
							   icon,
							   icon->x, icon->y,
							   icon->scale,
							   FALSE, TRUE, TRUE);
...
static gboolean
icon_set_selected (KrakenIconContainer *container,
		   KrakenIcon *icon,
		   gboolean select)
{
...
/* Utility functions for KrakenIconContainer.  */

gboolean
kraken_icon_container_scroll (KrakenIconContainer *container,
...
static void
pending_icon_to_reveal_destroy_callback (KrakenIconCanvasItem *item,
					 KrakenIconContainer *container)
{
...
static KrakenIcon*
get_pending_icon_to_reveal (KrakenIconContainer *container)
{
	return container->details->pending_icon_to_reveal;
}

static void
set_pending_icon_to_reveal (KrakenIconContainer *container, KrakenIcon *icon)
{
	KrakenIcon *old_icon;
...
static void
item_get_canvas_bounds (KrakenIconContainer *container,
                        EelCanvasItem     *item,
                        EelIRect          *bounds,
                        gboolean           safety_pad)
{
...
static void
icon_get_row_and_column_bounds (KrakenIconContainer *container,
				KrakenIcon *icon,
				EelIRect *bounds,
				gboolean safety_pad)
{
...
static void
reveal_icon (KrakenIconContainer *container,
	     KrakenIcon *icon)
{
	...
	if (!kraken_icon_container_icon_is_positioned (icon)) {
	...
	if (kraken_icon_container_is_auto_layout (container)) {
...
static void
process_pending_icon_to_reveal (KrakenIconContainer *container)
{
...
keyboard_icon_reveal_timeout_callback (gpointer data)
{
	...
	container = KRAKEN_ICON_CONTAINER (data);
...
static void
unschedule_keyboard_icon_reveal (KrakenIconContainer *container)
{
...
static void
schedule_keyboard_icon_reveal (KrakenIconContainer *container,
			       KrakenIcon *icon)
{
	KrakenIconContainerDetails *details;
...
static void
clear_keyboard_focus (KrakenIconContainer *container)
{
...
inline static void
emit_atk_focus_tracker_notify (KrakenIcon *icon)
{
...
static void
set_keyboard_focus (KrakenIconContainer *container,
		    KrakenIcon *icon)
{
	g_assert (icon != NULL);
...
static void
set_keyboard_rubberband_start (KrakenIconContainer *container,
			       KrakenIcon *icon)
{
...
static void
clear_keyboard_rubberband_start (KrakenIconContainer *container)
{
	container->details->keyboard_rubberband_start = NULL;
}

/* carbon-copy of eel_canvas_group_bounds(), but
 * for KrakenIconContainerItems it returns the
				   ...
				   KrakenIconCanvasItemBoundsUsage usage)
{
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (!KRAKEN_IS_ICON_CANVAS_ITEM (child)) {
			continue;
		}
		...
		if (child->flags & EEL_CANVAS_ITEM_VISIBLE) {
			...
			if (!KRAKEN_IS_ICON_CANVAS_ITEM (child) ||
				...
				kraken_icon_canvas_item_get_bounds_for_layout (KRAKEN_ICON_CANVAS_ITEM (child),
										 &minx, &miny, &maxx, &maxy);
			} else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
				kraken_icon_canvas_item_get_bounds_for_entire_item (KRAKEN_ICON_CANVAS_ITEM (child),
										      &minx, &miny, &maxx, &maxy);
			} else {
...
void
kraken_icon_container_get_all_icon_bounds (KrakenIconContainer *container,
		     double *x1, double *y1,
		     double *x2, double *y2,
		     KrakenIconCanvasItemBoundsUsage usage)
{
...
void
kraken_icon_container_reset_scroll_region (KrakenIconContainer *container)
{
...
void
kraken_icon_container_update_scroll_region (KrakenIconContainer *container)
{
	double x1, y1, x2, y2;
	...
	if (kraken_icon_container_get_is_fixed_size (container)) {
	...
	reset_scroll_region = container->details->reset_scroll_region_trigger
		|| kraken_icon_container_is_empty (container)
		|| kraken_icon_container_is_auto_layout (container);
	 ...
	 */
	if (!kraken_icon_container_is_empty (container)) {
		container->details->reset_scroll_region_trigger = FALSE;
	}

	kraken_icon_container_get_all_icon_bounds (container, &x1, &y1, &x2, &y2, BOUNDS_USAGE_FOR_ENTIRE_ITEM);
	 ...
	 */
	if (kraken_icon_container_is_layout_vertical (container)) {
	...
	step_increment = kraken_get_icon_size_for_zoom_level
...
{
	KrakenIconContainerClass *klass;
	const KrakenIcon *icon_a, *icon_b;

	icon_a = a;
	icon_b = b;
	klass  = KRAKEN_ICON_CONTAINER_GET_CLASS (icon_container);

	return klass->compare_icons (icon_container, icon_a->data, icon_b->data);
}

static void
align_icons (KrakenIconContainer *container)
{
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->align_icons (container);
}

static void
redo_layout_internal (KrakenIconContainer *container)
{
    container->details->fixed_text_height = -1;

    if (KRAKEN_ICON_CONTAINER_GET_CLASS (container)->finish_adding_new_icons != NULL) {
        KRAKEN_ICON_CONTAINER_GET_CLASS (container)->finish_adding_new_icons (container);
    }
    ...
    if (container->details->auto_layout && container->details->drag_state != DRAG_STATE_STRETCH) {
        ...
        KRAKEN_ICON_CONTAINER_GET_CLASS (container)->lay_down_icons (container, container->details->icons, 0);
	}
	...
	kraken_icon_container_update_scroll_region (container);
...
redo_layout_callback (gpointer callback_data)
{
	...
	container = KRAKEN_ICON_CONTAINER (callback_data);
...
static void
unschedule_redo_layout (KrakenIconContainer *container)
{
...
static void
schedule_redo_layout (KrakenIconContainer *container)
{
	if (container->details->idle_id == 0
...
void
kraken_icon_container_redo_layout (KrakenIconContainer *container)
{
...
static void
reload_icon_positions (KrakenIconContainer *container)
{
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->reload_icon_positions (container);
}
...
static void
invalidate_label_sizes (KrakenIconContainer *container)
{
	GList *p;
	KrakenIcon *icon;
...
static void
update_icons (KrakenIconContainer *container)
{
    GList *p;
    KrakenIcon *icon;
...
static gboolean
select_range (KrakenIconContainer *container,
	      KrakenIcon *icon1,
	      KrakenIcon *icon2,
...
static gboolean
select_one_unselect_others (KrakenIconContainer *container,
			    KrakenIcon *icon_to_select)
{
	gboolean selection_changed;
	GList *p;
	KrakenIcon *icon;
...
static gboolean
unselect_all (KrakenIconContainer *container)
{
...
static void
rubberband_select (KrakenIconContainer *container,
		   ...
		   const EelDRect *current_rect)
{
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		is_in = kraken_icon_canvas_item_hit_test_rectangle (icon->item, canvas_rect);
...
rubberband_timeout_callback (gpointer data)
{
	KrakenIconContainer *container;
	GtkWidget *widget;
	KrakenIconRubberbandInfo *band_info;
	int x, y;
	double x1, y1, x2, y2;
	...
	kraken_icon_container_scroll (container, x_scroll, y_scroll);
...
static void
start_rubberbanding (KrakenIconContainer *container,
		     GdkEventButton *event)
{
	...
	band_info->selection_rectangle = eel_canvas_item_new
		(eel_canvas_root
		 (EEL_CANVAS (container)),
		 KRAKEN_TYPE_SELECTION_CANVAS_ITEM,
		 "x1", band_info->start_x,
		 "y1", band_info->start_y,
		 "x2", band_info->start_x,
		 "y2", band_info->start_y,
...
static void
stop_rubberbanding (KrakenIconContainer *container,
		    guint32 time)
{
	KrakenIconRubberbandInfo *band_info;
	GList *icons;
	...
	if (enable_animation) {
		kraken_selection_canvas_item_fade_out (KRAKEN_SELECTION_CANVAS_ITEM (band_info->selection_rectangle), 150);
	} else {
	...
	icons = kraken_icon_container_get_selected_icons (container);
...
typedef gboolean (* IsBetterIconFunction) (KrakenIconContainer *container,
					   KrakenIcon *start_icon,
					   KrakenIcon *best_so_far,
					   KrakenIcon *candidate,
					   void *data);

static KrakenIcon *
find_best_icon (KrakenIconContainer *container,
		KrakenIcon *start_icon,
		IsBetterIconFunction function,
		void *data)
{
	GList *p;
	KrakenIcon *best, *candidate;
...
static KrakenIcon *
find_best_selected_icon (KrakenIconContainer *container,
			 KrakenIcon *start_icon,
			 IsBetterIconFunction function,
			 void *data)
{
...
static int
compare_icons_by_uri (KrakenIconContainer *container,
		      KrakenIcon *icon_a,
		      KrakenIcon *icon_b)
{
	char *uri_a, *uri_b;
	...
	uri_a = kraken_icon_container_get_icon_uri (container, icon_a);
	uri_b = kraken_icon_container_get_icon_uri (container, icon_b);
...
static int
get_cmp_point_x (KrakenIconContainer *container,
		 EelDRect icon_rect)
{
	if (container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE) {
...
static int
get_cmp_point_y (KrakenIconContainer *container,
		 EelDRect icon_rect)
{
	if (container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE) {
...
static int
compare_icons_horizontal (KrakenIconContainer *container,
			  KrakenIcon *icon_a,
			  KrakenIcon *icon_b)
{
	EelDRect world_rect;
	...
	world_rect = kraken_icon_canvas_item_get_icon_rectangle (icon_a->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 ...
		 &ax,
		 NULL);
	world_rect = kraken_icon_canvas_item_get_icon_rectangle (icon_b->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static int
compare_icons_vertical (KrakenIconContainer *container,
			KrakenIcon *icon_a,
			KrakenIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_horizontal_first (KrakenIconContainer *container,
				KrakenIcon *icon_a,
				KrakenIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_vertical_first (KrakenIconContainer *container,
			      KrakenIcon *icon_a,
			      KrakenIcon *icon_b)
{
	EelDRect world_rect;
...
static gboolean
leftmost_in_top_row (KrakenIconContainer *container,
		     KrakenIcon *start_icon,
		     KrakenIcon *best_so_far,
		     KrakenIcon *candidate,
		     void *data)
...
static gboolean
rightmost_in_top_row (KrakenIconContainer *container,
		      KrakenIcon *start_icon,
		      KrakenIcon *best_so_far,
		      KrakenIcon *candidate,
		      void *data)
...
static gboolean
rightmost_in_bottom_row (KrakenIconContainer *container,
			 KrakenIcon *start_icon,
			 KrakenIcon *best_so_far,
			 KrakenIcon *candidate,
			 void *data)
...
static int
compare_with_start_row (KrakenIconContainer *container,
			KrakenIcon *icon)
{
	EelCanvasItem *item;
...
static int
compare_with_start_column (KrakenIconContainer *container,
			   KrakenIcon *icon)
{
	EelCanvasItem *item;
...
static gboolean
same_row_right_side_leftmost (KrakenIconContainer *container,
			      KrakenIcon *start_icon,
			      KrakenIcon *best_so_far,
			      KrakenIcon *candidate,
			      void *data)
...
static gboolean
same_row_left_side_rightmost (KrakenIconContainer *container,
			      KrakenIcon *start_icon,
			      KrakenIcon *best_so_far,
			      KrakenIcon *candidate,
			      void *data)
...
static gboolean
next_row_leftmost (KrakenIconContainer *container,
		   KrakenIcon *start_icon,
	           KrakenIcon *best_so_far,
		   KrakenIcon *candidate,
		   void *data)
...
static gboolean
next_row_rightmost (KrakenIconContainer *container,
		    KrakenIcon *start_icon,
		    KrakenIcon *best_so_far,
		    KrakenIcon *candidate,
		    void *data)
...
static gboolean
next_column_bottommost (KrakenIconContainer *container,
			KrakenIcon *start_icon,
			KrakenIcon *best_so_far,
			KrakenIcon *candidate,
			void *data)
...
static gboolean
previous_row_rightmost (KrakenIconContainer *container,
		        KrakenIcon *start_icon,
			KrakenIcon *best_so_far,
			KrakenIcon *candidate,
			void *data)
...
static gboolean
same_column_above_lowest (KrakenIconContainer *container,
			  KrakenIcon *start_icon,
			  KrakenIcon *best_so_far,
			  KrakenIcon *candidate,
			  void *data)
...
static gboolean
same_column_below_highest (KrakenIconContainer *container,
			   KrakenIcon *start_icon,
			   KrakenIcon *best_so_far,
			   KrakenIcon *candidate,
			   void *data)
...
static gboolean
previous_column_highest (KrakenIconContainer *container,
			 KrakenIcon *start_icon,
			 KrakenIcon *best_so_far,
			 KrakenIcon *candidate,
			 void *data)
...
static gboolean
next_column_highest (KrakenIconContainer *container,
		     KrakenIcon *start_icon,
		     KrakenIcon *best_so_far,
		     KrakenIcon *candidate,
		     void *data)
...
static gboolean
previous_column_lowest (KrakenIconContainer *container,
		        KrakenIcon *start_icon,
			KrakenIcon *best_so_far,
			KrakenIcon *candidate,
			void *data)
...
static gboolean
last_column_lowest (KrakenIconContainer *container,
		    KrakenIcon *start_icon,
		    KrakenIcon *best_so_far,
		    KrakenIcon *candidate,
		    void *data)
...
static gboolean
closest_in_90_degrees (KrakenIconContainer *container,
		       KrakenIcon *start_icon,
		       KrakenIcon *best_so_far,
		       KrakenIcon *candidate,
		       void *data)
{
	...
	world_rect = kraken_icon_canvas_item_get_icon_rectangle (candidate->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static EelDRect
get_rubberband (KrakenIcon *icon1,
		KrakenIcon *icon2)
{
	EelDRect rect1;
	EelDRect rect2;
...
static void
keyboard_move_to (KrakenIconContainer *container,
		  KrakenIcon *icon,
		  KrakenIcon *from,
		  GdkEventKey *event)
{
...
static void
keyboard_home (KrakenIconContainer *container,
	       GdkEventKey *event)
{
	KrakenIcon *from;
	KrakenIcon *to;
...
static void
keyboard_end (KrakenIconContainer *container,
	      GdkEventKey *event)
{
	KrakenIcon *to;
	KrakenIcon *from;
	...
	to = find_best_icon (container, NULL,
			     kraken_icon_container_is_layout_vertical (container) ?
			     last_column_lowest :
			     rightmost_in_bottom_row,
			     NULL);
...
static void
record_arrow_key_start (KrakenIconContainer *container,
			KrakenIcon *icon,
			GtkDirectionType direction)
{
	EelDRect world_rect;

	world_rect = kraken_icon_canvas_item_get_icon_rectangle (icon->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static void
keyboard_arrow_key (KrakenIconContainer *container,
		    GdkEventKey *event,
		    GtkDirectionType direction,
...
static void
keyboard_right (KrakenIconContainer *container,
		GdkEventKey *event)
{
	...
	keyboard_arrow_key (container,
			    ...
			    GTK_DIR_RIGHT,
			    rightmost_in_bottom_row,
			    kraken_icon_container_is_layout_rtl (container) ?
...
static void
keyboard_left (KrakenIconContainer *container,
	       GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_down (KrakenIconContainer *container,
	       GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_up (KrakenIconContainer *container,
	     GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_space (KrakenIconContainer *container,
		GdkEventKey *event)
{
	KrakenIcon *icon;
...
static void
select_previous_or_next_icon (KrakenIconContainer *container,
			      gboolean next,
			      GdkEventKey *event)
{
	KrakenIcon *icon;
...
destroy (GtkWidget *object)
{
	...
	container = KRAKEN_ICON_CONTAINER (object);

        kraken_icon_container_clear (container);
	...
	GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->destroy (object);
}
...
finalize (GObject *object)
{
	...
	details = KRAKEN_ICON_CONTAINER (object)->details;
	...
	G_OBJECT_CLASS (kraken_icon_container_parent_class)->finalize (object);
}
	       ...
	       GtkAllocation *allocation)
{
	...
	container = KRAKEN_ICON_CONTAINER (widget);
	...
	GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->size_allocate (widget, allocation);
...
realize (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->realize (widget);

	container = KRAKEN_ICON_CONTAINER (widget);

	/* Set up DnD.  */
	kraken_icon_dnd_init (container);
...
unrealize (GtkWidget *widget)
{
	...
	kraken_icon_dnd_fini (container);
	remove_search_entry_timeout (container);

	GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->unrealize (widget);
}
...
style_updated (GtkWidget *widget)
{
	 ...
	 */
	if (!kraken_icon_container_get_is_desktop (container)) {
		GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->style_updated (widget);
	}
		    ...
		    GdkEventButton *event)
{
	...
	clicked_on_item = GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->button_press_event (widget, event);
    ...
    if (clicked_on_item) {
        KrakenIcon *icon; // current icon which was clicked on
        ...
        if (clicked_on_icon (container, icon, event) &&
            icon == kraken_icon_container_get_icon_being_renamed (container)) {
            kraken_icon_container_end_renaming_mode (container, TRUE);
        }
...
static void
kraken_icon_container_did_not_drag (KrakenIconContainer *container,
				      GdkEventButton *event)
{
	KrakenIconContainerDetails *details;
	gboolean selection_changed;
	...
	if (details->drag_icon != NULL &&
		...
		if (click_count == 0 &&
			 ...
			 * KrakenList goes the other way because its "links" seem
			 * much more link-like.
			 */
			if (event->button == MIDDLE_BUTTON) {
				activate_selected_items_alternate (container, NULL);
    ...
    if (details->drag_icon != NULL &&
        ...
        if (!details->skip_rename_on_release)
            kraken_icon_container_start_renaming_selected_item (container, FALSE);
    }
}

static gboolean
clicked_within_double_click_interval (KrakenIconContainer *container)
{
	static gint64 last_click_time = 0;
	static gint click_count = 0;
	gint64 current_time;
...
static gboolean
clicked_within_slow_click_interval_on_text (KrakenIconContainer *container, KrakenIcon *icon, GdkEventButton *event)
{
...
static void
clear_drag_state (KrakenIconContainer *container)
{
...
static gboolean
start_stretching (KrakenIconContainer *container)
{
	KrakenIconContainerDetails *details;
	KrakenIcon *icon;
	GtkWidget *toplevel;
	...
	if (!kraken_icon_canvas_item_hit_test_stretch_handles (icon->item,
...
static gboolean
update_stretch_at_idle (KrakenIconContainer *container)
{
	KrakenIconContainerDetails *details;
	KrakenIcon *icon;
	double world_x, world_y;
	...
	kraken_icon_container_icon_set_position (container, icon, world_x, world_y);
...
static void
continue_stretching (KrakenIconContainer *container,
		     double world_x, double world_y)
{

	g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
...
static gboolean
keyboard_stretching (KrakenIconContainer *container,
		     GdkEventKey           *event)
{
	KrakenIcon *icon;
	guint size;
...
static void
ungrab_stretch_icon (KrakenIconContainer *container)
{
...
static void
end_stretching (KrakenIconContainer *container,
		double world_x, double world_y)
{
	...
	if (kraken_icon_container_is_layout_rtl (container)) {
		position.x = icon->saved_ltr_x = kraken_icon_container_get_mirror_x_position (container, icon, icon->x);
	} else {
		position.x = icon->x;
	}
	position.y = icon->y;
...
static gboolean
undo_stretching (KrakenIconContainer *container)
{
	KrakenIcon *stretched_icon;
	...
	kraken_icon_canvas_item_set_show_stretch_handles
		(stretched_icon->item, FALSE);

	kraken_icon_container_icon_set_position (container, stretched_icon,
		      ...
		      GdkEventButton *event)
{
	...
	if (event->button == RUBBERBAND_BUTTON && details->rubberband_info.active) {
        ...
        return GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->button_release_event (widget, event);
	}
	...
	if (event->button == details->drag_button) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			if (!details->drag_started) {
				kraken_icon_container_did_not_drag (container, event);
			} else {
				kraken_icon_dnd_end_drag (container);
		     ...
		     GdkEventMotion *event)
{
	...
	if (details->drag_button != 0) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			...
			if (gtk_drag_check_threshold (widget,
				...
				kraken_icon_dnd_begin_drag (container,
							      actions,
							      details->drag_button,
							      event,
							      canvas_x,
	...
	return GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->motion_notify_event (widget, event);
}

static void
kraken_icon_container_search_position_func (KrakenIconContainer *container,
					      GtkWidget *search_dialog)
{
    ...
    monitor_num = kraken_desktop_utils_get_monitor_for_widget (GTK_WIDGET (container));
     ...
     */
    kraken_desktop_utils_get_monitor_work_rect (monitor_num, &monitor);
...
static void
kraken_icon_container_search_dialog_hide (GtkWidget *search_dialog,
					    KrakenIconContainer *container)
{
...
static gboolean
kraken_icon_container_search_entry_flush_timeout (gpointer data)
{
	KrakenIconContainer *container = data;

	container->details->typeselect_flush_timeout = 0;
	kraken_icon_container_search_dialog_hide (container->details->search_window, container);

	return FALSE;
}

static void
add_search_entry_timeout (KrakenIconContainer *container)
{
	container->details->typeselect_flush_timeout =
		g_timeout_add_seconds (KRAKEN_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT,
				       kraken_icon_container_search_entry_flush_timeout,
				       container);
}

static void
remove_search_entry_timeout (KrakenIconContainer *container)
{
...
static void
reset_search_entry_timeout (KrakenIconContainer *container)
{
	remove_search_entry_timeout (container);
 ...
 */
static void
kraken_icon_container_search_preedit_changed (GtkEntry *entry,
						gchar *preedit,
						KrakenIconContainer *container)
{
...
static void
kraken_icon_container_search_activate (GtkEntry *entry,
					 KrakenIconContainer *container)
{
	kraken_icon_container_search_dialog_hide (container->details->search_window,
						    container);
...
static gboolean
kraken_icon_container_search_delete_event (GtkWidget *widget,
					     GdkEventAny *event,
					     KrakenIconContainer *container)
{
	kraken_icon_container_search_dialog_hide (widget, container);

	return TRUE;
}

static gboolean
kraken_icon_container_search_button_press_event (GtkWidget *widget,
						   GdkEventButton *event,
						   KrakenIconContainer *container)
{
	kraken_icon_container_search_dialog_hide (widget, container);
...
static gboolean
kraken_icon_container_search_entry_button_press_event (GtkWidget *widget,
							 GdkEventButton *event,
							 KrakenIconContainer *container)
{
	reset_search_entry_timeout (container);
...
static void
kraken_icon_container_search_populate_popup (GtkEntry *entry,
					       GtkMenu *menu,
					       KrakenIconContainer *container)
{
	remove_search_entry_timeout (container);
...
void
kraken_icon_container_get_icon_text (KrakenIconContainer *container,
				       KrakenIconData      *data,
				       char                 **editable_text,
				       ...
				       gboolean               include_invisible)
{
	...
	klass = KRAKEN_ICON_CONTAINER_GET_CLASS (container);
...
static gboolean
kraken_icon_container_search_iter (KrakenIconContainer *container,
				     const char *key, gint n)
{
	...
	for (p = container->details->icons; p != NULL && count != n; p = p->next) {
		icon = p->data;
		kraken_icon_container_get_icon_text (container, icon->data, &name,
						       NULL, NULL, NULL, TRUE);
...
static void
kraken_icon_container_search_move (GtkWidget *window,
				     KrakenIconContainer *container,
				     gboolean up)
{
	gboolean ret;
	...
	ret = kraken_icon_container_search_iter (container, text,
	...
	if (ret) {
		...
		/* return to old iter */
		kraken_icon_container_search_iter (container, text,
					container->details->selected_iter);
	}
}

static gboolean
kraken_icon_container_search_scroll_event (GtkWidget *widget,
					     ...
					     KrakenIconContainer *container)
{
	...
	if (event->direction == GDK_SCROLL_UP) {
		kraken_icon_container_search_move (widget, container, TRUE);
		retval = TRUE;
	} else if (event->direction == GDK_SCROLL_DOWN) {
		kraken_icon_container_search_move (widget, container, FALSE);
		retval = TRUE;
	}
...
static gboolean
kraken_icon_container_search_key_press_event (GtkWidget *widget,
						GdkEventKey *event,
						KrakenIconContainer *container)
{
	gboolean retval = FALSE;
	...
	if (event->keyval == GDK_KEY_Return && event->state & GDK_SHIFT_MASK) {
		kraken_icon_container_search_dialog_hide (widget,
							    container);
...
static void
kraken_icon_container_search_init (GtkWidget   *entry,
				     KrakenIconContainer *container)
{
	gint ret;
	gint len;
	...
	ret = kraken_icon_container_search_iter (container, text, 1);
...
static void
kraken_icon_container_ensure_interactive_directory (KrakenIconContainer *container)
{
	...
	g_signal_connect (container->details->search_window, "delete_event",
			  G_CALLBACK (kraken_icon_container_search_delete_event),
			  container);
	g_signal_connect (container->details->search_window, "key_press_event",
			  G_CALLBACK (kraken_icon_container_search_key_press_event),
			  container);
	g_signal_connect (container->details->search_window, "button_press_event",
			  G_CALLBACK (kraken_icon_container_search_button_press_event),
			  container);
	g_signal_connect (container->details->search_window, "scroll_event",
			  G_CALLBACK (kraken_icon_container_search_scroll_event),
			  container);
	...
	g_signal_connect (container->details->search_entry, "populate-popup",
			  G_CALLBACK (kraken_icon_container_search_populate_popup),
			  container);
	g_signal_connect (container->details->search_entry, "activate",
			  G_CALLBACK (kraken_icon_container_search_activate),
			  container);
	g_signal_connect (container->details->search_entry, "preedit-changed",
			  G_CALLBACK (kraken_icon_container_search_preedit_changed),
			  container);
	g_signal_connect (container->details->search_entry, "button-press-event",
			  G_CALLBACK (kraken_icon_container_search_entry_button_press_event),
			  container);
 ...
 */
static gboolean
kraken_icon_container_start_interactive_search (KrakenIconContainer *container)
{
	...
	kraken_icon_container_ensure_interactive_directory (container);

	/* done, show it */
	kraken_icon_container_search_position_func (container, container->details->search_window);
	...
	if (container->details->search_entry_changed_id == 0) {
		container->details->search_entry_changed_id =
			g_signal_connect (container->details->search_entry, "changed",
				G_CALLBACK (kraken_icon_container_search_init),
				container);
	}
	...
	* really non-kosher. */
	...
	kraken_icon_container_search_init (container->details->search_entry, container);

	return TRUE;
}

static gboolean
handle_popups (KrakenIconContainer *container,
	       GdkEventKey           *event,
	       const char            *signal)
{
		 ...
		 GdkEventKey *event)
{
	...
	if (!handled) {
		handled = GTK_WIDGET_CLASS (kraken_icon_container_parent_class)->key_press_event (widget, event);
	}
	...
	if (!handled &&
		...
		if (container->details->imcontext_changed ||    /* we're in a preedit */
			...
			if (kraken_icon_container_start_interactive_search (container)) {
                        ...
                        cairo_t   *cr)
{
     ...
     * This is overridden by kraken-icon-view-grid-container. */
    return;
}
...
get_accessible (GtkWidget *widget)
{
	...
	accessible = g_object_new
		(kraken_icon_container_accessible_get_type (), NULL);
...
static void
real_lay_down_icons (KrakenIconContainer *container,
                     GList             *icons,
                     double start_y)
{
    g_assert_not_reached ();
...
static void
real_icon_set_position (KrakenIconContainer *container,
                        KrakenIcon          *icon,
                        double x,
                        double y)
{
...
static void
real_move_icon (KrakenIconContainer *container,
                KrakenIcon *icon,
                int x, int y,
                double scale,
                gboolean raise,
...
static void
real_update_icon (KrakenIconContainer *container,
                  KrakenIcon *icon,
                  gboolean visible)
{
    g_assert_not_reached ();
...
static void
real_align_icons (KrakenIconContainer *container)
{
    g_assert_not_reached ();
}

static void
real_icon_get_bounding_box (KrakenIcon *icon,
...
static void
real_set_zoom_level (KrakenIconContainer *container,
                     gint               new_level)
{
    g_assert_not_reached ();
}
...
static void
kraken_icon_container_class_init (KrakenIconContainerClass *class)
{
	...
	signals[SELECTION_CHANGED]
		= g_signal_new ("selection_changed",
		                ...
		                G_SIGNAL_RUN_LAST,
		                G_STRUCT_OFFSET (KrakenIconContainerClass,
						 selection_changed),
		                NULL, NULL,
...
static void
update_selected (KrakenIconContainer *container)
{
	GList *node;
	KrakenIcon *icon;
...
{
	update_selected (KRAKEN_ICON_CONTAINER (widget));
...
handle_focus_out_event (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	...
	kraken_icon_container_end_renaming_mode (KRAKEN_ICON_CONTAINER (widget), TRUE);
	update_selected (KRAKEN_ICON_CONTAINER (widget));
...
static void
kraken_icon_container_init (KrakenIconContainer *container)
{
	KrakenIconContainerDetails *details;

	details = g_new0 (KrakenIconContainerDetails, 1);

	details->icon_set = g_hash_table_new (g_direct_hash, g_direct_equal);
	details->layout_timestamp = UNDEFINED_TIME;
	details->zoom_level = KRAKEN_ZOOM_LEVEL_STANDARD;

	details->font_size_table[KRAKEN_ZOOM_LEVEL_SMALLEST] = -2 * PANGO_SCALE;
	details->font_size_table[KRAKEN_ZOOM_LEVEL_SMALLER] = -2 * PANGO_SCALE;
	details->font_size_table[KRAKEN_ZOOM_LEVEL_SMALL] = -0 * PANGO_SCALE;
	details->font_size_table[KRAKEN_ZOOM_LEVEL_STANDARD] = 0 * PANGO_SCALE;
	details->font_size_table[KRAKEN_ZOOM_LEVEL_LARGE] = 0 * PANGO_SCALE;
	details->font_size_table[KRAKEN_ZOOM_LEVEL_LARGER] = 0 * PANGO_SCALE;
	details->font_size_table[KRAKEN_ZOOM_LEVEL_LARGEST] = 0 * PANGO_SCALE;

    details->fixed_text_height = -1;

    details->view_constants = g_new0 (KrakenViewLayoutConstants, 1);
    ...
    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIPS_DESKTOP,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIPS_ICON_VIEW,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIP_FILE_TYPE,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIP_MOD_DATE,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIP_ACCESS_DATE,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIP_FULL_PATH,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);
...
static gboolean
handle_icon_double_click (KrakenIconContainer *container,
			  KrakenIcon *icon,
			  GdkEventButton *event)
{
	KrakenIconContainerDetails *details;
...
static gboolean
handle_icon_slow_two_click (KrakenIconContainer *container,
                                     KrakenIcon *icon,
                               GdkEventButton *event)
{
    KrakenIconContainerDetails *details;

    KrakenFile *file = KRAKEN_FILE (icon->data);

    if (!kraken_file_can_rename (file))
        return FALSE;
    ...
    GList *selection = kraken_icon_container_get_selection (container);
...
static gboolean
handle_icon_button_press (KrakenIconContainer *container,
			  KrakenIcon *icon,
			  GdkEventButton *event)
{
	KrakenIconContainerDetails *details;
    ...
    GList *sel = kraken_icon_container_get_selected_icons (container);
		     ...
		     gpointer data)
{
	...
	icon = KRAKEN_ICON_CANVAS_ITEM (item)->user_data;
	g_assert (icon != NULL);
...
GtkWidget *
kraken_icon_container_new (void)
{
	return gtk_widget_new (KRAKEN_TYPE_ICON_CONTAINER, NULL);
}

/* Clear all of the icons in the container. */
void
kraken_icon_container_clear (KrakenIconContainer *container)
{
	KrakenIconContainerDetails *details;
	GList *p;
...
gboolean
kraken_icon_container_is_empty (KrakenIconContainer *container)
{
	return container->details->icons == NULL;
}

KrakenIconData *
kraken_icon_container_get_first_visible_icon (KrakenIconContainer *container)
{
	GList *l;
	KrakenIcon *icon, *best_icon;
	double x, y;
	double x1, y1, x2, y2;
	double *pos, best_pos;
...
void
kraken_icon_container_scroll_to_icon (KrakenIconContainer  *container,
					KrakenIconData       *data)
{
	GList *l;
	KrakenIcon *icon;
	...
	kraken_icon_container_layout_now (container);
	...
	while (l != NULL) {
		...
		if (icon->data == data &&
		    kraken_icon_container_icon_is_positioned (icon)) {
...
void
kraken_icon_container_for_each (KrakenIconContainer *container,
				  KrakenIconCallback callback,
				  gpointer callback_data)
{
	CallbackAndData callback_and_data;
...
static void
icon_destroy (KrakenIconContainer *container,
	      KrakenIcon *icon)
{
	KrakenIconContainerDetails *details;
	gboolean was_selected;
	KrakenIcon *icon_to_focus;
	GList *item;
...
static void
activate_selected_items (KrakenIconContainer *container)
{
	GList *selection;

	g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));

	selection = kraken_icon_container_get_selection (container);
	if (selection != NULL) {
	  	g_signal_emit (container,
				 signals[ACTIVATE], 0,
				 selection);
...
static void
preview_selected_items (KrakenIconContainer *container)
{
	GList *selection;
	GArray *locations;
	guint idx;
	...
	selection = kraken_icon_container_get_selection (container);
	locations = kraken_icon_container_get_selected_icon_locations (container);
...
static void
activate_selected_items_alternate (KrakenIconContainer *container,
				   KrakenIcon *icon)
{
	GList *selection;
...
KrakenIconInfo *
kraken_icon_container_get_icon_images (KrakenIconContainer *container,
                                     KrakenIconData      *data,
                                     int                size,
...
static void
kraken_icon_container_freeze_updates (KrakenIconContainer *container)
{
	KrakenIconContainerClass *klass;
...
static void
kraken_icon_container_unfreeze_updates (KrakenIconContainer *container)
{
	KrakenIconContainerClass *klass;
...
static gboolean
update_visible_icons_cb (KrakenIconContainer *container)
{
	GtkAdjustment *vadj, *hadj;
	double min_y, max_y;
	double min_x, max_x;
	...
	for (node = g_list_last (container->details->icons); node != NULL; node = node->prev) {
		...
		if (kraken_icon_container_icon_is_positioned (icon)) {
			...
			if (visible) {
				kraken_icon_canvas_item_set_is_visible (icon->item, TRUE);
                KrakenFile *file = KRAKEN_FILE (icon->data);
                ...
                if (!icon->ok_to_show_thumb) {
                    ...
                    if (kraken_file_get_load_deferred_attrs (file) == KRAKEN_FILE_LOAD_DEFERRED_ATTRS_NO) {
                        kraken_file_set_load_deferred_attrs (file, KRAKEN_FILE_LOAD_DEFERRED_ATTRS_YES);
                    }

                    kraken_file_invalidate_attributes (file, KRAKEN_FILE_DEFERRED_ATTRIBUTES);
                } else {
                    gchar *uri = kraken_file_get_uri (file);
                    kraken_thumbnail_prioritize (uri);
                    g_free (uri);
                }

                kraken_icon_container_update_icon (container, icon);
			} else {
				kraken_icon_canvas_item_set_is_visible (icon->item, FALSE);
			}
		}
	}
...
static void
queue_update_visible_icons(KrakenIconContainer *container,
                           gint               delay)
{
    KrakenIconContainerDetails *details = container->details;
...
static gboolean
is_old_or_unknown_icon_data (KrakenIconContainer *container,
			     KrakenIconData *data)
{
	time_t timestamp;
	gboolean success;
    ...
    is_transient = KRAKEN_IS_DESKTOP_ICON_FILE (data) && container->details->keep_aligned;
...
gboolean
kraken_icon_container_icon_is_new_for_monitor (KrakenIconContainer *container,
                                             KrakenIcon          *icon,
                                             gint               current_monitor)
{
    ...
    return kraken_file_get_monitor_number (KRAKEN_FILE (icon->data)) != current_monitor;
}

/**
 * kraken_icon_container_add:
 * @container: A KrakenIconContainer
 * @data: Icon data.
 *
 ...
 **/
gboolean
kraken_icon_container_add (KrakenIconContainer *container,
			     KrakenIconData *data)
{
	KrakenIconContainerDetails *details;
	KrakenIcon *icon;
	...
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);
	...
	icon->scale = 1.0;
 	icon->item = KRAKEN_ICON_CANVAS_ITEM
		(eel_canvas_item_new (EEL_CANVAS_GROUP (EEL_CANVAS (container)->root),
				      kraken_icon_canvas_item_get_type (),
				      "visible", FALSE,
				      NULL));
	...
	item = EEL_CANVAS_ITEM (icon->item);
	band = KRAKEN_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle;
	if (band) {
...
void
kraken_icon_container_layout_now (KrakenIconContainer *container)
{
	if (container->details->idle_id != 0) {
		unschedule_redo_layout (container);
		redo_layout_internal (container);
 ...
 **/
gboolean
kraken_icon_container_remove (KrakenIconContainer *container,
				KrakenIconData *data)
{
	KrakenIcon *icon;
 ...
 **/
void
kraken_icon_container_request_update (KrakenIconContainer *container,
					KrakenIconData *data)
{
	KrakenIcon *icon;
...
void
kraken_icon_container_invalidate_labels (KrakenIconContainer *container)
{
    GList *p;
    KrakenIcon *icon;
    ...
    for (p = container->details->icons; p != NULL; p = p->next) {
        ...
        kraken_icon_canvas_item_invalidate_label (icon->item);
    }
}
...
KrakenZoomLevel
kraken_icon_container_get_zoom_level (KrakenIconContainer *container)
{
    return container->details->zoom_level;
}

void
kraken_icon_container_set_zoom_level (KrakenIconContainer *container,
                                    gint               new_level)
{
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->set_zoom_level (container, new_level);
...
/**
 * kraken_icon_container_request_update_all:
 ...
 **/
void
kraken_icon_container_request_update_all (KrakenIconContainer *container)
{
	GList *node;
	KrakenIcon *icon;
 ...
 */
void
kraken_icon_container_reveal (KrakenIconContainer *container, KrakenIconData *data)
{
	KrakenIcon *icon;
...
GList *
kraken_icon_container_get_real_selection (KrakenIconContainer *container)
{
    GList *list, *p;
...
/**
 * kraken_icon_container_get_selection:
 * @container: An icon container.
 *
 ...
 **/
GList *
kraken_icon_container_get_selection (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), NULL);

    kraken_icon_container_update_selection (container);

    return g_list_copy (container->details->current_selection);
}

/**
 * kraken_icon_container_peek_selection:
 * @container: An icon container.
 *
 ...
 * KrakenIconContainer and should not be freed.
 **/
GList *
kraken_icon_container_peek_selection (KrakenIconContainer *container)
{
    g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), NULL);
...
gint
kraken_icon_container_get_selection_count (KrakenIconContainer *container)
{
    g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), 0);
...
static GList *
kraken_icon_container_get_selected_icons (KrakenIconContainer *container)
{
	GList *list, *p;
 ...
 **/
void
kraken_icon_container_invert_selection (KrakenIconContainer *container)
{
	GList *p;
...
static GArray *
kraken_icon_container_get_icon_locations (KrakenIconContainer *container,
					    GList *icons)
{
	GArray *result;
	GList *node;
	...
	for (index = 0, node = icons; node != NULL; index++, node = node->next) {
	     	g_array_index (result, GdkPoint, index).x =
	     		((KrakenIcon *)node->data)->x;
	     	g_array_index (result, GdkPoint, index).y =
			((KrakenIcon *)node->data)->y;
	}
...
/**
 * kraken_icon_container_get_selected_icon_locations:
 * @container: An icon container widget.
 *
 * Returns an array of GdkPoints of locations of the selected icons.
 **/
GArray *
kraken_icon_container_get_selected_icon_locations (KrakenIconContainer *container)
{
	GArray *result;
	GList *icons;
	...
	icons = kraken_icon_container_get_selected_icons (container);
	result = kraken_icon_container_get_icon_locations (container, icons);
	g_list_free (icons);
...
/**
 * kraken_icon_container_select_all:
 * @container: An icon container widget.
 *
 * Select all the icons in @container at once.
 **/
void
kraken_icon_container_select_all (KrakenIconContainer *container)
{
	gboolean selection_changed;
	GList *p;
	KrakenIcon *icon;
...
/**
 * kraken_icon_container_select_list_unselect_others.
 * @container: An icon container widget.
 * @selection: A list of KrakenIcon *.
 *
 * Set the selection to exactly the icons in @selection.
 **/
void
kraken_icon_container_select_list_unselect_others (KrakenIconContainer *container,
						     GList *selection)
{
	gboolean selection_changed;
	GHashTable *hash;
...
/**
 * kraken_icon_container_unselect_all:
 * @container: An icon container widget.
 *
 * Deselect all the icons in @container.
 **/
void
kraken_icon_container_unselect_all (KrakenIconContainer *container)
{
...
/**
 * kraken_icon_container_get_icon_by_uri:
 * @container: An icon container widget.
 ...
 **/
KrakenIcon *
kraken_icon_container_get_icon_by_uri (KrakenIconContainer *container,
					 const char *uri)
{
	KrakenIconContainerDetails *details;
	GList *p;
	...
	*/
	...
	for (p = details->icons; p != NULL; p = p->next) {
		...
		icon_uri = kraken_icon_container_get_icon_uri
			(container, icon);
...
static KrakenIcon *
get_nth_selected_icon (KrakenIconContainer *container, int index)
{
	GList *p;
	KrakenIcon *icon;
	int selection_count;
...
static KrakenIcon *
get_first_selected_icon (KrakenIconContainer *container)
{
        return get_nth_selected_icon (container, 1);
}

static gboolean
has_multiple_selection (KrakenIconContainer *container)
{
        return get_nth_selected_icon (container, 2) != NULL;
}

static gboolean
all_selected (KrakenIconContainer *container)
{
	GList *p;
	KrakenIcon *icon;
...
static gboolean
has_selection (KrakenIconContainer *container)
{
        return get_nth_selected_icon (container, 1) != NULL;
}

/**
 * kraken_icon_container_show_stretch_handles:
 * @container: An icon container widget.
 *
 * Makes stretch handles visible on the first selected icon.
 **/
void
kraken_icon_container_show_stretch_handles (KrakenIconContainer *container)
{
	KrakenIconContainerDetails *details;
	KrakenIcon *icon;
	guint initial_size;
	...
	kraken_icon_canvas_item_set_show_stretch_handles (icon->item, TRUE);
...
/**
 * kraken_icon_container_has_stretch_handles
 * @container: An icon container widget.
 *
 * Returns true if the first selected item has stretch handles.
 **/
gboolean
kraken_icon_container_has_stretch_handles (KrakenIconContainer *container)
{
	KrakenIcon *icon;
...
/**
 * kraken_icon_container_is_stretched
 * @container: An icon container widget.
 *
 * Returns true if the any selected item is stretched to a size other than 1.0.
 **/
gboolean
kraken_icon_container_is_stretched (KrakenIconContainer *container)
{
	GList *p;
	KrakenIcon *icon;
...
/**
 * kraken_icon_container_unstretch
 * @container: An icon container widget.
 *
 * Gets rid of any icon stretching.
 **/
void
kraken_icon_container_unstretch (KrakenIconContainer *container)
{
	GList *p;
	KrakenIcon *icon;
...
char *
kraken_icon_container_get_icon_uri (KrakenIconContainer *container,
				      KrakenIcon *icon)
{
	char *uri;
...
char *
kraken_icon_container_get_icon_drop_target_uri (KrakenIconContainer *container,
				   	     	  KrakenIcon *icon)
{
	char *uri;
...
void
kraken_icon_container_update_tooltip_text (KrakenIconContainer  *container,
 ...
 */
static void
reset_scroll_region_if_not_empty (KrakenIconContainer *container)
{
	if (!kraken_icon_container_is_empty (container)) {
		kraken_icon_container_reset_scroll_region (container);
	}
}
 ...
 */
void
kraken_icon_container_set_auto_layout (KrakenIconContainer *container,
...
void
kraken_icon_container_set_horizontal_layout (KrakenIconContainer *container,
                                           gboolean           horizontal)
{
    ...
    if (dir == GTK_TEXT_DIR_LTR) {
        layout_mode = horizontal ? KRAKEN_ICON_LAYOUT_L_R_T_B : KRAKEN_ICON_LAYOUT_T_B_L_R;
    } else {
        layout_mode = horizontal ? KRAKEN_ICON_LAYOUT_R_L_T_B : KRAKEN_ICON_LAYOUT_T_B_R_L;
    }
...
gboolean
kraken_icon_container_get_horizontal_layout (KrakenIconContainer *container)
{
    g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);
...
void
kraken_icon_container_set_grid_adjusts (KrakenIconContainer *container,
                                      gint               h_adjust,
                                      gint               v_adjust)
{
    g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
...
gboolean
kraken_icon_container_is_keep_aligned (KrakenIconContainer *container)
{
...
static void
unschedule_align_icons (KrakenIconContainer *container)
{
        if (container->details->align_idle_id != 0) {
		g_source_remove (container->details->align_idle_id);
		container->details->align_idle_id = 0;
...
static void
schedule_align_icons (KrakenIconContainer *container)
{
	if (container->details->align_idle_id == 0
	    && container->details->has_been_allocated) {
...
void
kraken_icon_container_set_keep_aligned (KrakenIconContainer *container,
...
void
kraken_icon_container_set_layout_mode (KrakenIconContainer *container,
...
void
kraken_icon_container_set_label_position (KrakenIconContainer *container,
					    KrakenIconLabelPosition position)
{
	g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
 ...
 */
void
kraken_icon_container_freeze_icon_positions (KrakenIconContainer *container)
{
	gboolean changed;
	GList *p;
	KrakenIcon *icon;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		position.y = icon->y;
		position.scale = icon->scale;
        position.monitor = kraken_desktop_utils_get_monitor_for_widget (GTK_WIDGET (container));
...
void
kraken_icon_container_sort (KrakenIconContainer *container)
{
	gboolean changed;
...
gboolean
kraken_icon_container_is_auto_layout (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);
...
static void
pending_icon_to_rename_destroy_callback (KrakenIconCanvasItem *item, KrakenIconContainer *container)
{
...
static KrakenIcon*
get_pending_icon_to_rename (KrakenIconContainer *container)
{
	return container->details->pending_icon_to_rename;
}

static void
set_pending_icon_to_rename (KrakenIconContainer *container, KrakenIcon *icon)
{
	KrakenIcon *old_icon;
...
static void
process_pending_icon_to_rename (KrakenIconContainer *container)
{
...
static gboolean
is_renaming_pending (KrakenIconContainer *container)
{
	return get_pending_icon_to_rename (container) != NULL;
}

static gboolean
is_renaming (KrakenIconContainer *container)
{
 ...
 **/
void
kraken_icon_container_start_renaming_selected_item (KrakenIconContainer *container,
						      gboolean select_all)
{
	KrakenIconContainerDetails *details;
	KrakenIcon *icon;
	...
	editable_text = kraken_icon_canvas_item_get_editable_text (icon->item);
	...
	kraken_icon_container_freeze_updates (container);
	...
	if (details->rename_widget == NULL) {
		...
		if (details->label_position != KRAKEN_ICON_LABEL_POSITION_BESIDE) {
	...
	icon_rect = kraken_icon_canvas_item_get_icon_rectangle (icon->item);
    text_rect = kraken_icon_canvas_item_get_text_rectangle (icon->item, TRUE);

	if (kraken_icon_container_is_layout_vertical (container) &&
	    container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE) {
		/* for one-line editables, the width changes dynamically */
		width = -1;
	} else {
		width = kraken_icon_canvas_item_get_max_text_width (icon->item);
	}
	...
	if (select_all) {
		...
		if (kraken_file_is_directory (KRAKEN_FILE (icon->data))) {
			start_offset = 0;
			end_offset = -1;
		} else {
	...
	kraken_icon_canvas_item_set_renaming (icon->item, TRUE);
}

KrakenIcon *
kraken_icon_container_get_icon_being_renamed (KrakenIconContainer *container)
{
    KrakenIcon *rename_icon;
...
void
kraken_icon_container_end_renaming_mode (KrakenIconContainer *container, gboolean commit)
{
	KrakenIcon *icon;
	...
	kraken_icon_canvas_item_set_renaming (icon->item, FALSE);
    ...
    kraken_icon_container_unfreeze_updates (container);
}

void
kraken_icon_container_set_single_click_mode (KrakenIconContainer *container,
...
void
kraken_icon_container_set_click_to_rename_enabled (KrakenIconContainer *container,
                                                           gboolean enabled)
{
    g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
...
gboolean
kraken_icon_container_get_is_fixed_size (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);
...
void
kraken_icon_container_set_is_fixed_size (KrakenIconContainer *container,
...
gboolean
kraken_icon_container_get_is_desktop (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);
...
void
kraken_icon_container_set_is_desktop (KrakenIconContainer *container,
					   gboolean is_desktop)
{
    g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
    ...
    if (is_desktop) {
        ...
        context = gtk_widget_get_style_context (GTK_WIDGET (container));
        gtk_style_context_add_class (context, "kraken-desktop");
        ...
        g_signal_connect_swapped (kraken_icon_view_preferences,
                                  "changed::" KRAKEN_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT,
                                  G_CALLBACK (text_ellipsis_limit_changed_container_callback),
                                  container);
    }
}
...
void
kraken_icon_container_set_margins (KrakenIconContainer *container,
				     int left_margin,
				     int right_margin,
				     int top_margin,
				     int bottom_margin)
...
void
kraken_icon_container_set_use_drop_shadows (KrakenIconContainer  *container,
...
void
kraken_icon_container_set_font (KrakenIconContainer *container,
				  const char *font)
{
	g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
...
void
kraken_icon_container_set_font_size_table (KrakenIconContainer *container,
					     const int font_size_table[KRAKEN_ZOOM_LEVEL_LARGEST + 1])
{
	...
	for (i = 0; i <= KRAKEN_ZOOM_LEVEL_LARGEST; i++) {
...
/**
 * kraken_icon_container_get_icon_description
 * @container: An icon container widget.
 * @data: Icon data
 *
 ...
 **/
char*
kraken_icon_container_get_icon_description (KrakenIconContainer *container,
				              KrakenIconData      *data)
{
	KrakenIconContainerClass *klass;
...
gboolean
kraken_icon_container_get_allow_moves (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);
...
void
kraken_icon_container_set_allow_moves	(KrakenIconContainer *container,
...
void
kraken_icon_container_set_forced_icon_size (KrakenIconContainer *container,
...
void
kraken_icon_container_set_all_columns_same_width (KrakenIconContainer *container,
 ...
 **/
void
kraken_icon_container_set_highlighted_for_clipboard (KrakenIconContainer *container,
...
/* KrakenIconContainerAccessible */

static KrakenIconContainerAccessiblePrivate *
accessible_get_priv (AtkObject *accessible)
{
	KrakenIconContainerAccessiblePrivate *priv;
...
static gboolean
kraken_icon_container_accessible_do_action (AtkAction *accessible, int i)
{
	GtkWidget *widget;
	KrakenIconContainer *container;
	GList *selection;
	...
	case ACTION_MENU :
		...
		break;
	default :
		g_warning ("Invalid action passed to KrakenIconContainerAccessible::do_action");
		return FALSE;
	}
	return TRUE;
}
...
static int
kraken_icon_container_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
kraken_icon_container_accessible_action_get_description (AtkAction *accessible,
							   int i)
{
	KrakenIconContainerAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return kraken_icon_container_accessible_action_descriptions[i];
	}
}

static const char *
kraken_icon_container_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return kraken_icon_container_accessible_action_names[i];
}

static const char *
kraken_icon_container_accessible_action_get_keybinding (AtkAction *accessible,
							  int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
kraken_icon_container_accessible_action_set_description (AtkAction *accessible,
							   int i,
...
static void
kraken_icon_container_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = kraken_icon_container_accessible_do_action;
	iface->get_n_actions = kraken_icon_container_accessible_get_n_actions;
	iface->get_description = kraken_icon_container_accessible_action_get_description;
	iface->get_name = kraken_icon_container_accessible_action_get_name;
	iface->get_keybinding = kraken_icon_container_accessible_action_get_keybinding;
	iface->set_description = kraken_icon_container_accessible_action_set_description;
}

/* AtkSelection interface */

static void
kraken_icon_container_accessible_update_selection (AtkObject *accessible)
{
	KrakenIconContainer *container;
	KrakenIconContainerAccessiblePrivate *priv;
	GList *l;
	...
	container = KRAKEN_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
...
static void
kraken_icon_container_accessible_selection_changed_cb (KrakenIconContainer *container,
							 gpointer data)
{
...
static void
kraken_icon_container_accessible_icon_added_cb (KrakenIconContainer *container,
...
static void
kraken_icon_container_accessible_icon_removed_cb (KrakenIconContainer *container,
						    KrakenIconData *icon_data,
						    gpointer data)
{
	KrakenIcon *icon;
...
static void
kraken_icon_container_accessible_cleared_cb (KrakenIconContainer *container,
					       gpointer data)
{
...
static gboolean
kraken_icon_container_accessible_add_selection (AtkSelection *accessible,
						  int i)
{
	GtkWidget *widget;
	KrakenIconContainer *container;
	...
	if (icon) {
		...
		selection = g_list_prepend (selection,
					    icon->data);
		kraken_icon_container_set_selection (container, selection);
...
static gboolean
kraken_icon_container_accessible_clear_selection (AtkSelection *accessible)
{
	GtkWidget *widget;
	KrakenIconContainer *container;
	...
	kraken_icon_container_unselect_all (container);

	return TRUE;
}

static AtkObject *
kraken_icon_container_accessible_ref_selection (AtkSelection *accessible,
						  int i)
{
	...
	kraken_icon_container_accessible_update_selection (ATK_OBJECT (accessible));
	priv = accessible_get_priv (ATK_OBJECT (accessible));
...
static int
kraken_icon_container_accessible_get_selection_count (AtkSelection *accessible)
{
	int count;
	KrakenIconContainerAccessiblePrivate *priv;
...
static gboolean
kraken_icon_container_accessible_is_child_selected (AtkSelection *accessible,
						      int i)
{
	KrakenIconContainer *container;
	KrakenIcon *icon;
...
static gboolean
kraken_icon_container_accessible_remove_selection (AtkSelection *accessible,
						     int i)
{
	KrakenIconContainer *container;
	KrakenIconContainerAccessiblePrivate *priv;
...
static gboolean
kraken_icon_container_accessible_select_all_selection (AtkSelection *accessible)
{
	KrakenIconContainer *container;
	GtkWidget *widget;
	...
	kraken_icon_container_select_all (container);

	return TRUE;
}

void
kraken_icon_container_widget_to_file_operation_position (KrakenIconContainer *container,
							   GdkPoint              *position)
{
	double x, y;
	...
	position->x -= kraken_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
	position->y -= kraken_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
}

static void
kraken_icon_container_accessible_selection_interface_init (AtkSelectionIface *iface)
{
	iface->add_selection = kraken_icon_container_accessible_add_selection;
	iface->clear_selection = kraken_icon_container_accessible_clear_selection;
	iface->ref_selection = kraken_icon_container_accessible_ref_selection;
	iface->get_selection_count = kraken_icon_container_accessible_get_selection_count;
	iface->is_child_selected = kraken_icon_container_accessible_is_child_selected;
	iface->remove_selection = kraken_icon_container_accessible_remove_selection;
	iface->select_all_selection = kraken_icon_container_accessible_select_all_selection;
}


static gint
kraken_icon_container_accessible_get_n_children (AtkObject *accessible)
{
	KrakenIconContainer *container;
	GtkWidget *widget;
	gint i;
...
static AtkObject*
kraken_icon_container_accessible_ref_child (AtkObject *accessible, int i)
{
        AtkObject *atk_object;
        KrakenIconContainer *container;
        KrakenIcon *icon;
...
static void
kraken_icon_container_accessible_initialize (AtkObject *accessible,
					       gpointer data)
{
	KrakenIconContainer *container;
	KrakenIconContainerAccessiblePrivate *priv;
	...
	priv = g_new0 (KrakenIconContainerAccessiblePrivate, 1);
	g_object_set_qdata (G_OBJECT (accessible),
			    accessible_private_data_quark,
			    priv);

	if (GTK_IS_ACCESSIBLE (accessible)) {
		kraken_icon_container_accessible_update_selection
			(ATK_OBJECT (accessible));

		container = KRAKEN_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
		g_signal_connect (G_OBJECT (container), "selection_changed",
				  G_CALLBACK (kraken_icon_container_accessible_selection_changed_cb),
				  accessible);
		g_signal_connect (G_OBJECT (container), "icon_added",
				  G_CALLBACK (kraken_icon_container_accessible_icon_added_cb),
				  accessible);
		g_signal_connect (G_OBJECT (container), "icon_removed",
				  G_CALLBACK (kraken_icon_container_accessible_icon_removed_cb),
				  accessible);
		g_signal_connect (G_OBJECT (container), "cleared",
				  G_CALLBACK (kraken_icon_container_accessible_cleared_cb),
				  accessible);
	}
}

static void
kraken_icon_container_accessible_finalize (GObject *object)
{
	KrakenIconContainerAccessiblePrivate *priv;
	int i;
...
static void
kraken_icon_container_accessible_class_init (AtkObjectClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	accessible_parent_class = g_type_class_peek_parent (klass);

	gobject_class->finalize = kraken_icon_container_accessible_finalize;

	klass->get_n_children = kraken_icon_container_accessible_get_n_children;
	klass->ref_child = kraken_icon_container_accessible_ref_child;
	klass->initialize = kraken_icon_container_accessible_initialize;

	accessible_private_data_quark = g_quark_from_static_string ("icon-container-accessible-private-data");
}

static GType
kraken_icon_container_accessible_get_type (void)
{
        static GType type = 0;

        if (!type) {
                static GInterfaceInfo atk_action_info = {
                        (GInterfaceInitFunc) kraken_icon_container_accessible_action_interface_init,
                        (GInterfaceFinalizeFunc) NULL,
                        NULL
                };

                static GInterfaceInfo atk_selection_info = {
                        (GInterfaceInitFunc) kraken_icon_container_accessible_selection_interface_init,
                        (GInterfaceFinalizeFunc) NULL,
                        NULL
                };
		...
		type = eel_accessibility_create_derived_type
			("KrakenIconContainerAccessible",
			 EEL_TYPE_CANVAS,
			 kraken_icon_container_accessible_class_init);
...
gboolean
kraken_icon_container_is_layout_rtl (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), 0);
...
gboolean
kraken_icon_container_is_layout_vertical (KrakenIconContainer *container)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CONTAINER (container), FALSE);

	return (container->details->layout_mode == KRAKEN_ICON_LAYOUT_T_B_L_R ||
		container->details->layout_mode == KRAKEN_ICON_LAYOUT_T_B_R_L);
}

gint
kraken_icon_container_get_max_layout_lines_for_pango (KrakenIconContainer  *container)
{
    return KRAKEN_ICON_CONTAINER_GET_CLASS (container)->get_max_layout_lines_for_pango (container);
}

gint
kraken_icon_container_get_max_layout_lines (KrakenIconContainer  *container)
{
    return KRAKEN_ICON_CONTAINER_GET_CLASS (container)->get_max_layout_lines (container);
}

void
kraken_icon_container_begin_loading (KrakenIconContainer *container)
{
	gboolean dummy;

    clear_drag_state (container);

	if (kraken_icon_container_get_store_layout_timestamps (container)) {
...
void
kraken_icon_container_store_layout_timestamps_now (KrakenIconContainer *container)
{
	KrakenIcon *icon;
	GList *p;
	gboolean dummy;
...
void
kraken_icon_container_end_loading (KrakenIconContainer *container,
				     gboolean               all_icons_added)
{
	if (all_icons_added &&
	    kraken_icon_container_get_store_layout_timestamps (container)) {
...
gboolean
kraken_icon_container_get_store_layout_timestamps (KrakenIconContainer *container)
{
...
void
kraken_icon_container_set_store_layout_timestamps (KrakenIconContainer *container,
...
gint
kraken_icon_container_get_canvas_height (KrakenIconContainer *container,
                                       GtkAllocation      allocation)
{
...
gint
kraken_icon_container_get_canvas_width (KrakenIconContainer *container,
                                      GtkAllocation      allocation)
{
...
double
kraken_icon_container_get_mirror_x_position (KrakenIconContainer *container, KrakenIcon *icon, double x)
{
    ...
    return kraken_icon_container_get_canvas_width (container, allocation) - x - (icon_bounds.x1 - icon_bounds.x0);
}

void
kraken_icon_container_set_rtl_positions (KrakenIconContainer *container)
{
    GList *l;
    KrakenIcon *icon;
    double x;
    ...
    for (l = container->details->icons; l != NULL; l = l->next) {
        icon = l->data;
        x = kraken_icon_container_get_mirror_x_position (container, icon, icon->saved_ltr_x);
        kraken_icon_container_icon_set_position (container, icon, x, icon->y);
    }
}

void
kraken_icon_container_sort_icons (KrakenIconContainer *container,
        GList                **icons)
{
    KrakenIconContainerClass *klass;
...
void
kraken_icon_container_resort (KrakenIconContainer *container)
{
    kraken_icon_container_sort_icons (container, &container->details->icons);
}

void
kraken_icon_container_icon_raise (KrakenIconContainer *container, KrakenIcon *icon)
{
...
void
kraken_icon_container_finish_adding_icon (KrakenIconContainer *container,
            KrakenIcon *icon)
{
...
void
kraken_icon_container_update_selection (KrakenIconContainer *container)
{
    g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
    ...
    container->details->current_selection = kraken_icon_container_get_real_selection (container);
    container->details->current_selection_count = g_list_length (container->details->current_selection);
}

void
kraken_icon_container_move_icon (KrakenIconContainer *container,
                   KrakenIcon *icon,
                   int x, int y,
                   double scale,
                   gboolean raise,
                   ...
                   gboolean update_position)
{
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->move_icon (container,
                                                          icon,
                                                          x, y,
                                                          scale,
                                                          raise,
...
void
kraken_icon_container_icon_set_position (KrakenIconContainer *container,
                                       KrakenIcon          *icon,
                                       gdouble            x,
                                       gdouble            y)
{
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->icon_set_position (container, icon, x, y);
}

void
kraken_icon_container_icon_get_bounding_box (KrakenIconContainer *container,
                                           KrakenIcon *icon,
                                           int *x1_return, int *y1_return,
                                           int *x2_return, int *y2_return,
                                           KrakenIconCanvasItemBoundsUsage usage)
{
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->icon_get_bounding_box (icon, x1_return, y1_return, x2_return, y2_return, usage);
}

void
kraken_icon_container_update_icon (KrakenIconContainer *container,
                                 KrakenIcon          *icon)
{
    gboolean ok = FALSE;
    ...
    if (icon != NULL) {
        ...
        ok = icon->ok_to_show_thumb ||
             (kraken_file_get_load_deferred_attrs (file) == KRAKEN_FILE_LOAD_DEFERRED_ATTRS_PRELOAD);
    }

    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->update_icon (container, icon, ok);
}

gint
kraken_icon_container_get_additional_text_line_count (KrakenIconContainer *container)
{
    return KRAKEN_ICON_CONTAINER_GET_CLASS (container)->get_additional_text_line_count (container);
}

void
kraken_icon_container_set_ok_to_load_deferred_attrs (KrakenIconContainer *container,
                                                   gboolean           ok)
{
...
void
kraken_self_check_icon_container (void)
{
