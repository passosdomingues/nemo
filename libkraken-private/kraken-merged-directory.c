/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-merged-directory.c: Subclass of KrakenDirectory to implement the
...
#include <config.h>
#include "kraken-merged-directory.h"
...
struct KrakenMergedDirectoryDetails {
	GList *directories;
...
G_DEFINE_TYPE (KrakenMergedDirectory, kraken_merged_directory,
...
merged_callback_destroy (MergedCallback *merged_callback)
{
	...
	g_assert (KRAKEN_IS_MERGED_DIRECTORY (merged_callback->merged));

	g_list_free (merged_callback->non_ready_directories);
	kraken_file_list_free (merged_callback->merged_file_list);
...
merged_callback_check_done (MergedCallback *merged_callback)
{
	...
	(* merged_callback->callback) (KRAKEN_DIRECTORY (merged_callback->merged),
...
static void
directory_ready_callback (KrakenDirectory *directory,
			  GList *files,
			  gpointer callback_data)
{
...
static void
merged_call_when_ready (KrakenDirectory *directory,
			...
			gpointer callback_data)
{
	...
	merged = KRAKEN_MERGED_DIRECTORY (directory);
...
static void
merged_cancel_callback (KrakenDirectory *directory,
			KrakenDirectoryCallback callback,
			gpointer callback_data)
{
	KrakenMergedDirectory *merged;
	...
	for (node = merged_callback->non_ready_directories; node != NULL; node = node->next) {
		kraken_directory_cancel_callback
			(node->data,
			 directory_ready_callback, merged_callback);
	}
	merged_callback_destroy (merged_callback);
...
static void
build_merged_callback_list (KrakenDirectory *directory,
			    GList *file_list,
			    gpointer callback_data)
{
	GList **merged_list;
	...
	*merged_list = g_list_concat (*merged_list,
				      kraken_file_list_copy (file_list));
}

/* Create a monitor on each of the directories in the list. */
static void
merged_monitor_add (KrakenDirectory *directory,
		    ...
		    gpointer callback_data)
{
	...
	kraken_file_list_free (merged_callback_list);
}

static void
merged_monitor_destroy (KrakenMergedDirectory *merged, MergedMonitor *monitor)
{
	GList *node;

	/* Call through to the real directory remove calls. */
	for (node = merged->details->directories; node != NULL; node = node->next) {
		kraken_directory_file_monitor_remove (node->data, monitor);
	}
...
static void
merged_monitor_remove (KrakenDirectory *directory,
		       gconstpointer client)
{
	KrakenMergedDirectory *merged;
	MergedMonitor *monitor;
...
static void
merged_force_reload (KrakenDirectory *directory)
{
	KrakenMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		kraken_directory_force_reload (node->data);
	}
}

/* Return true if any directory in the list does. */
static gboolean
merged_contains_file (KrakenDirectory *directory,
		      KrakenFile *file)
{
	KrakenMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		if (kraken_directory_contains_file (node->data, file)) {
			return TRUE;
		}
	}
	return FALSE;
...
static gboolean
merged_are_all_files_seen (KrakenDirectory *directory)
{
	KrakenMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		if (!kraken_directory_are_all_files_seen (node->data)) {
			return FALSE;
		}
	}
	return TRUE;
...
/* Return true if any directory in the list does. */
static gboolean
merged_is_not_empty (KrakenDirectory *directory)
{
	KrakenMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		if (kraken_directory_is_not_empty (node->data)) {
			return TRUE;
		}
	}
	return FALSE;
...
static GList *
merged_get_file_list (KrakenDirectory *directory)
{
	...
	dirs_file_list = NULL;
	dir_list = KRAKEN_MERGED_DIRECTORY (directory)->details->directories;

	for (cur_node = dir_list; cur_node != NULL; cur_node = cur_node->next) {
		KrakenDirectory *cur_dir;

		cur_dir = KRAKEN_DIRECTORY (cur_node->data);
		dirs_file_list = g_list_concat (dirs_file_list,
						 kraken_directory_get_file_list (cur_dir));
	}

	merged_dir_file_list = KRAKEN_DIRECTORY_CLASS 
		(kraken_merged_directory_parent_class)->get_file_list (directory);

	return g_list_concat (dirs_file_list, merged_dir_file_list);
}

static void
forward_files_added_cover (KrakenDirectory *real_directory,
			   GList *files,
			   gpointer callback_data)
{
	kraken_directory_emit_files_added (KRAKEN_DIRECTORY (callback_data), files);
}

static void
forward_files_changed_cover (KrakenDirectory *real_directory,
			     GList *files,
			     gpointer callback_data)
{
	kraken_directory_emit_files_changed (KRAKEN_DIRECTORY (callback_data), files);
}

static void
done_loading_callback (KrakenDirectory *real_directory,
		       KrakenMergedDirectory *merged)
{
	...
	if (merged->details->directories_not_done_loading == NULL) {
		kraken_directory_emit_done_loading (KRAKEN_DIRECTORY (merged));
	}
}
		       ...
		       gpointer callback_data)
{
	...
	monitor = value;
	kraken_directory_file_monitor_add
		(KRAKEN_DIRECTORY (callback_data), monitor,
...
static void
merged_add_real_directory (KrakenMergedDirectory *merged,
			   KrakenDirectory *real_directory)
{
	g_return_if_fail (KRAKEN_IS_MERGED_DIRECTORY (merged));
	g_return_if_fail (KRAKEN_IS_DIRECTORY (real_directory));
	g_return_if_fail (!KRAKEN_IS_MERGED_DIRECTORY (real_directory));
	g_return_if_fail (g_list_find (merged->details->directories, real_directory) == NULL);

	/* Add to our list of directories. */
	kraken_directory_ref (real_directory);
...
void
kraken_merged_directory_add_real_directory (KrakenMergedDirectory *merged,
					      KrakenDirectory *real_directory)
{
	g_return_if_fail (KRAKEN_IS_MERGED_DIRECTORY (merged));
	g_return_if_fail (KRAKEN_IS_DIRECTORY (real_directory));
...
GList *
kraken_merged_directory_get_real_directories (KrakenMergedDirectory *merged)
{
					...
					gpointer callback_data)
{
	merged_callback_remove_directory
		(value, KRAKEN_DIRECTORY (callback_data));
}
...
monitor_remove_directory (gpointer key,
			  ...
			  gpointer callback_data)
{
	kraken_directory_file_monitor_remove
		(KRAKEN_DIRECTORY (callback_data), value);
}

static void
real_directory_notify_files_removed (KrakenDirectory *real_directory)
{
	GList *files, *l;

	files = kraken_directory_get_file_list (real_directory);
	...
	if (files) {
		kraken_directory_notify_files_removed_by_uri (files);
	}
...
static void
merged_remove_real_directory (KrakenMergedDirectory *merged,
			      KrakenDirectory *real_directory)
{
	g_return_if_fail (KRAKEN_IS_MERGED_DIRECTORY (merged));
	g_return_if_fail (KRAKEN_IS_DIRECTORY (real_directory));
	...
	merged->details->directories_not_done_loading = g_list_remove
		(merged->details->directories_not_done_loading, real_directory);
	kraken_directory_unref (real_directory);
}

void
kraken_merged_directory_remove_real_directory (KrakenMergedDirectory *merged,
						 KrakenDirectory *real_directory)
{
	g_return_if_fail (KRAKEN_IS_MERGED_DIRECTORY (merged));
...
merged_finalize (GObject *object)
{
	...
	merged = KRAKEN_MERGED_DIRECTORY (object);
	...
	kraken_directory_list_free (merged->details->directories);
	g_list_free (merged->details->directories_not_done_loading);

	G_OBJECT_CLASS (kraken_merged_directory_parent_class)->finalize (object);
}

static void
kraken_merged_directory_init (KrakenMergedDirectory *merged)
{
	merged->details = G_TYPE_INSTANCE_GET_PRIVATE (merged, KRAKEN_TYPE_MERGED_DIRECTORY,
						       KrakenMergedDirectoryDetails);
...
static void
kraken_merged_directory_class_init (KrakenMergedDirectoryClass *class)
{
	 ...
	 * the files from each of the directories in KrakenMergedDirectory->details->directories.
         */
	...
	g_type_class_add_private (class, sizeof (KrakenMergedDirectoryDetails));

	signals[ADD_REAL_DIRECTORY] 
		= g_signal_new ("add_real_directory",
		                G_TYPE_FROM_CLASS (class),
		                G_SIGNAL_RUN_LAST,
		                G_STRUCT_OFFSET (KrakenMergedDirectoryClass, 
						 add_real_directory),
		                NULL, NULL,
