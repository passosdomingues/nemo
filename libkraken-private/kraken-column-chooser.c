...
/* kraken-column-chooser.h - A column chooser widget
...
#include <config.h>
#include "kraken-column-chooser.h"

#include <string.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

#include "kraken-column-utilities.h"

struct _KrakenColumnChooserDetails
{
	GtkTreeView *view;
	GtkListStore *store;
...
G_DEFINE_TYPE(KrakenColumnChooser, kraken_column_chooser, GTK_TYPE_BOX);

static void kraken_column_chooser_constructed (GObject *object);

static void
kraken_column_chooser_set_property (GObject *object,
				      guint param_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
	KrakenColumnChooser *chooser;

	chooser = KRAKEN_COLUMN_CHOOSER (object);
...
static void
kraken_column_chooser_class_init (KrakenColumnChooserClass *chooser_class)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (chooser_class);

	oclass->set_property = kraken_column_chooser_set_property;
	oclass->constructed = kraken_column_chooser_constructed;

	signals[CHANGED] = g_signal_new
		("changed",
		 G_TYPE_FROM_CLASS (chooser_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (KrakenColumnChooserClass,
				  changed),
		 NULL, NULL,
	...
	g_type_class_add_private (chooser_class, sizeof (KrakenColumnChooserDetails));
}

static void 
update_buttons (KrakenColumnChooser *chooser)
{
...
static void
list_changed (KrakenColumnChooser *chooser) 
{
			  ...
			  gpointer user_data)
{
	...
	chooser = KRAKEN_COLUMN_CHOOSER (user_data);
...
{
	update_buttons (KRAKEN_COLUMN_CHOOSER (user_data));
}
...
row_deleted_callback (GtkTreeModel *model, 
		       ...
		       gpointer user_data)
{
	list_changed (KRAKEN_COLUMN_CHOOSER (user_data));
}

static void
add_tree_view (KrakenColumnChooser *chooser)
{
	GtkWidget *scrolled;
	GtkWidget *view;
	GtkListStore *store;
...
{
	g_signal_emit (KRAKEN_COLUMN_CHOOSER (user_data), 
...
static void
add_buttons (KrakenColumnChooser *chooser)
{
	GtkWidget *box;
	GtkWidget *separator;
...
static void
populate_tree (KrakenColumnChooser *chooser)
{
	GList *columns;
	GList *l;

	columns = kraken_get_columns_for_file (chooser->details->file);
...
static void
kraken_column_chooser_constructed (GObject *object)
{
	KrakenColumnChooser *chooser;
...
static void
kraken_column_chooser_init (KrakenColumnChooser *chooser)
{	
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE ((chooser), KRAKEN_TYPE_COLUMN_CHOOSER, KrakenColumnChooserDetails);
...
static void 
set_visible_columns (KrakenColumnChooser *chooser,
...
static char **
get_column_names (KrakenColumnChooser *chooser, gboolean only_visible)
{
	GPtrArray *ret;
	GtkTreeIter iter;
...
static gboolean
get_column_iter (KrakenColumnChooser *chooser, 
		 KrakenColumn *column,
		 GtkTreeIter *iter)
{
	char *column_name;

	g_object_get (KRAKEN_COLUMN (column), "name", &column_name, NULL);
...
static void
set_column_order (KrakenColumnChooser *chooser,
		  char **column_order)

{
	...
	columns = kraken_get_columns_for_file (chooser->details->file);	
	columns = kraken_sort_columns (columns, column_order);
	...
	for (l = columns; l != NULL; l = l->next) {
		...
		if (get_column_iter (chooser, KRAKEN_COLUMN (l->data), &iter)) {
			GtkTreeIter before;
			if (path) {
				gtk_tree_model_get_iter (GTK_TREE_MODEL (chooser->details->store),
							 &before, path);
...
void
kraken_column_chooser_set_settings (KrakenColumnChooser *chooser,
				      char **visible_columns,
				      char **column_order)
{
	g_return_if_fail (KRAKEN_IS_COLUMN_CHOOSER (chooser));
...
void
kraken_column_chooser_get_settings (KrakenColumnChooser *chooser,
				      char ***visible_columns,
				      char ***column_order)
{
	g_return_if_fail (KRAKEN_IS_COLUMN_CHOOSER (chooser));
...
GtkWidget *
kraken_column_chooser_new (KrakenFile *file)
{
	return g_object_new (KRAKEN_TYPE_COLUMN_CHOOSER, "file", file, NULL);
}
