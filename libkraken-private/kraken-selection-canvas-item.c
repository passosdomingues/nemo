...
struct _KrakenSelectionCanvasItemDetails {
...
G_DEFINE_TYPE (KrakenSelectionCanvasItem, kraken_selection_canvas_item, EEL_TYPE_CANVAS_ITEM);

#define DASH_ON 0.8
#define DASH_OFF 1.7
static void
kraken_selection_canvas_item_draw (EelCanvasItem *item,
				     cairo_t *cr,
				     cairo_region_t *region)
{
	KrakenSelectionCanvasItem *self;
	double x1, y1, x2, y2;
	int cx1, cy1, cx2, cy2;
	double i2w_dx, i2w_dy;

	self = KRAKEN_SELECTION_CANVAS_ITEM (item);
...
static double
kraken_selection_canvas_item_point (EelCanvasItem *item,
				      double x,
				      double y,
				      int cx,
				      int cy,
...
static void
kraken_selection_canvas_item_update (EelCanvasItem *item,
				       double i2w_dx,
				       double i2w_dy,
				       gint flags)
{
	...
	if (EEL_CANVAS_ITEM_CLASS (kraken_selection_canvas_item_parent_class)->update)
		(* EEL_CANVAS_ITEM_CLASS (kraken_selection_canvas_item_parent_class)->update) (item, i2w_dx, i2w_dy, flags);
...
static void
kraken_selection_canvas_item_translate (EelCanvasItem *item,
					  double dx,
					  double dy)
{
	KrakenSelectionCanvasItem *self;
...
static void
kraken_selection_canvas_item_bounds (EelCanvasItem *item,
				       double *x1,
				       double *y1,
				       double *x2,
				       double *y2)
...
{
	KrakenSelectionCanvasItem *self = user_data;
...
void
kraken_selection_canvas_item_fade_out (KrakenSelectionCanvasItem *self,
...
static void
kraken_selection_canvas_item_dispose (GObject *obj)
{
	KrakenSelectionCanvasItem *self = KRAKEN_SELECTION_CANVAS_ITEM (obj);
	...
	G_OBJECT_CLASS (kraken_selection_canvas_item_parent_class)->dispose (obj);
}

static void
do_set_fill (KrakenSelectionCanvasItem *self,
	     gboolean fill_set)
{
...
static void
do_set_outline (KrakenSelectionCanvasItem *self,
...
static void
kraken_selection_canvas_item_set_property (GObject *object,
					     guint param_id,
					     const GValue *value,
					     GParamSpec *pspec)
{
	...
	self = KRAKEN_SELECTION_CANVAS_ITEM (object);
...
static void
kraken_selection_canvas_item_get_property (GObject *object,
					     guint param_id,
					     GValue *value,
					     GParamSpec *pspec)
{
...
static void
kraken_selection_canvas_item_class_init (KrakenSelectionCanvasItemClass *klass)
{
	...
	gobject_class->set_property = kraken_selection_canvas_item_set_property;
	gobject_class->get_property = kraken_selection_canvas_item_get_property;
	gobject_class->dispose = kraken_selection_canvas_item_dispose;

	item_class->draw = kraken_selection_canvas_item_draw;
	item_class->point = kraken_selection_canvas_item_point;
	item_class->update = kraken_selection_canvas_item_update;
	item_class->bounds = kraken_selection_canvas_item_bounds;
	item_class->translate = kraken_selection_canvas_item_translate;
	...
	g_type_class_add_private (klass, sizeof (KrakenSelectionCanvasItemDetails));
}

static void
kraken_selection_canvas_item_init (KrakenSelectionCanvasItem *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, KRAKEN_TYPE_SELECTION_CANVAS_ITEM,
						  KrakenSelectionCanvasItemDetails);
}
