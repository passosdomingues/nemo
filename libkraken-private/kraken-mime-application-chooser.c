...
/*
 *  kraken-mime-application-chooser.c: an mime-application chooser
 *
...
#include <config.h>
#include "kraken-mime-application-chooser.h"
...
struct _KrakenMimeApplicationChooserDetails {
	GList *files;
	char *uri;
...
G_DEFINE_TYPE (KrakenMimeApplicationChooser, kraken_mime_application_chooser, GTK_TYPE_BOX);

static void
add_clicked_cb (GtkButton *button,
		gpointer user_data)
{
	KrakenMimeApplicationChooser *chooser = user_data;
	GAppInfo *info;
...
reset_clicked_cb (GtkButton *button,
                  gpointer   user_data)
{
	KrakenMimeApplicationChooser *chooser;

	chooser = KRAKEN_MIME_APPLICATION_CHOOSER (user_data);
...
static void
custom_app_set (KrakenMimeApplicationChooser *chooser,
...
static gchar *
get_extension_from_file (KrakenFile *nfile)
{
	char *name;
	char *extension;

	name = kraken_file_get_name (nfile);
...
static void
kraken_mime_application_chooser_apply_labels (KrakenMimeApplicationChooser *chooser)
{
	...
	if (chooser->details->files != NULL) {
		...
		if (g_content_type_is_unknown (chooser->details->content_type)) {
			extension = get_extension_from_file (KRAKEN_FILE (chooser->details->files->data));
...
static void
kraken_mime_application_chooser_build_ui (KrakenMimeApplicationChooser *chooser)
{
...
static void
kraken_mime_application_chooser_init (KrakenMimeApplicationChooser *chooser)
{
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE (chooser, KRAKEN_TYPE_MIME_APPLICATION_CHOOSER,
							KrakenMimeApplicationChooserDetails);
...
static void
kraken_mime_application_chooser_constructed (GObject *object)
{
	KrakenMimeApplicationChooser *chooser = KRAKEN_MIME_APPLICATION_CHOOSER (object);

	if (G_OBJECT_CLASS (kraken_mime_application_chooser_parent_class)->constructed != NULL)
		G_OBJECT_CLASS (kraken_mime_application_chooser_parent_class)->constructed (object);

    gtk_widget_set_halign (GTK_WIDGET (object), GTK_ALIGN_CENTER);

	kraken_mime_application_chooser_build_ui (chooser);
	kraken_mime_application_chooser_apply_labels (chooser);
}

static void
kraken_mime_application_chooser_finalize (GObject *object)
{
	KrakenMimeApplicationChooser *chooser;

	chooser = KRAKEN_MIME_APPLICATION_CHOOSER (object);
	...
	G_OBJECT_CLASS (kraken_mime_application_chooser_parent_class)->finalize (object);
}

static void
kraken_mime_application_chooser_get_property (GObject *object,
						guint property_id,
						GValue *value,
						GParamSpec *pspec)
{
...
static void
kraken_mime_application_chooser_set_property (GObject *object,
						guint property_id,
						const GValue *value,
						GParamSpec *pspec)
{
...
static void
kraken_mime_application_chooser_class_init (KrakenMimeApplicationChooserClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	gobject_class->set_property = kraken_mime_application_chooser_set_property;
	gobject_class->get_property = kraken_mime_application_chooser_get_property;
	gobject_class->finalize = kraken_mime_application_chooser_finalize;
	gobject_class->constructed = kraken_mime_application_chooser_constructed;
	...
	g_type_class_add_private (class, sizeof (KrakenMimeApplicationChooserDetails));
}

GtkWidget *
kraken_mime_application_chooser_new (const char *uri,
                                        GList *files,
                                    ...
                                    GtkWidget *ok_button)
{
	...
	chooser = g_object_new (KRAKEN_TYPE_MIME_APPLICATION_CHOOSER,
				"uri", uri,
				"files", files,
...
GAppInfo *
kraken_mime_application_chooser_get_info (KrakenMimeApplicationChooser *chooser)
{
    if (chooser->details->custom_info == NULL)
...
const gchar *
kraken_mime_application_chooser_get_uri (KrakenMimeApplicationChooser *chooser)
{
