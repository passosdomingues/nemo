...
/* kraken-column-utilities.h - Utilities related to column specifications
...
get_builtin_columns (void)
{
	...
	columns = g_list_append (NULL,
				 g_object_new (KRAKEN_TYPE_COLUMN,
					       "name", "name",
					       "attribute", "name",
					       "label", _("Name"),
...
get_extension_columns (void)
{
	...
	providers = kraken_module_get_extensions_for_type (KRAKEN_TYPE_COLUMN_PROVIDER);
	
	columns = NULL;
	
	for (l = providers; l != NULL; l = l->next) {
		KrakenColumnProvider *provider;
		GList *provider_columns;
		
		provider = KRAKEN_COLUMN_PROVIDER (l->data);
		provider_columns = kraken_column_provider_get_columns (provider);
...
get_trash_columns (void)
{
	...
	return kraken_column_list_copy (columns);
}
...
GList *
kraken_get_common_columns (void)
{
	static GList *columns = NULL;
...
GList *
kraken_get_all_columns (void)
{
    GList *columns = NULL;
	GList *with_search_columns = NULL;

	columns = g_list_concat (kraken_get_common_columns (),
	                         get_trash_columns ());
...
GList *
kraken_get_columns_for_file (KrakenFile *file)
{
	GList *columns;
    ...
    if (file != NULL && kraken_file_is_in_search (file)) {
        columns = g_list_concat (columns,
                                 get_search_columns ());
    }
...
GList *
kraken_column_list_copy (GList *columns) 
{
	GList *ret;
	GList *l;
...
void
kraken_column_list_free (GList *columns)
{
	GList *l;
...
static int
column_compare (KrakenColumn *a, KrakenColumn *b, char **column_order)
{
	int index_a;
	int index_b;
	char *name;
...
GList *
kraken_sort_columns (GList  *columns, 
		       char  **column_order)
{
	if (!column_order) {
		return NULL;
