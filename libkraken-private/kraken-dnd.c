...
/* kraken-dnd.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
void
kraken_drag_init (KrakenDragInfo     *drag_info,
...
void
kraken_drag_finalize (KrakenDragInfo *drag_info)
{
	gtk_target_list_unref (drag_info->target_list);
	kraken_drag_destroy_selection_list (drag_info->selection_list);
...
/* Functions to deal with KrakenDragSelectionItems.  */

KrakenDragSelectionItem *
kraken_drag_selection_item_new (void)
{
	return g_new0 (KrakenDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (KrakenDragSelectionItem *item)
{
	g_free (item->uri);
	g_free (item);
}
...
void
kraken_drag_destroy_selection_list (GList *list)
{
	GList *p;
...
char **
kraken_drag_uri_array_from_selection_list (const GList *selection_list)
{
	GList *uri_list;
	char **uris;

	uri_list = kraken_drag_uri_list_from_selection_list (selection_list);
	uris = kraken_drag_uri_array_from_list (uri_list);
...
GList *
kraken_drag_uri_list_from_selection_list (const GList *selection_list)
{
	KrakenDragSelectionItem *selection_item;
	GList *uri_list;
	const GList *l;

	uri_list = NULL;
	for (l = selection_list; l != NULL; l = l->next) {
		selection_item = (KrakenDragSelectionItem *) l->data;
...
char **
kraken_drag_uri_array_from_list (const GList *uri_list)
{
	const GList *l;
	char **uris;
	int i;
...
GList *
kraken_drag_uri_list_from_array (const char **uris)
{
	GList *uri_list;
	int i;
...
GList *
kraken_drag_build_selection_list (GtkSelectionData *data)
{
	GList *result;
	const guchar *p, *oldp;
	int size;
...
static gboolean
kraken_drag_file_local_internal (const char *target_uri_string,
...
gboolean
kraken_drag_uris_local (const char *target_uri,
			  const GList *source_uri_list)
{
	/* must have at least one item */
	g_assert (source_uri_list);
	
	return kraken_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
kraken_drag_items_local (const char *target_uri_string,
			   const GList *selection_list)
{
	/* must have at least one item */
	g_assert (selection_list);

	return kraken_drag_file_local_internal (target_uri_string,
						  ((KrakenDragSelectionItem *)selection_list->data)->uri);
}

gboolean
kraken_drag_items_in_trash (const GList *selection_list)
{
	 ...
	 * and currently dropped items can only be from the same container
	 */
	return eel_uri_is_trash (((KrakenDragSelectionItem *)selection_list->data)->uri);
}

gboolean
kraken_drag_items_on_desktop (const GList *selection_list)
{
	char *uri;
	 ...
	 * and currently dropped items can only be from the same container
	 */
	uri = ((KrakenDragSelectionItem *)selection_list->data)->uri;
	if (eel_uri_is_desktop (uri)) {
		return TRUE;
	}
	
	desktop = kraken_get_desktop_location ();
...
GdkDragAction
kraken_drag_default_drop_action_for_netscape_url (GdkDragContext *context)
{
...
static gboolean
check_same_fs (KrakenFile    *target_file,
               KrakenFile    *source_file,
               const gchar *source_fs_for_desktop)
{
    ...
    if (target_file != NULL && source_file != NULL) {
        source_id = kraken_file_get_filesystem_id (source_file);
        target_id = kraken_file_get_filesystem_id (target_file);
...
{
	KrakenFile *naut_file;
	gboolean ret;

	/* if there's no a cached KrakenFile, it returns NULL */
	naut_file = kraken_file_get_existing (file);
	if (naut_file == NULL) {
		return FALSE;
	}
	
	ret = kraken_file_can_delete (naut_file);
	kraken_file_unref (naut_file);
...
uri_contains_desktop (const gchar *uri)
{
    ...
    real_desktop_uri = kraken_get_desktop_directory_uri ();
...
void
kraken_drag_default_drop_action_for_icons (GdkDragContext *context,
                                         ...
                                         gboolean       *can_delete_source)
{
	...
	KrakenFile *dropped_file, *target_file;
    ...
    dropped_uri = ((KrakenDragSelectionItem *)items->data)->uri;
    ...
    dropped_file = kraken_file_get_existing_by_uri (dropped_uri);

    /* To/from desktop preparation - since we are separate processes, we don't have the full filesystem
     * info on the source and destination - we only have the destination info.  Creating a KrakenFile for it
	...
	target_file = kraken_file_get_existing_by_uri (target_uri_string);
	...
	if (eel_uri_is_trash (target_uri_string)) {
		...
		kraken_file_unref (dropped_file);
		kraken_file_unref (target_file);
		return;

	} else if (dropped_file != NULL && kraken_file_is_launcher (dropped_file)) {
		if (actions & GDK_ACTION_MOVE) {
			*action = GDK_ACTION_MOVE;
		}
		kraken_file_unref (dropped_file);
		...
		target = kraken_get_desktop_location ();

		kraken_file_unref (target_file);
		target_file = kraken_file_get (target);
	...
	} else if (target_file != NULL && kraken_file_is_archive (target_file)) {
		*action = GDK_ACTION_COPY;
...
GdkDragAction
kraken_drag_default_drop_action_for_uri_list (GdkDragContext *context,
						const char *target_uri_string)
{
...
gboolean
kraken_drag_drag_data_get (GtkWidget *widget,
...
GdkDragAction
kraken_drag_drop_action_ask (GtkWidget *widget,
			       GdkDragAction actions)
{
	GtkWidget *menu;
	GtkWidget *menu_item;
...
gboolean
kraken_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
kraken_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
kraken_drag_autoscroll_start (KrakenDragInfo *drag_info,
				GtkWidget        *widget,
				GSourceFunc       callback,
				gpointer          user_data)
{
	if (kraken_drag_autoscroll_in_scroll_region (widget)) {
...
void
kraken_drag_autoscroll_stop (KrakenDragInfo *drag_info)
{
	if (drag_info->auto_scroll_timeout_id != 0) {
		g_source_remove (drag_info->auto_scroll_timeout_id);
		drag_info->auto_scroll_timeout_id = 0;
...
gboolean
kraken_drag_selection_includes_special_link (GList *selection_list)
{
	GList *node;
	char *uri;

	for (node = selection_list; node != NULL; node = node->next) {
		uri = ((KrakenDragSelectionItem *) node->data)->uri;
