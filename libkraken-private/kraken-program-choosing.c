...
/* kraken-program-choosing.c - functions for selecting and activating
...
void
kraken_launch_application_for_mount (GAppInfo *app_info,
				       GMount *mount,
				       GtkWindow *parent_window)
{
	...
	kraken_launch_application (app_info,
				     files,
				     parent_window);

	g_list_free_full (files, (GDestroyNotify) kraken_file_unref);
}
 ...
 */
void
kraken_launch_application (GAppInfo *application, 
			     GList *files,
			     GtkWindow *parent_window)
{
	GList *uris, *l;
	...
	for (l = files; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, kraken_file_get_activation_uri (l->data));
	}
	uris = g_list_reverse (uris);
	kraken_launch_application_by_uri (application, uris,
					    parent_window);
...
void
kraken_launch_application_by_uri (GAppInfo *application, 
				    GList *uris,
				    GtkWindow *parent_window)
{
	char *uri;
	...
	file = kraken_file_get_by_uri (uris->data);
	icon = kraken_file_get_icon (file,
                               48, 0,
	...
	if (icon) {
		gdk_app_launch_context_set_icon_name (launch_context,
							kraken_icon_info_get_used_name (icon));
	...
	if (result) {
		for (l = uris; l != NULL; l = l->next) {
			file = kraken_file_get_by_uri (l->data);
			kraken_recent_add_file (file, application);
			kraken_file_unref (file);
		}
	}
...
/**
 * kraken_launch_application_from_command:
 * 
 ...
 */
void
kraken_launch_application_from_command (GdkScreen  *screen,
 ...
 */
void
kraken_launch_application_from_command_array (GdkScreen  *screen,
						const char *command_string,
						gboolean use_terminal,
...
void
kraken_launch_desktop_file (GdkScreen   *screen,
