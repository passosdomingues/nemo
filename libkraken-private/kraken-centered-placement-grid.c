...
kraken_centered_placement_grid_new (KrakenIconContainer *container, gboolean horizontal)
{
    ...
    width  = kraken_icon_container_get_canvas_width (container, allocation);
    ...
    grid = g_new0 (KrakenCenteredPlacementGrid, 1);
...
void
kraken_centered_placement_grid_free (KrakenCenteredPlacementGrid *grid)
{
    if (grid == NULL) {
        return;
    }
...
static gboolean
kraken_centered_placement_grid_position_is_free (KrakenCenteredPlacementGrid *grid,
                                               gint                       grid_x,
                                               gint                       grid_y)
{
...
static void
kraken_centered_placement_grid_mark (KrakenCenteredPlacementGrid *grid,
                                   gint                       grid_x,
                                   gint                       grid_y)
{
    g_assert (grid_x >= 0 && grid_x < grid->num_columns);
...
static void
kraken_centered_placement_grid_unmark (KrakenCenteredPlacementGrid *grid,
                                     gint                       grid_x,
                                     gint                       grid_y)
{
    g_assert (grid_x >= 0 && grid_x < grid->num_columns);
...
static void
kraken_centered_placement_grid_get_next_grid_position (KrakenCenteredPlacementGrid *grid,
                                                     gint                       x_in,
                                                     gint                       y_in,
                                                     gint                      *x_out,
                                                     gint                      *y_out)
...
void
kraken_centered_placement_grid_nominal_to_icon_position (KrakenCenteredPlacementGrid *grid,
                                                       KrakenIcon                  *icon,
                                                       gint                       x_nominal,
                                                       gint                       y_nominal,
                                                       gint                      *x_adjusted,
...
void
kraken_centered_placement_grid_icon_position_to_nominal (KrakenCenteredPlacementGrid *grid,
                                                       KrakenIcon                  *icon,
                                                       gint                       x_adjusted,
                                                       gint                       y_adjusted,
                                                       gint                      *x_nominal,
...
void
kraken_centered_placement_grid_mark_icon (KrakenCenteredPlacementGrid *grid, KrakenIcon *icon)
{
    gint grid_x, grid_y;
    GdkRectangle rect;
    ...
    kraken_centered_placement_grid_get_current_position_rect (grid, icon->x, icon->y, &rect, NULL);
    ...
    if ((grid_x >= 0 && grid_x < grid->num_columns) &&
       (grid_y >= 0 && grid_y < grid->num_rows)) {
        kraken_centered_placement_grid_mark (grid, grid_x, grid_y);
    }
}

void
kraken_centered_placement_grid_unmark_icon (KrakenCenteredPlacementGrid *grid,
                                          KrakenIcon                  *icon)
{
    gint grid_x, grid_y;
    GdkRectangle rect;
    ...
    if ((grid_x >= 0 && grid_x < grid->num_columns) &&
       (grid_y >= 0 && grid_y < grid->num_rows)) {
        kraken_centered_placement_grid_unmark (grid, grid_x, grid_y);
    }
}

void
kraken_centered_placement_grid_mark_position (KrakenCenteredPlacementGrid *grid,
                                            gint                       x,
                                            gint                       y)
{
    GdkRectangle rect;
...
void
kraken_centered_placement_grid_unmark_position (KrakenCenteredPlacementGrid *grid,
                                              gint                       x,
                                              gint                       y)
{
    GdkRectangle rect;
...
void
kraken_centered_placement_grid_pre_populate (KrakenCenteredPlacementGrid *grid,
                                           GList                     *icons,
                                           gboolean                   ignore_lazy)
{
    ...
    for (p = icons; p != NULL; p = p->next) {
        ...
        if (ignore_lazy) {
            should_mark = kraken_icon_container_icon_is_positioned (icon) && !icon->has_lazy_position;
        } else {
            should_mark = kraken_icon_container_icon_is_positioned (icon);
        }
...
void
kraken_centered_placement_grid_get_next_position_rect (KrakenCenteredPlacementGrid *grid,
                                                     ...
                                                     gboolean                  *is_free)
{
    ...
    kraken_centered_placement_grid_get_next_grid_position (grid,
                                                         index_x,
                                                         index_y,
                                                         &next_index_x,
                                                         &next_index_y);
    ...
    if (is_free) {
        if ((next_index_x == grid->num_columns - 1) && (next_index_y == grid->num_rows - 1)) {
            ...
            *is_free = kraken_centered_placement_grid_position_is_free (grid, next_index_x, next_index_y);
        }
    }
}

void
kraken_centered_placement_grid_get_current_position_rect (KrakenCenteredPlacementGrid *grid,
                                                        gint                       x,
                                                        gint                       y,
                                                        GdkRectangle              *rect,
                                                        gboolean                  *is_free)
{
    ...
    if (is_free) {
        if ((index_x == grid->num_columns - 1) && (index_y == grid->num_rows - 1)) {
            ...
            *is_free = kraken_centered_placement_grid_position_is_free (grid, index_x, index_y);
        }
    }
}

static KrakenIcon *
get_icon_at_grid_position (KrakenCenteredPlacementGrid *grid,
                           gint                       grid_x,
                           gint                       grid_y)
{
    KrakenIcon *icon;
...
GList *
kraken_centered_placement_grid_clear_grid_for_selection (KrakenCenteredPlacementGrid *grid,
                                                       gint                       start_x,
                                                       gint                       start_y,
                                                       GList                     *drag_sel_list)
{
    ...
    if (!insert_before) {
        kraken_centered_placement_grid_get_next_grid_position (grid, grid_x, grid_y, &new_grid_x, &new_grid_y);
    } else {
        new_grid_x = grid_x;
        new_grid_y = grid_y;
    }
    ...
    if (grid->horizontal) {
        for (iter_y = new_grid_y; iter_y < grid->num_rows; iter_y++) {
            for (iter_x = first_line ? new_grid_x : 0; iter_x < grid->num_columns; iter_x++) {
                ...
                if (kraken_centered_placement_grid_position_is_free (grid, iter_x, iter_y)) {
                    space_needed--;
                } else {
                    icon = get_icon_at_grid_position (grid, iter_x, iter_y);

                    icon_list = g_list_prepend (icon_list, icon);
                    kraken_centered_placement_grid_unmark (grid, iter_x, iter_y);
                }
    ...
    for (p = drag_sel_list; p != NULL; p = p->next) {
        ...
        kraken_centered_placement_grid_get_next_grid_position (grid, iter_x, iter_y, &iter_x, &iter_y);
    }

    /* Build a KrakenDragSelectionItem list from the KrakenIcon list we picked up, setting the appropriate positions
    ...
    for (p = icon_list; p != NULL; p = p->next) {
        ...
        item->uri = kraken_file_get_uri (KRAKEN_FILE (icon->data));
        item->got_icon_position = TRUE;
