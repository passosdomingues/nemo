...
/* kraken-tree-view-drag-dest.c: Handles drag and drop for treeviews which
 *                                 contain a hierarchy of files
 */

#include <config.h>

#include "kraken-tree-view-drag-dest.h"
...
struct _KrakenTreeViewDragDestDetails {
...
G_DEFINE_TYPE (KrakenTreeViewDragDest, kraken_tree_view_drag_dest,
	       G_TYPE_OBJECT);
...
static void
remove_scroll_timeout (KrakenTreeViewDragDest *dest)
{
...
expand_timeout (gpointer data)
{
    ...
    KrakenTreeViewDragDest *dest = KRAKEN_TREE_VIEW_DRAG_DEST (data);
...
static void
remove_expand_timeout (KrakenTreeViewDragDest *dest)
{
...
static void
set_widget_highlight (KrakenTreeViewDragDest *dest, gboolean highlight)
{
...
static void
set_drag_dest_row (KrakenTreeViewDragDest *dest,
		   GtkTreePath *path)
{
	if (path) {
...
static void
clear_drag_dest_row (KrakenTreeViewDragDest *dest)
{
...
static gboolean
get_drag_data (KrakenTreeViewDragDest *dest,
	       ...
	       guint32 time)
{
	...
	if (target == gdk_atom_intern (KRAKEN_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE) &&
	    !dest->details->drop_occurred) {
		dest->details->drag_type = KRAKEN_ICON_DND_XDNDDIRECTSAVE;
...
static void
free_drag_data (KrakenTreeViewDragDest *dest)
{
	...
	if (dest->details->drag_list) {
		kraken_drag_destroy_selection_list (dest->details->drag_list);
...
static char *
get_root_uri (KrakenTreeViewDragDest *dest)
{
	char *uri;
...
static KrakenFile *
file_for_path (KrakenTreeViewDragDest *dest, GtkTreePath *path)
{
	KrakenFile *file;
	char *uri;
...
static GtkTreePath *
get_drop_path (KrakenTreeViewDragDest *dest,
	       GtkTreePath *path)
{
	KrakenFile *file;
	GtkTreePath *ret;
	...
	while (file == NULL /* dummy row */ ||
	       !kraken_drag_can_accept_info (file,
...
static char *
get_drop_target_uri_for_path (KrakenTreeViewDragDest *dest,
			      GtkTreePath *path)
{
	KrakenFile *file;
	char *target;
	...
	target = kraken_file_get_drop_target_uri (file);
	kraken_file_unref (file);
...
static guint
get_drop_action (KrakenTreeViewDragDest *dest,
		 GdkDragContext *context,
		 GtkTreePath *path)
{
	char *drop_target;
	...
	if (!dest->details->have_drag_data ||
	    (dest->details->drag_type == KRAKEN_ICON_DND_GNOME_ICON_LIST &&
	     dest->details->drag_list == NULL)) {
		return 0;
	}
		      ...
		      gpointer data)
{
	...
	dest = KRAKEN_TREE_VIEW_DRAG_DEST (data);
	...
	if (action) {
        ...
        if (g_settings_get_boolean (kraken_list_view_preferences, KRAKEN_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION) &&
            g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_EXPAND_ROW_ON_DND_DWELL)) {
...
static char *
get_drop_target_uri_at_pos (KrakenTreeViewDragDest *dest, int x, int y)
{
	char *drop_target;
	GtkTreePath *path;
	GtkTreePath *drop_path;
...
static void
receive_uris (KrakenTreeViewDragDest *dest,
	      GdkDragContext *context,
	      GList *source_uris,
	      int x, int y)
{
	...
	if (real_action == GDK_ACTION_ASK) {
		if (kraken_drag_selection_includes_special_link (dest->details->drag_list)) {
			/* We only want to move the trash */
			action = GDK_ACTION_MOVE;
		} else {
			action = GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK;
		}
		real_action = kraken_drag_drop_action_ask
	...
	if (real_action > 0) {
        if (g_strcmp0 (drop_target, "favorites:///") == 0) {
            ...
            if (!kraken_drag_uris_local (drop_target, source_uris)
...
static void
receive_dropped_icons (KrakenTreeViewDragDest *dest,
		       GdkDragContext *context,
		       int x, int y)
{
	GList *source_uris;
	...
	for (l = dest->details->drag_list; l != NULL; l = l->next) {
		source_uris = g_list_prepend (source_uris,
					      ((KrakenDragSelectionItem *)l->data)->uri);
	}
...
static void
receive_dropped_uri_list (KrakenTreeViewDragDest *dest,
			  GdkDragContext *context,
			  int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_text (KrakenTreeViewDragDest *dest,
		      GdkDragContext *context,
		      int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_raw (KrakenTreeViewDragDest *dest,
...
static void
receive_dropped_netscape_url (KrakenTreeViewDragDest *dest,
			      GdkDragContext *context,
			      int x, int y)
{
	char *drop_target;
...
static gboolean
receive_xds (KrakenTreeViewDragDest *dest,
	     GtkWidget *widget,
	     guint32 time,
	     GdkDragContext *context,
	     int x, int y)
			     ...
			     gpointer data)
{
	...
	if (!dest->details->have_drag_data) {
		...
		if (info == KRAKEN_ICON_DND_GNOME_ICON_LIST) {
			dest->details->drag_list =
				kraken_drag_build_selection_list (selection_data);
		}
	}
...
static gboolean
set_direct_save_uri (KrakenTreeViewDragDest *dest,
		     GdkDragContext *context,
		     int x, int y)
{
	GFile *base, *child;
		       ...
		       GObject *object)
{
	...
	dest = KRAKEN_TREE_VIEW_DRAG_DEST (user_data);
...
static void
kraken_tree_view_drag_dest_dispose (GObject *object)
{
	KrakenTreeViewDragDest *dest;

	dest = KRAKEN_TREE_VIEW_DRAG_DEST (object);
	...
	G_OBJECT_CLASS (kraken_tree_view_drag_dest_parent_class)->dispose (object);
}

static void
kraken_tree_view_drag_dest_finalize (GObject *object)
{
	KrakenTreeViewDragDest *dest;
	...
	G_OBJECT_CLASS (kraken_tree_view_drag_dest_parent_class)->finalize (object);
}

static void
kraken_tree_view_drag_dest_init (KrakenTreeViewDragDest *dest)
{
	dest->details = G_TYPE_INSTANCE_GET_PRIVATE (dest, KRAKEN_TYPE_TREE_VIEW_DRAG_DEST,
						     KrakenTreeViewDragDestDetails);
}

static void
kraken_tree_view_drag_dest_class_init (KrakenTreeViewDragDestClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);

	gobject_class->dispose = kraken_tree_view_drag_dest_dispose;
	gobject_class->finalize = kraken_tree_view_drag_dest_finalize;

	g_type_class_add_private (class, sizeof (KrakenTreeViewDragDestDetails));

	signals[GET_ROOT_URI] =
		g_signal_new ("get_root_uri",
			      G_TYPE_FROM_CLASS (class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (KrakenTreeViewDragDestClass,
					       get_root_uri),
			      NULL, NULL,
...
KrakenTreeViewDragDest *
kraken_tree_view_drag_dest_new (GtkTreeView *tree_view, gboolean strict_drop)
{
	KrakenTreeViewDragDest *dest;
	GtkTargetList *targets;

	dest = g_object_new (KRAKEN_TYPE_TREE_VIEW_DRAG_DEST, NULL);
