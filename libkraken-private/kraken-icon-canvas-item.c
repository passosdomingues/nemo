...
#include <config.h>
#include <math.h>
#include "kraken-icon-canvas-item.h"
...
/* Private part of the KrakenIconCanvasItem structure. */
struct KrakenIconCanvasItemDetails {
	/* The image, text, font. */
	double x, y;
	GdkPixbuf *pixbuf;
...
static void kraken_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface);
static GType kraken_icon_canvas_item_accessible_factory_get_type (void);

G_DEFINE_TYPE_WITH_CODE (KrakenIconCanvasItem, kraken_icon_canvas_item, EEL_TYPE_CANVAS_ITEM,
			 G_IMPLEMENT_INTERFACE (EEL_TYPE_ACCESSIBLE_TEXT,
						kraken_icon_canvas_item_text_interface_init));

/* private */
static void     draw_label_text                      (KrakenIconCanvasItem        *item,
						      cairo_t                       *cr,
						      EelIRect                       icon_rect);
static void     measure_label_text                   (KrakenIconCanvasItem        *item);
...
static PangoLayout *get_label_layout                 (PangoLayout                  **layout,
						      ...
						      const char                    *text);
static gboolean hit_test_stretch_handle              (KrakenIconCanvasItem        *item,
						      EelIRect                       canvas_rect,
						      GtkCornerType *corner);

static void       kraken_icon_canvas_item_ensure_bounds_up_to_date (KrakenIconCanvasItem *icon_item);

/* Object initialization function for the icon item. */
static void
kraken_icon_canvas_item_init (KrakenIconCanvasItem *icon_item)
{
	icon_item->details = G_TYPE_INSTANCE_GET_PRIVATE ((icon_item), KRAKEN_TYPE_ICON_CANVAS_ITEM, KrakenIconCanvasItemDetails);
	kraken_icon_canvas_item_invalidate_label_size (icon_item);
}

static void
kraken_icon_canvas_item_finalize (GObject *object)
{
	KrakenIconCanvasItemDetails *details;

	g_assert (KRAKEN_IS_ICON_CANVAS_ITEM (object));

	details = KRAKEN_ICON_CANVAS_ITEM (object)->details;
	...
	G_OBJECT_CLASS (kraken_icon_canvas_item_parent_class)->finalize (object);
}
...
static void
kraken_icon_canvas_item_invalidate_bounds_cache (KrakenIconCanvasItem *item)
{
...
void
kraken_icon_canvas_item_invalidate_label_size (KrakenIconCanvasItem *item)
{
	...
	kraken_icon_canvas_item_invalidate_bounds_cache (item);
...
static void
kraken_icon_canvas_item_set_property (GObject        *object,
					guint           property_id,
					const GValue   *value,
					GParamSpec     *pspec)
{
	...
	item = KRAKEN_ICON_CANVAS_ITEM (object);
	...
	case PROP_EDITABLE_TEXT:
		...
		kraken_icon_canvas_item_invalidate_label_size (item);
    ...
    case PROP_PINNED:
        ...
        kraken_icon_canvas_item_invalidate_label (item);
	...
	default:
		g_warning ("kraken_icons_view_item_item_set_arg on unknown argument");
		return;
	}
...
static void
kraken_icon_canvas_item_get_property (GObject        *object,
					guint           property_id,
					GValue         *value,
					GParamSpec     *pspec)
{
...
static void
get_scaled_icon_size (KrakenIconCanvasItem *item,
             gint *width,
             gint *height)
{
   EelCanvas *canvas;
...
cairo_surface_t *
kraken_icon_canvas_item_get_drag_surface (KrakenIconCanvasItem *item)
{
	...
	g_return_val_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (item), NULL);
	...
	gtk_style_context_save (context);
        ...
        gtk_style_context_add_class (context, "kraken-canvas-item");
...
void
kraken_icon_canvas_item_set_image (KrakenIconCanvasItem *item,
				     GdkPixbuf *image)
{
	KrakenIconCanvasItemDetails *details;

	g_return_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (item));
 ...
 */
static void
recompute_bounding_box (KrakenIconCanvasItem *icon_item,
...
static EelIRect
compute_text_rectangle (const KrakenIconCanvasItem *item,
			...
			KrakenIconCanvasItemBoundsUsage usage)
{
	...
	if (KRAKEN_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE) {
		if (!kraken_icon_container_is_layout_rtl (KRAKEN_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas))) {
...
void
kraken_icon_canvas_item_update_bounds (KrakenIconCanvasItem *item,
					 double i2w_dx, double i2w_dy)
{
	...
	kraken_icon_canvas_item_get_icon_canvas_rectangle (item, &item->details->canvas_rect);
...
static void
kraken_icon_canvas_item_update (EelCanvasItem *item,
				  double i2w_dx, double i2w_dy,
				  gint flags)
{
	kraken_icon_canvas_item_update_bounds (KRAKEN_ICON_CANVAS_ITEM (item), i2w_dx, i2w_dy);

	eel_canvas_item_request_redraw (EEL_CANVAS_ITEM (item));

	EEL_CANVAS_ITEM_CLASS (kraken_icon_canvas_item_parent_class)->update (item, i2w_dx, i2w_dy, flags);
}
...
in_single_click_mode (void)
{
	...
	return click_policy == KRAKEN_CLICK_POLICY_SINGLE;
}
...
#define IS_COMPACT_VIEW(container) \
        ((container->details->layout_mode == KRAKEN_ICON_LAYOUT_T_B_L_R || \
...
static void
prepare_pango_layout_width (KrakenIconCanvasItem *item,
			    PangoLayout *layout)
{
	if (kraken_icon_canvas_item_get_max_text_width (item) < 0) {
		pango_layout_set_width (layout, -1);
	} else {
		pango_layout_set_width (layout, floor (kraken_icon_canvas_item_get_max_text_width (item)) * PANGO_SCALE);
		pango_layout_set_ellipsize (layout, PANGO_ELLIPSIZE_END);
	}
}

static void
prepare_pango_layout_for_measure_entire_text (KrakenIconCanvasItem *item,
					      PangoLayout *layout)
{
	...
	container = KRAKEN_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
...
static void
prepare_pango_layout_for_draw (KrakenIconCanvasItem *item,
			       PangoLayout *layout)
{
	KrakenIconCanvasItemDetails *details;
	KrakenIconContainer *container;
	...
	if (IS_COMPACT_VIEW (container)) {
		...
		pango_layout_set_height (layout,
					 kraken_icon_container_get_max_layout_lines_for_pango (container));
	}
}

static void
measure_label_text (KrakenIconCanvasItem *item)
{
	KrakenIconCanvasItemDetails *details;
	KrakenIconContainer *container;
	...
	if (have_editable) {
		...
		layout_get_size_for_layout (editable_layout,
					    kraken_icon_container_get_max_layout_lines (container),
					    editable_height_for_entire_text,
...
static void
draw_label_text (KrakenIconCanvasItem *item,
                 cairo_t *cr,
		 EelIRect icon_rect)
{
	KrakenIconCanvasItemDetails *details;
	...
	is_rtl_label_beside = kraken_icon_container_is_layout_rtl (container) &&
			      container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE;
	...
	max_text_width = floor (kraken_icon_canvas_item_get_max_text_width (item));
...
void
kraken_icon_canvas_item_set_is_visible (KrakenIconCanvasItem       *item,
					  gboolean                      visible)
{
...
void
kraken_icon_canvas_item_invalidate_label (KrakenIconCanvasItem     *item)
{
	kraken_icon_canvas_item_invalidate_label_size (item);
...
get_knob_pixbuf (void)
{
	...
	knob_pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (),
						...
						8, 0, NULL);
	if (!knob_pixbuf) {
		GInputStream *stream = g_resources_open_stream ("/org/kraken/icons/knob.png", 0, NULL);
		if (stream != NULL) {
...
static void
draw_stretch_handles (KrakenIconCanvasItem *item,
                      cairo_t *cr,
		      const EelIRect *rect)
{
	GtkWidget *widget;
...
real_map_surface (KrakenIconCanvasItem *icon_item)
{
	EelCanvas *canvas;
...
static cairo_surface_t *
map_surface (KrakenIconCanvasItem *icon_item)
{
#if 0
...
static void
kraken_icon_canvas_item_draw (EelCanvasItem *item,
                                cairo_t *cr,
                                cairo_region_t *region)
{
	KrakenIconContainer *container;
	KrakenIconCanvasItem *icon_item;
	KrakenIconCanvasItemDetails *details;
	EelIRect icon_rect;
    cairo_surface_t *temp_surface;
	GtkStyleContext *context;

	container = KRAKEN_ICON_CONTAINER (item->canvas);
	icon_item = KRAKEN_ICON_CANVAS_ITEM (item);
...
static PangoLayout *
create_label_layout (KrakenIconCanvasItem *item,
		     const char *text)
{
	PangoLayout *layout;
	...
	container = KRAKEN_ICON_CONTAINER (canvas_item->canvas);
...
static int
kraken_icon_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
	KrakenIconCanvasItem *icon_item;
	GdkCursor *cursor;
	GdkWindow *cursor_window;
    ...
    if (event->type == GDK_ENTER_NOTIFY) {
        kraken_icon_container_update_tooltip_text (KRAKEN_ICON_CONTAINER (item->canvas), icon_item);
		if (!icon_item->details->is_prelit) {
			icon_item->details->is_prelit = TRUE;
			kraken_icon_canvas_item_invalidate_label_size (icon_item);
			eel_canvas_item_request_update (item);
			eel_canvas_item_send_behind (item,
						     KRAKEN_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle);
        ...
        kraken_icon_container_update_tooltip_text (KRAKEN_ICON_CONTAINER (item->canvas), NULL);
		if (icon_item->details->is_prelit
...
static gboolean
hit_test (KrakenIconCanvasItem *icon_item, EelIRect canvas_rect)
{
	KrakenIconCanvasItemDetails *details;
...
static double
kraken_icon_canvas_item_point (EelCanvasItem *item, double x, double y, int cx, int cy,
				 EelCanvasItem **actual_item)
{
	...
	*actual_item = item;
	...
	canvas_rect.x1 = cx + 1;
	canvas_rect.y1 = cy + 1;
	if (hit_test (KRAKEN_ICON_CANVAS_ITEM (item), canvas_rect)) {
		return 0.0;
	} else {
...
static void
kraken_icon_canvas_item_translate (EelCanvasItem *item, double dx, double dy)
{
	KrakenIconCanvasItem *icon_item;
	KrakenIconCanvasItemDetails *details;
...
void
kraken_icon_canvas_item_get_bounds_for_layout (KrakenIconCanvasItem *icon_item,
						 double *x1, double *y1, double *x2, double *y2)
{
	...
	kraken_icon_canvas_item_ensure_bounds_up_to_date (icon_item);
...
void
kraken_icon_canvas_item_get_bounds_for_entire_item (KrakenIconCanvasItem *icon_item,
						      double *x1, double *y1, double *x2, double *y2)
{
	KrakenIconCanvasItemDetails *details;
	EelIRect *total_rect;
...
static void
kraken_icon_canvas_item_bounds (EelCanvasItem *item,
				  double *x1, double *y1, double *x2, double *y2)
{
	KrakenIconCanvasItem *icon_item;
	KrakenIconCanvasItemDetails *details;
...
static void
kraken_icon_canvas_item_ensure_bounds_up_to_date (KrakenIconCanvasItem *icon_item)
{
	KrakenIconCanvasItemDetails *details;
...
EelDRect
kraken_icon_canvas_item_get_icon_rectangle (const KrakenIconCanvasItem *item)
{
	EelDRect rectangle;
	double pixels_per_unit;
    gint width, height;

	g_return_val_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (item), eel_drect_empty);

	rectangle.x0 = item->details->x;
	rectangle.y0 = item->details->y;

	pixels_per_unit = EEL_CANVAS_ITEM (item)->canvas->pixels_per_unit;
    get_scaled_icon_size (KRAKEN_ICON_CANVAS_ITEM (item), &width, &height);
...
EelDRect
kraken_icon_canvas_item_get_text_rectangle (KrakenIconCanvasItem *item,
					      gboolean for_layout)
{
	/* FIXME */
	EelIRect icon_rectangle;
...
void
kraken_icon_canvas_item_get_icon_canvas_rectangle (KrakenIconCanvasItem *item,
                                                 EelIRect *rect)
{
    gint width, height;

	g_assert (KRAKEN_IS_ICON_CANVAS_ITEM (item));
	g_assert (rect != NULL);
...
void
kraken_icon_canvas_item_set_show_stretch_handles (KrakenIconCanvasItem *item,
...
static gboolean
hit_test_stretch_handle (KrakenIconCanvasItem *item,
...
gboolean
kraken_icon_canvas_item_hit_test_stretch_handles (KrakenIconCanvasItem *item,
						    gdouble world_x,
						    gdouble world_y,
						    GtkCornerType *corner)
{
	...
	g_return_val_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (item), FALSE);
...
/* kraken_icon_canvas_item_hit_test_rectangle
 *
 * Check and see if there is an intersection between the item and the
 * canvas rect.
 */
gboolean
kraken_icon_canvas_item_hit_test_rectangle (KrakenIconCanvasItem *item, EelIRect canvas_rect)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (item), FALSE);
...
const char *
kraken_icon_canvas_item_get_editable_text (KrakenIconCanvasItem *icon_item)
{
	g_return_val_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (icon_item), NULL);

	return icon_item->details->editable_text;
}

void
kraken_icon_canvas_item_set_renaming (KrakenIconCanvasItem *item, gboolean state)
{
	g_return_if_fail (KRAKEN_IS_ICON_CANVAS_ITEM (item));
...
double
kraken_icon_canvas_item_get_max_text_width (KrakenIconCanvasItem *item)
{
    EelCanvasItem *canvas_item;
    KrakenIconContainer *container;
    ...
    if (container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE) {
        if (container->details->layout_mode == KRAKEN_ICON_LAYOUT_T_B_L_R ||
            container->details->layout_mode == KRAKEN_ICON_LAYOUT_T_B_R_L) {
            /* compact view */
        ...
        if (container->details->is_desktop) {
            return kraken_get_desktop_text_width_for_zoom_level (kraken_icon_container_get_zoom_level (container));
        } else {
            return kraken_get_icon_text_width_for_zoom_level (kraken_icon_container_get_zoom_level (container));
        }
    }
}

void
kraken_icon_canvas_item_set_entire_text (KrakenIconCanvasItem       *item,
...
gint
kraken_icon_canvas_item_get_fixed_text_height_for_layout (KrakenIconCanvasItem *item)
{
    KrakenIconContainer *container;
    PangoLayout *layout;
    ...
    if (kraken_icon_container_get_zoom_level (container) == KRAKEN_ZOOM_LEVEL_SMALLEST) {
        // No label/info, just a bit of padding.
        return 0;
    }

    lines = kraken_icon_container_get_max_layout_lines (container);
    lines += kraken_icon_container_get_additional_text_line_count (container);
...
static void
kraken_icon_canvas_item_class_init (KrakenIconCanvasItemClass *class)
{
	...
	object_class->finalize = kraken_icon_canvas_item_finalize;
	object_class->set_property = kraken_icon_canvas_item_set_property;
	object_class->get_property = kraken_icon_canvas_item_get_property;
	...
	item_class->update = kraken_icon_canvas_item_update;
	item_class->draw = kraken_icon_canvas_item_draw;
	item_class->point = kraken_icon_canvas_item_point;
	item_class->translate = kraken_icon_canvas_item_translate;
	item_class->bounds = kraken_icon_canvas_item_bounds;
	item_class->event = kraken_icon_canvas_item_event;

	atk_registry_set_factory_type (atk_get_default_registry (),
				       KRAKEN_TYPE_ICON_CANVAS_ITEM,
				       kraken_icon_canvas_item_accessible_factory_get_type ());

	g_type_class_add_private (class, sizeof (KrakenIconCanvasItemDetails));
}

static GailTextUtil *
kraken_icon_canvas_item_get_text (GObject *text)
{
	return KRAKEN_ICON_CANVAS_ITEM (text)->details->text_util;
}

static void
kraken_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface)
{
	iface->get_text = kraken_icon_canvas_item_get_text;
}

/* ============================= a11y interfaces =========================== */

static const char *kraken_icon_canvas_item_accessible_action_names[] = {
        "open",
        "menu",
        NULL
};

static const char *kraken_icon_canvas_item_accessible_action_descriptions[] = {
        "Open item",
        "Popup context menu",
        NULL
};
...
typedef struct {
	...
	char *description;
} KrakenIconCanvasItemAccessiblePrivate;

typedef struct {
	KrakenIconCanvasItem *item;
	gint action_number;
} KrakenIconCanvasItemAccessibleActionContext;

static GType kraken_icon_canvas_item_accessible_get_type (void);

#define GET_PRIV(o) \
	G_TYPE_INSTANCE_GET_PRIVATE(o,\
				    kraken_icon_canvas_item_accessible_get_type (),\
				    KrakenIconCanvasItemAccessiblePrivate);

/* accessible AtkAction interface */
static gboolean
kraken_icon_canvas_item_accessible_idle_do_action (gpointer data)
{
	KrakenIconCanvasItem *item;
	KrakenIconCanvasItemAccessibleActionContext *ctx;
	KrakenIcon *icon;
	KrakenIconContainer *container;
	GList* selection;
	GList file_list;
...
static gboolean
kraken_icon_canvas_item_accessible_do_action (AtkAction *accessible,
						int i)
{
	KrakenIconCanvasItem *item;
	KrakenIconCanvasItemAccessibleActionContext *ctx;
	...
	item = KRAKEN_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (accessible)));
	if (!item) {
		return FALSE;
	}
	...
	case ACTION_MENU:
		...
		ctx = g_new (KrakenIconCanvasItemAccessibleActionContext, 1);
		ctx->action_number = i;
		ctx->item = item;
		g_queue_push_head (container->details->a11y_item_action_queue, ctx);
		if (container->details->a11y_item_action_idle_handler == 0) {
			container->details->a11y_item_action_idle_handler = g_idle_add (kraken_icon_canvas_item_accessible_idle_do_action, container);
		}
		break;
        default :
                g_warning ("Invalid action passed to KrakenIconCanvasItemAccessible::do_action");
                return FALSE;
        }
...
static int
kraken_icon_canvas_item_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
kraken_icon_canvas_item_accessible_action_get_description (AtkAction *accessible,
                                                             int i)
{
	KrakenIconCanvasItemAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return kraken_icon_canvas_item_accessible_action_descriptions[i];
	}
}

static const char *
kraken_icon_canvas_item_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return kraken_icon_canvas_item_accessible_action_names[i];
}

static const char *
kraken_icon_canvas_item_accessible_action_get_keybinding (AtkAction *accessible,
							    int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
kraken_icon_canvas_item_accessible_action_set_description (AtkAction *accessible,
							     int i,
...
static void
kraken_icon_canvas_item_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = kraken_icon_canvas_item_accessible_do_action;
	iface->get_n_actions = kraken_icon_canvas_item_accessible_get_n_actions;
	iface->get_description = kraken_icon_canvas_item_accessible_action_get_description;
	iface->get_keybinding = kraken_icon_canvas_item_accessible_action_get_keybinding;
	iface->get_name = kraken_icon_canvas_item_accessible_action_get_name;
	iface->set_description = kraken_icon_canvas_item_accessible_action_set_description;
}

static const gchar *
kraken_icon_canvas_item_accessible_get_name (AtkObject *accessible)
{
	KrakenIconCanvasItem *item;
...
static const gchar*
kraken_icon_canvas_item_accessible_get_description (AtkObject *accessible)
{
	KrakenIconCanvasItem *item;
...
static AtkObject *
kraken_icon_canvas_item_accessible_get_parent (AtkObject *accessible)
{
	KrakenIconCanvasItem *item;
...
static int
kraken_icon_canvas_item_accessible_get_index_in_parent (AtkObject *accessible)
{
	KrakenIconCanvasItem *item;
	KrakenIconContainer *container;
	GList *l;
...
static const gchar *
kraken_icon_canvas_item_accessible_get_image_description (AtkImage *image)
{
	KrakenIconCanvasItemAccessiblePrivate *priv;
	KrakenIconCanvasItem *item;
	KrakenIcon *icon;
	...
	if (priv->image_description) {
		...
		item = KRAKEN_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (image)));
		if (item == NULL) {
			return NULL;
		}
		icon = item->user_data;
		container = KRAKEN_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
		description = kraken_icon_container_get_icon_description (container, icon->data);
...
static void
kraken_icon_canvas_item_accessible_get_image_size
	(AtkImage *image,
	 gint     *width,
	 gint     *height)
{
...
static void
kraken_icon_canvas_item_accessible_get_image_position
	(AtkImage		 *image,
	 gint                    *x,
	 gint	                 *y,
...
static gboolean
kraken_icon_canvas_item_accessible_set_image_description (AtkImage    *image,
...
static void
kraken_icon_canvas_item_accessible_image_interface_init (AtkImageIface *iface)
{
	iface->get_image_description = kraken_icon_canvas_item_accessible_get_image_description;
	iface->set_image_description = kraken_icon_canvas_item_accessible_set_image_description;
	iface->get_image_size        = kraken_icon_canvas_item_accessible_get_image_size;
	iface->get_image_position    = kraken_icon_canvas_item_accessible_get_image_position;
}

/* accessible text interface */
static gint
kraken_icon_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
                                                          gint           x,
                                                          gint           y,
                                                          AtkCoordType coords)
{
	...
	item = KRAKEN_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (text)));
...
static void
kraken_icon_canvas_item_accessible_get_character_extents (AtkText	   *text,
                                                            gint	   offset,
                                                            gint	   *x,
                                                            gint	   *y,
                                                            gint	   *width,
...
static void
kraken_icon_canvas_item_accessible_text_interface_init (AtkTextIface *iface)
{
	...
	iface->get_character_extents   = kraken_icon_canvas_item_accessible_get_character_extents;
	iface->get_offset_at_point     = kraken_icon_canvas_item_accessible_get_offset_at_point;
}

typedef struct {
	AtkGObjectAccessible parent;
} KrakenIconCanvasItemAccessible;

typedef struct {
	AtkGObjectAccessibleClass parent_class;
} KrakenIconCanvasItemAccessibleClass;

G_DEFINE_TYPE_WITH_CODE (KrakenIconCanvasItemAccessible,
			 kraken_icon_canvas_item_accessible,
			 ATK_TYPE_GOBJECT_ACCESSIBLE,
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_IMAGE,
						kraken_icon_canvas_item_accessible_image_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_TEXT,
						kraken_icon_canvas_item_accessible_text_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
						kraken_icon_canvas_item_accessible_action_interface_init));

static AtkStateSet*
kraken_icon_canvas_item_accessible_ref_state_set (AtkObject *accessible)
{
	...
	state_set = ATK_OBJECT_CLASS (kraken_icon_canvas_item_accessible_parent_class)->ref_state_set (accessible);
...
static void
kraken_icon_canvas_item_accessible_finalize (GObject *object)
{
	KrakenIconCanvasItemAccessiblePrivate *priv;
	int i;
	...
	g_free (priv->description);
        ...
        G_OBJECT_CLASS (kraken_icon_canvas_item_accessible_parent_class)->finalize (object);
}

static void
kraken_icon_canvas_item_accessible_initialize (AtkObject *accessible,
						 gpointer widget)
{
	ATK_OBJECT_CLASS (kraken_icon_canvas_item_accessible_parent_class)->initialize (accessible, widget);

	atk_object_set_role (accessible, ATK_ROLE_ICON);
}

static void
kraken_icon_canvas_item_accessible_class_init (KrakenIconCanvasItemAccessibleClass *klass)
{
	AtkObjectClass *aclass = ATK_OBJECT_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = kraken_icon_canvas_item_accessible_finalize;

	aclass->initialize = kraken_icon_canvas_item_accessible_initialize;

	aclass->get_name = kraken_icon_canvas_item_accessible_get_name;
	aclass->get_description = kraken_icon_canvas_item_accessible_get_description;
	aclass->get_parent = kraken_icon_canvas_item_accessible_get_parent;
	aclass->get_index_in_parent = kraken_icon_canvas_item_accessible_get_index_in_parent;
	aclass->ref_state_set = kraken_icon_canvas_item_accessible_ref_state_set;

	g_type_class_add_private (klass, sizeof (KrakenIconCanvasItemAccessiblePrivate));
}

static void
kraken_icon_canvas_item_accessible_init (KrakenIconCanvasItemAccessible *self)
{
}

/* dummy typedef */
typedef AtkObjectFactory      KrakenIconCanvasItemAccessibleFactory;
typedef AtkObjectFactoryClass KrakenIconCanvasItemAccessibleFactoryClass;

G_DEFINE_TYPE (KrakenIconCanvasItemAccessibleFactory, kraken_icon_canvas_item_accessible_factory,
	       ATK_TYPE_OBJECT_FACTORY);

static AtkObject *
kraken_icon_canvas_item_accessible_factory_create_accessible (GObject *for_object)
{
	AtkObject *accessible;
	KrakenIconCanvasItem *item;
	GString *item_text;

	item = KRAKEN_ICON_CANVAS_ITEM (for_object);
	g_assert (item != NULL);
	...
	accessible = g_object_new (kraken_icon_canvas_item_accessible_get_type (), NULL);
...
static GType
kraken_icon_canvas_item_accessible_factory_get_accessible_type (void)
{
	return kraken_icon_canvas_item_accessible_get_type ();
}

static void
kraken_icon_canvas_item_accessible_factory_init (KrakenIconCanvasItemAccessibleFactory *self)
{
}

static void
kraken_icon_canvas_item_accessible_factory_class_init (KrakenIconCanvasItemAccessibleFactoryClass *klass)
{
	klass->create_accessible = kraken_icon_canvas_item_accessible_factory_create_accessible;
	klass->get_accessible_type = kraken_icon_canvas_item_accessible_factory_get_accessible_type;
}
