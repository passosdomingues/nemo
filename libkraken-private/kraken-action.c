...
#include "kraken-action-symbols.h"
...
#include "kraken-program-choosing.h"
#include "kraken-ui-utilities.h"

#define DEBUG_FLAG KRAKEN_DEBUG_ACTIONS
...
G_DEFINE_TYPE_WITH_PRIVATE (KrakenAction, kraken_action, GTK_TYPE_ACTION)

static void     kraken_action_get_property  (GObject                    *object,
                                           guint                       param_id,
                                           GValue                     *value,
                                           GParamSpec                 *pspec);
static void     kraken_action_set_property  (GObject                    *object,
                                           guint                       param_id,
                                           const GValue               *value,
                                           GParamSpec                 *pspec);
static void     kraken_action_constructed (GObject *object);
static void     kraken_action_finalize (GObject *gobject);
...
static void
kraken_action_init (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static void
kraken_action_class_init (KrakenActionClass *klass)
{
    GObjectClass         *object_class = G_OBJECT_CLASS(klass);
    object_class->finalize = kraken_action_finalize;
    object_class->set_property = kraken_action_set_property;
    object_class->get_property = kraken_action_get_property;
    object_class->constructed = kraken_action_constructed;
...
static gboolean
recalc_dbus_conditions (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static void
queue_recalc_dbus_conditions (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static void
setup_dbus_condition (KrakenAction *action, const gchar *condition)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static gboolean
recalc_gsettings_conditions (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
        ...
        if (len == 3) {
            ...
            if (g_variant_type_equal (setting_type, target_type)) {
                ...
                if (target_var != NULL) {
                    ...
                    g_warning ("Kraken Action: gsettings value could not be parsed into a valid GVariant");
                }
            }
...
static void
queue_recalc_gsettings_conditions (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static void
setup_gsettings_condition (KrakenAction *action,
                           const gchar *condition)
{
    ...
    if (len == 6 &&
        ...
        g_warning ("Kraken Action: Either gsettings variant type (%s) or operator (%s) is invalid.",
...
static void
populate_patterns_and_filenames (KrakenAction   *action,
                                 gchar       **array,
                                 GList       **allowed_patterns,
                                 GList       **forbidden_patterns,
                                 GList       **allowed_filenames,
...
void
kraken_action_constructed (GObject *object)
{
    KrakenAction *action = KRAKEN_ACTION (object);
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    GIcon *gicon;

    G_OBJECT_CLASS (kraken_action_parent_class)->constructed (object);
    ...
    if (conditions && condition_count > 0) {
        ...
        for (j = 0; j < condition_count; j++) {
            ...
            if (g_str_has_prefix (condition, "exec")) {
                /* handled in kraken_action_get_visibility */
            }
            else
            ...
            if (g_strcmp0 (condition, "removable") == 0) {
                /* this is handled in kraken_action_get_visibility() */
            }
            else {
                g_warning ("Ignoring invalid condition: %s."
                           " See sample action at /usr/share/kraken/actions/sample.kraken_action", condition);
            }
        }
    }
...
KrakenAction *
kraken_action_new (const gchar *name, 
                 const gchar *path)
{
    GKeyFile *key_file = g_key_file_new();
    ...
    if (!g_key_file_has_group (key_file, ACTION_FILE_GROUP)) {
        DEBUG ("Action file '%s' is missing [Kraken Action] group, skipping.", path);
        g_key_file_free (key_file);
        return NULL;
    }
    ...
    if (deps != NULL) {
        ...
        for (i = 0; i < g_strv_length (deps); i++) {
            ...
            if (g_path_is_absolute (prg_name)) {
                if ((!kraken_path_is_network_safe (prg_name)) && g_file_test (prg_name, G_FILE_TEST_EXISTS)) {
                    found = TRUE;
                }
            } else {
    ...
    return finish ? g_object_new (KRAKEN_TYPE_ACTION,
                                  "name", name,
                                  "key-file-path", path,
                                  NULL): NULL;
}
...
static void
kraken_action_finalize (GObject *object)
{
    KrakenAction *action = KRAKEN_ACTION (object);
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    ...
    G_OBJECT_CLASS (kraken_action_parent_class)->finalize (object);
}

static void
kraken_action_set_property (GObject         *object,
                          guint            prop_id,
                          const GValue    *value,
                          GParamSpec      *pspec)
{
    ...
    action = KRAKEN_ACTION (object);

    switch (prop_id)
    {
    case PROP_KEY_FILE_PATH:
      action->key_file_path = g_strdup (g_value_get_string (value));
      action->uuid = kraken_make_action_uuid_for_path (action->key_file_path);
      break;
    default:
...
static void
kraken_action_get_property (GObject    *object,
             guint       prop_id,
             GValue     *value,
             GParamSpec *pspec)
{
...
static gchar *
get_path (KrakenAction *action, KrakenFile *file)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    gchar *ret, *orig;
    GFile *location;
...
static GString *
score_append (KrakenAction *action, GString *str, const gchar *c)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static GString *
insert_separator (KrakenAction *action, GString *str)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static GString *
insert_quote (KrakenAction *action, GString *str)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
static gchar *
get_device_path (KrakenAction *action, KrakenFile *file)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    GMount *mount = kraken_file_get_mount (file);
...
static gchar *
get_insertion_string (KrakenAction *action,
                      TokenType   token_type,
                      GList      *selection,
                      KrakenFile   *parent,
                      GtkWindow  *window)
        ...
        case TOKEN_PATH_LIST:
            if (g_list_length (selection) > 0) {
                for (l = selection; l != NULL; l = l->next) {
                    ...
                    gchar *path = get_path (action, KRAKEN_FILE (l->data));
                    if (path)
        ...
        case TOKEN_URI_LIST:
            if (g_list_length (selection) > 0) {
                for (l = selection; l != NULL; l = l->next) {
                    ...
                    if (kraken_file_is_in_recent (KRAKEN_FILE (l->data))) {
                        uri = kraken_file_get_activation_uri (KRAKEN_FILE (l->data));
                    } else {
                        uri = kraken_file_get_uri (KRAKEN_FILE (l->data));
                    }
...
default_parent_path:
            ...
            if (parent != NULL) {
                ...
                if (path == NULL) {
                    gchar *name = kraken_file_get_display_name (parent);
                    if (g_strcmp0 (name, "x-kraken-desktop") == 0)
                ...
                if (g_list_length (selection) > 0 && kraken_file_is_in_recent (KRAKEN_FILE (selection->data))) {
...
default_parent_uri:
            ...
            if (parent != NULL) {
                ...
                gchar *name = kraken_file_get_display_name (parent);
                if (g_strcmp0 (name, "x-kraken-desktop") == 0) {
                    gchar *real_desktop_path = kraken_get_desktop_directory ();
                    ...
                    uri = kraken_file_get_uri (parent);
                }
        ...
        case TOKEN_FILE_DISPLAY_NAME:
            if (g_list_length (selection) > 0) {
                gchar *file_display_name = kraken_file_get_display_name (KRAKEN_FILE (selection->data));
...
default_parent_display_name:
            ...
            if (parent != NULL) {
                ...
                gchar *real_display_name = kraken_file_get_display_name (parent);
                if (g_strcmp0 (real_display_name, "x-kraken-desktop") == 0)
                    parent_display_name = g_strdup_printf (_("Desktop"));
                else
                    parent_display_name = kraken_file_get_display_name (parent);
            ...
            if (g_list_length (selection) > 0) {
                for (l = selection; l != NULL; l = l->next) {
                    if (!first)
                        str = insert_separator (action, str);
                    str = insert_quote (action, str);
                    gchar *dev = get_device_path (action, KRAKEN_FILE (l->data));
                    if (dev)
...
static GString *
expand_action_string (KrakenAction *action,
                      GList      *selection,
                      KrakenFile   *parent,
                      GString    *str,
                      GtkWindow  *window)
{
    ...
    while (ptr != NULL) {
         ...
         * tokens (%U, %F, etc...).
         *
         * See: https://github.com/linuxmint/kraken/issues/1956
         */
...
void
kraken_action_activate (KrakenAction *action,
                      GList      *selection,
                      KrakenFile   *parent,
                      GtkWindow  *window)
{
    ...
    if (priv->run_in_terminal) {
        ...
        if (g_shell_parse_argv (exec->str, &argcp, &argvp, &error)) {
            kraken_launch_application_from_command_array (gdk_screen_get_default (),
                                                        argvp[0],
                                                        TRUE,
...
const gchar *
kraken_action_get_orig_label (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
...
void
kraken_action_override_label (KrakenAction  *action,
                            const gchar *label)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    g_free (priv->orig_label);
...
void
kraken_action_override_icon (KrakenAction  *action,
...
const gchar *
kraken_action_get_orig_tt (KrakenAction *action)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    return priv->orig_tt;
}
...
static gchar *
get_final_label (KrakenAction *action,
                 GList      *selection,
                 KrakenFile   *parent,
                 GtkWindow  *window)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    const gchar *orig_label = kraken_action_get_orig_label (action);
...
static gchar *
get_final_tt (KrakenAction *action,
              GList      *selection,
              KrakenFile   *parent,
              GtkWindow  *window)
{
    KrakenActionPrivate *priv = kraken_action_get_instance_private (action);
    const gchar *orig_tt = kraken_action_get_orig_tt (action);
...
static void
finalize_strings (KrakenAction *action,
                  GList      *selection,
                  KrakenFile   *parent,
                  GtkWindow  *window)
{
...
static gboolean
check_exec_condition (KrakenAction  *action,
                      const gchar *condition,
                      GList       *selection,
                      KrakenFile    *parent,
                      GtkWindow   *window)
...
static gboolean
get_is_dir (KrakenFile *file)
{
    gboolean ret = FALSE;

    GFile *f = kraken_file_get_location (file);

    if (g_file_is_native (f) && (!kraken_location_is_network_safe (f))) {
        gchar *path;
        ...
        ret = kraken_file_is_directory (file);
    }
...
static gboolean
check_is_allowed (KrakenAction *action,
                  KrakenFile   *parent,
                  GList      *selection,
                  GList      *allowed_patterns,
                  GList      *forbidden_patterns,
                  ...
                  GList      *forbidden_names)
{
    ...
    if (parent != NULL) {
        DEBUG ("Checking pattern/name matching for location: %s", kraken_file_peek_name (parent));
    ...
    for (l = files; l != NULL; l = l->next) {
        ...
        g_autofree gchar *path = kraken_file_get_path (file);
        ...
        const gchar *name = kraken_file_peek_name (file);
...
static gboolean
get_visibility (KrakenAction *action,
                GList      *selection,
                KrakenFile   *parent,
                gboolean    for_places,
                GtkWindow  *window)
{
    ...
    if ((priv->uri_scheme != NULL) && !kraken_file_has_uri_scheme (parent, priv->uri_scheme)) {
        return FALSE;
    }
    ...
    if (ext_count == 1 && g_strcmp0 (extensions[0], "any") == 0) {
      ...
      for (iter = selection; iter != NULL && found_match; iter = iter->next) {
          found_match = FALSE;
          gboolean is_dir;
          gchar *raw_fn = kraken_file_get_name (KRAKEN_FILE (iter->data));
          ...
          if (mime_count > 0) {
              for (i = 0; i < mime_count; i++) {
                  if (kraken_file_is_mime_type (KRAKEN_FILE (iter->data), mimetypes[i])) {
                      found_match = TRUE;
                      break;
                  }
              }
          ...
          if (kraken_file_is_mime_type (KRAKEN_FILE (iter->data), "application/x-kraken-link")) {
              found_match = FALSE;
          }
      }
    ...
    if (condition_count > 0) {
        ...
        for (j = 0; j < condition_count; j++) {
            ...
            if (g_strcmp0 (condition, "desktop") == 0) {
                gchar *name = kraken_file_get_display_name (parent);
                if (g_strcmp0 (name, "x-kraken-desktop") != 0 && !(parent == NULL && priv->show_in_blank_desktop))
                ...
                if (g_list_length (selection) > 0) {
                    ...
                    if (!mount && for_places) {
                        ...
                        f = kraken_file_get_location (file);

                        if (g_file_is_native (f)) {
                            mount = kraken_get_mount_for_location_safe (f);
...
void
kraken_action_update_display_state (KrakenAction *action,
                                  GList      *selection,
                                  KrakenFile   *parent,
                                  gboolean    for_places,
                                  GtkWindow  *window)
