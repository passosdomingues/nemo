...
/* kraken-file-operations.c - Kraken file operations.
...
#include "kraken-file-changes-queue.h"
#include "kraken-lib-self-check-functions.h"

#include "kraken-progress-info.h"
...
typedef struct {
	...
	gchar *target_name;
	KrakenCopyCallback  done_callback;
...
typedef struct {
	...
	KrakenDeleteCallback done_callback;
	gpointer done_callback_data;
} DeleteJob;
...
typedef struct {
	...
	GFile *created_file;
	KrakenCreateCallback done_callback;
	gpointer done_callback_data;
} CreateJob;
...
typedef struct {
	...
	KrakenOpCallback done_callback;
	gpointer done_callback_data;
} EmptyTrashJob;
...
static void
generate_initial_job_details (KrakenProgressInfo *info,
                              OpKind            kind,
                              GList            *files,
                              GFile            *destination)
{
    ...
    kraken_progress_info_take_initial_details (info, s);
}
	     ...
	     GtkWindow *parent_window)
{
	...
	common->progress = kraken_progress_info_new ();
	common->cancellable = kraken_progress_info_get_cancellable (common->progress);
	common->inhibit_cookie = -1;
    common->monitor_num = 0;
	if (parent_window) {
        common->monitor_num = kraken_desktop_utils_get_monitor_for_widget (GTK_WIDGET (parent_window));
	}
...
{
	kraken_progress_info_finish (common->progress);

	if (common->inhibit_cookie != -1) {
		kraken_uninhibit_power_manager (common->inhibit_cookie);
	}
	...
	if (common->undo_info != NULL) {
		kraken_file_undo_manager_set_action (common->undo_info);
...
can_delete_without_confirm (GFile *file)
{
	if (g_file_has_uri_scheme (file, "burn") ||
	    ...
	    g_file_has_uri_scheme (file, "x-kraken-desktop")) {
		return TRUE;
	}
		      ...
		      va_list varargs)
{
    ...
    kraken_progress_info_pause (job->progress);
    ...
    kraken_progress_info_resume (job->progress);
...
{
	job->inhibit_cookie = kraken_inhibit_power_manager (message);
}
...
should_confirm_move_to_trash (void)
{
	...
	confirm_move_to_trash = g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_CONFIRM_MOVE_TO_TRASH);
...
should_confirm_trash (void)
{
	...
	confirm_trash = g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_CONFIRM_TRASH);
			...
			TransferInfo *transfer_info)
{
	...
	kraken_progress_info_take_status (job->progress,
					    f (_("Deleting files")));

	elapsed = kraken_progress_info_get_elapsed_time (job->progress);
	if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE) {
        if (kraken_progress_info_get_is_paused (job->progress)) {
            kraken_progress_info_set_details (job->progress, _("Paused"));
        } else {
            kraken_progress_info_set_details (job->progress, files_left_s);
        }
	} else {
		...
		kraken_progress_info_take_details (job->progress, details);
	...
	if (source_info->num_files != 0) {
		kraken_progress_info_set_progress (job->progress, transfer_info->num_files, source_info->num_files);
	}
}
 ...
 retry:
	...
	if (!job_aborted (job) &&
		...
		skip:
			...
			kraken_file_changes_queue_file_removed (dir);
	     ...
	     gboolean toplevel)
{
	...
	if (file_delete_wrapper (file, job->cancellable, &error)) {
		...
		kraken_file_changes_queue_file_removed (file);
...
delete_files (CommonJob *job, GList *files, guint *files_skipped)
{
	...
	kraken_progress_info_start (job->progress);
		       ...
		       int total_files)
{
	...
	s = f (ngettext ("%'d file left to trash",
			 ...
			 files_left),
	       files_left);
	kraken_progress_info_take_details (job->progress, s);

	if (total_files != 0) {
		kraken_progress_info_set_progress (job->progress, files_trashed, total_files);
	}
}
	...
	for (l = files;
		...
		skip:
			...
			if (job->undo_info != NULL) {
				kraken_file_undo_info_trash_add_file (KRAKEN_FILE_UNDO_INFO_TRASH (job->undo_info), file);
			}
	    ...
	    gpointer user_data)
{
    ...
    kraken_progress_info_start (common->progress);
			  ...
			  gpointer                done_callback_data)
{
	...
	if (try_trash && !kraken_file_undo_manager_pop_flag ()) {
		job->common.undo_info = kraken_file_undo_info_trash_new (g_list_length (files));
	}
...
void
kraken_file_operations_trash_or_delete (GList                  *files,
					  GtkWindow              *parent_window,
					  KrakenDeleteCallback  done_callback,
					  gpointer                done_callback_data)
{
...
void
kraken_file_operations_delete (GList                  *files,
				 GtkWindow              *parent_window,
				 KrakenDeleteCallback  done_callback,
				 gpointer                done_callback_data)
{
...
typedef struct {
	...
	GtkWindow *parent_window;
	KrakenUnmountCallback callback;
...
void
kraken_file_operations_unmount_mount_full (GtkWindow                      *parent_window,
					     GMount                         *mount,
                         GMountOperation                *mount_operation,
					     gboolean                        eject,
...
void
kraken_file_operations_unmount_mount (GtkWindow                      *parent_window,
					GMount                         *mount,
					gboolean                        eject,
					gboolean                        check_trash)
{
	kraken_file_operations_unmount_mount_full (parent_window, mount, NULL, eject,
...
volume_mount_cb (GObject *source_object,
		 ...
		 gpointer user_data)
{
	KrakenMountCallback mount_callback;
	...
	mount_callback = (KrakenMountCallback)
...
void
kraken_file_operations_mount_volume (GtkWindow *parent_window,
				       GVolume *volume)
{
	kraken_file_operations_mount_volume_full (parent_window, volume,
						    NULL, NULL);
}

void
kraken_file_operations_mount_volume_full (GtkWindow *parent_window,
					    GVolume *volume,
					    KrakenMountCallback mount_callback,
					    GObject *mount_callback_data_object)
{
		       ...
		       SourceInfo *source_info)
{
	...
	kraken_progress_info_take_details (job->progress, details);
	kraken_progress_info_pulse_progress (job->progress);
}
		      ...
		      TransferInfo *transfer_info)
{
	...
	if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE &&
        ...
        if (kraken_progress_info_get_is_paused (job->progress)) {
            kraken_progress_info_take_details (job->progress, g_strdup (_("Paused")));
        } else {
            char *s;
	...
	kraken_progress_info_set_progress (job->progress, transfer_info->num_bytes, total_size);
}
		 ...
		 char **dest_fs_type)
{
 ...
 retry:
	...
	kraken_file_changes_queue_file_added (*dest);

	if (job->undo_info != NULL) {
		kraken_file_undo_info_ext_add_origin_target_pair (KRAKEN_FILE_UNDO_INFO_EXT (job->undo_info),
								    src, *dest);
	}
...
do_run_conflict_dialog (gpointer _data)
{
	...
	dialog = kraken_file_conflict_dialog_new (data->parent,
						    data->src,
						    data->dest,
						    data->dest_dir);
	...
	if (response == CONFLICT_RESPONSE_RENAME) {
		data->resp_data->new_name =
			kraken_file_conflict_dialog_get_new_name (KRAKEN_FILE_CONFLICT_DIALOG (dialog));
	} else if (response != GTK_RESPONSE_CANCEL ||
		   response != GTK_RESPONSE_NONE) {
		   data->resp_data->apply_to_all =
			   kraken_file_conflict_dialog_get_apply_to_all
				(KRAKEN_FILE_CONFLICT_DIALOG (dialog));
	}
 ...
 retry:
	...
	if (res) {
        ...
        if (debuting_files) {
            if (target_is_desktop && position) {
                kraken_file_changes_queue_schedule_position_set (dest, *position, job->monitor_num);
            } else if (source_is_desktop && copy_job->is_move) {
                kraken_file_changes_queue_schedule_position_remove (dest);
            }
		...
		if (copy_job->is_move) {
            ...
            g_free (src_uri);
            g_free (dest_uri);
			kraken_file_changes_queue_file_moved (src, dest);
		} else {
			kraken_file_changes_queue_file_added (dest);
		}
	...
	/* Needs to recurse */
		...
		if (overwrite && would_recurse) {
			...
			kraken_file_changes_queue_file_removed (dest);
		}
...
void
kraken_file_operations_copy_file (GFile *source_file,
				    GFile *target_dir,
...
void
kraken_file_operations_copy (GList *files,
			       ...
			       gpointer done_callback_data)
{
	...
	job = op_job_new (CopyMoveJob, parent_window);
	job->desktop_location = kraken_get_desktop_location ();
	job->done_callback = done_callback;
	job->done_callback_data = done_callback_data;
	job->files = eel_g_object_list_copy (files);
	job->destination = g_object_ref (target_dir);
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		...
		job->common.undo_info = kraken_file_undo_info_ext_new (KRAKEN_FILE_UNDO_OP_COPY,
									 g_list_length (files),
									 src_dir, target_dir);
...
report_move_progress (CopyMoveJob *move_job, int total, int left)
{
    ...
    if (kraken_progress_info_get_is_paused (job->progress)) {
    	...
    	kraken_progress_info_take_details (job->progress,
...
void
kraken_file_operations_move (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       KrakenCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		...
		if (g_file_has_uri_scheme (g_list_first (files)->data, "trash")) {
			job->common.undo_info = kraken_file_undo_info_ext_new (KRAKEN_FILE_UNDO_OP_RESTORE_FROM_TRASH,
										 g_list_length (files),
										 src_dir, target_dir);
		} else {
			job->common.undo_info = kraken_file_undo_info_ext_new (KRAKEN_FILE_UNDO_OP_MOVE,
										 g_list_length (files),
										 src_dir, target_dir);
		}
...
report_link_progress (CopyMoveJob *link_job, int total, int left)
{
	...
	kraken_progress_info_set_progress (job->progress, left, total);
}
	   ...
	   int files_left)
{
 ...
 retry:
	...
	if (path == NULL) {
		...
		if (common->undo_info != NULL) {
			kraken_file_undo_info_ext_add_origin_target_pair (KRAKEN_FILE_UNDO_INFO_EXT (common->undo_info),
									    src, dest);
		}
        ...
        if (target_is_desktop && position) {
            kraken_file_changes_queue_schedule_position_set (dest, *position, common->monitor_num);
        }
...
void
kraken_file_operations_link (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       KrakenCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		...
		src_dir = g_file_get_parent (files->data);
		job->common.undo_info = kraken_file_undo_info_ext_new (KRAKEN_FILE_UNDO_OP_CREATE_LINK,
									 g_list_length (files),
									 src_dir, target_dir);
		g_object_unref (src_dir);
	}
...
void
kraken_file_operations_duplicate (GList *files,
				    GArray *relative_item_points,
				    GtkWindow *parent_window,
				    KrakenCopyCallback  done_callback,
				    gpointer done_callback_data)
{
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		...
		job->common.undo_info =
			kraken_file_undo_info_ext_new (KRAKEN_FILE_UNDO_OP_DUPLICATE,
							 g_list_length (files),
							 src_dir, src_dir);
		g_object_unref (src_dir);
	}
		      ...
		      GFileInfo *info)
{
	...
	kraken_progress_info_pulse_progress (common->progress);
	...
	if (!job_aborted (common) &&
		...
		if (common->undo_info != NULL) {
			kraken_file_undo_info_rec_permissions_add_file (KRAKEN_FILE_UNDO_INFO_REC_PERMISSIONS (common->undo_info),
									  file, current);
		}
		     ...
		     gpointer user_data)
{
	...
	kraken_progress_info_set_status (common->progress,
...
void
kraken_file_set_permissions_recursive (const char *directory,
					 guint32         file_permissions,
					 guint32         file_mask,
					 guint32         dir_permissions,
					 guint32         dir_mask,
					 KrakenOpCallback  callback,
					 gpointer  callback_data)
{
	SetPermissionsJob *job;
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		job->common.undo_info =
			kraken_file_undo_info_rec_permissions_new (job->file,
...
typedef struct {
	KrakenCopyCallback real_callback;
	gpointer real_data;
} MoveTrashCBData;
...
void
kraken_file_operations_copy_move (const GList *item_uris,
				    GArray *relative_item_points,
				    ...
				    gpointer done_callback_data)
{
	...
	if (copy_action == GDK_ACTION_COPY) {
		...
		if (target_dir == NULL ||
			...
			kraken_file_operations_duplicate (locations,
							    relative_item_points,
							    parent_window,
							    done_callback, done_callback_data);
		} else {
			kraken_file_operations_copy (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		if (g_file_has_uri_scheme (dest, "trash")) {
			...
			kraken_file_operations_trash_or_delete (locations,
								  parent_window,
								  (KrakenDeleteCallback) callback_for_move_to_trash,
								  cb_data);
		} else {

			kraken_file_operations_move (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		kraken_file_operations_link (locations,
					       relative_item_points,
					       dest,
					       parent_window,
					       done_callback, done_callback_data);
 ...
 retry:
	...
	if (job->make_dir) {
		...
		if (res && common->undo_info != NULL) {
			kraken_file_undo_info_create_set_data (KRAKEN_FILE_UNDO_INFO_CREATE (common->undo_info),
								 dest, NULL, 0);
		}
	...
	if (res) {
		...
		kraken_file_changes_queue_file_added (dest);
		if (job->has_position) {
			kraken_file_changes_queue_schedule_position_set (dest, job->position, common->monitor_num);
		}
	} else {
		g_assert (error != NULL);
...
void
kraken_file_operations_new_folder (GtkWidget *parent_view,
				     ...
				     gpointer done_callback_data)
{
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		job->common.undo_info = kraken_file_undo_info_create_new (KRAKEN_FILE_UNDO_OP_CREATE_FOLDER);
	}
...
void
kraken_file_operations_new_file_from_template (GtkWidget *parent_view,
						 GdkPoint *target_point,
						 const char *parent_dir,
						 ...
						 gpointer done_callback_data)
{
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		job->common.undo_info = kraken_file_undo_info_create_new (KRAKEN_FILE_UNDO_OP_CREATE_FILE_FROM_TEMPLATE);
	}
...
void
kraken_file_operations_new_file (GtkWidget *parent_view,
				   GdkPoint *target_point,
				   const char *parent_dir,
				   const char *target_filename,
				   ...
				   gpointer done_callback_data)
{
	...
	if (!kraken_file_undo_manager_pop_flag ()) {
		job->common.undo_info = kraken_file_undo_info_create_new (KRAKEN_FILE_UNDO_OP_CREATE_EMPTY_FILE);
	}
		 ...
		 gpointer user_data)
{
	...
	if (confirmed) {
		kraken_progress_info_set_status (common->progress, _("Emptying Trash"));
		kraken_progress_info_set_details (common->progress, _("Emptying Trash"));
...
void
kraken_file_operations_empty_trash (GtkWidget *parent_view)
{
	EmptyTrashJob *job;
	GtkWindow *parent_window;
...
void
kraken_file_mark_desktop_file_trusted (GFile *file,
					 GtkWindow *parent_window,
					 gboolean interactive,
					 KrakenOpCallback done_callback,
					 gpointer done_callback_data)
...
job_is_local (GList *files, GFile *destination)
{
    ...
    KrakenFile *source = kraken_file_get_existing (G_FILE (files->data));

    if (source == NULL)
        return FALSE;

    KrakenFile *dest = destination != NULL ? kraken_file_get_existing (destination) : NULL;

    if (dest != NULL) {
        ret = kraken_file_is_local (source) &&
              kraken_file_is_local (dest);
    } else {
        ret = kraken_file_is_local (source);
    }

    kraken_file_unref (source);
    kraken_file_unref (dest);
...
job_is_same_fs (GList *files, GFile *destination)
{
    ...
    KrakenFile *dest = kraken_file_get_existing (destination);

    if (dest != NULL) {
        gchar *src_fs_id = kraken_file_get_filesystem_id (source);
        gchar *dst_fs_id = kraken_file_get_filesystem_id (dest);
...
job_has_no_folders (GList *files)
{
    ...
    for (l = files; l != NULL; l = l->next) {
        ...
        KrakenFile *file = kraken_file_get_existing (location);
...
job_is_small (GList *files)
{
    ...
    for (l = files; l != NULL; l = l->next) {
        ...
        size = size + kraken_file_get_size (file);
                                  ...
                                  OpKind  kind)
{
    ...
    KrakenJobQueue *job_queue = kraken_job_queue_get ();

    start_immediately = should_start_immediately (kind, user_data);

    kraken_job_queue_add_new_job (job_queue,
                                job_func,
                                user_data,
                                cancellable,
                                info,
...
void
kraken_self_check_file_operations (void)
{
