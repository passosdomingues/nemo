...
#include "kraken-action-manager.h"
...
} KrakenActionManagerPrivate;

struct _KrakenActionManager 
{
    GObject parent_instance;
};

G_DEFINE_TYPE_WITH_PRIVATE (KrakenActionManager, kraken_action_manager, G_TYPE_OBJECT)

static void     refresh_actions                 (KrakenActionManager *action_manager, const gchar *directory);
static void     add_action_to_action_list       (KrakenActionManager *action_manager, const gchar *path);
                 ...
                 gpointer           user_data)
{
    ...
    action_manager = KRAKEN_ACTION_MANAGER (user_data);
...
{
    g_return_if_fail (KRAKEN_IS_ACTION_MANAGER (user_data));

    KrakenActionManager *action_manager = KRAKEN_ACTION_MANAGER (user_data);
...
static void
add_directory_to_actions_directory_list (KrakenActionManager *action_manager,
                                         const gchar       *path)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
...
static void
process_directory_actions (KrakenActionManager *action_manager,
                           const gchar       *directory)
{
    ...
    disabled_actions = g_settings_get_strv (kraken_plugin_preferences, KRAKEN_PLUGIN_PREFERENCES_DISABLED_ACTIONS);
    ...
    if (dir) {
        ...
        while ((name = g_dir_read_name (dir))) {
            if (g_str_has_suffix (name, ".kraken_action")) {
...
static void
set_up_actions_directories (KrakenActionManager *action_manager)
{
    gchar *path;
    gchar **data_dirs;
    guint i;
    ...
    for (i = 0; i < g_strv_length (data_dirs); i++) {
        path = kraken_action_manager_get_system_directory_path (data_dirs[i]);
    ...
    if (!g_file_test (path, G_FILE_TEST_EXISTS)) {
        g_mkdir_with_parents (path, DEFAULT_KRAKEN_DIRECTORY_MODE);
    }
...
static char *
create_action_name (KrakenActionManager *manager,
                    const char        *uri)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (manager);
    GString *s;
...
static void
on_action_condition_changed (KrakenActionManager *action_manager)
{
...
static void
add_action_to_action_list (KrakenActionManager *action_manager,
                           const gchar       *path)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    ...
    action = kraken_action_new (action_name, path);
...
static void
void_actions_for_directory (KrakenActionManager *action_manager,
                            const gchar       *directory)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    ...
    for (l = priv->actions; l != NULL; l = l->next) {
        KrakenAction *action = KRAKEN_ACTION (l->data);
...
static void
reload_actions_layout (KrakenActionManager *action_manager)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    GFile *file;
    GFileInfo *info;
    ...
    path = g_build_filename (g_get_user_config_dir (), "kraken", LAYOUT_FILENAME, NULL);
...
static void
kraken_config_dir_changed (GFileMonitor      *monitor,
                         GFile             *file,
                         GFile             *other_file,
                         GFileMonitorEvent  event_type,
                         gpointer           user_data)
...
static void
monitor_kraken_config_dir (KrakenActionManager *action_manager)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    GFile *file;
    GFileMonitor *monitor;

    g_autofree gchar *path = g_build_filename (g_get_user_config_dir (), "kraken", NULL);

    file = g_file_new_for_path (path);
    monitor = g_file_monitor_directory (file, G_FILE_MONITOR_WATCH_MOVES | G_FILE_MONITOR_SEND_MOVED, NULL, NULL);
    g_object_unref (file);
    g_signal_connect (monitor, "changed", G_CALLBACK (kraken_config_dir_changed), action_manager);
    priv->dir_monitors = g_list_prepend (priv->dir_monitors, monitor);
}
...
{
    KrakenAction *action_a = (KrakenAction *) a;
    KrakenAction *action_b = (KrakenAction *) b;

    return g_strcmp0 (action_a->uuid, action_b->uuid);
}

static void
refresh_actions (KrakenActionManager *action_manager,
                 const gchar       *directory)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
...
static void
kraken_action_manager_init (KrakenActionManager *action_manager)
{
}

void
kraken_action_manager_constructed (GObject *object)
{
    G_OBJECT_CLASS (kraken_action_manager_parent_class)->constructed (object);

    KrakenActionManager *action_manager = KRAKEN_ACTION_MANAGER (object);
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    ...
    monitor_kraken_config_dir (action_manager);
    reload_actions_layout (action_manager);
    refresh_actions (action_manager, NULL);

    g_signal_connect (kraken_plugin_preferences,
                      "changed::" KRAKEN_PLUGIN_PREFERENCES_DISABLED_ACTIONS,
                      G_CALLBACK (plugin_prefs_changed), action_manager);
}

KrakenActionManager *
kraken_action_manager_new (void)
{
    return g_object_new (KRAKEN_TYPE_ACTION_MANAGER, NULL);
}

static void
kraken_action_manager_dispose (GObject *object)
{
    KrakenActionManager *action_manager = KRAKEN_ACTION_MANAGER (object);
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    ...
    g_clear_object (&priv->json_parser);
    g_signal_handlers_disconnect_by_func (kraken_plugin_preferences, G_CALLBACK (plugin_prefs_changed), action_manager);

    G_OBJECT_CLASS (kraken_action_manager_parent_class)->dispose (object);
}

static void
kraken_action_manager_finalize (GObject *object)
{
    KrakenActionManager *action_manager = KRAKEN_ACTION_MANAGER (object);
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    ...
    G_OBJECT_CLASS (kraken_action_manager_parent_class)->finalize (object);
}

static void
kraken_action_manager_class_init (KrakenActionManagerClass *klass)
{
    GObjectClass         *object_class = G_OBJECT_CLASS(klass);
    object_class->finalize = kraken_action_manager_finalize;
    object_class->dispose = kraken_action_manager_dispose;
    object_class->constructed = kraken_action_manager_constructed;

    signals[CHANGED] = g_signal_new ("changed",
                                     KRAKEN_TYPE_ACTION_MANAGER,
                                     G_SIGNAL_RUN_LAST,
                                     0,
                                     NULL, NULL, NULL,
                                     G_TYPE_NONE, 0);
...
GList *
kraken_action_manager_list_actions (KrakenActionManager *action_manager)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
...
KrakenAction *
kraken_action_manager_get_action (KrakenActionManager *action_manager,
                                const gchar       *uuid)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
    KrakenAction *action;
...
gchar *
kraken_action_manager_get_system_directory_path (const gchar *data_dir)
{
    g_autofree gchar *kraken_path = NULL;
    g_autofree gchar *target = NULL;

    kraken_path = g_build_filename (data_dir, "kraken", NULL);

    // For symbolic links, we try to figure out its actual path
    // to prevent possible duplicate right-click menu items.
    target = g_file_read_link (kraken_path, NULL);

    if (target) {
        return g_build_filename (target, "actions", NULL);
    } else {
        return g_build_filename (kraken_path, "actions", NULL);
    }
}

gchar *
kraken_action_manager_get_user_directory_path (void)
{
    return g_build_filename (g_get_user_data_dir (), "kraken", "actions", NULL);
}
...
typedef struct
{
    ...
    KrakenActionManagerIterFunc         func;
    gpointer                          user_data;
} ActionsIterData;
            ...
            const gchar               *path)
{
    ...
    if (g_strcmp0 (type, "action") == 0) {
        KrakenAction *action;
        g_autofree gchar *lookup_uuid = kraken_make_action_uuid_for_path (uuid);

        action = kraken_action_manager_get_action (idata->action_manager, lookup_uuid);
        ...
        if (user_label != NULL) {
            kraken_action_override_label (action, user_label);
        }

        if (user_icon != NULL) {
            kraken_action_override_icon (action, user_icon);
        }
...
static gboolean
iter_actions (KrakenActionManager  *action_manager,
...
static JsonReader *
get_layout_reader (KrakenActionManager *action_manager)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
...
void
kraken_action_manager_iterate_actions (KrakenActionManager                *action_manager,
                                     KrakenActionManagerIterFunc         func,
                                     gpointer                          user_data)
{
    KrakenActionManagerPrivate *priv = kraken_action_manager_get_instance_private (action_manager);
...
void
kraken_action_manager_add_action_ui (KrakenActionManager   *manager,
...
void
kraken_action_manager_update_action_states (KrakenActionManager *action_manager,
                                          GtkActionGroup    *action_group,
                                          GList             *selection,
                                          KrakenFile          *parent,
                                          gboolean           for_places,
                                          ...
                                          GtkWindow         *window)
{
    ...
    for (l = actions; l != NULL; l = l->next) {
        ...
        if (for_accelerators && !KRAKEN_ACTION (l->data)->has_accel) {
            DEBUG ("Skipping '%s' (no accelerator assigned to this action)", KRAKEN_ACTION (l->data)->uuid);
            continue;
        }

        kraken_action_update_display_state (KRAKEN_ACTION (l->data), selection, parent, for_places, window);
    }
