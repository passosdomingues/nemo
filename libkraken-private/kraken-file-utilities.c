...
/* kraken-file-utilities.c - implementation of file manipulation routines.
...
#define KRAKEN_USER_DIRECTORY_NAME "kraken"
...
static GFile *kraken_find_file_insensitive_next (GFile *parent, const gchar *name);

char *
kraken_compute_title_for_location (GFile *location)
{
	KrakenFile *file;
	char *title;
    char *builder;
	...
	builder = NULL;
	if (location) {
		file = kraken_file_get (location);
		builder = kraken_file_get_description (file);
		if (builder == NULL) {
			builder = kraken_file_get_display_name (file);
		}
    ...
    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_SHOW_FULL_PATH_TITLES)) {
        gchar *uri, *path;
        file = kraken_file_get (location);
...
// TODO: Maybe this can replace kraken_compute_title_for_location() all around?
char *
kraken_compute_search_title_for_location (GFile *location)
{
    GFile *home_file;
...
/**
 * kraken_get_user_directory:
 *
 * Get the path for the directory containing kraken settings.
 *
 * Return value: the directory path.
 **/
char *
kraken_get_user_directory (void)
{
	char *user_directory = NULL;

	user_directory = g_build_filename (g_get_user_config_dir (),
					   KRAKEN_USER_DIRECTORY_NAME,
					   NULL);

	if (!g_file_test (user_directory, G_FILE_TEST_EXISTS)) {
		g_mkdir_with_parents (user_directory, DEFAULT_KRAKEN_DIRECTORY_MODE);
		/* FIXME bugzilla.gnome.org 41286:
		 * How should we handle the case where this mkdir fails?
		 * Note that kraken_application_startup will refuse to launch if this
		 * directory doesn't get created, so that case is OK. But the directory
		 * could be deleted after Kraken was launched, and perhaps
...
/**
 * kraken_get_accel_map_file:
 *
 * Get the path for the filename containing kraken accelerator map.
 * The filename need not exist.
 *
 * Return value: the filename path, or NULL if the home directory could not be found
 **/
char *
kraken_get_accel_map_file (void)
{
	...
	if (override) {
		return g_build_filename (override, "accels/kraken", NULL);
	} else {
		return g_build_filename (g_get_home_dir (), ".gnome2/accels/kraken", NULL);
	}
}

/**
 * kraken_get_scripts_directory_path:
 *
 * Get the path for the directory containing kraken scripts.
 *
 * Return value: the directory path containing kraken scripts
 **/
char *
kraken_get_scripts_directory_path (void)
{
	return g_build_filename (g_get_user_data_dir (), "kraken", "scripts", NULL);
}
...
static void
xdg_dir_changed (KrakenFile *file,
		 XdgDirEntry *dir)
{
	...
	if (cached_xdg_dirs != NULL) {
		for (i = 0; cached_xdg_dirs[i].type != NULL; i++) {
			if (cached_xdg_dirs[i].file != NULL) {
				kraken_file_monitor_remove (cached_xdg_dirs[i].file,
							      &cached_xdg_dirs[i]);
				g_signal_handlers_disconnect_by_func (cached_xdg_dirs[i].file,
								      G_CALLBACK (xdg_dir_changed),
								      &cached_xdg_dirs[i]);
				kraken_file_unref (cached_xdg_dirs[i].file);
			}
...
update_xdg_dir_cache (void)
{
	...
	for (i = 0 ; cached_xdg_dirs[i].type != NULL; i++) {
		...
		if (strcmp (cached_xdg_dirs[i].path, g_get_home_dir ()) != 0) {
			...
			cached_xdg_dirs[i].file = kraken_file_get_by_uri (uri);
			kraken_file_monitor_add (cached_xdg_dirs[i].file,
						   &cached_xdg_dirs[i],
...
char *
kraken_get_xdg_dir (const char *type)
{
	int i;
...
{
	if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_DESKTOP_IS_HOME_DIR)) {
		return g_strdup (g_get_home_dir());
	} else {
		return kraken_get_xdg_dir ("DESKTOP");
	}
}

/**
 * kraken_get_desktop_directory:
 *
 ...
 **/
char *
kraken_get_desktop_directory (void)
{
...
GFile *
kraken_get_desktop_location (void)
{
	char *desktop_directory;
	GFile *res;
...
/**
 * kraken_get_desktop_directory_uri:
 *
 ...
 **/
char *
kraken_get_desktop_directory_uri (void)
{
	char *desktop_path;
	char *desktop_uri;
...
char *
kraken_get_desktop_directory_uri_no_create (void)
{
	char *desktop_path;
	char *desktop_uri;
...
char *
kraken_get_home_directory_uri (void)
{
...
gboolean
kraken_should_use_templates_directory (void)
{
	char *dir;
	gboolean res;

	dir = kraken_get_xdg_dir ("TEMPLATES");
...
char *
kraken_get_templates_directory (void)
{
	return kraken_get_xdg_dir ("TEMPLATES");
}

void
kraken_ensure_valid_templates_directory (void)
{
    ...
    templates_dir = kraken_get_xdg_dir ("TEMPLATES");
    ...
    if (g_strcmp0 (templates_dir, home_dir) == 0) {
        ...
        g_mkdir (fixed_templates_dir, DEFAULT_KRAKEN_DIRECTORY_MODE);
        ...
        g_mkdir (templates_dir, DEFAULT_KRAKEN_DIRECTORY_MODE);
    }
}

char *
kraken_get_templates_directory_uri (void)
{
	char *directory, *uri;

	directory = kraken_get_templates_directory ();
...
char *
kraken_get_searches_directory (void)
{
	char *user_dir;
	char *searches_dir;

	user_dir = kraken_get_user_directory ();
	searches_dir = g_build_filename (user_dir, "searches", NULL);
	g_free (user_dir);

	if (!g_file_test (searches_dir, G_FILE_TEST_EXISTS))
		g_mkdir (searches_dir, DEFAULT_KRAKEN_DIRECTORY_MODE);
...
gboolean
kraken_is_home_directory_file (GFile *dir,
				 const char *filename)
{
	char *dirname;
...
gboolean
kraken_is_home_directory (GFile *dir)
{
...
gboolean
kraken_is_root_directory (GFile *dir)
{
...
gboolean
kraken_is_desktop_directory_file (GFile *dir,
				    const char *file)
{
...
gboolean
kraken_is_desktop_directory (GFile *dir)
{
...
/**
 * kraken_get_gmc_desktop_directory:
 *
 ...
 **/
char *
kraken_get_gmc_desktop_directory (void)
{
	return g_build_filename (g_get_home_dir (), LEGACY_DESKTOP_DIRECTORY_NAME, NULL);
}

char *
kraken_get_data_file_path (const char *partial_path)
{
	char *path;
	char *user_directory;

	/* first try the user's home directory */
	user_directory = kraken_get_user_directory ();
	...
	path = g_build_filename (KRAKEN_DATADIR, partial_path, NULL);
	if (g_file_test (path, G_FILE_TEST_EXISTS)) {
		return path;
	}
	g_free (path);
...
char *
kraken_ensure_unique_file_name (const char *directory_uri,
...
char *
kraken_unique_temporary_file_name (void)
{
	const char *prefix = "/tmp/kraken-temp-file";
	char *file_name;
	int fd;
...
GFile *
kraken_find_existing_uri_in_hierarchy (GFile *location)
{
	GFileInfo *info;
	GFile *tmp;
...
kraken_find_file_insensitive (GFile *parent, const gchar *name)
{
	...
	for (i = 0; (component = split_path[i]) != NULL; i++) {
		if (!(next = kraken_find_file_insensitive_next (file,
		                                                  component))) {
			/* File does not exist */
			g_object_unref (file);
			file = NULL;
...
static GFile *
kraken_find_file_insensitive_next (GFile *parent, const gchar *name)
{
...
gboolean
kraken_is_file_roller_installed (void)
{
 ...
 * (e.g. whenever Kraken is doing file operations).
 *
 * Returns: an integer cookie, which must be passed to
 *    kraken_uninhibit_power_manager() to resume
 *    normal power management.
 */
int
kraken_inhibit_power_manager (const char *message)
{
...
/**
 * kraken_uninhibit_power_manager:
 * @cookie: the cookie value returned by kraken_inhibit_power_manager()
 *
 ...
 */
void
kraken_uninhibit_power_manager (gint cookie)
{
	GDBusConnection *connection;
	GVariant *result;
	GError *error = NULL;
...
gboolean
kraken_is_in_system_dir (GFile *file)
{
...
GHashTable *
kraken_trashed_files_get_original_directories (GList *files,
						 GList **unhandled_files)
{
	GHashTable *directories;
	KrakenFile *file, *original_file, *original_dir;
	GList *l, *m;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		original_file = kraken_file_get_trash_original_file (file);

		original_dir = NULL;
		if (original_file != NULL) {
			original_dir = kraken_file_get_parent (original_file);
		}

		if (original_dir != NULL) {
			if (directories == NULL) {
				directories = g_hash_table_new_full (g_direct_hash, g_direct_equal,
								     (GDestroyNotify) kraken_file_unref,
								     (GDestroyNotify) kraken_file_list_free);
			}
			kraken_file_ref (original_dir);
			m = g_hash_table_lookup (directories, original_dir);
			if (m != NULL) {
				g_hash_table_steal (directories, original_dir);
				kraken_file_unref (original_dir);
			}
			m = g_list_append (m, kraken_file_ref (file));
			g_hash_table_insert (directories, original_dir, m);
		} else if (unhandled_files != NULL) {
			*unhandled_files = g_list_append (*unhandled_files, kraken_file_ref (file));
		}
...
locations_from_file_list (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		file = KRAKEN_FILE (l->data);
		ret = g_list_prepend (ret, kraken_file_get_location (file));
	}
			   ...
			   gpointer user_data)
{
	...
	for (l = original_dirs; l != NULL; l = l->next) {
		original_dir = KRAKEN_FILE (l->data);
		original_dir_location = kraken_file_get_location (original_dir);

		files = g_hash_table_lookup (data->original_dirs_hash, original_dir);
		locations = locations_from_file_list (files);

		kraken_file_operations_move
			(locations, NULL,
...
void
kraken_restore_files_from_trash (GList *files,
				   GtkWindow *parent_window)
{
	KrakenFile *file;
	GHashTable *original_dirs_hash;
	...
	original_dirs_hash = kraken_trashed_files_get_original_directories (files, &unhandled_files);
	...
	kraken_file_list_unref (unhandled_files);
}
...
void
kraken_get_x_content_types_for_mount_async (GMount *mount,
					      KrakenMountGetContent callback,
					      GCancellable *cancellable,
					      gpointer user_data)
{
	...
	cached = g_object_get_data (G_OBJECT (mount), "kraken-content-type-cache");
	if (cached != NULL) {
		if (callback) {
...
char **
kraken_get_cached_x_content_types_for_mount (GMount *mount)
{
	char **cached;
...
debug_icon_names (const gchar *format, ...)
{
    ...
    if (g_once_init_enter (&once_init)) {
        debug_removable_device_icons = g_getenv ("KRAKEN_DEBUG_DEVICE_ICONS") != NULL;
                        ...
                        const gchar  *type_name)
{
    ...
    if (G_IS_THEMED_ICON (gicon)) {
        ...
        // but currently everywhere kraken needs this is looking for icon names, so this function
...
gchar *
kraken_get_mount_icon_name (GMount *mount)
{
...
gchar *
kraken_get_volume_icon_name (GVolume *volume)
{
    GtkIconTheme *icon_theme;
    GIcon *gicon;
    gchar *icon_name;
...
gchar *
kraken_get_drive_icon_name (GDrive *drive)
{
    GtkIconTheme *icon_theme;
    GIcon *gicon;
    gchar *icon_name;
...
gchar *
kraken_get_best_guess_file_mimetype (const gchar *filename,
                                   GFileInfo   *info,
                                   goffset      size)
{
...
gboolean
kraken_treating_root_as_normal (void)
{
    ...
    if (g_once_init_enter (&once_init)) {
        GSettings *prefs = g_settings_new("org.kraken.preferences");
...
GMount *
kraken_get_mount_for_location_safe (GFile *location)
{
...
gboolean
kraken_location_is_network_safe (GFile *location)
{
    GVolume *volume;
    GMount *mount;
    gboolean is_network = FALSE;

    mount = kraken_get_mount_for_location_safe (location);
    if (mount != NULL) {
...
gboolean
kraken_path_is_network_safe (const gchar *path)
{
    g_autoptr(GFile) location = g_file_new_for_path (path);

    return kraken_location_is_network_safe (location);
}

#if !defined (KRAKEN_OMIT_SELF_CHECK)

void
kraken_self_check_file_utilities (void)
{
}
