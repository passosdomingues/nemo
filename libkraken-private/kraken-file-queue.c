...
#include <config.h>
#include "kraken-file-queue.h"

#include <glib.h>

struct KrakenFileQueue {
	GList *head;
	GList *tail;
...
KrakenFileQueue *
kraken_file_queue_new (void)
{
	KrakenFileQueue *queue;
	
	queue = g_new0 (KrakenFileQueue, 1);
...
void
kraken_file_queue_destroy (KrakenFileQueue *queue)
{
	g_hash_table_destroy (queue->item_to_link_map);
	kraken_file_list_free (queue->head);
	g_free (queue);
}

void
kraken_file_queue_enqueue (KrakenFileQueue *queue,
			     KrakenFile      *file)
{
...
KrakenFile *
kraken_file_queue_dequeue (KrakenFileQueue *queue)
{
	KrakenFile *file;

	file = kraken_file_queue_head (queue);
	kraken_file_queue_remove (queue, file);
...
void
kraken_file_queue_remove (KrakenFileQueue *queue,
			    KrakenFile *file)
{
	GList *link;
...
KrakenFile *
kraken_file_queue_head (KrakenFileQueue *queue)
{
	if (queue->head == NULL) {
		return NULL;
	}

	return KRAKEN_FILE (queue->head->data);
}

gboolean
kraken_file_queue_is_empty (KrakenFileQueue *queue)
{
