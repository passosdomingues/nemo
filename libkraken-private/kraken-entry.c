...
/* KrakenEntry: one-line text editing widget. This consists of bug fixes
...
#include <config.h>
#include "kraken-entry.h"

#include <string.h>
#include "kraken-global-preferences.h"
#include "kraken-undo-signal-handlers.h"
#include <gdk/gdkkeysyms.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

struct KrakenEntryDetails {
	gboolean user_edit;
...
static void kraken_entry_editable_init (GtkEditableInterface *iface);

G_DEFINE_TYPE_WITH_CODE (KrakenEntry, kraken_entry, GTK_TYPE_ENTRY,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_EDITABLE,
						kraken_entry_editable_init));

static GtkEditableInterface *parent_editable_interface = NULL;

static void
kraken_entry_init (KrakenEntry *entry)
{
	entry->details = g_new0 (KrakenEntryDetails, 1);
	
	entry->details->user_edit = TRUE;

	kraken_undo_set_up_kraken_entry_for_undo (entry);
}

GtkWidget *
kraken_entry_new (void)
{
	return gtk_widget_new (KRAKEN_TYPE_ENTRY, NULL);
}

GtkWidget *
kraken_entry_new_with_max_length (guint16 max)
{
	GtkWidget *widget;

	widget = gtk_widget_new (KRAKEN_TYPE_ENTRY, NULL);
...
static void
kraken_entry_finalize (GObject *object)
{
	KrakenEntry *entry;

	entry = KRAKEN_ENTRY (object);
	...
	G_OBJECT_CLASS (kraken_entry_parent_class)->finalize (object);
}

static gboolean
kraken_entry_key_press (GtkWidget *widget, GdkEventKey *event)
{
	KrakenEntry *entry;
	...
	result = GTK_WIDGET_CLASS (kraken_entry_parent_class)->key_press_event (widget, event);
...
static gboolean
kraken_entry_motion_notify (GtkWidget *widget, GdkEventMotion *event)
{
	int result;
	...
	result = GTK_WIDGET_CLASS (kraken_entry_parent_class)->motion_notify_event (widget, event);
 ...
 **/
void
kraken_entry_select_all (KrakenEntry *entry)
{
	g_return_if_fail (KRAKEN_IS_ENTRY (entry));
...
select_all_at_idle (gpointer callback_data)
{
	...
	entry = KRAKEN_ENTRY (callback_data);

	kraken_entry_select_all (entry);
...
/**
 * kraken_entry_select_all_at_idle
 *
 ...
 **/
void
kraken_entry_select_all_at_idle (KrakenEntry *entry)
{
	g_return_if_fail (KRAKEN_IS_ENTRY (entry));
...
void
kraken_entry_set_text (KrakenEntry *entry, const gchar *text)
{
	g_return_if_fail (KRAKEN_IS_ENTRY (entry));
...
static void
kraken_entry_set_selection_bounds (GtkEditable *editable,
				     int start_pos,
				     int end_pos)
{
...
static gboolean
kraken_entry_button_press (GtkWidget *widget,
			     GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (kraken_entry_parent_class)->button_press_event (widget, event);
...
static gboolean
kraken_entry_button_release (GtkWidget *widget,
			       GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (kraken_entry_parent_class)->button_release_event (widget, event);
...
static void
kraken_entry_insert_text (GtkEditable *editable, const gchar *text,
			    int length, int *position)
{
	KrakenEntry *entry;

	entry = KRAKEN_ENTRY(editable);
...
static void 
kraken_entry_delete_text (GtkEditable *editable, int start_pos, int end_pos)
{
	KrakenEntry *entry;
 ...
 * this with type-completion behavior in KrakenLocationBar (see bug 5313).
 * There's a FIXME comment that seems to be about this same issue in
 * gtk+/gtkselection.c, gtk_selection_clear.
 */
static gboolean
kraken_entry_selection_clear (GtkWidget *widget,
			        GdkEventSelection *event)
{
	...
	return GTK_WIDGET_CLASS (kraken_entry_parent_class)->selection_clear_event (widget, event);
}

static void
kraken_entry_editable_init (GtkEditableInterface *iface)
{
	parent_editable_interface = g_type_interface_peek_parent (iface);

	iface->insert_text = kraken_entry_insert_text;
	iface->delete_text = kraken_entry_delete_text;
	iface->set_selection_bounds = kraken_entry_set_selection_bounds;
...
static void
kraken_entry_class_init (KrakenEntryClass *class)
{
	...
	widget_class->button_press_event = kraken_entry_button_press;
	widget_class->button_release_event = kraken_entry_button_release;
	widget_class->key_press_event = kraken_entry_key_press;
	widget_class->motion_notify_event = kraken_entry_motion_notify;
	widget_class->selection_clear_event = kraken_entry_selection_clear;
	
	gobject_class->finalize = kraken_entry_finalize;
	...
	signals[USER_CHANGED] = g_signal_new
		("user_changed",
		 ...
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (KrakenEntryClass, user_changed),
		 NULL, NULL,
	...
	signals[SELECTION_CHANGED] = g_signal_new
		("selection_changed",
		 G_TYPE_FROM_CLASS (class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (KrakenEntryClass, selection_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__VOID,
		 G_TYPE_NONE, 0);
}
...
void
kraken_entry_set_special_tab_handling (KrakenEntry *entry,
					 gboolean special_tab_handling)
{
	g_return_if_fail (KRAKEN_IS_ENTRY (entry));
