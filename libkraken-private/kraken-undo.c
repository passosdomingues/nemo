...
/* kraken-undo.c - public interface for objects that implement
...
#include <config.h>
#include "kraken-undo.h"

#include "kraken-undo-private.h"
#include "kraken-undo-transaction.h"
#include <gtk/gtk.h>

#define KRAKEN_UNDO_MANAGER_DATA "Kraken undo manager"

/* Register a simple undo action by calling kraken_undo_register_full. */
void
kraken_undo_register (GObject *target,
			KrakenUndoCallback callback,
			...
			const char *redo_menu_item_hint)
{
	 ...
	 * undo manager.
	 */
	kraken_undo_register_full (&single_atom_list,
				     target,
				     operation_name,
...
void
kraken_undo_register_full (GList *atoms,
			     ...
			     const char *redo_menu_item_hint)
{
	...
	transaction = kraken_undo_transaction_new (operation_name,
						     undo_menu_item_label,
						     undo_menu_item_hint, 
						     redo_menu_item_label,
						     redo_menu_item_hint);
	for (p = atoms; p != NULL; p = p->next) {
		kraken_undo_transaction_add_atom (transaction, p->data);
	}
	kraken_undo_transaction_add_to_undo_manager
		(transaction,
		 kraken_undo_get_undo_manager (undo_manager_search_start_object));
...
void
kraken_undo_unregister (GObject *target)
{
	/* Perhaps this should also unregister all children if called on a
	 * GtkContainer? That might be handy.
	 */
	kraken_undo_transaction_unregister_object (target);
}

void
kraken_undo (GObject *undo_manager_search_start_object)
{
	KrakenUndoManager *manager;

	g_return_if_fail (G_IS_OBJECT (undo_manager_search_start_object));

	manager = kraken_undo_get_undo_manager (undo_manager_search_start_object);
	if (manager != NULL) {
		kraken_undo_manager_undo (manager);
	}
}

KrakenUndoManager *
kraken_undo_get_undo_manager (GObject *start_object)
{
	KrakenUndoManager *manager;
	GtkWidget *parent;
	...
	manager = g_object_get_data (start_object, KRAKEN_UNDO_MANAGER_DATA);
	if (manager != NULL) {
		return manager;
	}
	...
	if (GTK_IS_WIDGET (start_object)) {
		...
		if (parent != NULL) {
			manager = kraken_undo_get_undo_manager (G_OBJECT (parent));
			if (manager != NULL) {
				return manager;
			}
		}
		...
		if (GTK_IS_WINDOW (start_object)) {
			...
			if (transient_parent != NULL) {
				manager = kraken_undo_get_undo_manager (G_OBJECT (transient_parent));
				if (manager != NULL) {
					return manager;
				}
			}
...
void
kraken_undo_attach_undo_manager (GObject *object,
				   KrakenUndoManager *manager)
{
	g_return_if_fail (G_IS_OBJECT (object));

	if (manager == NULL) {
		g_object_set_data (object, KRAKEN_UNDO_MANAGER_DATA, NULL);
	} else {
		g_object_ref (manager);
		g_object_set_data_full
			(object, KRAKEN_UNDO_MANAGER_DATA,
...
void
kraken_undo_share_undo_manager (GObject *destination_object,
				  GObject *source_object)
{
	KrakenUndoManager *manager;

	manager = kraken_undo_get_undo_manager (source_object);
	kraken_undo_attach_undo_manager (destination_object, manager);
}
