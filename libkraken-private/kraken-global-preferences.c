...
/* kraken-global-preferences.c - Kraken specific preference keys and
                                   functions.
...
#define DEBUG_FLAG KRAKEN_DEBUG_PREFERENCES
#include <libkraken-private/kraken-debug.h>

GSettings *kraken_preferences;
GSettings *kraken_icon_view_preferences;
GSettings *kraken_list_view_preferences;
GSettings *kraken_compact_view_preferences;
GSettings *kraken_desktop_preferences;
GSettings *kraken_tree_sidebar_preferences;
GSettings *kraken_window_state;
GSettings *gtk_filechooser_preferences;
GSettings *kraken_plugin_preferences;
GSettings *kraken_menu_config_preferences;
GSettings *kraken_search_preferences;
...
KrakenDateFormat  prefs_current_date_format;

GTimer    *kraken_startup_timer;
 ...
 */
char *
kraken_global_preferences_get_default_folder_viewer_preference_as_iid (void)
{
	int preference_value;
	const char *viewer_iid;

	preference_value =
		g_settings_get_enum (kraken_preferences, KRAKEN_PREFERENCES_DEFAULT_FOLDER_VIEWER);

	if (preference_value == KRAKEN_DEFAULT_FOLDER_VIEWER_LIST_VIEW) {
		viewer_iid = KRAKEN_LIST_VIEW_IID;
	} else if (preference_value == KRAKEN_DEFAULT_FOLDER_VIEWER_COMPACT_VIEW) {
		viewer_iid = KRAKEN_COMPACT_VIEW_IID;
	} else {
		viewer_iid = KRAKEN_ICON_VIEW_IID;
	}
...
gboolean
kraken_global_preferences_get_inherit_folder_viewer_preference (void)
{
    return inherit_folder_view_preference;
}

gboolean
kraken_global_preferences_get_ignore_view_metadata (void)
{
    return ignore_view_metadata;
}

gboolean
kraken_global_preferences_get_inherit_show_thumbnails_preference (void)
{
    return inherit_show_thumbnails_preference;
}

int
kraken_global_preferences_get_size_prefix_preference (void)
{
...
char *
kraken_global_preferences_get_desktop_iid (void)
{
    gboolean use_grid;
    const char *viewer_iid;

    use_grid = g_settings_get_boolean (kraken_desktop_preferences, KRAKEN_PREFERENCES_USE_DESKTOP_GRID);

    if (use_grid) {
        viewer_iid = KRAKEN_DESKTOP_ICON_GRID_VIEW_IID;
    } else {
        viewer_iid = KRAKEN_DESKTOP_ICON_VIEW_IID;
    }
...
gint
kraken_global_preferences_get_tooltip_flags (void)
{
    KrakenFileTooltipFlags flags = KRAKEN_FILE_TOOLTIP_FLAGS_NONE;

    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_TOOLTIP_FILE_TYPE))
        flags |= KRAKEN_FILE_TOOLTIP_FLAGS_FILE_TYPE;
    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_TOOLTIP_MOD_DATE))
        flags |= KRAKEN_FILE_TOOLTIP_FLAGS_MOD_DATE;
    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_TOOLTIP_ACCESS_DATE))
        flags |= KRAKEN_FILE_TOOLTIP_FLAGS_ACCESS_DATE;
    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_TOOLTIP_FULL_PATH))
        flags |= KRAKEN_FILE_TOOLTIP_FLAGS_PATH;
    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_TOOLTIP_CREATED_DATE))
        flags |= KRAKEN_FILE_TOOLTIP_FLAGS_CREATED_DATE;
    return flags;
}

gboolean
kraken_global_preferences_should_load_plugin (const gchar *name, const gchar *key)
{
    gchar **disabled_list = g_settings_get_strv (kraken_plugin_preferences, key);
...
{
    g_signal_connect (kraken_preferences,
                      "changed::" KRAKEN_PREFERENCES_IGNORE_VIEW_METADATA,
                      G_CALLBACK (boolean_changed_cb), &ignore_view_metadata);

    boolean_changed_cb (kraken_preferences, KRAKEN_PREFERENCES_IGNORE_VIEW_METADATA, &ignore_view_metadata);

    g_signal_connect (kraken_preferences,
                      "changed::" KRAKEN_PREFERENCES_INHERIT_FOLDER_VIEWER,
                      G_CALLBACK (boolean_changed_cb), &inherit_folder_view_preference);

    boolean_changed_cb (kraken_preferences, KRAKEN_PREFERENCES_INHERIT_FOLDER_VIEWER, &inherit_folder_view_preference);

    g_signal_connect (kraken_preferences,
                      "changed::" KRAKEN_PREFERENCES_INHERIT_SHOW_THUMBNAILS,
                      G_CALLBACK (boolean_changed_cb), &inherit_show_thumbnails_preference);

    boolean_changed_cb (kraken_preferences, KRAKEN_PREFERENCES_INHERIT_SHOW_THUMBNAILS, &inherit_show_thumbnails_preference);

    g_signal_connect (kraken_preferences,
                      "changed::" KRAKEN_PREFERENCES_SIZE_PREFIXES,
                      G_CALLBACK (enum_changed_cb), &size_prefixes_preference);

    enum_changed_cb (kraken_preferences, KRAKEN_PREFERENCES_SIZE_PREFIXES, &size_prefixes_preference);
}


gchar **
kraken_global_preferences_get_fileroller_mimetypes (void)
{
...
gchar *
kraken_global_preferences_get_mono_system_font (void)
{
    return g_settings_get_string (gnome_interface_preferences, KRAKEN_PREFERENCES_MONO_FONT_NAME);
}
...
gchar *
kraken_global_preferences_get_mono_font_family_match (const gchar *in_family_name)
{
    static gsize once_init = 0;
...
{
    prefs_current_date_format = g_settings_get_enum (kraken_preferences, KRAKEN_PREFERENCES_DATE_FORMAT);
...
void
kraken_global_preferences_init (void)
{
	...
	kraken_preferences = g_settings_new("org.kraken.preferences");
	kraken_window_state = g_settings_new("org.kraken.window-state");
	kraken_icon_view_preferences = g_settings_new("org.kraken.icon-view");
	kraken_list_view_preferences = g_settings_new("org.kraken.list-view");
	kraken_compact_view_preferences = g_settings_new("org.kraken.compact-view");
	kraken_desktop_preferences = g_settings_new("org.kraken.desktop");
	...
	kraken_tree_sidebar_preferences = g_settings_new("org.kraken.sidebar-panels.tree");
    kraken_plugin_preferences = g_settings_new("org.kraken.plugins");
    kraken_menu_config_preferences = g_settings_new("org.kraken.preferences.menu-config");
    kraken_search_preferences = g_settings_new("org.kraken.search");
    ...
    eel_debug_call_at_shutdown (kraken_global_preferences_finalize);
}

void
kraken_global_preferences_finalize (void)
{
    ...
    g_object_unref (kraken_preferences);
    g_object_unref (kraken_window_state);
    g_object_unref (kraken_icon_view_preferences);
    g_object_unref (kraken_list_view_preferences);
    g_object_unref (kraken_compact_view_preferences);
    g_object_unref (kraken_desktop_preferences);
    g_object_unref (kraken_tree_sidebar_preferences);
    g_object_unref (kraken_plugin_preferences);
    g_object_unref (kraken_menu_config_preferences);
    g_object_unref (kraken_search_preferences);
