...
} KrakenFileChangeKind;

typedef struct {
	KrakenFileChangeKind kind;
	GFile *from;
	GFile *to;
	GdkPoint point;
    int monitor;
...
typedef struct {
	...
	GList *tail;
	GMutex mutex;
} KrakenFileChangesQueue;

static KrakenFileChangesQueue *
kraken_file_changes_queue_new (void)
{
	KrakenFileChangesQueue *result;

	result = g_new0 (KrakenFileChangesQueue, 1);
...
static KrakenFileChangesQueue *
kraken_file_changes_queue_get (void)
{
	static KrakenFileChangesQueue *file_changes_queue;

	if (file_changes_queue == NULL) {
		file_changes_queue = kraken_file_changes_queue_new ();
	}
...
static void
kraken_file_changes_queue_add_common (KrakenFileChangesQueue *queue,
	KrakenFileChange *new_item)
{
...
void
kraken_file_changes_queue_file_added (GFile *location)
{
	KrakenFileChange *new_item;
	KrakenFileChangesQueue *queue;

	queue = kraken_file_changes_queue_get();

	new_item = g_new0 (KrakenFileChange, 1);
	new_item->kind = CHANGE_FILE_ADDED;
	new_item->from = g_object_ref (location);
	kraken_file_changes_queue_add_common (queue, new_item);
}

void
kraken_file_changes_queue_file_changed (GFile *location)
{
	KrakenFileChange *new_item;
	KrakenFileChangesQueue *queue;
...
void
kraken_file_changes_queue_file_removed (GFile *location)
{
	KrakenFileChange *new_item;
	KrakenFileChangesQueue *queue;
...
void
kraken_file_changes_queue_file_moved (GFile *from,
					GFile *to)
{
	KrakenFileChange *new_item;
	KrakenFileChangesQueue *queue;
	...
	new_item = g_new (KrakenFileChange, 1);
...
void
kraken_file_changes_queue_schedule_position_set (GFile *location,
						   GdkPoint point,
                           int monitor)
{
	KrakenFileChange *new_item;
...
void
kraken_file_changes_queue_schedule_position_remove (GFile *location)
{
	KrakenFileChange *new_item;
	KrakenFileChangesQueue *queue;
...
static KrakenFileChange *
kraken_file_changes_queue_get_change (KrakenFileChangesQueue *queue)
{
	GList *new_tail;
	KrakenFileChange *result;
...
{
	GList *p;
	KrakenFileChangesQueuePosition *item;
 ...
 * in a list to the different kraken_directory_notify calls
 */
void
kraken_file_changes_consume_changes (gboolean consume_all)
{
	KrakenFileChange *change;
	GList *additions, *changes, *deletions, *moves;
	GList *position_set_requests;
	GFilePair *pair;
	KrakenFileChangesQueuePosition *position_set;
	guint chunk_count;
	KrakenFileChangesQueue *queue;
	...
	for (chunk_count = 0; ; chunk_count++) {
		change = kraken_file_changes_queue_get_change (queue);
		...
		if (flush_needed) {
			...
			if (position_set_requests != NULL) {
				...
				kraken_directory_schedule_position_set (position_set_requests);
		...
		case CHANGE_POSITION_SET:
			position_set = g_new (KrakenFileChangesQueuePosition, 1);
