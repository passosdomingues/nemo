/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-desktop-directory-file.c: Subclass of KrakenFile to help implement the
   virtual desktop.
...
struct KrakenDesktopDirectoryFileDetails {
	KrakenDesktopDirectory *desktop_directory;

  	KrakenFile *real_dir_file;
...
typedef struct {
	KrakenDesktopDirectoryFile *desktop_file;
	KrakenFileCallback callback;
	gpointer callback_data;

	KrakenFileAttributes delegated_attributes;
	KrakenFileAttributes non_delegated_attributes;
...
G_DEFINE_TYPE (KrakenDesktopDirectoryFile, kraken_desktop_directory_file,
	       KRAKEN_TYPE_FILE);

static gchar *
get_indexed_key (KrakenFile *file)
{
    KrakenDesktopDirectory *desktop_directory;
    gchar *indexed_key;

    desktop_directory = KRAKEN_DESKTOP_DIRECTORY_FILE (file)->details->desktop_directory;

    indexed_key = g_strdup_printf ("desktop-monitor-%d",
                                   KRAKEN_DESKTOP_DIRECTORY (desktop_directory)->display_number);
...
static void
real_file_changed_callback (KrakenFile *real_file,
			    gpointer callback_data)
{
	KrakenDesktopDirectoryFile *desktop_file;
	
	desktop_file = KRAKEN_DESKTOP_DIRECTORY_FILE (callback_data);
	kraken_file_changed (KRAKEN_FILE (desktop_file));
}

static KrakenFileAttributes 
get_delegated_attributes_mask (void)
{
	return KRAKEN_FILE_ATTRIBUTE_DEEP_COUNTS |
		KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;
}

static void
partition_attributes (KrakenFileAttributes attributes,
		      KrakenFileAttributes *delegated_attributes,
		      KrakenFileAttributes *non_delegated_attributes)
{
	KrakenFileAttributes mask;
...
static void
desktop_directory_file_monitor_add (KrakenFile *file,
				    ...
				    KrakenFileAttributes attributes)
{
	...
	desktop_file = KRAKEN_DESKTOP_DIRECTORY_FILE (file);
	...
	kraken_file_monitor_add (desktop_file->details->real_dir_file,
...
static void
desktop_directory_file_monitor_remove (KrakenFile *file,
				       gconstpointer client)
{
	KrakenDesktopDirectoryFile *desktop_file;
	DesktopMonitor *monitor;
...
desktop_callback_destroy (DesktopCallback *desktop_callback)
{
	...
	g_assert (KRAKEN_IS_DESKTOP_DIRECTORY_FILE (desktop_callback->desktop_file));

	kraken_file_unref (KRAKEN_FILE (desktop_callback->desktop_file));
...
desktop_callback_check_done (DesktopCallback *desktop_callback)
{
    ...
    file = KRAKEN_FILE (desktop_callback->desktop_file);
    name = get_indexed_key (file);

    kraken_desktop_update_metadata_from_keyfile (file, name);
	...
	(* desktop_callback->callback) (KRAKEN_FILE (desktop_callback->desktop_file),
...
static void
ready_callback (KrakenFile *file,
		gpointer callback_data)
{
...
static void
desktop_directory_file_call_when_ready (KrakenFile *file,
					KrakenFileAttributes attributes,
					KrakenFileCallback callback,
					gpointer callback_data)
...
static void
desktop_directory_file_cancel_call_when_ready (KrakenFile *file,
					       KrakenFileCallback callback,
					       gpointer callback_data)
{
	KrakenDesktopDirectoryFile *desktop_file;
...
static gboolean
real_check_if_ready (KrakenFile *file,
		     KrakenFileAttributes attributes)
{
...
static gboolean
desktop_directory_file_check_if_ready (KrakenFile *file,
				       KrakenFileAttributes attributes)
{
	KrakenFileAttributes delegated_attributes, non_delegated_attributes;
	KrakenDesktopDirectoryFile *desktop_file;
	...
	return real_check_if_ready (file, non_delegated_attributes) &&
		kraken_file_check_if_ready (desktop_file->details->real_dir_file,
					      delegated_attributes);
}

static gboolean
desktop_directory_file_get_item_count (KrakenFile *file, 
				       guint *count,
				       gboolean *count_unreadable)
{
	...
	got_count = kraken_file_get_directory_item_count (desktop_file->details->real_dir_file,
							    count,
							    count_unreadable);
...
desktop_directory_file_get_deep_counts (KrakenFile *file,
					...
					goffset *total_size)
{
	...
	status = kraken_file_get_deep_counts (desktop_file->details->real_dir_file,
						directory_count,
						file_count,
...
static gboolean
desktop_directory_file_get_date (KrakenFile *file,
				 ...
				 time_t *date)
{
	...
	return kraken_file_get_date (desktop_file->details->real_dir_file,
				       date_type,
				       date);
}

static char *
desktop_directory_file_get_where_string (KrakenFile *file)
{
...
monitor_destroy (gpointer data)
{
	...
	kraken_file_monitor_remove
		(KRAKEN_FILE (monitor->desktop_file->details->real_dir_file), monitor);
	g_free (monitor);
}

static void
kraken_desktop_directory_file_set_metadata (KrakenFile           *file,
                                          const char         *key,
                                          const char         *value)
{
    gchar *name;
    ...
    kraken_desktop_set_metadata_string (file, name, key, value);

    g_free (name);
}

static void
kraken_desktop_directory_file_set_metadata_as_list (KrakenFile           *file,
                                                  const char         *key,
                                                  char              **value)
{
    gchar *name;
    ...
    kraken_desktop_set_metadata_stringv (file, name, key, (const gchar **) value);

    g_free (name);
}

static gchar *
kraken_desktop_directory_file_get_metadata (KrakenFile           *file,
                                          const char         *key)
{
    gchar *name;
    gchar *string;
    ...
    string = kraken_desktop_get_metadata_string (file, name, key);
...
static gchar **
kraken_desktop_directory_file_get_metadata_as_list (KrakenFile           *file,
                                                  const char         *key)
{
    gchar *name;
    gchar **stringv;
    ...
    stringv = kraken_desktop_get_metadata_stringv (file, name, key);
...
static void
kraken_desktop_directory_file_init (KrakenDesktopDirectoryFile *desktop_file)
{
	KrakenDesktopDirectory *desktop_directory;
	KrakenDirectory *real_dir;
	KrakenFile *real_dir_file;

	desktop_file->details = G_TYPE_INSTANCE_GET_PRIVATE (desktop_file,
							     KRAKEN_TYPE_DESKTOP_DIRECTORY_FILE,
							     KrakenDesktopDirectoryFileDetails);

	desktop_directory = KRAKEN_DESKTOP_DIRECTORY (kraken_directory_get_by_uri (EEL_DESKTOP_URI));
	...
	real_dir = kraken_desktop_directory_get_real_directory (desktop_directory);
	real_dir_file = kraken_directory_get_corresponding_file (real_dir);
	kraken_directory_unref (real_dir);
				    ...
				    gpointer callback_data)
{
	desktop_callback_remove_file
		(value, KRAKEN_FILE (callback_data));
}
...
desktop_finalize (GObject *object)
{
	...
	desktop_file = KRAKEN_DESKTOP_DIRECTORY_FILE (object);
	...
	kraken_file_unref (desktop_file->details->real_dir_file);
	kraken_directory_unref (KRAKEN_DIRECTORY (desktop_directory));

	G_OBJECT_CLASS (kraken_desktop_directory_file_parent_class)->finalize (object);
}

static void
kraken_desktop_directory_file_class_init (KrakenDesktopDirectoryFileClass *klass)
{
    ...
    file_class->set_metadata = kraken_desktop_directory_file_set_metadata;
	file_class->get_metadata = kraken_desktop_directory_file_get_metadata;
    file_class->set_metadata_as_list = kraken_desktop_directory_file_set_metadata_as_list;
	file_class->get_metadata_as_list = kraken_desktop_directory_file_get_metadata_as_list;

	g_type_class_add_private (klass, sizeof (KrakenDesktopDirectoryFileDetails));
}
