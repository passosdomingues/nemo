/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-directory-async.c: Kraken directory model state machine.
...
struct DirectoryLoadState {
	...
	KrakenFile *load_directory_file;
	int load_file_count;
};

struct MimeListState {
	KrakenDirectory *directory;
	KrakenFile *mime_list_file;
	GCancellable *cancellable;
	GFileEnumerator *enumerator;
...
typedef struct {
	KrakenFile *file; /* Which file, NULL means all. */
	union {
		KrakenDirectoryCallback directory;
		KrakenFileCallback file;
	} callback;
...
typedef gboolean (* FileCheck) (KrakenFile *);
...
static void     deep_count_load                               (DeepCountState         *state,
							       GFile                  *location);
static gboolean request_is_satisfied                          (KrakenDirectory      *directory,
							       KrakenFile           *file,
							       Request                 request);
static void     cancel_loading_attributes                     (KrakenDirectory      *directory,
							       KrakenFileAttributes  file_attributes);
static void     add_all_files_to_work_queue                   (KrakenDirectory      *directory);
static void     link_info_done                                (KrakenDirectory      *directory,
							       KrakenFile           *file,
							       const char             *uri,
							       const char             *name,
							       GIcon                  *icon,
							       ...
							       gboolean                is_foreign);
static void     move_file_to_low_priority_queue               (KrakenDirectory      *directory,
							       KrakenFile           *file);
static void     move_file_to_extension_queue                  (KrakenDirectory      *directory,
							       KrakenFile           *file);
static void     kraken_directory_invalidate_file_attributes (KrakenDirectory      *directory,
							       KrakenFileAttributes  file_attributes);

/* Some helpers for case-insensitive strings.
 * Move to kraken-glib-extensions?
 */
...
#if 0
static void
kraken_directory_verify_request_counts (KrakenDirectory *directory)
{
	GList *l;
 ...
 */
static gboolean
async_job_start (KrakenDirectory *directory,
		 const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
/* End a job. */
static void
async_job_end (KrakenDirectory *directory,
	       const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
async_job_wake_up (void)
{
	...
	while (async_job_count < MAX_ASYNC_JOBS) {
		...
		kraken_directory_async_state_changed
			(KRAKEN_DIRECTORY (value));
	}
	already_waking_up = FALSE;
}

static void
directory_count_cancel (KrakenDirectory *directory)
{
...
static void
deep_count_cancel (KrakenDirectory *directory)
{
	if (directory->details->deep_count_in_progress != NULL) {
		g_assert (KRAKEN_IS_FILE (directory->details->deep_count_file));
		
		g_cancellable_cancel (directory->details->deep_count_in_progress->cancellable);

		directory->details->deep_count_file->details->deep_counts_status = KRAKEN_REQUEST_NOT_STARTED;
...
static void
mime_list_cancel (KrakenDirectory *directory)
{
...
static void
link_info_cancel (KrakenDirectory *directory)
{
...
static void
thumbnail_cancel (KrakenDirectory *directory)
{
...
static void
mount_cancel (KrakenDirectory *directory)
{
...
static void
file_info_cancel (KrakenDirectory *directory)
{
...
static void
favorite_check_cancel (KrakenDirectory *directory)
{
...
static void
new_files_cancel (KrakenDirectory *directory)
{
	GList *l;
	NewFilesState *state;
...
static GList *
find_monitor (KrakenDirectory *directory,
	      KrakenFile *file,
	      gconstpointer client)
{
	Monitor monitor;
...
static void
remove_monitor_link (KrakenDirectory *directory,
		     GList *link)
{
	Monitor *monitor;
...
static void
remove_monitor (KrakenDirectory *directory,
		KrakenFile *file,
		gconstpointer client)
{
...
Request
kraken_directory_set_up_request (KrakenFileAttributes file_attributes)
{
	Request request;

	request = 0;
	
	if ((file_attributes & KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_DIRECTORY_COUNT);
	}
	
	if ((file_attributes & KRAKEN_FILE_ATTRIBUTE_DEEP_COUNTS) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_DEEP_COUNT);
	}

	if ((file_attributes & KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_MIME_LIST);
	}
	if ((file_attributes & KRAKEN_FILE_ATTRIBUTE_INFO) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}
	if (file_attributes & KRAKEN_FILE_ATTRIBUTE_LINK_INFO) {
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
		REQUEST_SET_TYPE (request, REQUEST_LINK_INFO);
	}

	if ((file_attributes & KRAKEN_FILE_ATTRIBUTE_EXTENSION_INFO) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_EXTENSION_INFO);
	}

	if (file_attributes & KRAKEN_FILE_ATTRIBUTE_THUMBNAIL) {
		REQUEST_SET_TYPE (request, REQUEST_THUMBNAIL);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if (file_attributes & KRAKEN_FILE_ATTRIBUTE_MOUNT) {
		REQUEST_SET_TYPE (request, REQUEST_MOUNT);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if (file_attributes & KRAKEN_FILE_ATTRIBUTE_FILESYSTEM_INFO) {
		REQUEST_SET_TYPE (request, REQUEST_FILESYSTEM_INFO);
	}

    if (file_attributes & KRAKEN_FILE_ATTRIBUTE_FAVORITE_CHECK) {
...
static void
mime_db_changed_callback (GObject *ignore, KrakenDirectory *dir)
{
	KrakenFileAttributes attrs;

	g_assert (dir != NULL);
	g_assert (dir->details != NULL);

	attrs = KRAKEN_FILE_ATTRIBUTE_INFO |
		KRAKEN_FILE_ATTRIBUTE_LINK_INFO |
		KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;

	kraken_directory_force_reload_internal (dir, attrs);
}

void
kraken_directory_monitor_add_internal (KrakenDirectory *directory,
					 KrakenFile *file,
					 gconstpointer client,
					 ...
					 gpointer callback_data)
{
	...
	monitor->client = client;
	monitor->request = kraken_directory_set_up_request (file_attributes);
	 ...
	 * kraken almost always shows the whole directory anyway, and
	 * it allows us to avoid one file monitor per file in a directory.
	 */
	if (directory->details->monitor == NULL) {
		directory->details->monitor = kraken_monitor_directory (directory->details->location);
	}
...
static void
set_file_unconfirmed (KrakenFile *file, gboolean unconfirmed)
{
	KrakenDirectory *directory;
...
{
	show_hidden_files = g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_SHOW_HIDDEN_FILES);
}

static gboolean
should_skip_file (KrakenDirectory *directory, GFileInfo *info)
{
...
static void
process_files_changed_while_being_added (KrakenDirectory *directory)
{
    if (directory->details->new_files_in_progress_changes == NULL)
    {
        return;
    ...
    kraken_directory_notify_files_changed (directory->details->new_files_in_progress_changes);
...
dequeue_pending_idle_callback (gpointer callback_data)
{
	...
	directory = KRAKEN_DIRECTORY (callback_data);
	...
	/* Build a list of KrakenFile objects. */
	for (node = pending_file_info; node != NULL; node = node->next) {
		...
		file = kraken_directory_find_file_by_name (directory, name);
		if (file != NULL) {
			...
			if (!file->details->is_added) {
				 ...
				 * This can happen if someone called kraken_file_get_by_uri()
			...
			} else if (kraken_file_update_info (file, file_info)) {
				/* File changed, notify about the change. */
				kraken_file_ref (file);
				changed_files = g_list_prepend (changed_files, file);
			}
		} else {
			/* new file, create a kraken file object and add it to the list */
			file = kraken_file_new_from_info (directory, file_info);
	...
	kraken_directory_emit_change_signals (directory, changed_files);
	kraken_file_list_free (changed_files);
	kraken_directory_emit_files_added (directory, added_files);
	kraken_file_list_free (added_files);

	if (directory->details->directory_loaded &&
	    !directory->details->directory_loaded_sent_notification) {
		/* Send the done_loading signal. */
		kraken_directory_emit_done_loading (directory);
...
void
kraken_directory_schedule_dequeue_pending (KrakenDirectory *directory)
{
...
static void
directory_load_one (KrakenDirectory *directory,
		    GFileInfo *info)
{
	if (info == NULL) {
		return;
	...
	kraken_directory_schedule_dequeue_pending (directory);
}

static void
directory_load_cancel (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
file_list_cancel (KrakenDirectory *directory)
{
...
static void
directory_load_done (KrakenDirectory *directory,
		     GError *error)
{
	GList *node;
	...
	if (error != NULL) {
		...
		for (node = directory->details->file_list;
			...
			set_file_unconfirmed (KRAKEN_FILE (node->data), FALSE);
		}

		kraken_directory_emit_load_error (directory, error);
	}
...
void
kraken_directory_monitor_remove_internal (KrakenDirectory *directory,
					    KrakenFile *file,
					    gconstpointer client)
{
	g_assert (KRAKEN_IS_DIRECTORY (directory));
	g_assert (file == NULL || KRAKEN_IS_FILE (file));
...
FileMonitors *
kraken_directory_remove_file_monitors (KrakenDirectory *directory,
					 KrakenFile *file)
{
...
void
kraken_directory_add_file_monitors (KrakenDirectory *directory,
				      KrakenFile *file,
...
static void
ready_callback_call (KrakenDirectory *directory,
...
void
kraken_directory_call_when_ready_internal (KrakenDirectory *directory,
					     KrakenFile *file,
					     KrakenFileAttributes file_attributes,
					     ...
					     gpointer callback_data)
{
	...
	g_assert (directory == NULL || KRAKEN_IS_DIRECTORY (directory));
	g_assert (file == NULL || KRAKEN_IS_FILE (file));
	...
	callback.request = kraken_directory_set_up_request (file_attributes);
...
gboolean      
kraken_directory_check_if_ready_internal (KrakenDirectory *directory,
					    KrakenFile *file,
					    KrakenFileAttributes file_attributes)
{
	Request request;
...
static void
remove_callback_link_keep_data (KrakenDirectory *directory,
				GList *link)
{
	ReadyCallback *callback;
...
static void
remove_callback_link (KrakenDirectory *directory,
		      GList *link)
{
	ReadyCallback *callback;
...
void
kraken_directory_cancel_callback_internal (KrakenDirectory *directory,
					     KrakenFile *file,
					     KrakenDirectoryCallback directory_callback,
					     KrakenFileCallback file_callback,
					     gpointer callback_data)
		    ...
		    gpointer user_data)
{
	...
	directory = kraken_directory_ref (state->directory);
...
void
kraken_directory_get_info_for_new_files (KrakenDirectory *directory,
					   GList *location_list)
{
	NewFilesState *state;
	GFile *location;
...
void
kraken_async_destroying_file (KrakenFile *file)
{
	KrakenDirectory *directory;
	gboolean changed;
	GList *node, *next;
...
static gboolean
lacks_directory_count (KrakenFile *file)
{
	return !file->details->directory_count_is_up_to_date
		&& kraken_file_should_show_directory_item_count (file);
}

static gboolean
should_get_directory_count_now (KrakenFile *file)
{
...
static gboolean
lacks_favorite_check (KrakenFile *file)
{
...
static gboolean
lacks_filesystem_info (KrakenFile *file)
{
	return kraken_file_is_directory (file) && !file->details->filesystem_info_is_up_to_date;
}

static gboolean
lacks_deep_count (KrakenFile *file)
{
	return file->details->deep_counts_status != KRAKEN_REQUEST_DONE;
}

static gboolean
lacks_mime_list (KrakenFile *file)
{
	return !file->details->mime_list_is_up_to_date;
}

static gboolean
should_get_mime_list (KrakenFile *file)
{
...
static gboolean
lacks_link_info (KrakenFile *file)
{
...
static gboolean
lacks_extension_info (KrakenFile *file)
{
    return file->details->load_deferred_attrs > KRAKEN_FILE_LOAD_DEFERRED_ATTRS_NO &&
        file->details->pending_info_providers != NULL;
}

static gboolean
lacks_thumbnail (KrakenFile *file)
{
    return file->details->load_deferred_attrs > KRAKEN_FILE_LOAD_DEFERRED_ATTRS_NO &&
	...
	return (!file->details->mount_is_up_to_date &&
		(
		 ...
		 (file->details->type == G_FILE_TYPE_DIRECTORY &&
		  kraken_file_is_self_owned (file)) ||
...
static gboolean
has_problem (KrakenDirectory *directory, KrakenFile *file, FileCheck problem)
{
	GList *node;
...
static gboolean
request_is_satisfied (KrakenDirectory *directory,
		      KrakenFile *file,
		      Request request)
{
...
static void
schedule_call_ready_callbacks (KrakenDirectory *directory)
{
 ...
 * before then */
static gboolean
call_ready_callbacks (KrakenDirectory *directory)
{
	gboolean found_any;
	GList *node, *next;
	ReadyCallback *callback;
...
gboolean
kraken_directory_has_active_request_for_file (KrakenDirectory *directory,
						KrakenFile *file)
{
	GList *node;
	ReadyCallback *callback;
...
gboolean
kraken_directory_is_anyone_monitoring_file_list (KrakenDirectory *directory)
{
...
gboolean
kraken_directory_is_file_list_monitored (KrakenDirectory *directory) 
{
	return directory->details->file_list_monitored;
}

static void
mark_all_files_unconfirmed (KrakenDirectory *directory)
{
	GList *node;
	KrakenFile *file;
...
directory_load_state_free (DirectoryLoadState *state)
{
	...
	kraken_file_unref (state->load_directory_file);
	g_object_unref (state->cancellable);
	g_free (state);
}
...
static void
start_monitoring_file_list (KrakenDirectory *directory)
{
	DirectoryLoadState *state;
	...
	if (!directory->details->file_list_monitored) {
		...
		kraken_file_list_ref (directory->details->file_list);
	}
...
void
kraken_directory_stop_monitoring_file_list (KrakenDirectory *directory)
{
	if (!directory->details->file_list_monitored) {
		g_assert (directory->details->directory_load_in_progress == NULL);
		return;
	...
	kraken_file_list_unref (directory->details->file_list);
	directory->details->directory_loaded = FALSE;
}

static void
file_list_start_or_stop (KrakenDirectory *directory)
{
	if (kraken_directory_is_anyone_monitoring_file_list (directory)) {
		start_monitoring_file_list (directory);
	} else {
		kraken_directory_stop_monitoring_file_list (directory);
	}
}

void
kraken_file_invalidate_count_and_mime_list (KrakenFile *file)
{
	KrakenFileAttributes attributes;
	
	attributes = KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		KRAKEN_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;
	
	kraken_file_invalidate_attributes (file, attributes);
}
 ...
 */
void
kraken_directory_invalidate_count_and_mime_list (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
kraken_directory_invalidate_file_attributes (KrakenDirectory      *directory,
					       KrakenFileAttributes  file_attributes)
{
	GList *node;
	...
	for (node = directory->details->file_list; node != NULL; node = node->next) {
		kraken_file_invalidate_attributes_internal (KRAKEN_FILE (node->data),
							      file_attributes);
	}

	if (directory->details->as_file != NULL) {
		kraken_file_invalidate_attributes_internal (directory->details->as_file,
							      file_attributes);
	}
}

void
kraken_directory_force_reload_internal (KrakenDirectory     *directory,
					  KrakenFileAttributes file_attributes)
{
	/* invalidate attributes that are getting reloaded for all files */
	kraken_directory_invalidate_file_attributes (directory, file_attributes);
...
static void
directory_count_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
count_children_done (KrakenDirectory *directory,
		     KrakenFile *count_file,
		     gboolean succeeded,
		     int count)
{
	g_assert (KRAKEN_IS_FILE (count_file));
	 ...
	 */
	kraken_file_changed (count_file);
...
directory_count_state_free (DirectoryCountState *state)
{
	...
	g_object_unref (state->cancellable);
	kraken_directory_unref (state->directory);
	g_free (state);
}
...
static void
directory_count_start (KrakenDirectory *directory,
		       KrakenFile *file,
		       gboolean *doing_io)
{
	DirectoryCountState *state;
	...
	*doing_io = TRUE;
	...
	state->directory = kraken_directory_ref (directory);
	state->cancellable = g_cancellable_new ();
...
deep_count_next_dir (DeepCountState *state)
{
	...
	if (state->deep_count_subdirectories != NULL) {
		...
		file->details->deep_counts_status = KRAKEN_REQUEST_DONE;
		directory->details->deep_count_file = NULL;
		directory->details->deep_count_in_progress = NULL;
	...
	kraken_file_updated_deep_count_in_progress (file);
...
static void
deep_count_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
deep_count_start (KrakenDirectory *directory,
		  KrakenFile *file,
		  gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	/* Start counting. */
	file->details->deep_counts_status = KRAKEN_REQUEST_IN_PROGRESS;
...
static void
mime_list_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
mime_list_start (KrakenDirectory *directory,
		 KrakenFile *file,
		 gboolean *doing_io)
{
	MimeListState *state;
		     ...
		     gpointer user_data)
{
	...
	g_assert (KRAKEN_IS_FILE (get_info_file));
	...
	if (info == NULL) {
		if (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_FOUND) {
			/* mark file as gone */
			kraken_file_mark_gone (get_info_file);
		}
		get_info_file->details->file_info_is_up_to_date = TRUE;
		kraken_file_clear_info (get_info_file);
		get_info_file->details->get_info_failed = TRUE;
		get_info_file->details->get_info_error = error;
	} else {
		kraken_file_update_info (get_info_file, info);
		g_object_unref (info);
	}

	kraken_file_changed (get_info_file);
	kraken_file_unref (get_info_file);
...
static void
file_info_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
file_info_start (KrakenDirectory *directory,
		 KrakenFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
favorite_check_callback (gpointer user_data)
{
    ...
    g_assert (KRAKEN_IS_FILE (favorite_check_file));
     ...
     * least long enough to send the change notification. 
     */
    kraken_file_ref (favorite_check_file);

    gchar *uri;
    gboolean is_favorite;

    uri = kraken_file_get_uri (favorite_check_file);
    ...
    if (!kraken_file_is_in_favorites (favorite_check_file) && 
        is_favorite != kraken_file_get_is_favorite (favorite_check_file)) {
        kraken_file_set_is_favorite (favorite_check_file, is_favorite);
        kraken_file_changed (favorite_check_file);
    }

    g_free (uri);
    kraken_file_unref (favorite_check_file);
...
static void
favorite_check_stop (KrakenDirectory *directory)
{
    KrakenFile *file;
...
static void
favorite_check_start (KrakenDirectory *directory,
                      KrakenFile *file,
                      gboolean *doing_io)
{
    FavoriteCheckState *state;
...
static gboolean
is_link_trusted (KrakenFile *file,
		 gboolean is_launcher)
{
	GFile *location;
	gboolean res;
	...
	if (kraken_file_can_execute (file)) {
		return TRUE;
	}
	...
	if (kraken_file_is_local (file)) {
		location = kraken_file_get_location (file);
		res = kraken_is_in_system_dir (location);
		g_object_unref (location);
	}
...
static void
link_info_done (KrakenDirectory *directory,
		KrakenFile *file,
		const char *uri,
		const char *name, 
		GIcon *icon,
		...
		gboolean is_foreign)
{
	...
	if (is_trusted) {
		kraken_file_set_display_name (file, name, name, TRUE);
	} else {
		kraken_file_set_display_name (file, NULL, NULL, TRUE);
	}
...
static void
link_info_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
link_info_got_data (KrakenDirectory *directory,
		    KrakenFile *file,
		    gboolean result,
		    goffset bytes_read,
		    char *file_contents)
{
	...
	/* Handle the case where we read the Kraken link. */
	if (result) {
		link_uri = kraken_file_get_uri (file);
		kraken_link_get_link_info_given_file_contents (file_contents, bytes_read, link_uri,
...
static void
link_info_kraken_link_read_callback (GObject *source_object,
				       GAsyncResult *res,
				       gpointer user_data)
{
...
static void
link_info_start (KrakenDirectory *directory,
		 KrakenFile *file,
		 gboolean *doing_io)
{
	GFile *location;
	gboolean kraken_style_link;
	LinkInfoReadState *state;
	...
	*doing_io = TRUE;
	...
	kraken_style_link = kraken_file_is_kraken_link (file);
	location = kraken_file_get_location (file);
	
	/* If it's not a link we are done. If it is, we need to read it. */
	if (!kraken_style_link) {
		...
		g_file_load_contents_async (location,
					    state->cancellable,
					    link_info_kraken_link_read_callback,
					    state);
	}
	g_object_unref (location);
}
...
static void
thumbnail_done (KrakenDirectory *directory,
		KrakenFile *file,
		GdkPixbuf *pixbuf,
		gboolean tried_original)
{
...
static void
thumbnail_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
thumbnail_got_pixbuf (KrakenDirectory *directory,
		      KrakenFile *file,
		      GdkPixbuf *pixbuf,
		      gboolean tried_original)
{
				...
				gpointer user_data)
{
	...
	/* cf. kraken_file_get_icon() */
	max_thumbnail_size = KRAKEN_ICON_SIZE_LARGEST * cached_thumbnail_size / KRAKEN_ICON_SIZE_STANDARD;
...
static void
thumbnail_start (KrakenDirectory *directory,
		 KrakenFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
static void
mount_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
got_mount (MountState *state, GMount *mount)
{
	...
	file = kraken_file_ref (state->file);

	file->details->mount_is_up_to_date = TRUE;
	kraken_file_set_mount (file, mount);
			       ...
			       gpointer user_data)
{
	...
	if (mount) {
		...
		location = kraken_file_get_location (state->file);
...
static void
mount_start (KrakenDirectory *directory,
	     KrakenFile *file,
	     gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	if (file->details->type == G_FILE_TYPE_MOUNTABLE) {
		...
		mount = NULL;
		target = kraken_file_get_activation_location (file);
		if (target != NULL) {
...
static void
filesystem_info_cancel (KrakenDirectory *directory)
{
	if (directory->details->filesystem_info_state != NULL) {
...
static void
filesystem_info_stop (KrakenDirectory *directory)
{
	KrakenFile *file;
...
static void
filesystem_info_start (KrakenDirectory *directory,
		       KrakenFile *file,
		       gboolean *doing_io)
{
	GFile *location;
...
static void
extension_info_cancel (KrakenDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		if (directory->details->extension_info_idle > 0) {
			...
			kraken_info_provider_cancel_update 
...
static void
extension_info_stop (KrakenDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		KrakenFile *file;
...
static void
finish_info_provider (KrakenDirectory *directory,
		      KrakenFile *file,
		      KrakenInfoProvider *provider)
{
	...
	if (file->details->pending_info_providers == NULL) {
		kraken_file_info_providers_done (file);
	}
...
info_provider_idle_callback (gpointer user_data)
{
	...
	if (response->handle != directory->details->extension_info_in_progress
		...
		g_warning ("Unexpected plugin response.  This probably indicates a bug in a Kraken extension: handle=%p", response->handle);
	} else {
		KrakenFile *file;
		async_job_end (directory, "extension info");
...
static void
info_provider_callback (KrakenInfoProvider *provider,
			KrakenOperationHandle *handle,
			KrakenOperationResult result,
			gpointer user_data)
{
	...
	response->directory = KRAKEN_DIRECTORY (user_data);
...
static void
extension_info_start (KrakenDirectory *directory,
		      KrakenFile *file,
		      gboolean *doing_io)
{
	KrakenInfoProvider *provider;
	...
	*doing_io = TRUE;
	...
	result = kraken_info_provider_update_file_info
		(provider, 
		 KRAKEN_FILE_INFO (file), 
		 update_complete, 
		 &handle);

	if (result == KRAKEN_OPERATION_COMPLETE ||
	    result == KRAKEN_OPERATION_FAILED) {
...
static void
start_or_stop_io (KrakenDirectory *directory)
{
	KrakenFile *file;
	gboolean doing_io;
	...
	while (!kraken_file_queue_is_empty (directory->details->high_priority_queue)) {
		file = kraken_file_queue_head (directory->details->high_priority_queue);
	...
	while (!kraken_file_queue_is_empty (directory->details->low_priority_queue)) {
		file = kraken_file_queue_head (directory->details->low_priority_queue);
	...
	while (!kraken_file_queue_is_empty (directory->details->extension_queue)) {
		file = kraken_file_queue_head (directory->details->extension_queue);
 ...
 */
void
kraken_directory_async_state_changed (KrakenDirectory *directory)
{
...
void
kraken_directory_cancel (KrakenDirectory *directory)
{
...
static void
cancel_directory_count_for_file (KrakenDirectory *directory,
				 KrakenFile      *file)
{
	if (directory->details->count_in_progress != NULL &&
	    directory->details->count_in_progress->count_file == file) {
...
static void
cancel_deep_counts_for_file (KrakenDirectory *directory,
			     KrakenFile      *file)
{
	if (directory->details->deep_count_file == file) {
		deep_count_cancel (directory);
...
static void
cancel_mime_list_for_file (KrakenDirectory *directory,
			   KrakenFile      *file)
{
	if (directory->details->mime_list_in_progress != NULL &&
	    directory->details->mime_list_in_progress->mime_list_file == file) {
...
static void
cancel_file_info_for_file (KrakenDirectory *directory,
			   KrakenFile      *file)
{
	if (directory->details->get_info_file == file) {
		file_info_cancel (directory);
...
static void
cancel_favorite_check_for_file (KrakenDirectory *directory,
                                KrakenFile      *file)
{
    if (directory->details->favorite_check_file == file) {
        favorite_check_cancel (directory);
...
static void
cancel_thumbnail_for_file (KrakenDirectory *directory,
			   KrakenFile      *file)
{
	if (directory->details->thumbnail_state != NULL &&
	    directory->details->thumbnail_state->file == file) {
...
static void
cancel_mount_for_file (KrakenDirectory *directory,
			   KrakenFile      *file)
{
	if (directory->details->mount_state != NULL &&
	    directory->details->mount_state->file == file) {
...
static void
cancel_filesystem_info_for_file (KrakenDirectory *directory,
				 KrakenFile      *file)
{
	if (directory->details->filesystem_info_state != NULL &&
	    directory->details->filesystem_info_state->file == file) {
...
static void
cancel_link_info_for_file (KrakenDirectory *directory,
			   KrakenFile      *file)
{
	if (directory->details->link_info_read_state != NULL &&
	    directory->details->link_info_read_state->file == file) {
...
static void
cancel_loading_attributes (KrakenDirectory *directory,
			   KrakenFileAttributes file_attributes)
{
	Request request;
...
void
kraken_directory_cancel_loading_file_attributes (KrakenDirectory      *directory,
						   KrakenFile           *file,
						   KrakenFileAttributes  file_attributes)
{
	Request request;
...
void
kraken_directory_add_file_to_work_queue (KrakenDirectory *directory,
					   KrakenFile *file)
{
	g_return_if_fail (file->details->directory == directory);

	kraken_file_queue_enqueue (directory->details->high_priority_queue,
				     file);
}


static void
add_all_files_to_work_queue (KrakenDirectory *directory)
{
	GList *node;
	KrakenFile *file;
...
void
kraken_directory_remove_file_from_work_queue (KrakenDirectory *directory,
						KrakenFile *file)
{
	kraken_file_queue_remove (directory->details->high_priority_queue,
				    file);
	kraken_file_queue_remove (directory->details->low_priority_queue,
				    file);
	kraken_file_queue_remove (directory->details->extension_queue,
				    file);
}


static void
move_file_to_low_priority_queue (KrakenDirectory *directory,
				 KrakenFile *file)
{
	/* Must add before removing to avoid ref underflow */
	kraken_file_queue_enqueue (directory->details->low_priority_queue,
				     file);
	kraken_file_queue_remove (directory->details->high_priority_queue,
				    file);
}
...
static void
move_file_to_extension_queue (KrakenDirectory *directory,
			      KrakenFile *file)
{
	/* Must add before removing to avoid ref underflow */
	kraken_file_queue_enqueue (directory->details->extension_queue,
				     file);
	kraken_file_queue_remove (directory->details->low_priority_queue,
				    file);
}
