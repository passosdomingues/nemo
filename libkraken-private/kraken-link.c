/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-link.c: .desktop link files.
...
#define KRAKEN_LINK_GENERIC_TAG	"Link"
#define KRAKEN_LINK_TRASH_TAG 	"X-kraken-trash"
#define KRAKEN_LINK_MOUNT_TAG 	"FSDevice"
#define KRAKEN_LINK_HOME_TAG 		"X-kraken-home"
...
gboolean
kraken_link_local_create (const char     *directory_uri,
			    ...
			    gboolean        unique_filename)
{
	...
	if (eel_uri_is_desktop (directory_uri)) {
		real_directory_uri = kraken_get_desktop_directory_uri ();
	} else {
		real_directory_uri = g_strdup (directory_uri);
	}

	if (unique_filename) {
		uri = kraken_ensure_unique_file_name (real_directory_uri,
							base_name, ".desktop");
		if (uri == NULL) {
	...
	kraken_directory_notify_files_added (&dummy_list);
	...
	if (point != NULL) {
		...
		dummy_list.next = NULL;
		dummy_list.prev = NULL;
		kraken_directory_schedule_position_set (&dummy_list);
	}
...
static gboolean
kraken_link_local_set_key (const char *uri,
			     const char *key,
			     const char *value,
			     gboolean    localize)
{
...
gboolean
kraken_link_local_set_text (const char *uri,
			      const char *text)
{
	return kraken_link_local_set_key (uri, "Name", text, TRUE);
}


gboolean
kraken_link_local_set_icon (const char        *uri,
			      const char        *icon)
{
	return kraken_link_local_set_key (uri, "Icon", icon, FALSE);
}

char *
kraken_link_local_get_text (const char *path)
{
	return slurp_key_string (path, "Name", TRUE);
}

static char *
kraken_link_get_link_uri_from_desktop (GKeyFile *key_file, const char *desktop_file_uri)
{
	GFile *file, *parent;
	char *type;
	char *retval;
	...
	} else if ((strcmp (type, KRAKEN_LINK_GENERIC_TAG) == 0) ||
		   (strcmp (type, KRAKEN_LINK_MOUNT_TAG) == 0) ||
		   (strcmp (type, KRAKEN_LINK_TRASH_TAG) == 0) ||
		   (strcmp (type, KRAKEN_LINK_HOME_TAG) == 0)) {
...
static char *
kraken_link_get_link_name_from_desktop (GKeyFile *key_file)
{
	return g_key_file_get_locale_string (key_file, MAIN_GROUP, "Name", NULL, NULL);
}

static GIcon *
kraken_link_get_link_icon_from_desktop (GKeyFile *key_file)
{
	...
	if (icon_str == NULL) {
		if (g_strcmp0 (type, "Application") == 0) {
			...
			icon_str = g_strdup (KRAKEN_ICON_FOLDER);
...
char *
kraken_link_local_get_link_uri (const char *uri)
{
	GKeyFile *key_file;
	char *retval;
	...
	retval = kraken_link_get_link_uri_from_desktop (key_file, uri);
	g_key_file_free (key_file);
...
void
kraken_link_get_link_info_given_file_contents (const char  *file_contents,
						 int          link_file_size,
						 const char  *file_uri,
						 char       **uri,
						 char       **name,
						 ...
						 gboolean    *is_foreign)
{
	...
	*uri = kraken_link_get_link_uri_from_desktop (key_file, file_uri);
	*name = kraken_link_get_link_name_from_desktop (key_file);
	*icon = kraken_link_get_link_icon_from_desktop (key_file);
