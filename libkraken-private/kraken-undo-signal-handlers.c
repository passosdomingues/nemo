...
/* kraken_undo_set_up_kraken_entry_for_undo
 *
...
static void
kraken_entry_user_changed_callback (KrakenEntry *entry)
{
...
void
kraken_undo_set_up_kraken_entry_for_undo (KrakenEntry *entry)
{
	EditableUndoObjectData *data;

	if (!KRAKEN_IS_ENTRY (entry) ) {
		return;
	}
	...
	g_signal_connect (entry, "user_changed",
			  G_CALLBACK (kraken_entry_user_changed_callback),
			  NULL);
}

void
kraken_undo_tear_down_kraken_entry_for_undo (KrakenEntry *entry)
{
	if (!KRAKEN_IS_ENTRY (entry) ) {
		return;
	}
	...
	g_signal_handlers_disconnect_by_func
		(entry, G_CALLBACK (kraken_entry_user_changed_callback), NULL);
...
void
kraken_undo_set_up_editable_for_undo (GtkEditable *editable)
{
	EditableUndoObjectData *data;
...
void
kraken_undo_tear_down_editable_for_undo (GtkEditable *editable)
{
	if (!GTK_IS_EDITABLE (editable) ) {
		return;
	}
