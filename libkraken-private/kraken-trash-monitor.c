...
/* 
   kraken-trash-monitor.c: Kraken trash state watcher.
...
struct KrakenTrashMonitorDetails {
	gboolean empty;
	GIcon *icon;
...
static KrakenTrashMonitor *kraken_trash_monitor = NULL;

G_DEFINE_TYPE(KrakenTrashMonitor, kraken_trash_monitor, G_TYPE_OBJECT)

static void
kraken_trash_monitor_finalize (GObject *object)
{
	KrakenTrashMonitor *trash_monitor;

	trash_monitor = KRAKEN_TRASH_MONITOR (object);
	...
	G_OBJECT_CLASS (kraken_trash_monitor_parent_class)->finalize (object);
}

static void
kraken_trash_monitor_class_init (KrakenTrashMonitorClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = kraken_trash_monitor_finalize;

	signals[TRASH_STATE_CHANGED] = g_signal_new
		("trash_state_changed",
		 G_TYPE_FROM_CLASS (object_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (KrakenTrashMonitorClass, trash_state_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__BOOLEAN,
		 G_TYPE_NONE, 1,
		 G_TYPE_BOOLEAN);

	g_type_class_add_private (object_class, sizeof(KrakenTrashMonitorDetails));
}

static void
update_icon (KrakenTrashMonitor *trash_monitor)
{
	g_clear_object (&trash_monitor->details->icon);

	if (trash_monitor->details->empty) {
		trash_monitor->details->icon = g_themed_icon_new (KRAKEN_ICON_TRASH);
        trash_monitor->details->symbolic_icon_name = KRAKEN_ICON_SYMBOLIC_TRASH;
	} else {
		trash_monitor->details->icon = g_themed_icon_new (KRAKEN_ICON_TRASH_FULL);
        trash_monitor->details->symbolic_icon_name = KRAKEN_ICON_SYMBOLIC_TRASH_FULL;
	}
}

static void
update_empty_info (KrakenTrashMonitor *trash_monitor,
		   gboolean is_empty)
{
...
trash_query_info_cb (GObject *source,
                     ...
                     gpointer user_data)
{
        KrakenTrashMonitor *trash_monitor = user_data;
        GFileInfo *info;
        guint32 item_count;
...
static void
schedule_update_info (KrakenTrashMonitor *trash_monitor)
{
	GFile *location;
	      ...
	      gpointer user_data)
{
	...
	trash_monitor = KRAKEN_TRASH_MONITOR (user_data);

	schedule_update_info (trash_monitor);
}

static void
kraken_trash_monitor_init (KrakenTrashMonitor *trash_monitor)
{
	GFile *location;

	trash_monitor->details = G_TYPE_INSTANCE_GET_PRIVATE (trash_monitor,
							      KRAKEN_TYPE_TRASH_MONITOR,
							      KrakenTrashMonitorDetails);
...
{
	g_object_unref (kraken_trash_monitor);
}

KrakenTrashMonitor *
kraken_trash_monitor_get (void)
{
	if (kraken_trash_monitor == NULL) {
		/* not running yet, start it up */

		kraken_trash_monitor = KRAKEN_TRASH_MONITOR
			(g_object_new (KRAKEN_TYPE_TRASH_MONITOR, NULL));
		eel_debug_call_at_shutdown (unref_trash_monitor);
	}

	return kraken_trash_monitor;
}

gboolean
kraken_trash_monitor_is_empty (void)
{
...
GIcon *
kraken_trash_monitor_get_icon (void)
{
	KrakenTrashMonitor *monitor;
...
gchar *
kraken_trash_monitor_get_symbolic_icon_name (void)
{
    KrakenTrashMonitor *monitor;
...
void
kraken_trash_monitor_add_new_trash_directories (void)
{
