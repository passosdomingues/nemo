...
struct _KrakenFileConflictDialogDetails
{
	/* conflicting objects */
	KrakenFile *source;
	KrakenFile *destination;
	KrakenFile *dest_dir;
...
G_DEFINE_TYPE (KrakenFileConflictDialog,
	       kraken_file_conflict_dialog,
	       GTK_TYPE_DIALOG);

#define KRAKEN_FILE_CONFLICT_DIALOG_GET_PRIVATE(object)		\
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), KRAKEN_TYPE_FILE_CONFLICT_DIALOG, \
				      KrakenFileConflictDialogDetails))

static void
file_icons_changed (KrakenFile *file,
		    KrakenFileConflictDialog *fcd)
{
	GdkPixbuf *pixbuf;

	pixbuf = kraken_file_get_icon_pixbuf (fcd->details->destination,
						KRAKEN_ICON_SIZE_LARGE,
						TRUE,
	...
	pixbuf = kraken_file_get_icon_pixbuf (fcd->details->source,
						KRAKEN_ICON_SIZE_LARGE,
						TRUE,
...
file_list_ready_cb (GList *files,
		    gpointer user_data)
{
	KrakenFileConflictDialog *fcd = user_data;
	KrakenFile *src, *dest, *dest_dir;
	...
	src_mtime = kraken_file_get_mtime (src);
	dest_mtime = kraken_file_get_mtime (dest);

	dest_name = kraken_file_get_display_name (dest);
	dest_dir_name = kraken_file_get_display_name (dest_dir);

	source_is_dir = kraken_file_is_directory (src);
	dest_is_dir = kraken_file_is_directory (dest);

	type = kraken_file_get_mime_type (dest);
	should_show_type = !kraken_file_is_mime_type (src, type);
    ...
    /* Set up file icons */
    pixbuf = kraken_file_get_icon_pixbuf (dest,
                                        KRAKEN_ICON_SIZE_LARGE,
                                        TRUE,
                                        ui_scale,
                                        KRAKEN_FILE_ICON_FLAGS_USE_THUMBNAILS);
    ...
    pixbuf = kraken_file_get_icon_pixbuf (src,
                                        KRAKEN_ICON_SIZE_LARGE,
                                        TRUE,
                                        ui_scale,
                                        KRAKEN_FILE_ICON_FLAGS_USE_THUMBNAILS);
	...
	date = kraken_file_get_string_attribute (dest,
						   "date_modified");
	size = kraken_file_get_string_attribute (dest, "size");

	if (should_show_type) {
		type = kraken_file_get_string_attribute (dest, "type");
	}
	...
	/* Second label */
	label = gtk_label_new (NULL);
	date = kraken_file_get_string_attribute (src,
						   "date_modified");
	size = kraken_file_get_string_attribute (src, "size");

	if (should_show_type) {
		type = kraken_file_get_string_attribute (src, "type");
	}
	...
	/* Populate the entry */
	edit_name = kraken_file_get_edit_name (dest);
	...
	kraken_file_monitor_add (src, fcd, KRAKEN_FILE_ATTRIBUTES_FOR_ICON);
	kraken_file_monitor_add (dest, fcd, KRAKEN_FILE_ATTRIBUTES_FOR_ICON);
...
static void
build_dialog_appearance (KrakenFileConflictDialog *fcd)
{
	GList *files = NULL;
	KrakenFileConflictDialogDetails *details = fcd->details;
	...
	kraken_file_list_call_when_ready (files,
...
set_source_and_destination (GtkWidget *w,
			    ...
			    GFile *dest_dir)
{
	KrakenFileConflictDialog *dialog;
	KrakenFileConflictDialogDetails *details;

	dialog = KRAKEN_FILE_CONFLICT_DIALOG (w);
	details = dialog->details;

	details->source = kraken_file_get (source);
	details->destination = kraken_file_get (destination);
	details->dest_dir = kraken_file_get (dest_dir);
...
static void
kraken_file_conflict_dialog_init (KrakenFileConflictDialog *fcd)
{
	...
	details = fcd->details = KRAKEN_FILE_CONFLICT_DIALOG_GET_PRIVATE (fcd);
...
{
	KrakenFileConflictDialogDetails *details =
		KRAKEN_FILE_CONFLICT_DIALOG (self)->details;

	g_free (details->conflict_name);

	if (details->handle != NULL) {
		kraken_file_list_cancel_call_when_ready (details->handle);
	}

	if (details->src_handler_id) {
		g_signal_handler_disconnect (details->source, details->src_handler_id);
		kraken_file_monitor_remove (details->source, self);
	}

	if (details->dest_handler_id) {
		g_signal_handler_disconnect (details->destination, details->dest_handler_id);
		kraken_file_monitor_remove (details->destination, self);
	}

	kraken_file_unref (details->source);
	kraken_file_unref (details->destination);
	kraken_file_unref (details->dest_dir);

	G_OBJECT_CLASS (kraken_file_conflict_dialog_parent_class)->finalize (self);
}

static void
kraken_file_conflict_dialog_class_init (KrakenFileConflictDialogClass *klass)
{
	G_OBJECT_CLASS (klass)->finalize = do_finalize;

	g_type_class_add_private (klass, sizeof (KrakenFileConflictDialogDetails));
}

char *
kraken_file_conflict_dialog_get_new_name (KrakenFileConflictDialog *dialog)
{
...
gboolean
kraken_file_conflict_dialog_get_apply_to_all (KrakenFileConflictDialog *dialog)
{
...
GtkWidget *
kraken_file_conflict_dialog_new (GtkWindow *parent,
				   GFile *source,
				   GFile *destination,
				   GFile *dest_dir)
{
	...
	dialog = GTK_WIDGET (g_object_new (KRAKEN_TYPE_FILE_CONFLICT_DIALOG,
