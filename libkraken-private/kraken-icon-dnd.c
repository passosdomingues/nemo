...
/* kraken-icon-dnd.c - Drag & drop handling for the icon container widget.
...
static const GtkTargetEntry drop_types [] = {
	...
	{ (char *)KRAKEN_ICON_DND_XDNDDIRECTSAVE_TYPE, 0, KRAKEN_ICON_DND_XDNDDIRECTSAVE },
	/* prefer "_NETSCAPE_URL" over "text/uri-list" to satisfy web browsers. */
	{ (char *)KRAKEN_ICON_DND_NETSCAPE_URL_TYPE, 0, KRAKEN_ICON_DND_NETSCAPE_URL },
	{ (char *)KRAKEN_ICON_DND_URI_LIST_TYPE, 0, KRAKEN_ICON_DND_URI_LIST },
	{ (char *)KRAKEN_ICON_DND_RAW_TYPE, 0, KRAKEN_ICON_DND_RAW },
	/* Must be last: */
	{ (char *)KRAKEN_ICON_DND_ROOTWINDOW_DROP_TYPE,  0, KRAKEN_ICON_DND_ROOTWINDOW_DROP }
};
...
static char * kraken_icon_container_find_drop_target (KrakenIconContainer *container,
							GdkDragContext *context,
							int x, int y, gboolean *icon_hit,
							gboolean rewrite_desktop);

static void
initialize_dnd_grid (KrakenIconContainer *container)
{
    GList *selection, *p;

    if (!KRAKEN_ICON_CONTAINER_GET_CLASS (container)->is_grid_container) {
        return;
    }

    if (container->details->dnd_grid != NULL) {
        kraken_centered_placement_grid_free (container->details->dnd_grid);
    }

    container->details->dnd_grid = kraken_centered_placement_grid_new (container,
                                                                     container->details->horizontal);

    kraken_centered_placement_grid_pre_populate (container->details->dnd_grid, container->details->icons, FALSE);
    ...
    for (p = selection; p != NULL; p = p->next) {
        ...
        icon = kraken_icon_container_get_icon_by_uri (container, uri);

        if (icon != NULL) {
            kraken_centered_placement_grid_unmark_icon (container->details->dnd_grid,
                                                      icon);
        }
...
static void
free_dnd_grid (KrakenIconContainer *container)
{
    g_clear_pointer (&container->details->dnd_grid,
                     kraken_centered_placement_grid_free);
...
create_selection_shadow (KrakenIconContainer *container,
			 GList *list)
{
...
typedef struct {
	...
	KrakenDragEachSelectedItemDataGet iteratee;
...
static gboolean
icon_get_data_binder (KrakenIcon *icon, gpointer data)
{
	...
	g_assert (KRAKEN_IS_ICON_CONTAINER (context->iterator_context));

	container = KRAKEN_ICON_CONTAINER (context->iterator_context);
	...
	uri = kraken_icon_container_get_icon_uri (container, icon);
	if (uri == NULL) {
		g_warning ("no URI for one of the iterated icons");
		return TRUE;
	}
	path_str = kraken_file_get_path (KRAKEN_FILE (icon->data));
...
/* Iterate over each selected icon in a KrakenIconContainer,
 * calling each_function on each.
 */
static void
kraken_icon_container_each_selected_icon (KrakenIconContainer *container,
	gboolean (*each_function) (KrakenIcon *, gpointer), gpointer data)
{
	GList *p;
	KrakenIcon *icon;
...
/* Adaptor function used with kraken_icon_container_each_selected_icon
 * to help iterate over all selected items, passing uris, x, y, w and h
 * values to the iteratee
 */
static void
each_icon_get_data_binder (KrakenDragEachSelectedItemDataGet iteratee,
	gpointer iterator_context, gpointer data)
{
	IconGetDataBinderContext context;
	KrakenIconContainer *container;

	g_assert (KRAKEN_IS_ICON_CONTAINER (iterator_context));
	container = KRAKEN_ICON_CONTAINER (iterator_context);

	context.iterator_context = iterator_context;
	context.iteratee = iteratee;
	context.iteratee_data = data;
	kraken_icon_container_each_selected_icon (container, icon_get_data_binder, &context);
}
			...
			gpointer data)
{
	...
	g_assert (KRAKEN_IS_ICON_CONTAINER (widget));
	g_return_if_fail (context != NULL);

	/* Call common function from kraken-drag that set's up
	 * the selection data in the right format. Pass it means to
	 * iterate all the selected icons.
	 */
	kraken_drag_drag_data_get (widget, context, selection_data,
...
static void
kraken_icon_container_position_shadow (KrakenIconContainer *container,
					 int x, int y)
{
...
static void
kraken_icon_container_dropped_icon_feedback (GtkWidget *widget,
					       ...
					       int x, int y)
{
	...
	kraken_drag_destroy_selection_list (dnd_info->drag_info.selection_list);
	...
	dnd_info->drag_info.selection_list = kraken_drag_build_selection_list (data);
	dnd_info->shadow = create_selection_shadow (container, dnd_info->drag_info.selection_list);
	kraken_icon_container_position_shadow (container, x, y);
}
...
get_direct_save_filename (GdkDragContext *context)
{
	...
	if (!gdk_property_get (gdk_drag_context_get_source_window (context), gdk_atom_intern (KRAKEN_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
static void
set_direct_save_uri (GtkWidget *widget, GdkDragContext *context, KrakenDragInfo *drag_info, int x, int y)
{
	GFile *base, *child;
	...
	drag_info->data_type = KRAKEN_ICON_DND_XDNDDIRECTSAVE;

	uri = NULL;

	filename = get_direct_save_filename (context);
	drop_target = kraken_icon_container_find_drop_target (KRAKEN_ICON_CONTAINER (widget),
	...
	if (filename != NULL && drop_target != NULL) {
		...
		gdk_property_change (gdk_drag_context_get_source_window (context),
				     gdk_atom_intern (KRAKEN_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
get_data_on_first_target_we_support (GtkWidget *widget, GdkDragContext *context, guint32 time, int x, int y)
{
	...
	if (drop_types_list == NULL) {
		...
		gtk_target_list_add_text_targets (drop_types_list, KRAKEN_ICON_DND_TEXT);
	}
	if (drop_types_list_root == NULL) {
		drop_types_list_root = gtk_target_list_new (drop_types,
							    G_N_ELEMENTS (drop_types));
		gtk_target_list_add_text_targets (drop_types_list_root, KRAKEN_ICON_DND_TEXT);
	}

	if (kraken_icon_container_get_is_desktop (KRAKEN_ICON_CONTAINER (widget))) {
	...
	if (target != GDK_NONE) {
		guint info;
		KrakenDragInfo *drag_info;
		gboolean found;

		drag_info = &(KRAKEN_ICON_CONTAINER (widget)->details->dnd_info->drag_info);

		found = gtk_target_list_find (list, target, &info);
		g_assert (found);

		/* Don't get_data for destructive ops */
		if ((info == KRAKEN_ICON_DND_ROOTWINDOW_DROP ||
		     info == KRAKEN_ICON_DND_XDNDDIRECTSAVE) &&
			...
			if (info == KRAKEN_ICON_DND_XDNDDIRECTSAVE) {
...
static void
kraken_icon_container_ensure_drag_data (KrakenIconContainer *container,
					  GdkDragContext *context,
					  guint32 time)
{
	KrakenIconDndInfo *dnd_info;
...
static KrakenIcon *
kraken_icon_container_item_at (KrakenIconContainer *container,
                                int x, int y)
{
	GList *p;
	int size;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		eel_canvas_w2c (EEL_CANVAS (container),
				...
				&canvas_point.x1,
				&canvas_point.y1);
		if (kraken_icon_canvas_item_hit_test_rectangle (icon->item, canvas_point)) {
			return icon;
		}
	}
...
static char *
get_container_uri (KrakenIconContainer *container)
{
	char *uri;
...
static gboolean
kraken_icon_container_selection_items_local (KrakenIconContainer *container,
					       GList *items)
{
	...
	if (eel_uri_is_desktop (container_uri_string)) {
		result = kraken_drag_items_on_desktop (items);
	} else {
		result = kraken_drag_items_local (container_uri_string, items);
	}
...
/* handle dropped url */
static void
receive_dropped_netscape_url (KrakenIconContainer *container, const char *encoded_url, GdkDragContext *context, int x, int y)
{
	char *drop_target;
	...
	drop_target = kraken_icon_container_find_drop_target (container, context, x, y, NULL, TRUE);
...
static void
receive_dropped_uri_list (KrakenIconContainer *container, const char *uri_list, GdkDragContext *context, int x, int y)
{
	char *drop_target;
...
/* handle dropped text */
static void
receive_dropped_text (KrakenIconContainer *container, const char *text, GdkDragContext *context, int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_raw (KrakenIconContainer *container, const char *raw_data, int length, const char *direct_save_uri, GdkDragContext *context, int x, int y)
{
	char *drop_target;
...
auto_scroll_timeout_callback (gpointer data)
{
	...
	g_assert (KRAKEN_IS_ICON_CONTAINER (data));
	...
	kraken_drag_autoscroll_calculate_delta (widget, &x_scroll_delta, &y_scroll_delta);
	...
	if (!kraken_icon_container_scroll (container, (int)x_scroll_delta, (int)y_scroll_delta)) {
...
static void
set_up_auto_scroll_if_needed (KrakenIconContainer *container)
{
	kraken_drag_autoscroll_start (&container->details->dnd_info->drag_info,
...
static void
stop_auto_scroll (KrakenIconContainer *container)
{
	kraken_drag_autoscroll_stop (&container->details->dnd_info->drag_info);
}

static KrakenIcon *
get_icon_from_drag_info (KrakenIconContainer     *container,
                         KrakenDragSelectionItem *item,
                         time_t                 now,
                         gint                   monitor)
{
    ...
    icon = kraken_icon_container_get_icon_by_uri (container, item->uri);

    if (icon == NULL) {
        /* probably dragged from another monitor or screen.  Add it to
         * this screen
         */
        file = kraken_file_get_by_uri (item->uri);
        kraken_file_set_time_metadata (file, KRAKEN_METADATA_KEY_ICON_POSITION_TIMESTAMP, now);
        kraken_file_set_is_desktop_orphan (file, FALSE);
        kraken_file_set_monitor_number (file, monitor);
        kraken_icon_container_add (container, KRAKEN_ICON_CONTAINER_ICON_DATA (file));
...
static void
handle_local_move (KrakenIconContainer *container,
           double world_x, double world_y)
{
    GList *moved_icons, *p;
    KrakenDragSelectionItem *item;
    KrakenIcon *icon;
    KrakenFile *file = NULL;
    gint monitor;
    time_t now;
    ...
    monitor = kraken_desktop_utils_get_monitor_for_widget (GTK_WIDGET (container));
    ...
    for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
        ...
        icon = kraken_icon_container_get_icon_by_uri
            (container, item->uri);
    ...
    kraken_icon_container_select_list_unselect_others (container, moved_icons);
...
static void
handle_local_grid_container_move (KrakenIconContainer *container,
                                  double world_x,
                                  double world_y)
{
    GList *moved_icons, *p;
    ...
    if (container->details->auto_layout) {
        if (kraken_icon_container_get_is_desktop (container)) {
            ...
            if (kraken_icon_container_get_icon_by_uri (container,
                                                     item->uri)) {
                return;
            }
        } else {
...
static void
handle_nonlocal_move (KrakenIconContainer *container,
		      ...
		      gboolean icon_hit)
{
	...
	for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
		...
		source_uris = g_list_prepend (source_uris, ((KrakenDragSelectionItem *)p->data)->uri);
	}
	source_uris = g_list_reverse (source_uris);

	is_rtl = kraken_icon_container_is_layout_rtl (container);
	...
	if (!icon_hit) {
        ...
        for (index = 0, p = container->details->dnd_info->drag_info.selection_list; p != NULL; index++, p = p->next) {
            item_x = ((KrakenDragSelectionItem *)p->data)->icon_x;
            if (is_rtl)
                item_x = -item_x - ((KrakenDragSelectionItem *)p->data)->icon_width;
            g_array_index (source_item_locations, GdkPoint, index).x = item_x;
            g_array_index (source_item_locations, GdkPoint, index).y = ((KrakenDragSelectionItem *)p->data)->icon_y;
		}
	}
	...
	if (eel_uri_is_desktop (target_uri)) {
		target_uri = kraken_get_desktop_directory_uri ();
		free_target_uri = TRUE;
	}

	if (is_rtl) {
		gtk_widget_get_allocation (GTK_WIDGET (container), &allocation);
		x = kraken_icon_container_get_canvas_width (container, allocation) - x;
	}
...
static char *
kraken_icon_container_find_drop_target (KrakenIconContainer *container,
					  GdkDragContext *context,
					  int x, int y,
					  gboolean *icon_hit,
					  gboolean rewrite_desktop)
{
	KrakenIcon *drop_target_icon;
	double world_x, world_y;
	KrakenFile *file;
	char *icon_uri;
	char *container_uri;
	...
	drop_target_icon = kraken_icon_container_item_at (container, world_x, world_y);
	if (drop_target_icon != NULL) {
		icon_uri = kraken_icon_container_get_icon_uri (container, drop_target_icon);
		if (icon_uri != NULL) {
			file = kraken_file_get_by_uri (icon_uri);

			if (!kraken_drag_can_accept_info (file,
	...
	if (drop_target_icon == NULL) {
		...
		if (rewrite_desktop &&
			...
			container_uri = kraken_get_desktop_directory_uri ();
		}
	...
	return kraken_icon_container_get_icon_drop_target_uri (container, drop_target_icon);
}

static void
prep_selection (KrakenIconContainer *container,
                double             world_x,
                double             world_y)
{
    ...
    if (container->details->keep_aligned) {
        if (drop_position_free) {
            ...
            for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
                 ...
                 */
                icon = kraken_icon_container_get_icon_by_uri (grid->container, item->uri);

                if (icon != NULL) {
                    kraken_centered_placement_grid_unmark_icon (grid, icon);
                }
                ...
                if (item->got_icon_position) {
                    ...
                    if (icon && icon->item) {
                        ...
                        kraken_icon_canvas_item_get_bounds_for_entire_item (icon->item, NULL, &y1, NULL, &y2);
                    ...
                    if (!iter_is_free) {
                        ...
                        kraken_centered_placement_grid_mark_position (grid,
                                                                    iter_rect.x,
                                                                    iter_rect.y);
            ...
            for (p = unplaced_list; p != NULL; p = p->next) {
                ...
                kraken_centered_placement_grid_get_next_position_rect (grid,
                                                                     &iter_rect,
                                                                     &iter_rect,
                                                                     &iter_is_free);
            ...
            push_list = kraken_centered_placement_grid_clear_grid_for_selection (grid,
                                                                               drop_x,
                                                                               drop_y,
                                                                               container->details->dnd_info->drag_info.selection_list);
            ...
            kraken_drag_destroy_selection_list (push_list);
        }
    } else {
        iter_rect.x = drop_rect.x;
        iter_rect.y = drop_rect.y;
...
static void
kraken_icon_container_receive_dropped_icons (KrakenIconContainer *container,
					       GdkDragContext *context,
					       int x, int y)
{
	char *drop_target;
	...
	if (real_action == GDK_ACTION_ASK) {
		...
		if (kraken_drag_selection_includes_special_link (container->details->dnd_info->drag_info.selection_list)) {
			/* We only want to move the trash */
			action = GDK_ACTION_MOVE;
		} else {
			action = GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK;
		}
		real_action = kraken_drag_drop_action_ask (GTK_WIDGET (container), action);
	}
	...
	if (real_action > 0) {
		...
		drop_target = kraken_icon_container_find_drop_target (container,
		...
		if (!icon_hit && real_action == GDK_ACTION_MOVE) {
			 ...
			 */
			local_move_only = kraken_icon_container_selection_items_local
	...
	g_free (drop_target);
	kraken_drag_destroy_selection_list (container->details->dnd_info->drag_info.selection_list);
...
static void
kraken_icon_container_get_drop_action (KrakenIconContainer *container,
					 GdkDragContext *context,
					 int x, int y,
					 int *action)
{
	...
	*action = 0;
	...
	case KRAKEN_ICON_DND_URI_LIST:
		drop_target = kraken_icon_container_find_drop_target (container,
									context, x, y, &icon_hit, FALSE);
		*action = kraken_drag_default_drop_action_for_uri_list (context, drop_target);

		g_free (drop_target);
		break;

	case KRAKEN_ICON_DND_NETSCAPE_URL:
		*action = kraken_drag_default_drop_action_for_netscape_url (context);
		break;
...
static void
set_drop_target (KrakenIconContainer *container,
		 KrakenIcon *icon)
{
	KrakenIcon *old_icon;
	...
	kraken_icon_container_update_icon (container, old_icon);
...
static void
kraken_icon_dnd_update_drop_target (KrakenIconContainer *container,
				      GdkDragContext *context,
				      int x, int y)
{
	KrakenIcon *icon;
	...
	/* Find the item we hit with our drop, if any. */
	icon = kraken_icon_container_item_at (container, world_x, world_y);
...
static void
kraken_icon_container_free_drag_data (KrakenIconContainer *container)
{
	KrakenIconDndInfo *dnd_info;
		     ...
		     gpointer data)
{
	...
	dnd_info = KRAKEN_ICON_CONTAINER (widget)->details->dnd_info;
	...
	set_drop_target (KRAKEN_ICON_CONTAINER (widget), NULL);
	stop_auto_scroll (KRAKEN_ICON_CONTAINER (widget));
	kraken_icon_container_free_drag_data(KRAKEN_ICON_CONTAINER (widget));
}
		     ...
		     gpointer        data)
{
	...
	start_y = container->details->dnd_info->drag_info.start_y +
        ...
        surface = kraken_icon_canvas_item_get_drag_surface (container->details->drag_icon->item);
...
void
kraken_icon_dnd_begin_drag (KrakenIconContainer *container,
		      ...
		      guint32 time)
{
	...
	kraken_icon_container_ensure_drag_data (container, context, time);
	kraken_icon_container_position_shadow (container, x, y);
	...
	kraken_icon_dnd_update_drop_target (container, context, x, y);
	set_up_auto_scroll_if_needed (container);
	/* Find out what the drop actions are based on our drag selection and
	 * the drop target.
	 */
	action = 0;
	kraken_icon_container_get_drop_action (container, context, x, y,
						 &action);
	if (action != 0) {
...
void
kraken_icon_dnd_end_drag (KrakenIconContainer *container)
{
	KrakenIconDndInfo *dnd_info;
			     ...
			     gpointer user_data)
{
	...
	switch (info) {
	case KRAKEN_ICON_DND_GNOME_ICON_LIST:
		kraken_icon_container_dropped_icon_feedback (widget, data, x, y);
		break;
	case KRAKEN_ICON_DND_URI_LIST:
	case KRAKEN_ICON_DND_TEXT:
	case KRAKEN_ICON_DND_XDNDDIRECTSAVE:
	...
	if (drag_info->drop_occured) {
		...
		switch (info) {
		case KRAKEN_ICON_DND_GNOME_ICON_LIST:
			kraken_icon_container_receive_dropped_icons
				(KRAKEN_ICON_CONTAINER (widget),
				 context, x, y);
			break;
		case KRAKEN_ICON_DND_NETSCAPE_URL:
			...
			success = TRUE;
            free_dnd_grid (KRAKEN_ICON_CONTAINER (widget));
		...
		case KRAKEN_ICON_DND_XDNDDIRECTSAVE:
		{
			...
			if (selection_format == 8 &&
				...
				gtk_drag_get_data (widget, context,
				                  gdk_atom_intern (KRAKEN_ICON_DND_RAW_TYPE,
				                                   FALSE),
				                  time);
				return;
				...
				kraken_file_changes_queue_file_added (location);
				p.x = x; p.y = y;
                kraken_file_changes_queue_schedule_position_set (
                                 location,
                                 p,
                                 kraken_desktop_utils_get_monitor_for_widget (widget));
				g_object_unref (location);
				kraken_file_changes_consume_changes (TRUE);
				success = TRUE;
			}
...
void
kraken_icon_dnd_init (KrakenIconContainer *container)
{
	...
	container->details->dnd_info = g_new0 (KrakenIconDndInfo, 1);
	kraken_drag_init (&container->details->dnd_info->drag_info,
	...
	gtk_target_list_add_text_targets (targets, KRAKEN_ICON_DND_TEXT);
...
void
kraken_icon_dnd_fini (KrakenIconContainer *container)
{
	g_return_if_fail (KRAKEN_IS_ICON_CONTAINER (container));
	...
	if (container->details->dnd_info != NULL) {
		...
		kraken_drag_finalize (&container->details->dnd_info->drag_info);
