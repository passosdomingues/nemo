...
locale.bindtextdomain("kraken", leconfig.LOCALE_DIR)
gettext.bindtextdomain("kraken", leconfig.LOCALE_DIR)
gettext.textdomain("kraken")
_ = gettext.gettext

gresources = Gio.Resource.load(os.path.join(leconfig.PKG_DATADIR, "kraken-action-layout-editor-resources.gresource"))
gresources._register()

JSON_FILE = Path(GLib.get_user_config_dir()).joinpath("kraken/actions-tree.json")
USER_ACTIONS_DIR = Path(GLib.get_user_data_dir()).joinpath("kraken/actions")
...
class Row():
    ...
    def get_icon_string(self, original=False):
        ...
        if icon_string is None:
            if self.keyfile is not None:
                try:
                    icon_string = self.keyfile.get_string('Kraken Action', 'Icon-Name')
                except GLib.Error:
                    pass
    ...
    def get_label(self):
        ...
        if label is None:
            if self.keyfile is not None:
                try:
                    label = self.keyfile.get_locale_string('Kraken Action', 'Name', None).replace("_", "")
...
class KrakenActionsOrganizer(Gtk.Box):
    def __init__(self, window, builder=None):
        Gtk.Box.__init__(self, orientation=Gtk.Orientation.VERTICAL)

        if builder is None:
            self.builder = Gtk.Builder.new_from_resource("/org/kraken/action-layout-editor/kraken-action-layout-editor.glade")
        else:
            self.builder = builder

        self.builtin_shortcuts = []
        self.load_kraken_shortcuts()
        ...
        self.kraken_plugin_settings = Gio.Settings(schema_id="org.kraken.plugins")
        # Disabled/Enabled may be toggled in kraken preferences directly, keep us in sync.
        self.kraken_plugin_settings.connect("changed", self.on_disabled_settings_list_changed)
    ...
    def load_kraken_shortcuts(self):
        source = Xmlb.BuilderSource()
        try:
            xml = Gio.resources_lookup_data("/org/kraken/action-layout-editor/kraken-shortcuts.ui", Gio.ResourceLookupFlags.NONE)
            ...
            print("Could not load kraken-shortcuts.ui from resource file - we won't be able to detect built-in shortcut collisions: %s" % e.message)
            return
    ...
    def monitor_action_dirs (self):
        ...
        for d in data_dirs:
            full = os.path.join(d, "kraken", "actions")
    ...
    def actions_folder_changed(self, monitor, file, other, event_type, data=None):
        if not file.get_basename().endswith(".kraken_action"):
            return
    ...
    def load_installed_actions(self):
        ...
        for data_dir in data_dirs:
            actions_dir = Path(data_dir).joinpath("kraken/actions")
            if actions_dir.is_dir():
                for path in actions_dir.iterdir():
                    file = Path(path)
                    if file.suffix == ".kraken_action":
                        uuid = file.name
    ...
    def fill_model(self, model, parent, items, installed_actions):
        disabled_actions = self.kraken_plugin_settings.get_strv("disabled-actions")
    ...
    def save_disabled_list(self):
        ...
        self.kraken_plugin_settings.set_strv("disabled-actions", disabled)
    ...
    def validate_accelerator(self, key, mods):
        # Check kraken's built-ins (copy, paste, etc...)
        for shortcut in self.builtin_shortcuts:
            if shortcut.key == key and shortcut.mods == mods:
                ...
                dialog = Gtk.MessageDialog(
                    ...
                    text=_("This key combination is already in use by Kraken (%s). It cannot be changed.") % label,
                    use_markup=True
                )
                dialog.run()
                dialog.destroy()
...
class EditorWindow():
    def __init__(self):
        ...
        self.editor = KrakenActionsOrganizer(self.main_window, self.builder)
