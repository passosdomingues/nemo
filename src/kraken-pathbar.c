...
#define KRAKEN_PATH_BAR_ICON_SIZE 16
#define KRAKEN_PATH_BAR_BUTTON_MAX_WIDTH 250
...
G_DEFINE_TYPE (KrakenPathBar, kraken_path_bar,
           GTK_TYPE_CONTAINER);

static GFile* get_xdg_dir               (GUserDirectory dir);
static void     kraken_path_bar_scroll_up                (KrakenPathBar *path_bar);
static void     kraken_path_bar_scroll_down              (KrakenPathBar *path_bar);
static void     kraken_path_bar_stop_scrolling           (KrakenPathBar *path_bar);
static gboolean kraken_path_bar_slider_button_press      (GtkWidget       *widget,
                                GdkEventButton  *event,
                                KrakenPathBar *path_bar);
static gboolean kraken_path_bar_slider_button_release    (GtkWidget       *widget,
                                GdkEventButton  *event,
                                KrakenPathBar *path_bar);
static void     kraken_path_bar_check_icon_theme         (KrakenPathBar *path_bar);
static void     kraken_path_bar_update_button_appearance (ButtonData      *button_data);
static void     kraken_path_bar_update_button_state      (ButtonData      *button_data,
                                gboolean         current_dir);
static gboolean kraken_path_bar_update_path              (KrakenPathBar *path_bar,
                                GFile           *file_path,
                                gboolean         emit_signal);

static GtkWidget *
get_slider_button (KrakenPathBar     *path_bar,
...
static void
update_button_types (KrakenPathBar *path_bar)
{
    GList *list;
    GFile *path = NULL;
    ...
    if (path != NULL) {
        kraken_path_bar_update_path (path_bar, path, TRUE);
        g_object_unref (path);
    }
}
...
desktop_location_changed_callback (gpointer user_data)
{
    ...
    path_bar = KRAKEN_PATH_BAR (user_data);

    g_object_unref (path_bar->priv->desktop_path);
    g_object_unref (path_bar->priv->home_path);
    path_bar->priv->desktop_path = kraken_get_desktop_location ();
...
slider_timeout (gpointer user_data)
{
    ...
    if (gtk_widget_get_visible (GTK_WIDGET (path_bar))) {
        if (path_bar->priv->drag_slider_timeout_for_up_button) {
            kraken_path_bar_scroll_up (path_bar);
        } else {
            kraken_path_bar_scroll_down (path_bar);
        }
    }
...
static void
kraken_path_bar_slider_drag_motion (GtkWidget      *widget,
...
static void
kraken_path_bar_slider_drag_leave (GtkWidget      *widget,
                     GdkDragContext *context,
                     unsigned int    time,
                     gpointer        user_data)
{
 ...
 * Ripped from kraken-file.c (kraken_file_is_user_special_directory) and slightly modified
 */
static GFile*
...
static void
kraken_path_bar_init (KrakenPathBar *path_bar)
{
    char *p;

    path_bar->priv = G_TYPE_INSTANCE_GET_PRIVATE (path_bar, KRAKEN_TYPE_PATH_BAR, KrakenPathBarDetails);
    ...
    p = kraken_get_desktop_directory ();
    ...
    g_signal_connect_swapped (path_bar->priv->up_slider_button, "clicked", G_CALLBACK (kraken_path_bar_scroll_up), path_bar);
    g_signal_connect_swapped (path_bar->priv->down_slider_button, "clicked", G_CALLBACK (kraken_path_bar_scroll_down), path_bar);

    g_signal_connect (path_bar->priv->up_slider_button, "button_press_event", G_CALLBACK (kraken_path_bar_slider_button_press), path_bar);
    g_signal_connect (path_bar->priv->up_slider_button, "button_release_event", G_CALLBACK (kraken_path_bar_slider_button_release), path_bar);
    g_signal_connect (path_bar->priv->down_slider_button, "button_press_event", G_CALLBACK (kraken_path_bar_slider_button_press), path_bar);
    g_signal_connect (path_bar->priv->down_slider_button, "button_release_event", G_CALLBACK (kraken_path_bar_slider_button_release), path_bar);
    ...
    g_signal_connect (path_bar->priv->up_slider_button,
              "drag-motion",
              G_CALLBACK (kraken_path_bar_slider_drag_motion),
              path_bar);
    g_signal_connect (path_bar->priv->up_slider_button,
              "drag-leave",
              G_CALLBACK (kraken_path_bar_slider_drag_leave),
              path_bar);
...
static void
kraken_path_bar_finalize (GObject *object)
{
    KrakenPathBar *path_bar;

    path_bar = KRAKEN_PATH_BAR (object);

    kraken_path_bar_stop_scrolling (path_bar);
    ...
    G_OBJECT_CLASS (kraken_path_bar_parent_class)->finalize (object);
}

/* Removes the settings signal handler.  It's safe to call multiple times */
static void
remove_settings_signal (KrakenPathBar *path_bar,
            GdkScreen  *screen)
{
...
static void
kraken_path_bar_dispose (GObject *object)
{
    remove_settings_signal (KRAKEN_PATH_BAR (object), gtk_widget_get_screen (GTK_WIDGET (object)));

    G_OBJECT_CLASS (kraken_path_bar_parent_class)->dispose (object);
}
 ...
 */
static void
kraken_path_bar_get_preferred_width (GtkWidget *widget,
                       gint      *minimum,
                       gint      *natural)
{
    ButtonData *button_data;
    ...
    path_bar = KRAKEN_PATH_BAR (widget);
...
static void
kraken_path_bar_get_preferred_height (GtkWidget *widget,
                    gint      *minimum,
                    gint      *natural)
{
    ButtonData *button_data;
...
static void
kraken_path_bar_update_slider_buttons (KrakenPathBar *path_bar)
{
...
static void
kraken_path_bar_unmap (GtkWidget *widget)
{
    kraken_path_bar_stop_scrolling (KRAKEN_PATH_BAR (widget));
    gdk_window_hide (KRAKEN_PATH_BAR (widget)->priv->event_window);

    GTK_WIDGET_CLASS (kraken_path_bar_parent_class)->unmap (widget);
}

static void
kraken_path_bar_map (GtkWidget *widget)
{
    gdk_window_show (KRAKEN_PATH_BAR (widget)->priv->event_window);

    GTK_WIDGET_CLASS (kraken_path_bar_parent_class)->map (widget);
}

static void
child_ordering_changed (KrakenPathBar *path_bar)
{
    GList *l;
...
static void
kraken_path_bar_size_allocate (GtkWidget     *widget,
                     GtkAllocation *allocation)
{
    ...
    if (need_sliders) {
        ...
        kraken_path_bar_update_slider_buttons (path_bar);
    } else {
        needs_reorder |= gtk_widget_get_child_visible (path_bar->priv->up_slider_button) == TRUE;
...
static void
kraken_path_bar_style_updated (GtkWidget *widget)
{
    GTK_WIDGET_CLASS (kraken_path_bar_parent_class)->style_updated (widget);

    kraken_path_bar_check_icon_theme (KRAKEN_PATH_BAR (widget));
}

static void
kraken_path_bar_screen_changed (GtkWidget *widget,
                      GdkScreen *previous_screen)
{
    if (GTK_WIDGET_CLASS (kraken_path_bar_parent_class)->screen_changed) {
        GTK_WIDGET_CLASS (kraken_path_bar_parent_class)->screen_changed (widget, previous_screen);
    }
        /* We might nave a new settings, so we remove the old one */
    if (previous_screen) {
        remove_settings_signal (KRAKEN_PATH_BAR (widget), previous_screen);
    }
    kraken_path_bar_check_icon_theme (KRAKEN_PATH_BAR (widget));
}

static gboolean
kraken_path_bar_scroll (GtkWidget      *widget,
...
static void
kraken_path_bar_realize (GtkWidget *widget)
{
    KrakenPathBar *path_bar;
    GtkAllocation allocation;
    GdkWindow *window;
...
static void
kraken_path_bar_unrealize (GtkWidget *widget)
{
    KrakenPathBar *path_bar;
    ...
    GTK_WIDGET_CLASS (kraken_path_bar_parent_class)->unrealize (widget);
}

static void
kraken_path_bar_add (GtkContainer *container,
               GtkWidget    *widget)
{
...
static void
kraken_path_bar_remove_1 (GtkContainer *container,
                    GtkWidget    *widget)
{
...
static void
kraken_path_bar_remove (GtkContainer *container,
                  GtkWidget    *widget)
{
    KrakenPathBar *path_bar;
    GList *children;

    path_bar = KRAKEN_PATH_BAR (container);

    if (widget == path_bar->priv->up_slider_button) {
            kraken_path_bar_remove_1 (container, widget);
...
static void
kraken_path_bar_forall (GtkContainer *container,
...
static void
kraken_path_bar_grab_notify (GtkWidget *widget,
...
static void
kraken_path_bar_state_changed (GtkWidget    *widget,
...
kraken_path_bar_get_path_for_child (GtkContainer *container,
                    GtkWidget *child)
{
    KrakenPathBar *path_bar = KRAKEN_PATH_BAR (container);
    GtkWidgetPath *path;
...
static void
kraken_path_bar_class_init (KrakenPathBarClass *path_bar_class)
{
    ...
    gobject_class->finalize = kraken_path_bar_finalize;
    gobject_class->dispose = kraken_path_bar_dispose;

    widget_class->get_preferred_height = kraken_path_bar_get_preferred_height;
    widget_class->get_preferred_width = kraken_path_bar_get_preferred_width;
    widget_class->realize = kraken_path_bar_realize;
    widget_class->unrealize = kraken_path_bar_unrealize;
    widget_class->unmap = kraken_path_bar_unmap;
    widget_class->map = kraken_path_bar_map;
    widget_class->size_allocate = kraken_path_bar_size_allocate;
    widget_class->style_updated = kraken_path_bar_style_updated;
    widget_class->screen_changed = kraken_path_bar_screen_changed;
    widget_class->grab_notify = kraken_path_bar_grab_notify;
    widget_class->state_changed = kraken_path_bar_state_changed;
    widget_class->scroll_event = kraken_path_bar_scroll;

    container_class->add = kraken_path_bar_add;
    container_class->forall = kraken_path_bar_forall;
    container_class->remove = kraken_path_bar_remove;
    container_class->get_path_for_child = kraken_path_bar_get_path_for_child;

    path_bar_signals [PATH_CLICKED] =
        g_signal_new ("path-clicked",
        G_OBJECT_CLASS_TYPE (path_bar_class),
        G_SIGNAL_RUN_FIRST,
        G_STRUCT_OFFSET (KrakenPathBarClass, path_clicked),
        NULL, NULL,
    ...
    path_bar_signals [PATH_SET] =
        ...
        G_OBJECT_CLASS_TYPE (path_bar_class),
        G_SIGNAL_RUN_FIRST,
        G_STRUCT_OFFSET (KrakenPathBarClass, path_set),
        NULL, NULL,
        g_cclosure_marshal_VOID__OBJECT,
        G_TYPE_NONE, 1,
        G_TYPE_FILE);
     ...
     g_type_class_add_private (path_bar_class, sizeof (KrakenPathBarDetails));
}

static void
kraken_path_bar_scroll_down (KrakenPathBar *path_bar)
{
    GList *list;
    GList *down_button;
    GList *up_button;
...
static void
kraken_path_bar_scroll_up (KrakenPathBar *path_bar)
{
    GList *list;
...
static gboolean
kraken_path_bar_scroll_timeout (KrakenPathBar *path_bar)
{
    ...
    if (path_bar->priv->timer) {
        ...
        if (path_bar->priv->need_timer) {
            ...
            path_bar->priv->timer = g_timeout_add (SCROLL_TIMEOUT,
                                   (GSourceFunc)kraken_path_bar_scroll_timeout,
                                   path_bar);
...
static void
kraken_path_bar_stop_scrolling (KrakenPathBar *path_bar)
{
    if (path_bar->priv->timer) {
        g_source_remove (path_bar->priv->timer);
...
static gboolean
kraken_path_bar_slider_button_press (GtkWidget       *widget,
                       GdkEventButton  *event,
                       KrakenPathBar *path_bar)
{
...
static gboolean
kraken_path_bar_slider_button_release (GtkWidget      *widget,
                         GdkEventButton *event,
                         KrakenPathBar     *path_bar)
{
...
static void
reload_icons (KrakenPathBar *path_bar)
{
    GList *list;
    ...
    for (list = path_bar->priv->button_list; list; list = list->next) {
        ...
        if (button_data->type != NORMAL_BUTTON || button_data->is_base_dir) {
            kraken_path_bar_update_button_appearance (button_data);
        }
    }
}
...
static void
kraken_path_bar_check_icon_theme (KrakenPathBar *path_bar)
{
    GtkSettings *settings;
...
void
kraken_path_bar_clear_buttons (KrakenPathBar *path_bar)
{
           ...
           gpointer   data)
{
    ...
    path_bar = KRAKEN_PATH_BAR (gtk_widget_get_parent (button));
...
button_data_free (ButtonData *button_data)
{
    ...
    if (button_data->file != NULL) {
        ...
        kraken_file_monitor_remove (button_data->file, button_data);
        kraken_file_unref (button_data->file);
    }
...
static void
kraken_path_bar_update_button_appearance (ButtonData *button_data)
{
    gchar *icon_name;
    ...
    if (button_data->image != NULL) {
        switch (button_data->type) {
            case ROOT_BUTTON:
                icon_name = g_strdup (KRAKEN_ICON_SYMBOLIC_FILESYSTEM);
                break;
            ...
            case DESKTOP_BUTTON:
            case XDG_BUTTON:
                icon_name = kraken_file_get_control_icon_name (button_data->file);
                break;
            case NORMAL_BUTTON:
...
static void
kraken_path_bar_update_button_state (ButtonData *button_data,
                                   gboolean    current_dir)
{
    if (button_data->label != NULL) {
...
setup_file_path_mounted_mount (GFile *location, ButtonData *button_data)
{
    ...
    for (l = mounts; l != NULL; l = l->next) {
        ...
        if (g_file_equal (location, root)) {
            ...
            if (button_data) {
                button_data->mount_icon_name = kraken_get_mount_icon_name (mount);
           ...
           GFile *location)
{
    ...
    } else if (kraken_is_home_directory (location)) {
        button_data->type = HOME_BUTTON;
        button_data->fake_root = TRUE;
    } else if (kraken_is_desktop_directory (location)) {
        if (!desktop_is_home) {
             ...
             gpointer            user_data)
{
    ...
    if (info == KRAKEN_ICON_DND_GNOME_ICON_LIST) {
        tmp = g_strdup_printf ("%s\r\n", uri_list[0]);
        gtk_selection_data_set (selection_data, gtk_selection_data_get_target (selection_data),
                    8, (const guchar *) tmp, strlen (tmp));
        g_free (tmp);
    } else if (info == KRAKEN_ICON_DND_URI_LIST) {
...
setup_button_drag_source (ButtonData *button_data)
{
    ...
    gtk_target_list_add_uri_targets (target_list, KRAKEN_ICON_DND_URI_LIST);
...
static void
button_data_file_changed (KrakenFile *file,
              ButtonData *button_data)
{
    ...
    path_bar = (KrakenPathBar *) gtk_widget_get_ancestor (button_data->button,
                                KRAKEN_TYPE_PATH_BAR);
    if (path_bar == NULL) {
        return;
    }
    ...
    if (!g_file_equal (button_data->path, location)) {
        ...
        if (renamed) {
            ...
            if (child) {
                 ...
                 */
                current_location = kraken_file_get_location (current_button_data->file);
            }

                kraken_path_bar_update_path (path_bar, location, FALSE);
                kraken_path_bar_set_path (path_bar, current_location);
            g_object_unref (location);
            g_object_unref (current_location);
            return;
        }
    } else if (kraken_file_is_gone (file)) {
        gint idx, position;
    ...
    if (button_data->type != MOUNT_BUTTON) {
        display_name = kraken_file_get_display_name (file);
...
static ButtonData *
make_directory_button (KrakenPathBar  *path_bar,
               KrakenFile     *file,
               gboolean          current_dir,
               gboolean          base_dir)
{
    ...
    path = kraken_file_get_location (file);
    child = NULL;
    ...
    if (button_data->dir_name == NULL) {
        button_data->dir_name = kraken_file_get_display_name (file);
    }
    if (button_data->file == NULL) {
        button_data->file = kraken_file_ref (file);
        kraken_file_monitor_add (button_data->file, button_data,
    ...
    kraken_path_bar_update_button_state (button_data, current_dir);
    ...
    kraken_drag_slot_proxy_init (button_data->button, button_data->file, NULL);
...
static gboolean
kraken_path_bar_check_parent_path (KrakenPathBar *path_bar,
                     GFile *location,
                     ButtonData **current_button_data)
{
    GList *list;
    ...
    if (current_path) {
        ...
        for (list = path_bar->priv->button_list; list; list = list->next) {
            kraken_path_bar_update_button_state (BUTTON_DATA (list->data),
...
static gboolean
kraken_path_bar_update_path (KrakenPathBar *path_bar,
                   GFile *file_path,
                   gboolean emit_signal)
{
    KrakenFile *file, *parent_file;
    gboolean first_directory, last_directory;
    gboolean result;
    GList *new_buttons, *l, *fake_root;
    ButtonData *button_data, *current_button_data;

    g_return_val_if_fail (KRAKEN_IS_PATH_BAR (path_bar), FALSE);
    ...
    file = kraken_file_get (file_path);
    ...
    kraken_path_bar_clear_buttons (path_bar);
...
gboolean
kraken_path_bar_set_path (KrakenPathBar *path_bar, GFile *file_path)
{
    ButtonData *button_data;
    ...
    if (kraken_path_bar_check_parent_path (path_bar, file_path, &button_data)) {
        if (path_bar->priv->current_path != NULL) {
            g_object_unref (path_bar->priv->current_path);
        }
    ...
    return kraken_path_bar_update_path (path_bar, file_path, TRUE);
}

GFile *
kraken_path_bar_get_path_for_button (KrakenPathBar *path_bar,
                       GtkWidget       *button)
{
    GList *list;

    g_return_val_if_fail (KRAKEN_IS_PATH_BAR (path_bar), NULL);
