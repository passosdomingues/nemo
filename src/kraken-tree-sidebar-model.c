...
typedef gboolean (* FilePredicate) (KrakenFile *);
...
static TreeNode *
tree_node_new (KrakenFile *file, FMTreeModelRoot *root)
{
	TreeNode *node;

	node = g_new0 (TreeNode, 1);
	node->file = kraken_file_ref (file);
	node->root = root;
...
tree_node_destroy (FMTreeModel *model, TreeNode *node)
{
	...
	kraken_directory_unref (node->directory);
                        ...
                        KrakenFileIconFlags flags)
{
	...
	gicon = G_ICON (kraken_file_get_icon_pixbuf (file, size, TRUE, node->icon_scale, flags));
	...
	emblem_icons = kraken_file_get_emblem_icons (node->file, parent_file);
...
{
	return tree_node_update_icon (node, &node->open_icon, KRAKEN_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
}
...
tree_node_update_display_name (TreeNode *node)
{
	...
	display_name = kraken_file_get_display_name (node->file);
...
tree_node_update_pinning (TreeNode *node)
{
    ...
    file_value = kraken_file_get_pinning (node->file);
...
tree_node_update_fav_unavailable (TreeNode *node)
{
    ...
    file_value = kraken_file_is_unavailable_favorite (node->file);
...
tree_node_get_open_icon (TreeNode *node)
{
	if (node->open_icon == NULL) {
		node->open_icon = tree_node_get_icon (node, KRAKEN_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
	}
...
{
	if (node->display_name == NULL) {
		node->display_name = kraken_file_get_display_name (node->file);
	}
...
static TreeNode *
get_node_from_file (FMTreeModelRoot *root, KrakenFile *file)
{
	return g_hash_table_lookup (root->file_to_node_map, file);
}

static TreeNode *
get_parent_node_from_file (FMTreeModelRoot *root, KrakenFile *file)
{
	KrakenFile *parent_file;
...
static TreeNode *
create_node_for_file (FMTreeModelRoot *root, KrakenFile *file)
{
	TreeNode *node;
...
get_node_uri (GtkTreeIter *iter)
{
	...
	node = iter->user_data;
	if (node != NULL) {
		return kraken_file_get_uri (node->file);
	}

	parent = iter->user_data2;
	parent_uri = kraken_file_get_uri (parent->file);
...
abandon_node_ref_count (FMTreeModel *model, TreeNode *node)
{
	if (node->parent != NULL) {
		...
		if (node->ref_count != 0) {
			...
			uri = kraken_file_get_uri (node->file);
...
report_node_inserted (FMTreeModel *model, TreeNode *node)
{
	...
	if (node->directory != NULL) {
        guint count;
        if (kraken_file_get_directory_item_count (node->file, &count, NULL)) {
...
stop_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
	...
	kraken_directory_file_monitor_remove (node->directory, model);
}
...
update_node_without_reporting (FMTreeModel *model, TreeNode *node)
{
	...
	if (node->directory == NULL &&
	    (kraken_file_is_directory (node->file) || node->parent == NULL)) {
		node->directory = kraken_directory_get_for_file (node->file);
...
static gboolean
should_show_file (FMTreeModel *model, KrakenFile *file)
{
	gboolean should;
	TreeNode *node;

	should = kraken_file_should_show (file,
	...
	if (should && kraken_file_is_gone (file)) {
		should = FALSE;
	}
...
update_node (FMTreeModel *model, TreeNode *node)
{
	...
	if (node->parent != NULL && node->parent->directory != NULL
	    && !kraken_directory_contains_file (node->parent->directory, node->file)) {
			...
			gpointer callback_data)
{
	...
	for (node = changed_files; node != NULL; node = node->next) {
		process_file_change (root, KRAKEN_FILE (node->data));
	}
}
		       ...
		       FMTreeModelRoot *root)
{
	...
	if (node == NULL) {
		 ...
		 * broken due to kraken_directory_remove_file()
...
start_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
	...
	set_done_loading (model, node, kraken_directory_are_all_files_seen (directory));

	attributes = get_tree_monitor_attributes ();
	kraken_directory_file_monitor_add (directory, model,
					     model->details->show_hidden_files,
...
#if 0
	g_warning ("Node '%s' %s",
		   node && node->file ? kraken_file_get_uri (node->file) : "no name",
...
fm_tree_model_add_root_uri (FMTreeModel *model, const char *root_uri, const char *display_name, GIcon *icon, GMount *mount)
{
	...
	file = kraken_file_get_by_uri (root_uri);
...
GMount *
fm_tree_model_get_mount_for_root_node_file (FMTreeModel *model, KrakenFile *file)
{
	TreeNode *node;
...
fm_tree_model_remove_root_uri (FMTreeModel *model, const char *uri)
{
	...
	if (node) {
		...
		kraken_file_monitor_remove (node->file, model);
		path = get_node_path (model, node);
					   ...
					   gboolean show_hidden_files)
{
	...
	if (!show_hidden_files) {
		destroy_by_function (model, kraken_file_is_hidden_file);
	}
	schedule_monitoring_update (model);
}

static gboolean
file_is_not_directory (KrakenFile *file)
{
...
fm_tree_model_iter_get_file (FMTreeModel *model, GtkTreeIter *iter)
{
	...
	node = iter->user_data;
	return node == NULL ? NULL : kraken_file_ref (node->file);
}
...
static void
do_update_node (KrakenFile *file,
                  FMTreeModel *model)
{
                                       ...
                                       GList *files)
{
	...
	if (model->details->highlighted_files != NULL) {
		...
		kraken_file_list_free (old_files);
	}
...
fm_tree_model_finalize (GObject *object)
{
	...
	if (model->details->highlighted_files != NULL) {
		kraken_file_list_free (model->details->highlighted_files);
	}
