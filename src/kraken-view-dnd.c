...
/*
 * kraken-view-dnd.c: DnD helpers for KrakenView
 *
...
static inline void
view_widget_to_file_operation_position (KrakenView *view,
                                        GdkPoint *position)
{
	KrakenViewClass *class = KRAKEN_VIEW_GET_CLASS (view);
...
static void
view_widget_to_file_operation_position_xy (KrakenView *view,
                                           int *x, int *y)
{
	GdkPoint position;
				  ...
				  gpointer user_data)
{
    ...
    monitor_num = kraken_desktop_utils_get_monitor_for_widget (GTK_WIDGET (data->view));

	kraken_link_local_create (data->target_uri,
				    link_name,
				    link_display_name,
				    icon_name,
				    data->url,
...
void
kraken_view_handle_netscape_url_drop (KrakenView  *view,
                                        ...
                                        int            y)
{
	...
	if (action == GDK_ACTION_LINK) {
		...
		kraken_view_move_copy_items (view, uri_list, points,
                                               target_uri,
                                               action, x, y);
...
void
kraken_view_handle_uri_list_drop (KrakenView  *view,
                                    ...
                                    int            y)
{
	...
	container_uri = NULL;
	if (target_uri == NULL) {
		container_uri = kraken_view_get_backing_uri (view);
		g_assert (container_uri != NULL);
	}

	if (action == GDK_ACTION_ASK) {
		action = kraken_drag_drop_action_ask
			(GTK_WIDGET (view),
	...
	kraken_view_move_copy_items (view, real_uri_list, points,
...
void
kraken_view_handle_text_drop (KrakenView  *view,
                                const char    *text,
                                const char    *target_uri,
                                GdkDragAction  action,
                                int            x,
                                int            y)
{
	...
	kraken_view_new_file_with_initial_contents (
...
void
kraken_view_handle_raw_drop (KrakenView *view,
                               const char   *raw_data,
                               int           length,
                               const char   *target_uri,
...
void 
kraken_view_drop_proxy_received_uris (KrakenView *view,
					...
					GdkDragAction action)
{
	...
	kraken_view_move_copy_items (view, source_uri_list, NULL,
				       target_uri != NULL ? target_uri : container_uri,
				       action, 0, 0);
