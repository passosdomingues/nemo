...
/*
 * kraken-progress-ui-handler.c: file operation progress user interface.
 *
...
#include "kraken-progress-info-widget.h"
...
struct _KrakenProgressUIHandlerPriv {
...
G_DEFINE_TYPE (KrakenProgressUIHandler, kraken_progress_ui_handler, G_TYPE_OBJECT);

static void
status_icon_activate_cb (XAppStatusIcon        *icon,
                         guint                  button,
                         guint                  _time,
                         KrakenProgressUIHandler *self)
{
...
static void
progress_ui_handler_ensure_status_icon (KrakenProgressUIHandler *self)
{
	XAppStatusIcon *status_icon;
    ...
    xapp_status_icon_set_icon_name (status_icon, "kraken-progress-0-symbolic");
...
get_icon_name_from_percent (guint pct)
{
    ...
    icon_name = g_strdup_printf ("kraken-progress-%d-symbolic", rounded);

    return icon_name;
}

static void
progress_ui_handler_update_status_icon (KrakenProgressUIHandler *self)
{
	gchar *tooltip;
...
static void
ensure_first_separator_hidden (KrakenProgressUIHandler *self)
{
    ...
    KrakenProgressInfoWidgetPriv *priv = KRAKEN_PROGRESS_INFO_WIDGET (l->data)->priv;
...
static void
progress_ui_handler_sort_by_active (KrakenProgressUIHandler *self)
{
    ...
    for (iter = l; iter != NULL; iter = iter->next) {
        KrakenProgressInfoWidgetPriv *priv = KRAKEN_PROGRESS_INFO_WIDGET (iter->data)->priv;

        if (kraken_progress_info_get_is_started (priv->info)) {
            if (first_pending > 0) {
...
static void
progress_ui_handler_ensure_window (KrakenProgressUIHandler *self)
{
    KrakenProgressUIHandlerPriv *priv = KRAKEN_PROGRESS_UI_HANDLER (self)->priv;
...
static void
progress_ui_handler_add_to_window (KrakenProgressUIHandler *self,
				   KrakenProgressInfo *info)
{
	GtkWidget *progress;

	progress = kraken_progress_info_widget_new (info);
...
static void
progress_ui_handler_show_complete_notification (KrakenProgressUIHandler *self)
{
	...
	g_application_send_notification (G_APPLICATION (kraken_application_get_singleton ()), NULL, complete_notification);
	g_object_unref (complete_notification);
}

static void
progress_ui_handler_hide_status (KrakenProgressUIHandler *self)
{
	if (self->priv->status_icon != NULL) {
        self->priv->should_show_status_icon = FALSE;
...
static void
progress_info_changed_cb (KrakenProgressInfo *info,
			   KrakenProgressUIHandler *self)
{	
	if (g_list_length(self->priv->infos) > 0) {
        KrakenProgressInfo *first_info = (KrakenProgressInfo *) g_list_first(self->priv->infos)->data;
        GList *l;
        g_autofree gchar *status = kraken_progress_info_get_status (first_info);
        double progress = 0.0;
        int i = 0;
        for (l = self->priv->infos; l != NULL; l = l->next) {
            if (kraken_progress_info_get_is_finished (l->data)) {
                continue;
            }
            progress = (progress + kraken_progress_info_get_progress (l->data)) / (double) ++i;
        }
        if (progress > 0) {
...
static void
progress_info_started_cb (KrakenProgressUIHandler *self)
{
    progress_ui_handler_sort_by_active (self);
    ensure_first_separator_hidden (self);
}
...
static void
handle_new_progress_info (KrakenProgressUIHandler *self,
			  KrakenProgressInfo *info)
{
	...
	if (self->priv->active_infos == 1) {
        ...
        gtk_window_present (GTK_WINDOW (self->priv->progress_window));
        gchar *details = kraken_progress_info_get_details (info);
...
static TimeoutData *
timeout_data_new (KrakenProgressUIHandler *self,
		  KrakenProgressInfo *info)
{
	TimeoutData *retval;
...
{
	KrakenProgressInfo *info = data->info;
	KrakenProgressUIHandler *self = data->self;

	if (kraken_progress_info_get_is_paused (info)) {
		return TRUE;
	}

	if (!kraken_progress_info_get_is_finished (info)) {
...
static void
release_application (KrakenProgressInfo *info,
		     KrakenProgressUIHandler *self)
{
...
static void
progress_info_queued_cb (KrakenProgressInfo *info,
			  KrakenProgressUIHandler *self)
{
	KrakenApplication *app;
	TimeoutData *data;
...
static void
new_progress_info_cb (KrakenProgressInfoManager *manager,
		      KrakenProgressInfo *info,
		      KrakenProgressUIHandler *self)
{
...
static void
kraken_progress_ui_handler_dispose (GObject *obj)
{
	KrakenProgressUIHandler *self = KRAKEN_PROGRESS_UI_HANDLER (obj);

	g_clear_object (&self->priv->manager);

	G_OBJECT_CLASS (kraken_progress_ui_handler_parent_class)->dispose (obj);
}

static void
kraken_progress_ui_handler_init (KrakenProgressUIHandler *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, KRAKEN_TYPE_PROGRESS_UI_HANDLER,
						  KrakenProgressUIHandlerPriv);

	self->priv->manager = kraken_progress_info_manager_new ();
...
static void
kraken_progress_ui_handler_class_init (KrakenProgressUIHandlerClass *klass)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (klass);
	oclass->dispose = kraken_progress_ui_handler_dispose;
	
	g_type_class_add_private (klass, sizeof (KrakenProgressUIHandlerPriv));
}

KrakenProgressUIHandler *
kraken_progress_ui_handler_new (void)
{
	return g_object_new (KRAKEN_TYPE_PROGRESS_UI_HANDLER, NULL);
}
