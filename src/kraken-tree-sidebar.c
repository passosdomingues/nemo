...
#include "kraken-tree-sidebar-model.h"
...
struct FMTreeViewDetails {
	...
	KrakenWindowOpenFlags activation_flags;
...
static void
notify_clipboard_info (KrakenClipboardMonitor *monitor,
...
static gboolean
show_iter_for_file (FMTreeView *view, KrakenFile *file, GtkTreeIter *iter)
{
	GtkTreeModel *model;
...
show_selection_idle_callback (gpointer callback_data)
{
	...
	file = kraken_file_get_by_uri (view->details->selection_location);
	if (file == NULL) {
		return FALSE;
	}
	...
	if (!kraken_file_is_directory (file)) {
		old_file = file;
		file = kraken_file_get_parent (file);
...
row_loaded_callback (GtkTreeModel     *tree_model,
		     ...
		     FMTreeView *view)
{
	KrakenFile *file, *tmp_file, *selection_file;
	...
	selection_file = kraken_file_get_by_uri (view->details->selection_location);
	while (selection_file != NULL) {
		if (file == selection_file) {
			kraken_file_unref (file);
			kraken_file_unref (selection_file);

			schedule_show_selection (view);
			return;
		}
		tmp_file = kraken_file_get_parent (selection_file);
		kraken_file_unref (selection_file);
...
static void
got_activation_uri_callback (KrakenFile *file, gpointer callback_data)
{
        char *uri, *file_uri;
        FMTreeView *view;
	GdkScreen *screen;
	...
	open_in_same_slot =
		(view->details->activation_flags &
		 (KRAKEN_WINDOW_OPEN_FLAG_NEW_WINDOW |
		  KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB)) == 0;

	slot = kraken_window_get_active_slot (view->details->window);
	...
	if (kraken_file_is_launcher (file)) {
		...
		kraken_launch_desktop_file (screen, file_uri, NULL, NULL);
		g_free (file_uri);
	} else if (uri != NULL
		...
		if (file_uri == NULL) {
			...
			kraken_window_slot_open_location
				(slot, location, 
				 view->details->activation_flags);
			g_object_unref (location);
		} else {
			DEBUG ("Tree sidebar, launching application for %s", file_uri);
			kraken_launch_application_from_command (screen, file_uri, FALSE, NULL);
			g_free (file_uri);
		}
	...
	g_free (uri);
	kraken_file_unref (view->details->activation_file);
...
selection_changed_timer_callback(FMTreeView *view)
{
	...
	attributes = KRAKEN_FILE_ATTRIBUTE_INFO | KRAKEN_FILE_ATTRIBUTE_LINK_INFO;
	kraken_file_call_when_ready (view->details->activation_file, attributes,
				       got_activation_uri_callback, view);
...
compare_rows (GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer callback_data)
{
	...
	if (file_a == file_b) {
        ...
        result = kraken_file_compare_for_sort (file_a, file_b,
	...
	kraken_file_unref (file_a);
	kraken_file_unref (file_b);
...
static gboolean
is_parent_writable (KrakenFile *file)
{
	KrakenFile *parent;
	gboolean result;
	...
	result = kraken_file_can_write (parent);
                      ...
                      gpointer          data)
{
    ...
    if (gtk_selection_data_get_data_type (selection_data) == copied_files_atom
        ...
        set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_PASTE, TRUE);
    }
                    ...
                    GdkEventButton *event)
{
    ...
    using_browser = g_settings_get_boolean (kraken_preferences,
    ...
    KrakenFile *file = sort_model_path_to_file (view, path);
    view->details->popup_file = kraken_file_ref (file);
    ...
    kraken_file_list_free (selected);
    kraken_file_unref (parent);

    is_dir = kraken_file_is_directory (file);
    can_write = kraken_file_can_write (file);

    is_toplevel = gtk_tree_path_get_depth (path) == 1;
    gtk_tree_path_free (path);

    set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_OPEN_ALTERNATE, is_dir);
    set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_OPEN_IN_NEW_TAB, is_dir);
    set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_OPEN_ALTERNATE, using_browser);
    set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_OPEN_IN_NEW_TAB, using_browser);
    set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_NEW_FOLDER, is_dir && can_write);
    set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_NEW_FOLDER, !kraken_file_is_in_favorites (file));
    set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_PASTE, FALSE);

    if (is_dir && can_write) {
        gtk_clipboard_request_contents (kraken_clipboard_get (GTK_WIDGET (view->details->tree_widget)),
                                        copied_files_atom,
                                        clipboard_contents_received_callback, g_object_ref (view));
    }

    can_move_file_to_trash = kraken_file_can_trash (file);
    set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_TRASH, can_move_file_to_trash);

    if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_ENABLE_DELETE)) {
    	parent_file_is_writable = is_parent_writable (file);
    	file_is_home_or_desktop = kraken_file_is_home (file)
    		|| kraken_file_is_desktop_directory (file);
    	file_is_special_link = KRAKEN_IS_DESKTOP_ICON_FILE (file);
    	
    	can_delete_file = parent_file_is_writable 
    		&& !file_is_home_or_desktop
    		&& !file_is_special_link;

        set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_DELETE, TRUE);
        set_action_sensitive (view->details->tv_action_group, KRAKEN_ACTION_DELETE, can_delete_file);
    } else {
        set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_DELETE, FALSE);
    }
    ...
    set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_UNMOUNT_VOLUME, show_unmount);
    set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_EJECT_VOLUME, show_eject);

    if (!is_toplevel && !kraken_file_is_in_favorites (file)) {
        set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_PIN_FILE,  !kraken_file_get_pinning (file));
        set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_UNPIN_FILE, kraken_file_get_pinning (file));
    } else {
        set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_PIN_FILE, FALSE);
        set_action_visible (view->details->tv_action_group, KRAKEN_ACTION_UNPIN_FILE, FALSE);
    }
}
			    ...
			    KrakenWindowOpenFlags flags)
{
	...
	view->details->activation_file = kraken_file_ref (file);
...
fm_tree_view_open_in_new_tab_cb (GtkAction *action,
				    FMTreeView *view)
{
	fm_tree_view_activate_file (view, view->details->popup_file, KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB);
}
...
fm_tree_view_open_in_new_window_cb (GtkAction *action,
				    FMTreeView *view)
{
	fm_tree_view_activate_file (view, view->details->popup_file, KRAKEN_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
		 ...
		 gpointer data)
{
	 ...
	 */
	list = g_list_prepend (NULL, kraken_file_get (new_folder));

	kraken_properties_window_present (list, GTK_WIDGET (data), NULL);
			       ...
			       FMTreeView *view)
{
	...
	parent_uri = kraken_file_get_uri (view->details->popup_file);
	kraken_file_operations_new_folder (GTK_WIDGET (view->details->tree_widget),
					     NULL,
					     parent_uri,
		   ...
		   gboolean cut)
{
	...
	kraken_clipboard_monitor_set_clipboard_info (kraken_clipboard_monitor_get (),
	                                               &info);
	g_list_free (info.files);

	name = kraken_file_get_display_name (view->details->popup_file);
	if (cut) {
	...
	kraken_window_push_status (view->details->window,
					  status_string);
					...
					gpointer          data)
{
	...
	directory_uri = kraken_file_get_uri (view->details->popup_file);
...
fm_tree_view_pin_unpin_cb (GtkAction *action,
                           FMTreeView *view)
{
    kraken_file_set_pinning (view->details->popup_file,
                           !kraken_file_get_pinning (view->details->popup_file));
}
		       ...
		       FMTreeView *view)
{
	...
	if (!kraken_file_can_trash (view->details->popup_file)) {
		return;
	}
	
	list = g_list_prepend (NULL,
			       kraken_file_get_location (view->details->popup_file));
	
	kraken_file_operations_trash_or_delete (list, 
		        ...
		        FMTreeView *view)
{
	...
	kraken_file_operations_delete (location_list, fm_tree_view_get_containing_window (view), NULL, NULL);
			    ...
			    FMTreeView *view)
{
	...
	list = g_list_prepend (NULL, kraken_file_ref (view->details->popup_file));

	kraken_properties_window_present (list, GTK_WIDGET (view->details->tree_widget), NULL);
...
fm_tree_view_unmount_cb (GtkAction *action,
			 FMTreeView *view)
{
	KrakenFile *file = view->details->popup_file;
	GMount *mount;
	...
	if (mount != NULL) {
		kraken_file_operations_unmount_mount (fm_tree_view_get_containing_window (view),
							mount, FALSE, TRUE);
	}
}
...
free_popup_file_in_idle_cb (gpointer data)
{
	...
	if (view->details->popup_file != NULL) {
		kraken_file_unref (view->details->popup_file);
...
run_action_callback (GtkAction *action, gpointer user_data)
{
    ...
    uri = kraken_file_get_uri (view->details->popup_file);
    ...
    kraken_action_activate (KRAKEN_ACTION (action), selection, parent, GTK_WINDOW (view->details->window));
...
{
    kraken_ui_unmerge_ui (view->details->ui_manager,
...
static const GtkActionEntry tree_sidebar_menu_entries[] = {
    { KRAKEN_ACTION_OPEN,                    "xsi-folder-open-symbolic",        N_("_Open"),                NULL, NULL, G_CALLBACK (fm_tree_view_open_cb)               },
    { KRAKEN_ACTION_OPEN_IN_NEW_TAB,         NULL,                          N_("Open in New _Tab"),     NULL, NULL, G_CALLBACK (fm_tree_view_open_in_new_tab_cb)    },
    { KRAKEN_ACTION_OPEN_ALTERNATE,          NULL,                          N_("Open in New _Window"),  NULL, NULL, G_CALLBACK (fm_tree_view_open_in_new_window_cb) },
    { KRAKEN_ACTION_NEW_FOLDER,              NULL,                          N_("Create New _Folder"),   NULL, NULL, G_CALLBACK (fm_tree_view_create_folder_cb)      },
    { KRAKEN_ACTION_CUT,                     "xsi-edit-cut-symbolic",           N_("Cu_t"),                 NULL, NULL, G_CALLBACK (fm_tree_view_cut_cb)                },
    { KRAKEN_ACTION_COPY,                    "xsi-edit-copy-symbolic",          N_("_Copy"),                NULL, NULL, G_CALLBACK (fm_tree_view_copy_cb)               },
    { KRAKEN_ACTION_PASTE,                   "xsi-edit-paste-symbolic",         N_("_Paste Into Folder"),   NULL, NULL, G_CALLBACK (fm_tree_view_paste_cb)              },
    { KRAKEN_ACTION_PIN_FILE,                "xsi-pin-symbolic",           N_("P_in"),                 NULL, NULL, G_CALLBACK (fm_tree_view_pin_unpin_cb)          },
    { KRAKEN_ACTION_UNPIN_FILE,              "xsi-unpin-symbolic",         N_("Unp_in"),               NULL, NULL, G_CALLBACK (fm_tree_view_pin_unpin_cb)          },
    { KRAKEN_ACTION_TRASH,                   "xsi-user-trash-full-symbolic",    N_("Mo_ve to Trash"),       NULL, NULL, G_CALLBACK (fm_tree_view_trash_cb)              },
    { KRAKEN_ACTION_DELETE,                  "xsi-edit-delete-symbolic",        N_("_Delete"),              NULL, NULL, G_CALLBACK (fm_tree_view_delete_cb)             },
    { KRAKEN_ACTION_UNMOUNT_VOLUME,          NULL,                          N_("_Unmount"),             NULL, NULL, G_CALLBACK (fm_tree_view_unmount_cb)            },
    { KRAKEN_ACTION_EJECT_VOLUME,            NULL,                          N_("_Eject"),               NULL, NULL, G_CALLBACK (fm_tree_view_eject_cb)              },
    { KRAKEN_ACTION_PROPERTIES,             "xsi-document-properties-symbolic", N_("_Properties"),          NULL, NULL, G_CALLBACK (fm_tree_view_properties_cb)         },
};
...
rebuild_menu (FMTreeView *view)
{
    ...
    kraken_ui_prepare_merge_ui (view->details->ui_manager,
                              "KrakenTreeviewSidebarFileActions",
                              &view->details->tv_action_group_merge_id,
                              &view->details->tv_action_group);

    kraken_ui_prepare_merge_ui (view->details->ui_manager,
                              "KrakenTreeviewSidebarActionActions",
                              &view->details->action_action_group_merge_id,
                              &view->details->action_action_group);

    view->details->tv_action_group_merge_id =
            gtk_ui_manager_add_ui_from_resource (view->details->ui_manager, "/org/kraken/kraken-tree-sidebar-ui.xml", NULL);
...
create_tree (FMTreeView *view)
{
	...
	view->details->drag_dest = 
		kraken_tree_view_drag_dest_new (view->details->tree_widget, FALSE);
...
update_filtering_from_preferences (FMTreeView *view)
{
    ...
    fm_tree_model_set_show_only_directories (view->details->child_model,
                                             g_settings_get_boolean (kraken_tree_sidebar_preferences,
...
sort_directories_first_changed_callback (gpointer callback_data)
{
    ...
    preference_value = g_settings_get_boolean (kraken_preferences,
...
fm_tree_view_init (FMTreeView *view)
{
	...
	g_signal_connect_swapped (kraken_tree_sidebar_preferences,
				  "changed::" KRAKEN_PREFERENCES_TREE_SHOW_ONLY_DIRECTORIES,
    ...
    view->details->sort_directories_first = g_settings_get_boolean (kraken_preferences,
                                                                    KRAKEN_PREFERENCES_SORT_DIRECTORIES_FIRST);
    view->details->sort_favorites_first = g_settings_get_boolean (kraken_preferences,
                                                                    KRAKEN_PREFERENCES_SORT_FAVORITES_FIRST);
...
static void 
hidden_files_mode_changed_callback (KrakenWindow *window,
                    FMTreeView *view)
{
    update_filtering_from_preferences (view);
}
...
fm_tree_view_dispose (GObject *object)
{
	...
	g_signal_handlers_disconnect_by_func (kraken_tree_sidebar_preferences,
...
GtkWidget *
kraken_tree_sidebar_new (KrakenWindow *window)
{
	FMTreeView *sidebar;
