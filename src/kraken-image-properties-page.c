...
struct KrakenImagePropertiesPageDetails {
...
typedef struct {
        GObject parent;
} KrakenImagePropertiesPageProvider;

typedef struct {
        GObjectClass parent;
} KrakenImagePropertiesPageProviderClass;


static GType kraken_image_properties_page_provider_get_type (void);
static void  property_page_provider_iface_init                (KrakenPropertyPageProviderIface *iface);


G_DEFINE_TYPE (KrakenImagePropertiesPage, kraken_image_properties_page, GTK_TYPE_BOX);

G_DEFINE_TYPE_WITH_CODE (KrakenImagePropertiesPageProvider, kraken_image_properties_page_provider, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (KRAKEN_TYPE_PROPERTY_PAGE_PROVIDER,
						property_page_provider_iface_init));

static void
kraken_image_properties_page_finalize (GObject *object)
{
	KrakenImagePropertiesPage *page;

	page = KRAKEN_IMAGE_PROPERTIES_PAGE (object);
	...
	G_OBJECT_CLASS (kraken_image_properties_page_parent_class)->finalize (object);
}
		     ...
		     gpointer      data)
{
	...
	page = KRAKEN_IMAGE_PROPERTIES_PAGE (data);
...
static void
append_item (KrakenImagePropertiesPage *page,
	     const char                  *name,
	     const char                  *value)
{
...
static gboolean
append_tag_value_pair (KrakenImagePropertiesPage *page,
		       ExifData *data,
		       ExifTag   tag,
		       char     *description)
{
...
#ifdef HAVE_EXEMPI
static void
append_xmp_value_pair (KrakenImagePropertiesPage *page,
		       XmpPtr      xmp,
		       const char *ns,
		       const char *propname,
		       char       *descr)
...
static gboolean
append_option_value_pair (KrakenImagePropertiesPage *page,
			  GdkPixbuf                   *pixbuf,
			  const char                  *key,
			  char                        *description)
{
...
static void
append_basic_info (KrakenImagePropertiesPage *page)
{
...
static void
append_options_info (KrakenImagePropertiesPage *page)
{
	GdkPixbuf *pixbuf;
...
static void
append_exif_info (KrakenImagePropertiesPage *page)
{
#ifdef HAVE_EXIF
	ExifData *exifdata;
...
static void
append_xmp_info (KrakenImagePropertiesPage *page)
{
#ifdef HAVE_EXEMPI
...
static void
load_finished (KrakenImagePropertiesPage *page)
{
	GtkWidget *label;
			...
			gpointer         callback_data)
{
	...
	page = KRAKEN_IMAGE_PROPERTIES_PAGE (callback_data);
		    ...
		    gpointer      user_data)
{
	...
	KrakenImagePropertiesPage *page = data->page;
	GFile *file;
	...
	if (stream) {
		...
		mime_type = kraken_file_info_get_mime_type (data->info);
...
static void
load_location (KrakenImagePropertiesPage *page,
	       KrakenFileInfo	       *info)
{
	GFile *file;
	char *uri;
	...
	g_assert (KRAKEN_IS_IMAGE_PROPERTIES_PAGE (page));
	g_assert (info != NULL);

	page->details->cancellable = g_cancellable_new ();

	uri = kraken_file_info_get_uri (info);
...
static void
kraken_image_properties_page_class_init (KrakenImagePropertiesPageClass *class)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (class);

	object_class->finalize = kraken_image_properties_page_finalize;

	g_type_class_add_private (object_class, sizeof(KrakenImagePropertiesPageDetails));
}

static void
kraken_image_properties_page_init (KrakenImagePropertiesPage *page)
{
	GtkWidget *sw;

	page->details = G_TYPE_INSTANCE_GET_PRIVATE (page,
						     KRAKEN_TYPE_IMAGE_PROPERTIES_PAGE,
						     KrakenImagePropertiesPageDetails);
...
static GList *
get_property_pages (KrakenPropertyPageProvider *provider,
                    GList *files)
{
	GList *pages;
	KrakenFileInfo *file;
	...
	pages = NULL;
        ...
        file = KRAKEN_FILE_INFO (files->data);

	mime_type = kraken_file_info_get_mime_type (file);
	if (mime_type != NULL && is_mime_type_supported (mime_type)) {
		KrakenImagePropertiesPage *page;
		KrakenPropertyPage *real_page;

		page = g_object_new (kraken_image_properties_page_get_type (), NULL);
		load_location (page, file);

		real_page = kraken_property_page_new ("KrakenImagePropertiesPage::property_page",
...
static void
property_page_provider_iface_init (KrakenPropertyPageProviderIface *iface)
{
...
static void
kraken_image_properties_page_provider_init (KrakenImagePropertiesPageProvider *sidebar)
{
}

static void
kraken_image_properties_page_provider_class_init (KrakenImagePropertiesPageProviderClass *class)
{
}

void
kraken_image_properties_page_register (void)
{
        kraken_module_add_type (kraken_image_properties_page_provider_get_type ());
}
