...
static int kraken_list_model_file_entry_compare_func (gconstpointer a,
							gconstpointer b,
							gpointer      user_data);
static void kraken_list_model_tree_model_init (GtkTreeModelIface *iface);
static void kraken_list_model_sortable_init (GtkTreeSortableIface *iface);
static void kraken_list_model_multi_drag_source_init (EggTreeMultiDragSourceIface *iface);

struct KrakenListModelDetails {
	GSequence *files;
...
G_DEFINE_TYPE_WITH_CODE (KrakenListModel, kraken_list_model, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_MODEL,
						kraken_list_model_tree_model_init)
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_SORTABLE,
						kraken_list_model_sortable_init)
			 G_IMPLEMENT_INTERFACE (EGG_TYPE_TREE_MULTI_DRAG_SOURCE,
						kraken_list_model_multi_drag_source_init));

static const GtkTargetEntry drag_types [] = {
	{ (char *)KRAKEN_ICON_DND_GNOME_ICON_LIST_TYPE, 0, KRAKEN_ICON_DND_GNOME_ICON_LIST },
	{ (char *)KRAKEN_ICON_DND_URI_LIST_TYPE, 0, KRAKEN_ICON_DND_URI_LIST },
};
...
{
	kraken_file_unref (file_entry->file);
	...
	if (file_entry->subdirectory != NULL) {
		kraken_directory_unref (file_entry->subdirectory);
	}
...
kraken_list_model_get_flags (GtkTreeModel *tree_model)
{
	return GTK_TREE_MODEL_ITERS_PERSIST;
}

static int
kraken_list_model_get_n_columns (GtkTreeModel *tree_model)
{
	return KRAKEN_LIST_MODEL_NUM_COLUMNS + KRAKEN_LIST_MODEL (tree_model)->details->columns->len;
}

static GType
kraken_list_model_get_column_type (GtkTreeModel *tree_model, int index)
{
	switch (index) {
	case KRAKEN_LIST_MODEL_FILE_COLUMN:
		return KRAKEN_TYPE_FILE;
	case KRAKEN_LIST_MODEL_SUBDIRECTORY_COLUMN:
		return KRAKEN_TYPE_DIRECTORY;
	case KRAKEN_LIST_MODEL_SMALLEST_ICON_COLUMN:
	case KRAKEN_LIST_MODEL_SMALLER_ICON_COLUMN:
	case KRAKEN_LIST_MODEL_SMALL_ICON_COLUMN:
	case KRAKEN_LIST_MODEL_STANDARD_ICON_COLUMN:
	case KRAKEN_LIST_MODEL_LARGE_ICON_COLUMN:
	case KRAKEN_LIST_MODEL_LARGER_ICON_COLUMN:
	case KRAKEN_LIST_MODEL_LARGEST_ICON_COLUMN:
		return CAIRO_GOBJECT_TYPE_SURFACE;
    case KRAKEN_LIST_MODEL_FILE_NAME_IS_EDITABLE_COLUMN:
		return G_TYPE_BOOLEAN;
    case KRAKEN_LIST_MODEL_TEXT_WEIGHT_COLUMN:
        return G_TYPE_INT;
	default:
		if (index < (int)(KRAKEN_LIST_MODEL_NUM_COLUMNS + KRAKEN_LIST_MODEL (tree_model)->details->columns->len)) {
...
static void
kraken_list_model_ptr_to_iter (KrakenListModel *model, GSequenceIter *ptr, GtkTreeIter *iter)
{
...
static gboolean
kraken_list_model_get_iter (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreePath *path)
{
	KrakenListModel *model;
	GSequence *files;
	GSequenceIter *ptr;
	...
	model = (KrakenListModel *)tree_model;
	ptr = NULL;
	...
	kraken_list_model_ptr_to_iter (model, ptr, iter);

	return TRUE;
}

static GtkTreePath *
kraken_list_model_get_path (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	GtkTreePath *path;
	KrakenListModel *model;
	GSequenceIter *ptr;
...
static gint
kraken_list_model_get_icon_scale (KrakenListModel *model)
{
   gint retval = -1;
...
static void
kraken_list_model_get_value (GtkTreeModel *tree_model, GtkTreeIter *iter, int column, GValue *value)
{
	KrakenListModel *model;
    KrakenFile *parent_file;
	FileEntry *file_entry;
	KrakenFile *file;
	char *str;
	...
	switch (column) {
	case KRAKEN_LIST_MODEL_FILE_COLUMN:
		g_value_init (value, KRAKEN_TYPE_FILE);

		g_value_set_object (value, file);
		break;
	case KRAKEN_LIST_MODEL_SUBDIRECTORY_COLUMN:
		g_value_init (value, KRAKEN_TYPE_DIRECTORY);
	...
	case KRAKEN_LIST_MODEL_LARGEST_ICON_COLUMN:
		...
		if (file != NULL) {
			...
			zoom_level = kraken_list_model_get_zoom_level_from_column_id (column);
			icon_size = kraken_get_list_icon_size_for_zoom_level (zoom_level);
            icon_scale = kraken_list_model_get_icon_scale (model);

			flags = KRAKEN_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE |
				KRAKEN_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM;
            ...
            icon_info = kraken_file_get_icon (file, icon_size, 0, icon_scale, flags);
            emblem_icons = kraken_file_get_emblem_icons (file, parent_file);
            ...
            if (emblem_icons) {
                ...
                initial_pixbuf = kraken_icon_info_get_pixbuf_at_size (icon_info, icon_size * icon_scale);
                ...
                bad_ratio = (int)(kraken_icon_get_emblem_size_for_icon_size (icon_size) * icon_scale) > (int)(w * 0.75) ||
                            (int)(kraken_icon_get_emblem_size_for_icon_size (icon_size) * icon_scale) > (int)(h * 0.75);
                ...
                icon_info = kraken_icon_info_lookup (gicon, icon_size, icon_scale);

                g_list_free_full (emblem_icons, g_object_unref);
                g_object_unref (gicon);
            }

			icon = kraken_icon_info_get_pixbuf_at_size (icon_info, icon_size * icon_scale);

			kraken_icon_info_unref (icon_info);

			if (model->details->highlight_files != NULL &&
			    g_list_find_custom (model->details->highlight_files,
			                        file, (GCompareFunc) kraken_file_compare_location))
			{
	...
	case KRAKEN_LIST_MODEL_FILE_NAME_IS_EDITABLE_COLUMN:
		g_value_init (value, G_TYPE_BOOLEAN);
                ...
                g_value_set_boolean (value, file != NULL && kraken_file_can_rename (file));
                break;
    case KRAKEN_LIST_MODEL_TEXT_WEIGHT_COLUMN:
        g_value_init (value, G_TYPE_INT);

        if (file != NULL) {
            if (kraken_file_is_unavailable_favorite (file)) {
                g_value_set_int (value, UNAVAILABLE_TEXT_WEIGHT);
            }
            else
            if (kraken_file_get_pinning (file) && !kraken_file_is_in_favorites (file)) {
        ...
        break;
    case KRAKEN_LIST_MODEL_ICON_SHOWN:
        g_value_init (value, G_TYPE_BOOLEAN);
        ...
        break;
 	default:
 		if (column >= KRAKEN_LIST_MODEL_NUM_COLUMNS || column < (int)(KRAKEN_LIST_MODEL_NUM_COLUMNS + model->details->columns->len)) {
			KrakenColumn *kraken_column;
			GQuark attribute;
			kraken_column = model->details->columns->pdata[column - KRAKEN_LIST_MODEL_NUM_COLUMNS];
			...
			if (file != NULL) {
                if (attribute == attribute_search_result_count_q) {
                    str = kraken_file_get_search_result_count_as_string (file, (gpointer) model->details->view_dir);
                } else {
                    str = kraken_file_get_string_attribute_with_default_q (file, attribute);
                }
...
static gboolean
kraken_list_model_iter_next (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	KrakenListModel *model;
...
static gboolean
kraken_list_model_iter_children (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent)
{
	KrakenListModel *model;
	GSequence *files;
	FileEntry *file_entry;
...
static gboolean
kraken_list_model_iter_has_child (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	FileEntry *file_entry;

	if (iter == NULL) {
		return !kraken_list_model_is_empty (KRAKEN_LIST_MODEL (tree_model));
	}
...
static int
kraken_list_model_iter_n_children (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	KrakenListModel *model;
	GSequence *files;
	FileEntry *file_entry;
...
static gboolean
kraken_list_model_iter_nth_child (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent, int n)
{
	KrakenListModel *model;
	GSequenceIter *child;
	GSequence *files;
...
static gboolean
kraken_list_model_iter_parent (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *child)
{
	KrakenListModel *model;
	FileEntry *file_entry;
...
lookup_file (KrakenListModel *model, KrakenFile *file,
	      ...
	      GHashTable *reverse_map)
{
	...
	if (ptr) {
		...
		kraken_list_model_ptr_to_iter (data->model, ptr, iter);
...
GList *
kraken_list_model_get_all_iters_for_file (KrakenListModel *model, KrakenFile *file)
{
	struct GetIters data;
...
gboolean
kraken_list_model_get_first_iter_for_file (KrakenListModel          *model,
					     KrakenFile         *file,
					     GtkTreeIter          *iter)
{
	GList *list;
	...
	list = kraken_list_model_get_all_iters_for_file (model, file);
	if (list != NULL) {
		res = TRUE;
...
gboolean
kraken_list_model_get_tree_iter_from_file (KrakenListModel *model, KrakenFile *file,
					     KrakenDirectory *directory,
					     GtkTreeIter *iter)
{
	GSequenceIter *ptr;
...
static int
kraken_list_model_file_entry_compare_func (gconstpointer a,
					     gconstpointer b,
					     gpointer      user_data)
{
	...
	model = (KrakenListModel *)user_data;

	file_entry1 = (FileEntry *)a;
	file_entry2 = (FileEntry *)b;

	if (file_entry1->file != NULL && file_entry2->file != NULL) {
		result = kraken_file_compare_for_sort_by_attribute_q (file_entry1->file, file_entry2->file,
...
int
kraken_list_model_compare_func (KrakenListModel *model,
				  KrakenFile *file1,
				  KrakenFile *file2)
{
	int result;

	result = kraken_file_compare_for_sort_by_attribute_q (file1, file2,
								model->details->sort_attribute,
								model->details->sort_directories_first,
								model->details->sort_favorites_first,
								(model->details->order == GTK_SORT_DESCENDING),
...
static void
kraken_list_model_sort_file_entries (KrakenListModel *model, GSequence *files, GtkTreePath *path)
{
	...
	for (i = 0; i < length; ++i) {
		...
		if (file_entry->files != NULL) {
			...
			kraken_list_model_sort_file_entries (model, file_entry->files, path);
	...
	/* sort */
	g_sequence_sort (files, kraken_list_model_file_entry_compare_func, model);
...
static void
kraken_list_model_sort (KrakenListModel *model)
{
	GtkTreePath *path;

	path = gtk_tree_path_new ();

	kraken_list_model_sort_file_entries (model, model->details->files, path);

	gtk_tree_path_free (path);
}

static gboolean
kraken_list_model_get_sort_column_id (GtkTreeSortable *sortable,
					gint            *sort_column_id,
					GtkSortType     *order)
{
	KrakenListModel *model;
	...
	model = (KrakenListModel *)sortable;

	id = kraken_list_model_get_sort_column_id_from_attribute
...
static void
kraken_list_model_set_sort_column_id (GtkTreeSortable *sortable, gint sort_column_id, GtkSortType order)
{
	KrakenListModel *model;

	model = (KrakenListModel *)sortable;

	model->details->sort_attribute = kraken_list_model_get_attribute_from_sort_column_id (model, sort_column_id);

	model->details->order = order;

	kraken_list_model_sort (model);
	gtk_tree_sortable_sort_column_changed (sortable);
}

static gboolean
kraken_list_model_has_default_sort_func (GtkTreeSortable *sortable)
{
	return FALSE;
}

static gboolean
kraken_list_model_multi_row_draggable (EggTreeMultiDragSource *drag_source, GList *path_list)
{
	return TRUE;
}

static void
each_path_get_data_binder (KrakenDragEachSelectedItemDataGet data_get,
			   gpointer context,
			   gpointer data)
{
	...
	for (l = info->path_list; l != NULL; l = l->next) {
		...
		file = kraken_list_model_file_for_path (info->model, path);
		if (file) {
			...
			uri = kraken_file_get_local_uri (file);
			path_str = kraken_file_get_path (file);
...
static gboolean
kraken_list_model_multi_drag_data_get (EggTreeMultiDragSource *drag_source,
					 GList *path_list,
					 GtkSelectionData *selection_data)
{
	...
	model = KRAKEN_LIST_MODEL (drag_source);

	context.model = model;
	context.path_list = path_list;

	if (!drag_target_list) {
		drag_target_list = kraken_list_model_get_drag_target_list ();
	}

	if (gtk_target_list_find (drag_target_list,
				  gtk_selection_data_get_target (selection_data),
				  &target_info)) {
		kraken_drag_drag_data_get (NULL,
					     NULL,
					     selection_data,
					     target_info,
					     GDK_CURRENT_TIME,
...
static gboolean
kraken_list_model_multi_drag_data_delete (EggTreeMultiDragSource *drag_source, GList *path_list)
{
	return TRUE;
}

static void
add_dummy_row (KrakenListModel *model, FileEntry *parent_entry)
{
	...
	dummy_file_entry->ptr = g_sequence_insert_sorted (parent_entry->files, dummy_file_entry,
							  kraken_list_model_file_entry_compare_func, model);
	iter.stamp = model->details->stamp;
...
gboolean
kraken_list_model_add_file (KrakenListModel *model, KrakenFile *file,
			      KrakenDirectory *directory)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	...
	file_entry->file = kraken_file_ref (file);
    ...
    if (kraken_file_is_directory (file)) {
        ...
        got_count = kraken_file_get_directory_item_count (file, &count, &unreadable);
...
static gboolean
update_dummy_row (KrakenListModel *model,
                  KrakenFile      *file,
                  FileEntry     *file_entry)
{
    GSequence *files;
...
void
kraken_list_model_file_changed (KrakenListModel *model, KrakenFile *file,
				  KrakenDirectory *directory)
{
	...
	pos_before = g_sequence_iter_get_position (ptr);
        ...
        if (!model->details->temp_unsorted)
                g_sequence_sort_changed (ptr, kraken_list_model_file_entry_compare_func, model);
	...
	if (pos_before != pos_after) {
		...
		if (parent_file_entry == NULL) {
			...
			has_iter = TRUE;
			kraken_list_model_ptr_to_iter (model, parent_file_entry->ptr, &iter);
...
gboolean
kraken_list_model_is_empty (KrakenListModel *model)
{
	return (g_sequence_get_length (model->details->files) == 0);
}

guint
kraken_list_model_get_length (KrakenListModel *model)
{
	return g_sequence_get_length (model->details->files);
}

static void
kraken_list_model_remove (KrakenListModel *model, GtkTreeIter *iter)
{
	...
	if (file_entry->files != NULL) {
		while (g_sequence_get_length (file_entry->files) > 0) {
			...
			if (child_file_entry->file != NULL) {
				kraken_list_model_remove_file (model,
	...
	if (parent_file_entry && g_sequence_get_length (parent_file_entry->files) == 1 &&
        ...
        got_count = kraken_file_get_directory_item_count (parent_file_entry->file, &count, &unreadable);
...
void
kraken_list_model_remove_file (KrakenListModel *model, KrakenFile *file,
			   KrakenDirectory *directory)
{
	GtkTreeIter iter;

	if (kraken_list_model_get_tree_iter_from_file (model, file, directory, &iter)) {
		kraken_list_model_remove (model, &iter);
	}
}

static void
kraken_list_model_clear_directory (KrakenListModel *model, GSequence *files)
{
	GtkTreeIter iter;
	FileEntry *file_entry;
	...
	while (g_sequence_get_length (files) > 0) {
		...
		file_entry = g_sequence_get (iter.user_data);
		if (file_entry->files != NULL) {
			kraken_list_model_clear_directory (model, file_entry->files);
		}
...
void
kraken_list_model_clear (KrakenListModel *model)
{
	g_return_if_fail (model != NULL);

	kraken_list_model_clear_directory (model, model->details->files);
}

KrakenFile *
kraken_list_model_file_for_path (KrakenListModel *model, GtkTreePath *path)
{
	KrakenFile *file;
	GtkTreeIter iter;
...
gboolean
kraken_list_model_load_subdirectory (KrakenListModel *model, GtkTreePath *path, KrakenDirectory **directory)
{
	GtkTreeIter iter;
	FileEntry *file_entry;
	KrakenDirectory *subdirectory;
	...
	subdirectory = kraken_directory_get_for_file (file_entry->file);

	if (g_hash_table_lookup (model->details->directory_reverse_map,
				 subdirectory) != NULL) {
		kraken_directory_unref (subdirectory);
	...
	/* Return a ref too */
	kraken_directory_ref (subdirectory);
...
void
kraken_list_model_unload_subdirectory (KrakenListModel *model, GtkTreeIter *iter)
{
	...
	while (g_sequence_get_length (file_entry->files) > 0) {
		...
		if (child_file_entry->file == NULL) {
			...
			kraken_list_model_ptr_to_iter (model, child_ptr, &child_iter);
			kraken_list_model_remove (model, &child_iter);
		}
	}
...
void
kraken_list_model_set_should_sort_directories_first (KrakenListModel *model, gboolean sort_directories_first)
{
...
void
kraken_list_model_set_should_sort_favorites_first (KrakenListModel *model, gboolean sort_favorites_first)
{
...
int
kraken_list_model_get_sort_column_id_from_attribute (KrakenListModel *model,
						       GQuark attribute)
{
	guint i;
	...
	for (i = 0; i < model->details->columns->len; i++) {
		...
		column =
			KRAKEN_COLUMN (model->details->columns->pdata[i]);
		g_object_get (G_OBJECT (column),
			      "attribute_q", &column_attribute,
			      NULL);
		if (column_attribute == attribute) {
			return KRAKEN_LIST_MODEL_NUM_COLUMNS + i;
		}
	}
...
GQuark
kraken_list_model_get_attribute_from_sort_column_id (KrakenListModel *model,
						       int sort_column_id)
{
	KrakenColumn *column;
	int index;
	...
	index = sort_column_id - KRAKEN_LIST_MODEL_NUM_COLUMNS;
	...
	column = KRAKEN_COLUMN (model->details->columns->pdata[index]);
...
KrakenZoomLevel
kraken_list_model_get_zoom_level_from_column_id (int column)
{
	switch (column) {
	case KRAKEN_LIST_MODEL_SMALLEST_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_SMALLEST;
	case KRAKEN_LIST_MODEL_SMALLER_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_SMALLER;
	case KRAKEN_LIST_MODEL_SMALL_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_SMALL;
	case KRAKEN_LIST_MODEL_STANDARD_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_STANDARD;
	case KRAKEN_LIST_MODEL_LARGE_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_LARGE;
	case KRAKEN_LIST_MODEL_LARGER_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_LARGER;
	case KRAKEN_LIST_MODEL_LARGEST_ICON_COLUMN:
		return KRAKEN_ZOOM_LEVEL_LARGEST;
        default:
                break;
	}

	g_return_val_if_reached (KRAKEN_ZOOM_LEVEL_STANDARD);
}

int
kraken_list_model_get_column_id_from_zoom_level (KrakenZoomLevel zoom_level)
{
	switch (zoom_level) {
	case KRAKEN_ZOOM_LEVEL_SMALLEST:
		return KRAKEN_LIST_MODEL_SMALLEST_ICON_COLUMN;
	case KRAKEN_ZOOM_LEVEL_SMALLER:
		return KRAKEN_LIST_MODEL_SMALLER_ICON_COLUMN;
	case KRAKEN_ZOOM_LEVEL_SMALL:
		return KRAKEN_LIST_MODEL_SMALL_ICON_COLUMN;
	case KRAKEN_ZOOM_LEVEL_STANDARD:
		return KRAKEN_LIST_MODEL_STANDARD_ICON_COLUMN;
	case KRAKEN_ZOOM_LEVEL_LARGE:
		return KRAKEN_LIST_MODEL_LARGE_ICON_COLUMN;
	case KRAKEN_ZOOM_LEVEL_LARGER:
		return KRAKEN_LIST_MODEL_LARGER_ICON_COLUMN;
	case KRAKEN_ZOOM_LEVEL_LARGEST:
		return KRAKEN_LIST_MODEL_LARGEST_ICON_COLUMN;
        case KRAKEN_ZOOM_LEVEL_NULL:
    default:
        g_return_val_if_reached (KRAKEN_LIST_MODEL_STANDARD_ICON_COLUMN);
	}
}

void
kraken_list_model_set_drag_view (KrakenListModel *model,
				   GtkTreeView *view,
				   int drag_begin_x,
				   int drag_begin_y)
{
...
GtkTargetList *
kraken_list_model_get_drag_target_list (void)
{
	GtkTargetList *target_list;

	target_list = gtk_target_list_new (drag_types, G_N_ELEMENTS (drag_types));
	gtk_target_list_add_text_targets (target_list, KRAKEN_ICON_DND_TEXT);

	return target_list;
}

int
kraken_list_model_add_column (KrakenListModel *model,
				KrakenColumn *column)
{
	g_ptr_array_add (model->details->columns, column);
	g_object_ref (column);

	return KRAKEN_LIST_MODEL_NUM_COLUMNS + (model->details->columns->len - 1);
}

int
kraken_list_model_get_column_number (KrakenListModel *model,
...
static void
kraken_list_model_dispose (GObject *object)
{
	KrakenListModel *model;
	guint i;

	model = KRAKEN_LIST_MODEL (object);
	...
	G_OBJECT_CLASS (kraken_list_model_parent_class)->dispose (object);
}

static void
kraken_list_model_finalize (GObject *object)
{
	KrakenListModel *model;
	...
	if (model->details->highlight_files != NULL) {
		kraken_file_list_free (model->details->highlight_files);
		model->details->highlight_files = NULL;
	}

	g_free (model->details);

	G_OBJECT_CLASS (kraken_list_model_parent_class)->finalize (object);
}

static void
kraken_list_model_init (KrakenListModel *model)
{
	model->details = g_new0 (KrakenListModelDetails, 1);
...
static void
kraken_list_model_class_init (KrakenListModelClass *klass)
{
	...
	object_class->finalize = kraken_list_model_finalize;
	object_class->dispose = kraken_list_model_dispose;

      list_model_signals[SUBDIRECTORY_UNLOADED] =
        g_signal_new ("subdirectory_unloaded",
                      KRAKEN_TYPE_LIST_MODEL,
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (KrakenListModelClass, subdirectory_unloaded),
                      NULL, NULL,
...
static void
kraken_list_model_tree_model_init (GtkTreeModelIface *iface)
{
	iface->get_flags = kraken_list_model_get_flags;
	iface->get_n_columns = kraken_list_model_get_n_columns;
	iface->get_column_type = kraken_list_model_get_column_type;
	iface->get_iter = kraken_list_model_get_iter;
	iface->get_path = kraken_list_model_get_path;
	iface->get_value = kraken_list_model_get_value;
	iface->iter_next = kraken_list_model_iter_next;
	iface->iter_children = kraken_list_model_iter_children;
	iface->iter_has_child = kraken_list_model_iter_has_child;
	iface->iter_n_children = kraken_list_model_iter_n_children;
	iface->iter_nth_child = kraken_list_model_iter_nth_child;
	iface->iter_parent = kraken_list_model_iter_parent;
}

static void
kraken_list_model_sortable_init (GtkTreeSortableIface *iface)
{
	iface->get_sort_column_id = kraken_list_model_get_sort_column_id;
	iface->set_sort_column_id = kraken_list_model_set_sort_column_id;
	iface->has_default_sort_func = kraken_list_model_has_default_sort_func;
}

static void
kraken_list_model_multi_drag_source_init (EggTreeMultiDragSourceIface *iface)
{
	iface->row_draggable = kraken_list_model_multi_row_draggable;
	iface->drag_data_get = kraken_list_model_multi_drag_data_get;
	iface->drag_data_delete = kraken_list_model_multi_drag_data_delete;
}

void
kraken_list_model_subdirectory_done_loading (KrakenListModel *model, KrakenDirectory *directory)
{
	GtkTreeIter iter;
	GtkTreePath *path;
             ...
             gpointer user_data)
{
	...
	iters = kraken_list_model_get_all_iters_for_file (model, file);
...
void
kraken_list_model_set_highlight_for_files (KrakenListModel *model,
					     GList *files)
{
	if (model->details->highlight_files != NULL) {
	...
	if (files != NULL) {
		model->details->highlight_files = kraken_file_list_copy (files);
		g_list_foreach (model->details->highlight_files,
		                refresh_row, model);
...
void
kraken_list_model_set_temporarily_disable_sort (KrakenListModel *model, gboolean disable)
{
...
gboolean
kraken_list_model_get_temporarily_disable_sort (KrakenListModel *model)
{
    return model->details->temp_unsorted;
}

void
kraken_list_model_set_expanding (KrakenListModel *model, KrakenDirectory *directory)
{
    FileEntry *entry;
    GSequenceIter *ptr;
    ...
    if (!ptr) {
        g_warning ("kraken_list_model_set_expanding: No pointer found, something's wrong?");
        return;
    }
...
void
kraken_list_model_set_view_directory (KrakenListModel *model, KrakenDirectory *dir)
{
