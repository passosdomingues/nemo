...
struct _KrakenNavigationStateDetails {
...
G_DEFINE_TYPE (KrakenNavigationState, kraken_navigation_state, G_TYPE_OBJECT);

static void
clear_bindings (KrakenNavigationState *self)
{
...
static void
update_bindings (KrakenNavigationState *self)
{
	gint length, idx;
	GBinding *binding;
...
static void
kraken_navigation_state_init (KrakenNavigationState *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, KRAKEN_TYPE_NAVIGATION_STATE,
						  KrakenNavigationStateDetails);
}

static void
kraken_navigation_state_set_property (GObject *object,
					guint property_id,
					const GValue *value,
					GParamSpec *pspec)
{
	KrakenNavigationState *self = KRAKEN_NAVIGATION_STATE (object);

	switch (property_id) {
	case PROP_SLAVE:
		self->priv->slave = g_value_dup_object (value);
		kraken_navigation_state_add_group (self, g_value_get_object (value));
		break;
	case PROP_MASTER:
...
static void
kraken_navigation_state_finalize (GObject *obj)
{
	KrakenNavigationState *self = KRAKEN_NAVIGATION_STATE (obj);

	g_strfreev (self->priv->action_names);

	G_OBJECT_CLASS (kraken_navigation_state_parent_class)->finalize (obj);	
}

static void
kraken_navigation_state_dispose (GObject *obj)
{
	KrakenNavigationState *self = KRAKEN_NAVIGATION_STATE (obj);
	...
	G_OBJECT_CLASS (kraken_navigation_state_parent_class)->dispose (obj);
}

static void
kraken_navigation_state_class_init (KrakenNavigationStateClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->dispose = kraken_navigation_state_dispose;
	oclass->finalize = kraken_navigation_state_finalize;
	oclass->set_property = kraken_navigation_state_set_property;
	...
	g_type_class_add_private (klass, sizeof (KrakenNavigationStateDetails));
}

KrakenNavigationState *
kraken_navigation_state_new (GtkActionGroup *slave,
			       const gchar **action_names)
{
	return g_object_new (KRAKEN_TYPE_NAVIGATION_STATE,
			     "slave", slave,
...
void
kraken_navigation_state_set_master (KrakenNavigationState *self,
				      GtkActionGroup *master)
{
...
void
kraken_navigation_state_add_group (KrakenNavigationState *self,
...
void
kraken_navigation_state_sync_all (KrakenNavigationState *self)
{
	GList *l;
	gint length, idx;
...
GtkActionGroup *
kraken_navigation_state_get_master (KrakenNavigationState *self)
{
	return self->priv->master;
}

void
kraken_navigation_state_set_boolean (KrakenNavigationState *self,
				       const gchar *action_name,
				       gboolean value)
{
	GtkAction *action;
