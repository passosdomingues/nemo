...
/* kraken-main.c: Implementation of the routines that drive program lifecycle and main window creation/destruction. */
...
main (int argc, char *argv[])
{
	...
	/* Kraken uses lots and lots of small and medium size allocations,
	 ...
	 * be mmaped. Unfortunately this triggers quickly for kraken when
	...
	g_set_prgname ("kraken");

#ifdef HAVE_EXEMPI
	xmp_init();
#endif

	/* Run the kraken application. */
	application = kraken_main_application_get_singleton ();

    /* hold indefinitely if we're asked to persist */
    if (g_getenv ("KRAKEN_PERSIST") != NULL) {
