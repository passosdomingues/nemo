...
struct KrakenBlankDesktopWindowDetails {
    gint monitor;
    GtkWidget *event_box;
...
G_DEFINE_TYPE (KrakenBlankDesktopWindow, kraken_blank_desktop_window, 
               GTK_TYPE_WINDOW);

static void update_actions_visibility (KrakenBlankDesktopWindow *window);
static void update_actions_menu (KrakenBlankDesktopWindow *window);
static void actions_changed (gpointer user_data);

static void
action_show_overlay (GtkAction *action, gpointer user_data)
{
    g_return_if_fail (KRAKEN_IS_BLANK_DESKTOP_WINDOW (user_data));
    KrakenBlankDesktopWindow *window = KRAKEN_BLANK_DESKTOP_WINDOW (user_data);
...
static void
reset_popup_menu (KrakenBlankDesktopWindow *window)
{
...
static void
do_popup_menu (KrakenBlankDesktopWindow *window, GdkEventButton *event)
{
...
static gboolean
on_popup_menu (GtkWidget *widget, KrakenBlankDesktopWindow *window)
{
...
static gboolean
on_button_press (GtkWidget *widget, GdkEventButton *event, KrakenBlankDesktopWindow *window)
{
...
static void
run_action_callback (KrakenAction *action, gpointer user_data)
{
    kraken_action_activate (action, NULL, NULL, GTK_WINDOW (user_data));
}

static void
update_actions_visibility (KrakenBlankDesktopWindow *window)
{
    kraken_action_manager_update_action_states (window->details->action_manager,
...
static void
clear_ui (KrakenBlankDesktopWindow *window)
{
...
static void
update_actions_menu (KrakenBlankDesktopWindow *window)
{
    ...
    window->details->action_group = gtk_action_group_new ("KrakenBlankDesktopActions");
    ...
    window->details->actions_merge_id =
            gtk_ui_manager_add_ui_from_resource (window->details->ui_manager, "/org/kraken/kraken-blank-desktop-window-ui.xml", NULL);

    kraken_action_manager_iterate_actions (window->details->action_manager,
...
static void
kraken_blank_desktop_window_dispose (GObject *obj)
{
    KrakenBlankDesktopWindow *window = KRAKEN_BLANK_DESKTOP_WINDOW (obj);
    ...
    g_signal_handlers_disconnect_by_func (kraken_menu_config_preferences, reset_popup_menu, window);

    G_OBJECT_CLASS (kraken_blank_desktop_window_parent_class)->dispose (obj);
}

static void
kraken_blank_desktop_window_finalize (GObject *obj)
{
    KrakenBlankDesktopWindow *window = KRAKEN_BLANK_DESKTOP_WINDOW (obj);

    g_object_unref (window->details->ui_manager);

    G_OBJECT_CLASS (kraken_blank_desktop_window_parent_class)->finalize (obj);
}
...
{
    KrakenBlankDesktopWindow *window;

    window = KRAKEN_BLANK_DESKTOP_WINDOW (widget);

    GTK_WIDGET_CLASS (kraken_blank_desktop_window_parent_class)->show (widget);
    gtk_ui_manager_ensure_update (window->details->ui_manager);
    actions_changed (window);
}

static void
kraken_blank_desktop_window_constructed (GObject *obj)
{
	AtkObject *accessible;
    KrakenApplication *application;
	KrakenBlankDesktopWindow *window = KRAKEN_BLANK_DESKTOP_WINDOW (obj);

	G_OBJECT_CLASS (kraken_blank_desktop_window_parent_class)->constructed (obj);
    ...
    window->details->action_manager = kraken_action_manager_new ();
    ...
    kraken_blank_desktop_window_update_geometry (window);
...
static void
kraken_blank_desktop_window_get_property (GObject *object,
                                           guint property_id,
                                         GValue *value,
                                     GParamSpec *pspec)
{
    KrakenBlankDesktopWindow *window = KRAKEN_BLANK_DESKTOP_WINDOW (object);
...
static void
kraken_blank_desktop_window_set_property (GObject *object,
                                           guint property_id,
                                   const GValue *value,
                                     GParamSpec *pspec)
{
...
static void
kraken_blank_desktop_window_init (KrakenBlankDesktopWindow *window)
{
	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, KRAKEN_TYPE_BLANK_DESKTOP_WINDOW,
						       KrakenBlankDesktopWindowDetails);
...
kraken_blank_desktop_window_new (gint monitor)
{
	KrakenBlankDesktopWindow *window;

	window = g_object_new (KRAKEN_TYPE_BLANK_DESKTOP_WINDOW,
                           "monitor", monitor,
                           NULL);
...
static gboolean
kraken_blank_desktop_window_delete_event (GtkWidget *widget,
                                        GdkEventAny *event)
{
...
map (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (kraken_blank_desktop_window_parent_class)->map (widget);
...
{
	GTK_WIDGET_CLASS (kraken_blank_desktop_window_parent_class)->unrealize (widget);
}
...
realize (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (kraken_blank_desktop_window_parent_class)->realize (widget);
}

static void
kraken_blank_desktop_window_class_init (KrakenBlankDesktopWindowClass *klass)
{
	GtkWidgetClass *wclass = GTK_WIDGET_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->constructed = kraken_blank_desktop_window_constructed;
    oclass->dispose = kraken_blank_desktop_window_dispose;
	oclass->finalize = kraken_blank_desktop_window_finalize;
    oclass->set_property = kraken_blank_desktop_window_set_property;
    oclass->get_property = kraken_blank_desktop_window_get_property;

	wclass->realize = realize;
	wclass->unrealize = unrealize;
	wclass->map = map;
    wclass->show = show;
	wclass->delete_event = kraken_blank_desktop_window_delete_event;
	...
	g_type_class_add_private (klass, sizeof (KrakenBlankDesktopWindowDetails));

    g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
}

void
kraken_blank_desktop_window_update_geometry (KrakenBlankDesktopWindow *window)
{
    GdkRectangle rect;
    ...
    DEBUG ("KrakenBlankDesktopWindow monitor:%d: x:%d, y:%d, w:%d, h:%d",
           window->details->monitor,
           rect.x, rect.y,
