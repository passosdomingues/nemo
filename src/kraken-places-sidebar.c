...
#include <libkraken-private/kraken-cell-renderer-disk.h>
#include <libkraken-private/kraken-places-tree-view.h>
...
#define DEBUG_FLAG KRAKEN_DEBUG_PLACES
...
typedef struct {
	...
	gboolean mounting;
	KrakenWindowSlot *go_to_after_mount_slot;
	KrakenWindowOpenFlags go_to_after_mount_flags;
...
} KrakenPlacesSidebarClass;

typedef struct {
        GObject parent;
} KrakenPlacesSidebarProvider;

typedef struct {
        GObjectClass parent;
} KrakenPlacesSidebarProviderClass;
...
static void  open_selected_bookmark                    (KrakenPlacesSidebar        *sidebar,
							GtkTreeModel                 *model,
							GtkTreeIter                  *iter,
							KrakenWindowOpenFlags flags);
static void  kraken_places_sidebar_style_set         (GtkWidget                    *widget,
							GtkStyle                     *previous_style);
static gboolean eject_or_unmount_bookmark              (KrakenPlacesSidebar *sidebar,
							GtkTreePath *path);
static gboolean eject_or_unmount_selection             (KrakenPlacesSidebar *sidebar);
...
static void update_places                              (KrakenPlacesSidebar *sidebar);
static void update_places_on_idle                      (KrakenPlacesSidebar *sidebar);
static void rebuild_menu                               (KrakenPlacesSidebar *sidebar);
...
static const GtkTargetEntry kraken_shortcuts_source_targets[] = {
	{ (char *)"GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, GTK_TREE_MODEL_ROW }
};

/* Target types for dropping into the shortcuts list */
static const GtkTargetEntry kraken_shortcuts_drop_targets [] = {
	{ (char *)"GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, GTK_TREE_MODEL_ROW },
...
typedef struct {
  ...
  KrakenPlacesSidebar *sidebar;
} KrakenShortcutsModel;

typedef struct {
  GtkTreeStoreClass parent_class;
} KrakenShortcutsModelClass;

#define KRAKEN_TYPE_SHORTCUTS_MODEL (_kraken_shortcuts_model_get_type ())
#define KRAKEN_SHORTCUTS_MODEL(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), KRAKEN_SHORTCUTS_MODEL_TYPE, KrakenShortcutsModel))

GType _kraken_shortcuts_model_get_type (void);
static void _kraken_shortcuts_model_drag_source_init (GtkTreeDragSourceIface *iface);
G_DEFINE_TYPE_WITH_CODE (KrakenShortcutsModel, _kraken_shortcuts_model, GTK_TYPE_TREE_STORE,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_DRAG_SOURCE,
						_kraken_shortcuts_model_drag_source_init));
static GtkTreeStore *kraken_shortcuts_model_new (KrakenPlacesSidebar *sidebar);

G_DEFINE_TYPE (KrakenPlacesSidebar, kraken_places_sidebar, GTK_TYPE_SCROLLED_WINDOW);

static void
breakpoint_changed_cb (KrakenPlacesSidebar *sidebar)
{
    sidebar->bookmark_breakpoint = g_settings_get_int (kraken_window_state, KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT);
    update_places (sidebar);
}

static void
increment_bookmark_breakpoint (KrakenPlacesSidebar *sidebar)
{
    g_signal_handlers_block_by_func (kraken_window_state, breakpoint_changed_cb, sidebar);

    sidebar->bookmark_breakpoint ++;
    g_settings_set_int (kraken_window_state, KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT, sidebar->bookmark_breakpoint);

    g_signal_handlers_unblock_by_func (kraken_window_state, breakpoint_changed_cb, sidebar);
}

static void
decrement_bookmark_breakpoint (KrakenPlacesSidebar *sidebar)
{
    g_signal_handlers_block_by_func (kraken_window_state, breakpoint_changed_cb, sidebar);
...
{
	return g_settings_get_boolean (kraken_desktop_preferences, KRAKEN_PREFERENCES_SHOW_DESKTOP) &&
	       !g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_DESKTOP_IS_HOME_DIR);
}

static gboolean
is_built_in_bookmark (KrakenFile *file)
{
	if (kraken_file_is_home (file)) {
		return TRUE;
	}

    if (kraken_file_is_desktop_directory (file) && should_show_desktop ())
        return TRUE;
    else
	    return FALSE;
}
...
static GtkTreeIter
add_heading (KrakenPlacesSidebar *sidebar,
...
static GtkTreeIter
check_heading_for_devices (KrakenPlacesSidebar *sidebar,
                           SectionType section_type,
			               GtkTreeIter cat_iter)
{
...
static GtkTreeIter
add_place (KrakenPlacesSidebar *sidebar,
	   PlaceType place_type,
	   SectionType section_type,
	   const char *name,
                              ...
                              gpointer user_data)
{
    PlaceType place_type;
    SectionType section_type;
    KrakenPlacesSidebar *sidebar = KRAKEN_PLACES_SIDEBAR (user_data);
...
static void
restore_expand_state (KrakenPlacesSidebar *sidebar)
{
...
static void expand_or_collapse_category (KrakenPlacesSidebar *sidebar,
...
static void
sidebar_update_restore_selection (KrakenPlacesSidebar *sidebar,
				  const gchar *location,
				  const gchar *last_uri)
{
...
static void
update_places (KrakenPlacesSidebar *sidebar)
{
	...
	slot = kraken_window_get_active_slot (sidebar->window);
	location = kraken_window_slot_get_current_uri (slot);
    ...
    if (should_show_desktop ()) {
        /* desktop */
        desktop_path = kraken_get_desktop_directory ();
    ...
    /* add bookmarks */
    bookmark_count = kraken_bookmark_list_length (sidebar->bookmarks);
    ...
    for (bookmark_index = 0; bookmark_index < temp_breakpoint; ++bookmark_index) {
        bookmark = kraken_bookmark_list_item_at (sidebar->bookmarks, bookmark_index);

        root = kraken_bookmark_get_location (bookmark);

        bookmark_name = kraken_bookmark_get_name (bookmark);
        icon = kraken_bookmark_get_icon_name (bookmark);
    ...
    /* Tags virtual folder */
    mount_uri = (char *)"kraken://tags/";
    icon = KRAKEN_ICON_SYMBOLIC_TAGS;
    cat_iter = add_place (sidebar, PLACES_BUILT_IN,
                          SECTION_COMPUTER,
                          _("Tags"), icon, mount_uri,
                          NULL, NULL, NULL, 0,
                          _("Virtual tag folders"), 0, FALSE, cat_iter);
    ...
    icon = KRAKEN_ICON_SYMBOLIC_FILESYSTEM;
    ...
    if (eel_vfs_supports_uri_scheme("trash")) {
        ...
        icon = kraken_trash_monitor_get_symbolic_icon_name ();
        cat_iter = add_place (sidebar, PLACES_BUILT_IN,
                               SECTION_COMPUTER,
        ...
        if (volumes != NULL) {
            for (ll = volumes; ll != NULL; ll = ll->next) {
                ...
                if (mount != NULL) {
                    ...
                    icon = kraken_get_mount_icon_name (mount);
                    root = g_mount_get_default_location (mount);
                     ...
                     */
                    gchar *volume_id;
                    icon = kraken_get_volume_icon_name (volume);
            ...
            if (g_drive_is_removable (drive) && !g_drive_is_media_check_automatic (drive)) {
                 ...
                 */
                icon = kraken_get_drive_icon_name (drive);
	...
	icon = KRAKEN_ICON_SYMBOLIC_NETWORK;
	cat_iter = add_place (sidebar, PLACES_BUILT_IN,
                		   SECTION_NETWORK,
                		   _("Network"), icon,
                		   mount_uri, NULL, NULL, NULL, 0,
...
static gboolean
over_eject_button (KrakenPlacesSidebar *sidebar,
		   gint x,
		   gint y,
		   GtkTreePath **path)
{
...
static gboolean
clicked_eject_button (KrakenPlacesSidebar *sidebar,
		      GtkTreePath **path)
{
	GdkEvent *event;
...
desktop_setting_changed_callback (gpointer user_data)
{
	...
	sidebar = KRAKEN_PLACES_SIDEBAR (user_data);

	update_places (sidebar);
}

static void
loading_uri_callback (KrakenWindow *window,
                            char *location,
               KrakenPlacesSidebar *sidebar)
{
    GtkTreeSelection *selection;
...
static gboolean
cat_is_expanded (KrakenPlacesSidebar *sidebar,
                 SectionType section_type)
{
    switch (section_type) {
        case SECTION_COMPUTER:
...
get_drag_type (KrakenPlacesSidebar *sidebar,
		       ...
		       KrakenPlacesSidebar *sidebar)
{
	...
	if (!cat_is_expanded (sidebar, section_type) && place_type == PLACES_HEADING) {
        ...
        if (section_type == SECTION_BOOKMARKS &&
            (int)kraken_bookmark_list_length (sidebar->bookmarks) == sidebar->bookmark_breakpoint) {
...
static void
free_drag_data (KrakenPlacesSidebar *sidebar)
{
	sidebar->drag_data_received = FALSE;

	if (sidebar->drag_list) {
		kraken_drag_destroy_selection_list (sidebar->drag_list);
...
static gboolean
can_accept_file_as_bookmark (KrakenFile *file)
{
	return (kraken_file_is_directory (file) &&
...
can_accept_items_as_bookmarks (const GList *items)
{
	...
	for (max = 100; items != NULL && max >= 0; items = items->next, max--) {
		uri = ((KrakenDragSelectionItem *)items->data)->uri;
	...
	if (pos == GTK_TREE_VIEW_DROP_BEFORE ||
		...
		if (sidebar->drag_list == NULL) {
			...
			gtk_tree_model_get (GTK_TREE_MODEL (sidebar->store_filter),
					    ...
					    PLACES_SIDEBAR_COLUMN_URI, &uri,
					    -1);
            kraken_drag_default_drop_action_for_icons (context,
                                                     uri,
                                                     sidebar->drag_list,
                                                     &action,
...
static void
bookmarks_drop_uris (KrakenPlacesSidebar *sidebar,
                          ...
                          SectionType   section_type)
{
	...
	for (i = 0; uris[i]; i++) {
		...
		uri = kraken_file_get_drop_target_uri (file);
		location = g_file_new_for_uri (uri);
		kraken_file_unref (file);

		bookmark = kraken_bookmark_new (location, NULL, NULL, NULL);

		if (!kraken_bookmark_list_contains (sidebar->bookmarks, bookmark)) {
			...
			kraken_bookmark_list_insert_item (sidebar->bookmarks, bookmark, position++);
		}
...
build_selection_list (const char *data)
{
	...
	for (i = 0; uris[i]; i++) {
		uri = uris[i];
		item = kraken_drag_selection_item_new ();
...
static gboolean
get_selected_iter (KrakenPlacesSidebar *sidebar,
		   GtkTreeIter *iter)
{
	GtkTreeSelection *selection;
...
static void
update_bookmark_breakpoint (KrakenPlacesSidebar *sidebar,
...
static void
reorder_bookmarks (KrakenPlacesSidebar *sidebar,
                                 int  new_position,
                         SectionType  new_section_type)
{
	...
	if (type != PLACES_BOOKMARK ||
	    old_position < 0 ||
	    old_position >= (int)kraken_bookmark_list_length (sidebar->bookmarks)) {
		return;
	}

    update_bookmark_breakpoint (sidebar, old_section_type, new_section_type);

	kraken_bookmark_list_move_item (sidebar->bookmarks, old_position,
					  new_position);
			     ...
			     KrakenPlacesSidebar *sidebar)
{
	...
	if (tree_pos == GTK_TREE_VIEW_DROP_BEFORE ||
        ...
        if (section_type == SECTION_COMPUTER && tree_pos == GTK_TREE_VIEW_DROP_BEFORE) {
            position = kraken_bookmark_list_length(sidebar->bookmarks);
		...
		if (real_action == GDK_ACTION_ASK) {
			real_action =
				kraken_drag_drop_action_ask (GTK_WIDGET (tree_view),
			...
			case TEXT_URI_LIST:
                ...
                if (g_strcmp0 (drop_uri, "favorites:///") == 0) {
                    for (l = uris; l != NULL; l = l->next) {
                        ...
                        KrakenFile *source_file = kraken_file_get_by_uri (uri);
                        kraken_file_set_is_favorite (source_file, TRUE);
                        kraken_file_unref (source_file);
                    }
                } else {
                    kraken_file_operations_copy_move (uris, NULL, drop_uri,
                                        real_action, GTK_WIDGET (tree_view),
                                        NULL, NULL);
                }

				kraken_drag_destroy_selection_list (selection_list);
				g_list_free (uris);
				success = TRUE;
				break;
			case GTK_TREE_MODEL_ROW:
...
static void
update_menu_states (KrakenPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	PlaceType type;
	GDrive *drive = NULL;
	...
	GMount *mount = NULL;
	GFile *location;
	KrakenDirectory *directory = NULL;
	gboolean show_mount;
	gboolean show_unmount;
	gboolean show_eject;
	gboolean show_rescan;
    ...
    if (uri) {
        KrakenFile *file = kraken_file_get_by_uri (uri);
        KrakenFile *parent = kraken_file_get_parent (file);

        GList *selection = g_list_prepend (NULL, file);

        kraken_action_manager_update_action_states (sidebar->action_manager,
    ...
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_ADD_BOOKMARK, (type == PLACES_MOUNTED_VOLUME));
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_SIDEBAR_REMOVE, (type == PLACES_BOOKMARK));
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_RENAME, (type == PLACES_BOOKMARK));
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_EMPTY_TRASH_CONDITIONAL, !kraken_trash_monitor_is_empty ());
	...
	if (mount != NULL) {
		...
		show_properties = kraken_directory_is_local (directory);

		kraken_directory_unref (directory);
		g_object_unref (location);
	}

    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_MOUNT_VOLUME, show_mount);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_UNMOUNT_VOLUME, show_unmount);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_EJECT_VOLUME, show_eject);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_SIDEBAR_DETECT_MEDIA, show_rescan);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_START_VOLUME, show_start);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_STOP_VOLUME, show_stop);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_EMPTY_TRASH_CONDITIONAL, show_empty_trash);
    set_action_visible (sidebar->bookmark_action_group, KRAKEN_ACTION_PROPERTIES, show_properties);

	/* Adjust start/stop items to reflect the type of the drive */
    GtkAction *start_action, *stop_action;

    start_action = gtk_action_group_get_action (sidebar->bookmark_action_group, KRAKEN_ACTION_START_VOLUME);
    stop_action = gtk_action_group_get_action (sidebar->bookmark_action_group, KRAKEN_ACTION_STOP_VOLUME);
		   ...
		   GObject *user_data)
{
	...
	if (mount != NULL) {
		...
		if (sidebar->go_to_after_mount_slot != NULL) {
			if ((sidebar->go_to_after_mount_flags & KRAKEN_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0) {
				kraken_window_slot_open_location (sidebar->go_to_after_mount_slot, location,
								    sidebar->go_to_after_mount_flags);
			} else {
				KrakenWindow *new, *cur;

				cur = KRAKEN_WINDOW (sidebar->window);
				new = kraken_application_create_window (kraken_application_get_singleton (),
									  gtk_window_get_screen (GTK_WINDOW (cur)));
				kraken_window_go_to (new, location);
			}
		}
...
static void
open_selected_bookmark (KrakenPlacesSidebar *sidebar,
			GtkTreeModel	      *model,
			GtkTreeIter	      *iter,
			KrakenWindowOpenFlags	      flags)
{
	...
	if (uri != NULL) {
		...
		if ((flags & KRAKEN_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0) {
			slot = kraken_window_get_active_slot (sidebar->window);
			kraken_window_slot_open_location (slot, location, flags);
		} else {
			KrakenWindow *cur, *new;
		...
		if (volume != NULL && !sidebar->mounting) {
			...
			slt = kraken_window_get_active_slot (sidebar->window);
			sidebar->go_to_after_mount_slot = slt;
			g_object_add_weak_pointer (G_OBJECT (sidebar->go_to_after_mount_slot),
						   (gpointer *) &sidebar->go_to_after_mount_slot);

			sidebar->go_to_after_mount_flags = flags | KRAKEN_WINDOW_OPEN_FLAG_MOUNT;

			kraken_file_operations_mount_volume_full (NULL, volume,
								    volume_mounted_cb,
								    G_OBJECT (sidebar));
...
static void
open_shortcut_from_menu (KrakenPlacesSidebar *sidebar,
			 KrakenWindowOpenFlags	       flags)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
open_shortcut_in_new_window_cb (GtkAction	      *item,
				KrakenPlacesSidebar *sidebar)
{
	open_shortcut_from_menu (sidebar, KRAKEN_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
...
open_shortcut_in_new_tab_cb (GtkAction	      *item,
				KrakenPlacesSidebar *sidebar)
{
	open_shortcut_from_menu (sidebar, KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB);
}

/* Add bookmark for the selected item */
static void
add_bookmark (KrakenPlacesSidebar *sidebar)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	char *uri;
	...
	if (get_selected_iter (sidebar, &iter)) {
		...
		if (!kraken_bookmark_list_contains (sidebar->bookmarks, bookmark)) {
			kraken_bookmark_list_append (sidebar->bookmarks, bookmark);
		}
...
static void
rename_selected_bookmark (KrakenPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	GtkTreeViewColumn *column;
...
static void
remove_selected_bookmarks (KrakenPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	PlaceType type;
	int index;
    ...
    kraken_bookmark_list_delete_item_at (sidebar->bookmarks, index);
}
		   ...
		   KrakenPlacesSidebar *sidebar)
{
	...
	if (volume != NULL) {
		kraken_file_operations_mount_volume (NULL, volume);
		g_object_unref (volume);
	}
}
...
show_unmount_progress_cb (GMountOperation *op,
                              const gchar *message,
                                  ...
                                  gpointer user_data)
{
    KrakenApplication *app = KRAKEN_APPLICATION (g_application_get_default ());

    if (bytes_left == 0) {
        kraken_application_notify_unmount_done (app, message);
    } else {
        kraken_application_notify_unmount_show (app, message);
    }
}
...
{
    KrakenApplication *app = KRAKEN_APPLICATION (g_application_get_default ());
    kraken_application_notify_unmount_done (app, NULL);
}

static GMountOperation *
get_unmount_operation (KrakenPlacesSidebar *sidebar)
{
    GMountOperation *mount_op;
	    ...
	    KrakenPlacesSidebar *sidebar)
{
	...
	if (mount != NULL) {
        ...
        kraken_file_operations_unmount_mount_full (NULL, mount, mount_op, FALSE, TRUE,
                                                 unmount_done,
...
static void
do_unmount_selection (KrakenPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	GMount *mount;
...
static gboolean
eject_or_unmount_bookmark (KrakenPlacesSidebar *sidebar,
			   GtkTreePath *path)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
static gboolean
eject_or_unmount_selection (KrakenPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	gboolean ret;
...
empty_trash_cb (GtkAction           *item,
		KrakenPlacesSidebar *sidebar)
{
	kraken_file_operations_empty_trash (GTK_WIDGET (sidebar->window));
}

static gboolean
find_prev_or_next_row (KrakenPlacesSidebar *sidebar,
		       GtkTreeIter *iter,
		       gboolean go_up)
{
...
static gboolean
find_prev_row (KrakenPlacesSidebar *sidebar, GtkTreeIter *iter)
{
	return find_prev_or_next_row (sidebar, iter, TRUE);
}

static gboolean
find_next_row (KrakenPlacesSidebar *sidebar, GtkTreeIter *iter)
{
	       ...
	       KrakenPlacesSidebar *sidebar)
{
	...
	if (uri != NULL) {
		...
		file = kraken_file_get_by_uri (uri);
		list = g_list_prepend (NULL, kraken_file_ref (file));

		kraken_properties_window_present (list, GTK_WIDGET (sidebar), NULL);
...
static gboolean
kraken_places_sidebar_focus (GtkWidget *widget,
			       GtkDirectionType direction)
{
	KrakenPlacesSidebar *sidebar = KRAKEN_PLACES_SIDEBAR (widget);
	GtkTreePath *path;
	...
	return GTK_WIDGET_CLASS (kraken_places_sidebar_parent_class)->focus (widget, direction);
}
...
run_action_callback (GtkAction *action, gpointer user_data)
{
    ...
    kraken_action_activate (KRAKEN_ACTION (action), selection, parent, GTK_WINDOW (sidebar->window));
...
static void
bookmarks_popup_menu (KrakenPlacesSidebar *sidebar,
...
static void
reset_menu (KrakenPlacesSidebar *sidebar)
{
...
static void
clear_ui (KrakenPlacesSidebar *sidebar)
{

    kraken_ui_unmerge_ui (sidebar->ui_manager,
...
static const GtkActionEntry bookmark_action_entries[] = {
    { KRAKEN_ACTION_OPEN,                    "xsi-folder-open-symbolic", N_("_Open"),                NULL, NULL, G_CALLBACK (open_shortcut_cb)               },
    { KRAKEN_ACTION_OPEN_IN_NEW_TAB,         NULL,                   N_("Open in New _Tab"),     NULL, NULL, G_CALLBACK (open_shortcut_in_new_tab_cb)    },
    { KRAKEN_ACTION_OPEN_ALTERNATE,          NULL,                   N_("Open in New _Window"),  NULL, NULL, G_CALLBACK (open_shortcut_in_new_window_cb) },
    { KRAKEN_ACTION_ADD_BOOKMARK,            NULL,                   N_("_Add Bookmark"),        NULL, NULL, G_CALLBACK (add_shortcut_cb)                },
    { KRAKEN_ACTION_SIDEBAR_REMOVE,          "xsi-list-remove-symbolic", N_("Remove"),               NULL, NULL, G_CALLBACK (remove_shortcut_cb)             },
    { KRAKEN_ACTION_RENAME,                  NULL,                   N_("_Rename..."),           NULL, NULL, G_CALLBACK (rename_shortcut_cb)             },
    { KRAKEN_ACTION_MOUNT_VOLUME,            NULL,                   N_("_Mount"),               NULL, NULL, G_CALLBACK (mount_shortcut_cb)              },
    { KRAKEN_ACTION_UNMOUNT_VOLUME,          NULL,                   N_("_Unmount"),             NULL, NULL, G_CALLBACK (unmount_shortcut_cb)            },
    { KRAKEN_ACTION_EJECT_VOLUME,            NULL,                   N_("_Eject"),               NULL, NULL, G_CALLBACK (eject_shortcut_cb)              },
    { KRAKEN_ACTION_SIDEBAR_DETECT_MEDIA,    NULL,                   N_("_Detect Media"),        NULL, NULL, G_CALLBACK (rescan_shortcut_cb)             },
    { KRAKEN_ACTION_START_VOLUME,            NULL,                   N_("_Start"),               NULL, NULL, G_CALLBACK (start_shortcut_cb)              },
    { KRAKEN_ACTION_STOP_VOLUME,             NULL,                   N_("_Stop"),                NULL, NULL, G_CALLBACK (stop_shortcut_cb)               },
    { KRAKEN_ACTION_EMPTY_TRASH_CONDITIONAL, NULL,                   N_("_Empty _Trash"),        NULL, NULL, G_CALLBACK (empty_trash_cb)                 },
    { KRAKEN_ACTION_PROPERTIES,              NULL,                   N_("_Properties"),          NULL, NULL, G_CALLBACK (properties_cb)                  },
};

static void
rebuild_menu (KrakenPlacesSidebar *sidebar)
{
    ...
    kraken_ui_prepare_merge_ui (sidebar->ui_manager,
                              "KrakenPlacesSidebarBookmarkActions",
                              &sidebar->bookmark_action_group_merge_id,
                              &sidebar->bookmark_action_group);

    kraken_ui_prepare_merge_ui (sidebar->ui_manager,
                              "KrakenPlacesSidebarActionActions",
                              &sidebar->action_action_group_merge_id,
                              &sidebar->action_action_group);

    sidebar->bookmark_action_group_merge_id =
            gtk_ui_manager_add_ui_from_resource (sidebar->ui_manager, "/org/kraken/kraken-places-sidebar-ui.xml", NULL);
    ...
    kraken_action_manager_iterate_actions (sidebar->action_manager,
                           ...
                           gboolean expanded)
{
    ...
    if (type == SECTION_COMPUTER) {
        ...
        g_settings_set_boolean (kraken_window_state, KRAKEN_WINDOW_STATE_MY_COMPUTER_EXPANDED, expanded);
    } else if (type == SECTION_BOOKMARKS) {
        sidebar->bookmarks_expanded = expanded;
        g_settings_set_boolean (kraken_window_state, KRAKEN_WINDOW_STATE_BOOKMARKS_EXPANDED, expanded);
    } else if (type == SECTION_DEVICES) {
        sidebar->devices_expanded = expanded;
        g_settings_set_boolean (kraken_window_state, KRAKEN_WINDOW_STATE_DEVICES_EXPANDED, expanded);
    } else if (type == SECTION_NETWORK) {
        sidebar->network_expanded = expanded;
        g_settings_set_boolean (kraken_window_state, KRAKEN_WINDOW_STATE_NETWORK_EXPANDED, expanded);
    }
}
		  ...
		  KrakenPlacesSidebar *sidebar)
{
	...
	gtk_tree_model_get (GTK_TREE_MODEL (sidebar->store_filter), &iter,
		            ...
		            -1);
	gtk_tree_path_free (path);
	bookmark = kraken_bookmark_list_item_at (sidebar->bookmarks, index);

	if (bookmark != NULL) {
		kraken_bookmark_set_custom_name (bookmark, new_text);
	}
}
...
static void
trash_state_changed_cb (KrakenTrashMonitor *trash_monitor,
			gboolean             state,
			gpointer             data)
{
	KrakenPlacesSidebar *sidebar;

	sidebar = KRAKEN_PLACES_SIDEBAR (data);
...
{
    KrakenPlacesSidebar *sidebar = KRAKEN_PLACES_SIDEBAR (data);
                         ...
                         gpointer      data)
{
    ...
    if ((int)kraken_bookmark_list_length (sidebar->bookmarks) > sidebar->bookmark_breakpoint)
        return TRUE;
...
static void
kraken_places_sidebar_init (KrakenPlacesSidebar *sidebar)
{
	GtkTreeView       *tree_view;
    ...
    sidebar->action_manager = kraken_action_manager_new ();
    ...
    sidebar->my_computer_expanded = g_settings_get_boolean (kraken_window_state,
                                                            KRAKEN_WINDOW_STATE_MY_COMPUTER_EXPANDED);
    sidebar->bookmarks_expanded = g_settings_get_boolean (kraken_window_state,
                                                             KRAKEN_WINDOW_STATE_BOOKMARKS_EXPANDED);
    sidebar->devices_expanded = g_settings_get_boolean (kraken_window_state,
                                                        KRAKEN_WINDOW_STATE_DEVICES_EXPANDED);
    sidebar->network_expanded = g_settings_get_boolean (kraken_window_state,
                                                        KRAKEN_WINDOW_STATE_NETWORK_EXPANDED);
	...
	gtk_style_context_add_class (style_context, "kraken-places-sidebar");

  	/* tree view */
	tree_view = GTK_TREE_VIEW (kraken_places_tree_view_new ());

	gtk_tree_view_set_headers_visible (tree_view, FALSE);

    primary_column = GTK_TREE_VIEW_COLUMN (gtk_tree_view_column_new ());
    gtk_tree_view_column_set_max_width (GTK_TREE_VIEW_COLUMN (primary_column), KRAKEN_ICON_SIZE_SMALLER);
    ...
    cell = sidebar->editable_renderer = kraken_cell_renderer_disk_new ();
    KRAKEN_CELL_RENDERER_DISK (cell)->direction = gtk_widget_get_direction (GTK_WIDGET (tree_view));
	...
	sidebar->store = kraken_shortcuts_model_new (sidebar);
	...
	gtk_tree_view_enable_model_drag_source (GTK_TREE_VIEW (tree_view),
						GDK_BUTTON1_MASK,
						kraken_shortcuts_source_targets,
						G_N_ELEMENTS (kraken_shortcuts_source_targets),
						GDK_ACTION_MOVE);
	gtk_drag_dest_set (GTK_WIDGET (tree_view),
			   0,
			   kraken_shortcuts_drop_targets, G_N_ELEMENTS (kraken_shortcuts_drop_targets),
	...
	g_signal_connect_swapped (kraken_desktop_preferences, "changed::" KRAKEN_PREFERENCES_SHOW_DESKTOP,
				  G_CALLBACK(desktop_setting_changed_callback),
				  sidebar);

    g_signal_connect_swapped (cinnamon_privacy_preferences, "changed::" KRAKEN_PREFERENCES_RECENT_ENABLED,
                  G_CALLBACK(desktop_setting_changed_callback),
                  sidebar);
...
static void
kraken_places_sidebar_dispose (GObject *object)
{
	KrakenPlacesSidebar *sidebar;

	sidebar = KRAKEN_PLACES_SIDEBAR (object);
	...
	G_OBJECT_CLASS (kraken_places_sidebar_parent_class)->dispose (object);
}

static void
kraken_places_sidebar_class_init (KrakenPlacesSidebarClass *class)
{
    GObjectClass *oclass = G_OBJECT_CLASS (class);
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (class);

    oclass->dispose = kraken_places_sidebar_dispose;

	widget_class->style_set = kraken_places_sidebar_style_set;
	widget_class->focus = kraken_places_sidebar_focus;
...
static gboolean
update_places_on_idle_callback (KrakenPlacesSidebar *sidebar)
{
    sidebar->update_places_on_idle_id = 0;
...
static void
update_places_on_idle (KrakenPlacesSidebar *sidebar)
{
    if (sidebar->update_places_on_idle_id != 0) {
        g_source_remove (sidebar->update_places_on_idle_id);
        sidebar->update_places_on_idle_id = 0;
...
static void
kraken_places_sidebar_set_parent_window (KrakenPlacesSidebar *sidebar,
					   KrakenWindow *window)
{
	KrakenWindowSlot *slot;
    gint breakpoint;
	...
	sidebar->bookmarks = kraken_bookmark_list_get_default ();
	sidebar->uri = kraken_window_slot_get_current_uri (slot);

    breakpoint = g_settings_get_int (kraken_window_state, KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT);

    if (breakpoint < 0) {     // Default gsettings value is -1 (which translates to 'not previously set')
        breakpoint = kraken_bookmark_list_length (sidebar->bookmarks);
        g_settings_set_int (kraken_window_state, KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT, breakpoint);
    }

    sidebar->bookmark_breakpoint = breakpoint;
    g_signal_connect_swapped (kraken_window_state, "changed::" KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT,
	...
	g_signal_connect_swapped (kraken_preferences, "changed::" KRAKEN_PREFERENCES_ALWAYS_USE_BROWSER,
				 G_CALLBACK (reset_menu), sidebar);
	update_places (sidebar);
}

static void
kraken_places_sidebar_style_set (GtkWidget *widget,
				   GtkStyle  *previous_style)
{
	KrakenPlacesSidebar *sidebar;

	sidebar = KRAKEN_PLACES_SIDEBAR (widget);
	update_places (sidebar);
}

GtkWidget *
kraken_places_sidebar_new (KrakenWindow *window)
{
	KrakenPlacesSidebar *sidebar;

	sidebar = g_object_new (KRAKEN_TYPE_PLACES_SIDEBAR, NULL);
	kraken_places_sidebar_set_parent_window (sidebar, window);
...
static gboolean
kraken_shortcuts_model_row_draggable (GtkTreeDragSource *drag_source,
					GtkTreePath       *path)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
static void
_kraken_shortcuts_model_class_init (KrakenShortcutsModelClass *klass)
{

}

static void
_kraken_shortcuts_model_init (KrakenShortcutsModel *model)
{
	model->sidebar = NULL;
}

static void
_kraken_shortcuts_model_drag_source_init (GtkTreeDragSourceIface *iface)
{
	iface->row_draggable = kraken_shortcuts_model_row_draggable;
}

static GtkTreeStore *
kraken_shortcuts_model_new (KrakenPlacesSidebar *sidebar)
{
	KrakenShortcutsModel *model;
	...
	model = g_object_new (KRAKEN_TYPE_SHORTCUTS_MODEL, NULL);
