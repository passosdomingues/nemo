...
static void find_empty_location (KrakenIconContainer *container,
                                 KrakenPlacementGrid *grid,
                                 KrakenIcon *icon,
                                 int start_x,
                                 int start_y,
                                 int *x,
...
G_DEFINE_TYPE (KrakenIconViewContainer, kraken_icon_view_container, KRAKEN_TYPE_ICON_CONTAINER);
...
get_icon_view (KrakenIconContainer *container)
{
	...
	return ((KrakenIconViewContainer *)container)->view;
}


static KrakenIconInfo *
kraken_icon_view_container_get_icon_images (KrakenIconContainer *container,
                                          KrakenIconData      *data,
                                          int                size,
                                          ...
                                          gboolean           visible)
{
	...
	*has_window_open = kraken_file_has_open_window (file);
	...
	icon_info = kraken_file_get_icon (file, size, size, scale, flags);
...
static char *
kraken_icon_view_container_get_icon_description (KrakenIconContainer *container,
						   KrakenIconData      *data)
{
	KrakenFile *file;
	char *mime_type;
 ...
 */
static GQuark *
kraken_icon_view_container_get_icon_text_attributes_from_preferences (void)
{
	 ...
	 *
	 * Thankfully, the Kraken preferences machinery deals with both of
	 * these cases.
	 *
 ...
 **/
static GQuark *
kraken_icon_view_container_get_icon_text_attribute_names (KrakenIconContainer *container,
							    int *len)
{
	GQuark *attributes;
	int piece_count;
	...
	const int pieces_by_level[] = {
		0,	/* KRAKEN_ZOOM_LEVEL_SMALLEST */
		0,	/* KRAKEN_ZOOM_LEVEL_SMALLER */
		0,	/* KRAKEN_ZOOM_LEVEL_SMALL */
		1,	/* KRAKEN_ZOOM_LEVEL_STANDARD */
		2,	/* KRAKEN_ZOOM_LEVEL_LARGE */
		2,	/* KRAKEN_ZOOM_LEVEL_LARGER */
		3	/* KRAKEN_ZOOM_LEVEL_LARGEST */
	};

	piece_count = pieces_by_level[kraken_icon_container_get_zoom_level (container)];

	attributes = kraken_icon_view_container_get_icon_text_attributes_from_preferences ();
 ...
 */
static void
kraken_icon_view_container_get_icon_text (KrakenIconContainer *container,
					    KrakenIconData      *data,
					    char                 **editable_text,
					    ...
					    gboolean               include_invisible)
{
	...
	if (kraken_icon_container_get_zoom_level (container) == KRAKEN_ZOOM_LEVEL_SMALLEST &&
		...
		*editable_text = kraken_file_get_display_name (file);
        if (pinned) {
            *pinned = kraken_file_get_pinning (file) && !kraken_file_is_in_favorites (file);
        }
	}

    if (fav_unavailable) {
        *fav_unavailable = kraken_file_is_unavailable_favorite (file);
    }
	...
	attributes = kraken_icon_view_container_get_icon_text_attribute_names (container,
									   &num_attributes);
	...
	for (i = 0; i < num_attributes; ++i) {
		...
		text_array[j++] =
			kraken_file_get_string_attribute_with_default_q (file, attributes[i]);
	}
	text_array[j] = NULL;
...
static int
fm_desktop_icon_container_icons_compare (KrakenIconContainer *container,
					 ...
					 KrakenIconData      *data_b)
{
	...
	directory_view = KRAKEN_VIEW (KRAKEN_ICON_VIEW_CONTAINER (container)->view);
	...
	if (category_a == category_b) {
		return kraken_file_compare_for_sort
			(file_a, file_b, KRAKEN_FILE_SORT_BY_DISPLAY_NAME,
			 kraken_view_should_sort_directories_first (directory_view),
			 kraken_view_should_sort_favorites_first (directory_view),
			 FALSE,
             NULL);
	}
...
static int
kraken_icon_view_container_compare_icons (KrakenIconContainer *container,
					    ...
					    KrakenIconData      *icon_b)
{
	...
	if (KRAKEN_ICON_VIEW_CONTAINER (container)->sort_for_desktop) {
...
static void
kraken_icon_view_container_freeze_updates (KrakenIconContainer *container)
{
	KrakenIconView *icon_view;
	icon_view = get_icon_view (container);
	g_return_if_fail (icon_view != NULL);
...
static void
kraken_icon_view_container_unfreeze_updates (KrakenIconContainer *container)
{
	KrakenIconView *icon_view;
	icon_view = get_icon_view (container);
	g_return_if_fail (icon_view != NULL);
...
inline static void
kraken_icon_view_container_icon_get_bounding_box (KrakenIcon *icon,
                          ...
                          KrakenIconPosition  *position)
{
    ...
    if (kraken_icon_view_is_compact (icon_view) || kraken_file_get_is_desktop_orphan (file)) {
        return FALSE;
    }
    ...
    if (kraken_icon_container_get_is_desktop (container)) {
        ...
        scale_string = kraken_file_get_metadata
            (file, KRAKEN_METADATA_KEY_ICON_SCALE, "1");
...
static void
lay_down_one_line (KrakenIconContainer *container,
           GList *line_start,
           GList *line_end,
           double y,
           double max_height,
           ...
           gint     gap)
{
    ...
    for (p = line_start; p != line_end; p = p->next) {
        ...
        position = &g_array_index (positions, KrakenCanvasRects, i++);
        ...
        kraken_icon_container_icon_set_position
            (container, icon,
             is_rtl ? kraken_icon_container_get_mirror_x_position (container, icon, x + position->x_offset) : x + position->x_offset,
             y + y_offset);
        kraken_icon_canvas_item_set_entire_text (icon->item, whole_text);

        icon->saved_ltr_x = is_rtl ? kraken_icon_container_get_mirror_x_position (container, icon, icon->x) : icon->x;
...
static void
lay_down_one_column (KrakenIconContainer *container,
             GList *line_start,
             GList *line_end,
             double x,
             double y_start,
...
static void
lay_down_icons_horizontal (KrakenIconContainer *container,
               GList *icons,
               double start_y)
{
    GList *p, *line_start;
    ...
    positions = g_array_new (FALSE, FALSE, sizeof (KrakenCanvasRects));
    gtk_widget_get_allocation (GTK_WIDGET (container), &allocation);

    /* Lay out icons a line at a time. */
    canvas_width = kraken_icon_container_get_canvas_width (container, allocation);
    ...
    icon_size = kraken_get_icon_size_for_zoom_level (container->details->zoom_level);
    text_size = kraken_get_icon_text_width_for_zoom_level (container->details->zoom_level);
    ...
    if (container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE) {
        ...
        for (p = icons; p != NULL; p = p->next) {
            ...
            text_bounds = kraken_icon_canvas_item_get_text_rectangle (icon->item, TRUE);
    ...
    line_width = container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE ? column_gap : 0;
    line_start = icons;
    y = start_y + row_gap;
    i = 0;
    ...
    for (p = icons; p != NULL; p = p->next) {
        ...
        if (container->details->fixed_text_height == -1) {
            container->details->fixed_text_height = kraken_icon_canvas_item_get_fixed_text_height_for_layout (icon->item) / ppu;
        }
...
static void
lay_down_icons_vertical (KrakenIconContainer *container,
             GList *icons,
             double start_y)
{
    GList *p, *line_start;
    ...
    g_assert (KRAKEN_IS_ICON_CONTAINER (container));
    g_assert (container->details->label_position == KRAKEN_ICON_LABEL_POSITION_BESIDE);
    ...
    canvas_height = kraken_icon_container_get_canvas_height (container, allocation);
...
static void
lay_down_icons_vertical_desktop (KrakenIconContainer *container, GList *icons)
{
    ...
    gtk_widget_get_allocation (GTK_WIDGET (container), &allocation);
    height = kraken_icon_container_get_canvas_height (container, allocation);
    ...
    if (placed > 0) {
        ...
        grid = kraken_placement_grid_new (container, FALSE);

        if (grid) {
            for (p = placed_icons; p != NULL; p = p->next) {
                kraken_placement_grid_mark_icon
                    (grid, (KrakenIcon*)p->data);
            }
            ...
            for (p = unplaced_icons; p != NULL; p = p->next) {
                ...
                icon->saved_ltr_x = x;
                kraken_placement_grid_mark_icon (grid, icon);
            }

            kraken_placement_grid_free (grid);
        }
        ...
        while (icons != NULL) {
            ...
            for (p = icons; p != NULL; p = p->next) {
                ...
                kraken_icon_container_icon_get_bounding_box (container, icon, NULL, &y1, NULL, &y2,
            ...
            for (p = icons; p != NULL; p = p->next) {
                ...
                kraken_icon_container_icon_set_position (container, icon,
...
static void
kraken_icon_view_container_lay_down_icons (KrakenIconContainer *container, GList *icons, double start_y)
{
    switch (container->details->layout_mode)
    {
    case KRAKEN_ICON_LAYOUT_L_R_T_B:
    case KRAKEN_ICON_LAYOUT_R_L_T_B:
        lay_down_icons_horizontal (container, icons, start_y);
        break;

    case KRAKEN_ICON_LAYOUT_T_B_L_R:
    case KRAKEN_ICON_LAYOUT_T_B_R_L:
        if (kraken_icon_container_get_is_desktop (container)) {
           ...
           int *x, int *y)
{
    ...
    icon_position = kraken_icon_canvas_item_get_icon_rectangle (icon->item);
    icon_width = icon_position.x1 - icon_position.x0;
    icon_height = icon_position.y1 - icon_position.y0;

    gtk_widget_get_allocation (GTK_WIDGET (container), &allocation);
    total_width = kraken_icon_container_get_canvas_width (container, allocation);
    total_height = kraken_icon_container_get_canvas_height (container, allocation);

    if (kraken_icon_container_is_layout_rtl (container))
        *x = kraken_icon_container_get_mirror_x_position (container, icon, *x);
...
compare_icons_by_position (gconstpointer a, gconstpointer b)
{
    ...
    kraken_icon_view_container_icon_get_bounding_box (icon_a, &x1, &y1, &x2, &y2,
                   BOUNDS_USAGE_FOR_DISPLAY);
    center_a = x1 + (x2 - x1) / 2;
    kraken_icon_view_container_icon_get_bounding_box (icon_b, &x1, &y1, &x2, &y2,
                   BOUNDS_USAGE_FOR_DISPLAY);
    center_b = x1 + (x2 - x1) / 2;
...
static void
kraken_icon_view_container_icon_set_position (KrakenIconContainer *container,
                                            KrakenIcon          *icon,
                                            double             x,
                                            double             y)
{
...
static void
kraken_icon_view_container_move_icon (KrakenIconContainer *container,
                   KrakenIcon *icon,
                   int x, int y,
                   double scale,
                   gboolean raise,
...
static void
kraken_icon_view_container_update_icon (KrakenIconContainer *container,
                                      KrakenIcon          *icon,
                                      gboolean           visible)
{
    KrakenIconContainerDetails *details;
    ...
    max_image_size = MAX (MAXIMUM_IMAGE_SIZE * EEL_CANVAS (container)->pixels_per_unit, KRAKEN_ICON_MAXIMUM_SIZE);
    ...
    /* Get the icons. */
    icon_info = kraken_icon_container_get_icon_images (container,
                                                     icon->data,
                                                     icon_size,
    ...
    if (container->details->forced_icon_size > 0) {
        ...
        pixbuf = kraken_icon_info_get_pixbuf_at_size (icon_info, icon_size * scale_factor);
    } else {
        pixbuf = kraken_icon_info_get_pixbuf (icon_info);
    }
...
static void
find_empty_location (KrakenIconContainer *container,
             KrakenPlacementGrid *grid,
             KrakenIcon *icon,
             int start_x,
             int start_y,
             ...
             int *y)
{
    ...
    kraken_icon_container_icon_get_bounding_box (container, icon,
    ...
    pixbuf_rect = kraken_icon_canvas_item_get_icon_rectangle (icon->item);
    ...
    do {
        ...
        kraken_placement_grid_canvas_position_to_grid_position (grid,
                          icon_position,
                          &grid_position);
        ...
        if (need_new_column ||
            !kraken_placement_grid_position_is_free (grid, grid_position)) {
...
static void
kraken_icon_view_container_align_icons (KrakenIconContainer *container)
{
    ...
    grid = kraken_placement_grid_new (container, TRUE);
...
static void
kraken_icon_view_container_reload_icon_positions (KrakenIconContainer *container)
{
    ...
    KRAKEN_ICON_CONTAINER_GET_CLASS (container)->lay_down_icons (container, no_position_icons, bottom + GET_VIEW_CONSTANT (container, icon_pad_bottom));
...
static void
kraken_icon_view_container_finish_adding_new_icons (KrakenIconContainer *container)
{
    ...
    for (p = new_icons; p != NULL; p = p->next) {
        icon = p->data;
        kraken_icon_container_update_icon (container, icon);
        if (icon->has_lazy_position || kraken_icon_container_icon_is_new_for_monitor (container, icon, current_monitor)) {
    ...
    if (no_position_icons != NULL) {
        ...
        if (kraken_icon_container_get_is_desktop (container)) {
            KRAKEN_ICON_CONTAINER_GET_CLASS (container)->lay_down_icons (container, no_position_icons, GET_VIEW_CONSTANT (container, container_pad_top));
        } else {
            kraken_icon_container_get_all_icon_bounds (container, NULL, NULL, NULL, &bottom, BOUNDS_USAGE_FOR_LAYOUT);
            KRAKEN_ICON_CONTAINER_GET_CLASS (container)->lay_down_icons (container, no_position_icons, bottom + GET_VIEW_CONSTANT (container, icon_pad_bottom));
        }
        g_list_free (no_position_icons);
    }
...
static void
kraken_icon_view_container_set_zoom_level (KrakenIconContainer *container, gint new_level)
{
    KrakenIconContainerDetails *details;
    int pinned_level;
    double pixels_per_unit;
    ...
    pixels_per_unit = (double) kraken_get_icon_size_for_zoom_level (pinned_level) / KRAKEN_ICON_SIZE_STANDARD;
    eel_canvas_set_pixels_per_unit (EEL_CANVAS (container), pixels_per_unit);
}

static int text_ellipsis_limits[KRAKEN_ZOOM_LEVEL_N_ENTRIES];
...
text_ellipsis_limit_changed_callback (gpointer callback_data)
{
    ...
    pref = g_settings_get_strv (kraken_icon_view_preferences,
                    KRAKEN_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT);

    /* set default */
    get_text_ellipsis_limit_for_zoom (pref, NULL, &one_limit);
    for (i = 0; i < KRAKEN_ZOOM_LEVEL_N_ENTRIES; i++) {
...
desktop_text_ellipsis_limit_changed_callback (gpointer callback_data)
{
    ...
    pref = g_settings_get_int (kraken_desktop_preferences, KRAKEN_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT);
...
static gint
kraken_icon_view_container_get_max_layout_lines_for_pango (KrakenIconContainer  *container)
{
    int limit;
...
static gint
kraken_icon_view_container_get_max_layout_lines (KrakenIconContainer  *container)
{
    int limit;
...
static gint
kraken_icon_view_container_get_additional_text_line_count (KrakenIconContainer *container)
{
    GQuark *attributes;
    gint len, i, real_count;

    attributes = kraken_icon_view_container_get_icon_text_attribute_names (container, &len);
...
finalize (GObject *object)
{
    ...
    G_OBJECT_CLASS (kraken_icon_view_container_parent_class)->finalize (object);
}

static void
kraken_icon_view_container_class_init (KrakenIconViewContainerClass *klass)
{
	...
	ic_class->get_icon_text = kraken_icon_view_container_get_icon_text;
	ic_class->get_icon_images = kraken_icon_view_container_get_icon_images;
	ic_class->get_icon_description = kraken_icon_view_container_get_icon_description;
    ic_class->get_max_layout_lines_for_pango = kraken_icon_view_container_get_max_layout_lines_for_pango;
    ic_class->get_max_layout_lines = kraken_icon_view_container_get_max_layout_lines;

	ic_class->compare_icons = kraken_icon_view_container_compare_icons;
	ic_class->freeze_updates = kraken_icon_view_container_freeze_updates;
	ic_class->unfreeze_updates = kraken_icon_view_container_unfreeze_updates;
    ic_class->lay_down_icons = kraken_icon_view_container_lay_down_icons;
    ic_class->icon_set_position = kraken_icon_view_container_icon_set_position;
    ic_class->move_icon = kraken_icon_view_container_move_icon;
    ic_class->update_icon = kraken_icon_view_container_update_icon;
    ic_class->align_icons = kraken_icon_view_container_align_icons;
    ic_class->reload_icon_positions = kraken_icon_view_container_reload_icon_positions;
    ic_class->finish_adding_new_icons = kraken_icon_view_container_finish_adding_new_icons;
    ic_class->icon_get_bounding_box = kraken_icon_view_container_icon_get_bounding_box;
    ic_class->set_zoom_level = kraken_icon_view_container_set_zoom_level;
    ic_class->get_additional_text_line_count = kraken_icon_view_container_get_additional_text_line_count;
}

static void
kraken_icon_view_container_init (KrakenIconViewContainer *icon_container)
{
    ...
    if (!setup_prefs) {
        ...
        g_signal_connect (kraken_icon_view_preferences,
...
KrakenIconContainer *
kraken_icon_view_container_construct (KrakenIconViewContainer *icon_container,
                                    KrakenIconView          *view,
                                    gboolean               is_desktop)
{
	AtkObject *atk_obj;
...
KrakenIconContainer *
kraken_icon_view_container_new (KrakenIconView *view,
                              gboolean      is_desktop)
{
    return kraken_icon_view_container_construct (g_object_new (KRAKEN_TYPE_ICON_VIEW_CONTAINER, NULL),
                                               view,
                                               is_desktop);
}

void
kraken_icon_view_container_set_sort_desktop (KrakenIconViewContainer *container,
					       gboolean         desktop)
{
