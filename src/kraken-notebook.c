...
static int  kraken_notebook_insert_page	 (GtkNotebook *notebook,
					  GtkWidget *child,
					  GtkWidget *tab_label,
					  GtkWidget *menu_label,
					  int position);
static void kraken_notebook_remove	 (GtkContainer *container,
...
G_DEFINE_TYPE (KrakenNotebook, kraken_notebook, GTK_TYPE_NOTEBOOK);

static void
kraken_notebook_class_init (KrakenNotebookClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);
	GtkNotebookClass *notebook_class = GTK_NOTEBOOK_CLASS (klass);

	container_class->remove = kraken_notebook_remove;

	notebook_class->insert_page = kraken_notebook_insert_page;

	signals[TAB_CLOSE_REQUEST] =
		g_signal_new ("tab-close-request",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (KrakenNotebookClass, tab_close_request),
			      NULL, NULL,
...
find_notebook_at_pointer (gint abs_x, gint abs_y)
{
	...
	/* toplevel should be an KrakenWindow */
	if (toplevel != NULL && KRAKEN_IS_WINDOW (toplevel))
	{
		return KRAKEN_NOTEBOOK (KRAKEN_WINDOW (toplevel)->details->active_pane->notebook);
	}
...
static gboolean
is_in_notebook_window (KrakenNotebook *notebook,
		       gint abs_x, gint abs_y)
{
	KrakenNotebook *nb_at_pointer;
...
gint
kraken_notebook_find_tab_num_at_pos (KrakenNotebook *notebook,
				   gint 	 abs_x,
				   gint 	 abs_y)
{
...
static gboolean
control_key_checker_cb(KrakenNotebook *notebook, GdkEventKey *event, gpointer user_data)
{
...
static gboolean
notebook_tab_shortcut_cb(KrakenNotebook *notebook, GtkDirectionType direction, gpointer user_data)
{
...
static void
kraken_notebook_init (KrakenNotebook *notebook)
{
...
void
kraken_notebook_sync_loading (KrakenNotebook *notebook,
				KrakenWindowSlot *slot)
{
	GtkWidget *tab_label, *spinner, *icon;
	gboolean active;

	g_return_if_fail (KRAKEN_IS_NOTEBOOK (notebook));
...
void
kraken_notebook_sync_tab_label (KrakenNotebook *notebook,
				  KrakenWindowSlot *slot)
{
			 ...
			 KrakenWindowSlot *slot)
{
	...
	notebook = gtk_widget_get_ancestor (GTK_WIDGET (slot), KRAKEN_TYPE_NOTEBOOK);
	if (notebook != NULL) {
...
static GtkWidget *
build_tab_label (KrakenNotebook *nb, KrakenWindowSlot *slot)
{
	...
	gtk_widget_set_name (close_button, "kraken-tab-close-button");
	...
	kraken_drag_slot_proxy_init (hbox, NULL, slot);
...
static int
kraken_notebook_insert_page (GtkNotebook *gnotebook,
			       GtkWidget *tab_widget,
			       GtkWidget *tab_label,
			       GtkWidget *menu_label,
			       int position)
{
	...
	position = GTK_NOTEBOOK_CLASS (kraken_notebook_parent_class)->insert_page (gnotebook,
										     tab_widget,
										     tab_label,
										     menu_label,
										     position);
...
int
kraken_notebook_add_tab (KrakenNotebook *notebook,
			   KrakenWindowSlot *slot,
			   int position,
			   gboolean jump_to)
{
	...
	g_return_val_if_fail (KRAKEN_IS_NOTEBOOK (notebook), -1);
	g_return_val_if_fail (KRAKEN_IS_WINDOW_SLOT (slot), -1);
...
static void
kraken_notebook_remove (GtkContainer *container,
			  GtkWidget *tab_widget)
{
	GtkNotebook *gnotebook = GTK_NOTEBOOK (container);
	GTK_CONTAINER_CLASS (kraken_notebook_parent_class)->remove (container, tab_widget);
...
void
kraken_notebook_reorder_child_relative (KrakenNotebook *notebook,
				      int    	    page_num,
				      int 	    offset)
{
	...
	if (!kraken_notebook_can_reorder_child_relative (
...
void
kraken_notebook_set_current_page_relative (KrakenNotebook *notebook,
					     int offset)
{
	GtkNotebook *gnotebook;
	int page;
	...
	if (!kraken_notebook_can_set_current_page_relative (notebook, offset)) {
		return;
	}
...
static gboolean
kraken_notebook_is_valid_relative_position (KrakenNotebook *notebook,
					  int		page_num,
					  int 		offset)
{
	GtkNotebook *gnotebook;
...
gboolean
kraken_notebook_can_reorder_child_relative (KrakenNotebook *notebook,
					  int		page_num,
					  int 		offset)
{
	g_return_val_if_fail (KRAKEN_IS_NOTEBOOK (notebook), FALSE);

	return kraken_notebook_is_valid_relative_position (
		notebook, page_num, offset);
}

gboolean
kraken_notebook_can_set_current_page_relative (KrakenNotebook *notebook,
						 int offset)
{
	int page_num;
