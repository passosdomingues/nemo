...
#include <libkraken-extension/kraken-extension-types.h>
...
static void
kraken_desktop_item_properties_url_drag_data_received (GtkWidget *widget, GdkDragContext *context,
                                                         int x, int y,
...
static void
kraken_desktop_item_properties_exec_drag_data_received (GtkWidget *widget, GdkDragContext *context,
                                                          int x, int y,
                                                          GtkSelectionData *selection_data,
                                                          guint info, guint time,
                                                          GtkEntry *entry)
{
	...
	file = kraken_file_get_by_uri (uris[0]);
	...
	for (l = entries; l; l = l->next) {
		...
		if (item_entry->filename) {
			...
			g_signal_connect (entry, "drag_data_received",
					  G_CALLBACK (kraken_desktop_item_properties_url_drag_data_received),
					  entry);
			...
			g_signal_connect (entry, "drag_data_received",
					  G_CALLBACK (kraken_desktop_item_properties_exec_drag_data_received),
					  entry);
		}
	}
...
static void
kraken_desktop_item_properties_create_begin (const char *uri,
                                               GtkWidget *box)
{
	GFile *location;
...
GtkWidget *
kraken_desktop_item_properties_make_box (GtkSizeGroup *label_size_group,
                                           GList *files)
{
	KrakenFileInfo *info;
	char *uri;
	...
	g_assert (kraken_desktop_item_properties_should_show (files));

	box = gtk_box_new (GTK_ORIENTATION_VERTICAL, 6);
	g_object_set_data_full (G_OBJECT (box), "label-size-group",
				label_size_group, (GDestroyNotify) g_object_unref);

	info = KRAKEN_FILE_INFO (files->data);

	uri = kraken_file_info_get_uri (info);
	kraken_desktop_item_properties_create_begin (uri, box);
	g_free (uri);
...
gboolean
kraken_desktop_item_properties_should_show (GList *files)
{
	KrakenFileInfo *info;
	...
	if (!kraken_file_info_is_mime_type (info, "application/x-desktop")) {
		return FALSE;
	}
