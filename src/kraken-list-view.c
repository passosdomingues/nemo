...
#include <libkraken-extension/kraken-column-provider.h>
#include <libkraken-private/kraken-clipboard-monitor.h>
#include <libkraken-private/kraken-column-chooser.h>
#include <libkraken-private/kraken-column-utilities.h>
...
#include <libkraken-private/kraken-tree-view-drag-dest.h>
#include <libkraken-private/kraken-clipboard.h>

#define DEBUG_FLAG KRAKEN_DEBUG_LIST_VIEW
#include <libkraken-private/kraken-debug.h>

struct KrakenListViewDetails {
	GtkTreeView *tree_view;
	KrakenListModel *model;
	...
	KrakenTreeViewDragDest *drag_dest;
...
static GList *kraken_list_view_get_selection                   (KrakenView   *view);
static void   kraken_list_view_update_selection                (KrakenView *view);
static GList *kraken_list_view_get_selection_for_file_transfer (KrakenView   *view);
static void   kraken_list_view_set_zoom_level                  (KrakenListView        *view,
								  KrakenZoomLevel  new_level,
								  gboolean           always_set_level);
static void   kraken_list_view_scale_font_size                 (KrakenListView        *view,
								  KrakenZoomLevel  new_level);
static void   kraken_list_view_scroll_to_file                  (KrakenListView        *view,
								  KrakenFile      *file);
static void   kraken_list_view_rename_callback                 (KrakenFile      *file,
								  GFile             *result_location,
								  GError            *error,
								  gpointer           callback_data);

static void kraken_list_view_start_renaming_file               (KrakenView *view,
                                                              KrakenFile *file,
                                                              gboolean  select_all);

static void   apply_columns_settings                             (KrakenListView *list_view,
                                                                  char **column_order,
                                                                  char **visible_columns);
static char **get_visible_columns                                (KrakenListView *list_view);
static char **get_default_visible_columns                        (KrakenListView *list_view);
static char **get_column_order                                   (KrakenListView *list_view);
static char **get_default_column_order                           (KrakenListView *list_view);

static void   set_columns_settings_from_metadata_and_preferences (KrakenListView *list_view);
static void   queue_update_visible_icons (KrakenListView *view, gint delay);
static KrakenZoomLevel kraken_list_view_get_zoom_level (KrakenView *view);
static void   prioritize_visible_files (KrakenListView *view);

G_DEFINE_TYPE (KrakenListView, kraken_list_view, KRAKEN_TYPE_VIEW);

static gint click_policy = KRAKEN_CLICK_POLICY_SINGLE;
...
{
	KrakenFileSortType default_sort_order;
	...
	retval = kraken_file_get_default_sort_attribute (file, reversed);

	if (retval == NULL) {
		default_sort_order = g_settings_get_enum (kraken_preferences,
							  KRAKEN_PREFERENCES_DEFAULT_SORT_ORDER);
		default_sort_reversed = g_settings_get_boolean (kraken_preferences,
...
static void
tooltip_prefs_changed_callback (KrakenListView *view)
{
    view->details->show_tooltips = g_settings_get_boolean (kraken_preferences,
                                                           KRAKEN_PREFERENCES_TOOLTIPS_LIST_VIEW);

    view->details->tooltip_flags = kraken_global_preferences_get_tooltip_flags ();
}

static void
expanders_enabled_changed_cb (KrakenListView *view)
{
    g_return_if_fail (KRAKEN_IS_LIST_VIEW (view));
    ...
    gtk_tree_view_set_show_expanders (view->details->tree_view,
                                      g_settings_get_boolean (kraken_list_view_preferences,
                                                             KRAKEN_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION));
}
...
static void
preview_selected_items (KrakenListView *view)
{
	GList *file_list;

	file_list = kraken_list_view_get_selection (KRAKEN_VIEW (view));

	if (file_list != NULL) {
		kraken_view_preview_files (KRAKEN_VIEW (view),
					     file_list, NULL);
...
static void
activate_selected_items (KrakenListView *view)
{
	GList *file_list;
	...
	kraken_view_activate_files (KRAKEN_VIEW (view),
				      file_list,
				      0, TRUE);
	kraken_file_list_free (file_list);
...
static void
activate_selected_items_alternate (KrakenListView *view,
				   KrakenFile *file,
...
static void
kraken_list_view_did_not_drag (KrakenListView *view,
				 GdkEventButton *event)
{
	...
	if (gtk_tree_view_get_path_at_pos (tree_view, event->x, event->y,
		...
		if ((click_policy == KRAKEN_CLICK_POLICY_SINGLE)
        ...
        if (view->details->rename_on_release) {
            KrakenFile *file = kraken_list_model_file_for_path (view->details->model, path);
            kraken_list_view_start_renaming_file (KRAKEN_VIEW (view),
                                                file,
...
static void
stop_drag_check (KrakenListView *view)
{
	view->details->drag_button = 0;
}

static cairo_surface_t *
get_drag_surface (KrakenListView *view)
{
	GtkTreeModel *model;
	GtkTreePath *path;
	GtkTreeIter iter;
	...
	if (gtk_tree_view_get_path_at_pos (view->details->tree_view,
		...
		gtk_tree_model_get (model, &iter,
				    kraken_list_model_get_column_id_from_zoom_level (view->details->zoom_level),
				    &ret,
				    -1);
			...
			gpointer callback_data)
{
	...
	view = KRAKEN_LIST_VIEW (callback_data);
    ...
    if (view->details->drag_button != 0 && !view->details->rubber_banding) {
        ...
        if (!source_target_list) {
            source_target_list = kraken_list_model_get_drag_target_list ();
        }
...
query_tooltip_callback (GtkWidget *widget,
                        ...
                        gpointer user_data)
{
    KrakenListView *list_view;
    gboolean ret;

    ret = FALSE;

    list_view = KRAKEN_LIST_VIEW (user_data);
        ...
        if (gtk_tree_view_get_tooltip_context (GTK_TREE_VIEW (widget), &x, &y,
            ...
            if (!gtk_tree_view_is_blank_at_pos (GTK_TREE_VIEW (widget), x, y, NULL, NULL, NULL, NULL)) {
                gtk_tree_model_get (GTK_TREE_MODEL (list_view->details->model),
                                    &iter,
                                    KRAKEN_LIST_MODEL_FILE_COLUMN, &file,
                                    -1);
                if (file) {
                    gchar *tooltip_text;

                    tooltip_text = kraken_file_construct_tooltip (file,
                                                                list_view->details->tooltip_flags,
                                                                kraken_view_get_model (KRAKEN_VIEW (list_view)));
...
static void
do_popup_menu (GtkWidget *widget, KrakenListView *view, GdkEventButton *event)
{
 	if (tree_view_has_selection (GTK_TREE_VIEW (widget))) {
		kraken_view_pop_up_selection_context_menu (KRAKEN_VIEW (view), event);
	} else {
                kraken_view_pop_up_background_context_menu (KRAKEN_VIEW (view), event);
	}
}

static void
row_activated_callback (GtkTreeView *treeview, GtkTreePath *path,
			GtkTreeViewColumn *column, KrakenListView *view)
{
	activate_selected_items (view);
}
...
columns_reordered_callback (AtkObject *atk,
                            gpointer user_data)
{
    KrakenListView *view = KRAKEN_LIST_VIEW (user_data);

    gchar **columns;
    GList *vis_columns = NULL;
    int i;
    KrakenFile *file = kraken_view_get_directory_as_file (KRAKEN_VIEW (view));
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        kraken_window_set_ignore_meta_column_order (kraken_view_get_kraken_window (KRAKEN_VIEW (view)), list);
    } else if (kraken_file_is_in_search (file)) {
        gchar **column_array = string_array_from_string_glist (list);

        g_settings_set_strv (kraken_search_preferences,
                             KRAKEN_PREFERENCES_SEARCH_VISIBLE_COLUMNS,
                             (const gchar **) column_array);
    } else {
        kraken_file_set_metadata_list (file,
                                     KRAKEN_METADATA_KEY_LIST_VIEW_COLUMN_ORDER,
                                     list);
    }
...
static gboolean
clicked_on_text_in_name_cell (KrakenListView *view, GtkTreePath *path, GdkEventButton *event)
{
    gboolean ret = FALSE;

    KrakenListViewDetails *details = view->details;
    ...
    if (g_settings_get_boolean (kraken_list_view_preferences, KRAKEN_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION)) {
...
static gboolean
clicked_within_double_click_interval (KrakenListView *view)
{
...
static gboolean
clicked_within_slow_click_interval_on_text (KrakenListView *view, GtkTreePath *path, GdkEventButton *event)
{
...
static gboolean
handle_icon_double_click (KrakenListView *view, GtkTreePath *path, GdkEventButton *event, gboolean on_expander)
{
    ...
    if (clicked_within_double_click_interval (view) &&
        ...
        if (!button_event_modifies_selection (event)) {
            ...
            return TRUE;
               ...
               (event->state & GDK_SHIFT_MASK) != 0) {
            KrakenFile *file;
            file = kraken_list_model_file_for_path (view->details->model, path);
            if (file != NULL) {
...
static gboolean
handle_icon_slow_two_click (KrakenListView *view, GtkTreePath *path, GdkEventButton *event)
{
    ...
    file = kraken_list_model_file_for_path (view->details->model, path);
    can_rename = kraken_file_can_rename (file);
    kraken_file_unref (file);
...
button_press_callback (GtkWidget *widget, GdkEventButton *event, gpointer callback_data)
{
    ...
    if (event->type == GDK_2BUTTON_PRESS || event->type == GDK_3BUTTON_PRESS) {
        if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_CLICK_DOUBLE_PARENT_FOLDER) &&
            ...
            if (!gtk_tree_view_get_path_at_pos (tree_view, event->x, event->y,
                                                NULL, NULL, NULL, NULL)) {
                KrakenWindowSlot *slot = kraken_view_get_kraken_window_slot (KRAKEN_VIEW (view));
    ...
    if (!kraken_view_get_active (KRAKEN_VIEW (view)) && gtk_tree_selection_count_selected_rows (selection) > 0) {
        KrakenWindowSlot *slot = kraken_view_get_kraken_window_slot (KRAKEN_VIEW (view));
        kraken_window_slot_make_hosting_pane_active (slot);
        return GDK_EVENT_STOP;
    }

	kraken_list_model_set_drag_view
		(KRAKEN_LIST_MODEL (gtk_tree_view_get_model (tree_view)),
		 tree_view,
		 event->x, event->y);
	...
	if (gtk_tree_view_get_path_at_pos (tree_view, event->x, event->y,
					   &path, NULL, NULL, NULL)) {
        if (g_settings_get_boolean (kraken_list_view_preferences,
                                      KRAKEN_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION)) {
			 ...
			 gpointer callback_data)
{
	...
	if (event->button == view->details->drag_button) {
		...
		if (!view->details->drag_started &&
			...
			kraken_list_view_did_not_drag (view, event);
		}
	}
	return FALSE;
}
...
static void
subdirectory_done_loading_callback (KrakenDirectory *directory, KrakenListView *view)
{
	kraken_list_model_subdirectory_done_loading (view->details->model, directory);
...
row_expanded_callback (GtkTreeView *treeview, GtkTreeIter *iter, GtkTreePath *path, gpointer callback_data)
{
	...
	if (kraken_list_model_load_subdirectory (view->details->model, path, &directory)) {
		char *uri;
		...
		kraken_view_add_subdirectory (KRAKEN_VIEW (view), directory);
        kraken_list_model_set_expanding (view->details->model, directory);

		if (kraken_directory_are_all_files_seen (directory)) {
			kraken_list_model_subdirectory_done_loading (view->details->model,
								 directory);
		} else {
...
unload_file_timeout (gpointer data)
{
	...
	if (unload_data->view != NULL) {
		...
		if (kraken_list_model_get_tree_iter_from_file (model,
							   unload_data->file,
			...
			if (!gtk_tree_view_row_expanded (unload_data->view->details->tree_view,
							 path)) {
				kraken_list_model_unload_subdirectory (model, &iter);
			}
			gtk_tree_path_free (path);
		}
	...
	if (unload_data->directory) {
		kraken_directory_unref (unload_data->directory);
	}
	kraken_file_unref (unload_data->file);
	g_free (unload_data);
	return FALSE;
}
...
row_collapsed_callback (GtkTreeView *treeview, GtkTreeIter *iter, GtkTreePath *path, gpointer callback_data)
{
	...
	if (gtk_tree_model_iter_parent (model, &parent, iter)) {
		gtk_tree_model_get (model, &parent,
				    KRAKEN_LIST_MODEL_SUBDIRECTORY_COLUMN, &directory,
				    -1);
	}
...
static void
subdirectory_unloaded_callback (KrakenListModel *model,
				KrakenDirectory *directory,
				gpointer callback_data)
{
	KrakenListView *view;

	g_return_if_fail (KRAKEN_IS_LIST_MODEL (model));
	...
	g_signal_handlers_disconnect_by_func (directory,
					      G_CALLBACK (subdirectory_done_loading_callback),
					      view);
	kraken_view_remove_subdirectory (KRAKEN_VIEW (view), directory);
}
...
key_press_callback (GtkWidget *widget, GdkEventKey *event, gpointer callback_data)
{
    ...
    if (event->keyval == GDK_KEY_slash ||
        ...
        if (gtk_bindings_activate_event (G_OBJECT (kraken_view_get_kraken_window (view)), event)) {
            return GDK_EVENT_STOP;
        }
    }
	...
	case GDK_KEY_F10:
		if (event->state & GDK_CONTROL_MASK) {
			kraken_view_pop_up_background_context_menu (view, &button_event);
			handled = TRUE;
		}
		break;
	case GDK_KEY_Right:
	...
	case GDK_KEY_space:
		...
		if (!gtk_widget_has_focus (GTK_WIDGET (KRAKEN_LIST_VIEW (view)->details->tree_view))) {
			handled = FALSE;
			break;
		}
		if ((event->state & GDK_SHIFT_MASK) != 0) {
			activate_selected_items_alternate (KRAKEN_LIST_VIEW (view), NULL, TRUE);
		} else {
			preview_selected_items (KRAKEN_LIST_VIEW (view));
		}
		handled = TRUE;
		break;
	case GDK_KEY_Return:
	case GDK_KEY_KP_Enter:
		if ((event->state & GDK_SHIFT_MASK) != 0) {
			...
			activate_selected_items (KRAKEN_LIST_VIEW (view));
		}
		handled = TRUE;
		break;
	case GDK_KEY_v:
...
static void
set_ok_to_load_deferred_attrs (KrakenListView  *list_view,
                       gboolean       ok)
{
...
static void
kraken_list_view_reveal_selection (KrakenView *view)
{
	GList *selection;
	...
	if (selection != NULL) {
		...
		list_view = KRAKEN_LIST_VIEW (view);
		file = selection->data;
		if (kraken_list_model_get_first_iter_for_file (list_view->details->model, file, &iter)) {
			      ...
			      KrakenListView *view)
{
	...
	sort_attr = kraken_list_model_get_attribute_from_sort_column_id (view->details->model, sort_column_id);

	default_sort_column_id = kraken_list_model_get_sort_column_id_from_attribute (view->details->model,
										  g_quark_from_string (get_default_sort_order (file, &default_sort_reversed)));
	default_sort_attr = kraken_list_model_get_attribute_from_sort_column_id (view->details->model, default_sort_column_id);

        if (kraken_global_preferences_get_ignore_view_metadata ())
                kraken_window_set_ignore_meta_sort_column (kraken_view_get_kraken_window (KRAKEN_VIEW (view)),
                                                         g_quark_to_string (sort_attr));
        else if (kraken_file_is_in_search (file)) {
            g_settings_set_string (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_SORT_COLUMN, g_quark_to_string (sort_attr));
        } else {
            kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
	...
	if (view->details->last_sort_attr != sort_attr &&
		...
		if (sort_attr == default_sort_attr) {
			...
			reversed = g_settings_get_boolean (kraken_preferences,
							   KRAKEN_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER);
		} else {
			reversed = kraken_file_is_date_sort_attribute_q (sort_attr);
		}
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        kraken_window_set_ignore_meta_sort_direction (kraken_view_get_kraken_window (KRAKEN_VIEW (view)),
                                                    reversed ? SORT_DESCENDING : SORT_ASCENDING);
    } else if (kraken_file_is_in_search (file)) {
        g_settings_set_boolean (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_REVERSE_SORT, reversed);
    } else {
        reversed_attr = (reversed ? (char *)"true" : (char *)"false");
        kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
                                default_reversed_attr, reversed_attr);
    }

	/* Make sure selected item(s) is visible after sort */
	kraken_list_view_reveal_selection (KRAKEN_VIEW (view));
...
editable_focus_out_cb (GtkWidget *widget,
		       ...
		       gpointer user_data)
{
	KrakenListView *view = user_data;

	kraken_view_set_is_renaming (KRAKEN_VIEW (view), FALSE);
	kraken_view_unfreeze_updates (KRAKEN_VIEW (view));
				  ...
				  KrakenListView *list_view)
{
	...
	kraken_clipboard_set_up_editable
		(GTK_EDITABLE (entry),
		 kraken_view_get_ui_manager (KRAKEN_VIEW (list_view)),
		 FALSE);
}
		      ...
		      KrakenListView    *view)
{
	...
	if (strcmp (new_text, view->details->original_name) != 0) {
		view->details->renaming_file = kraken_file_ref (file);
		view->details->rename_done = FALSE;
		kraken_rename_file (file, new_text, kraken_list_view_rename_callback, g_object_ref (view));
...
static char *
get_root_uri_callback (KrakenTreeViewDragDest *dest,
		       gpointer user_data)
{
	KrakenListView *view;

	view = KRAKEN_LIST_VIEW (user_data);

	return kraken_view_get_uri (KRAKEN_VIEW (view));
}
...
static KrakenFile *
get_file_for_path_callback (KrakenTreeViewDragDest *dest,
			    GtkTreePath *path,
			    gpointer user_data)
{
    KrakenListView *view;
    ...
    return kraken_list_model_file_for_path (view->details->model, path);
}

/* Handles an URL received from Mozilla */
static void
list_view_handle_netscape_url (KrakenTreeViewDragDest *dest, const char *encoded_url,
			       const char *target_uri, GdkDragAction action, int x, int y, KrakenListView *view)
{
...
static void
list_view_handle_uri_list (KrakenTreeViewDragDest *dest, const char *item_uris,
			   const char *target_uri,
			   GdkDragAction action, int x, int y, KrakenListView *view)
{
...
static void
list_view_handle_text (KrakenTreeViewDragDest *dest, const char *text,
		       const char *target_uri,
		       GdkDragAction action, int x, int y, KrakenListView *view)
{
...
static void
list_view_handle_raw (KrakenTreeViewDragDest *dest, const char *raw_data,
...
static void
move_copy_items_callback (KrakenTreeViewDragDest *dest,
                            ...
                            KrakenListView *list_view)
{
    ...
    file = kraken_view_get_directory_as_file (KRAKEN_VIEW (list_view));
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        kraken_window_set_ignore_meta_visible_columns (kraken_view_get_kraken_window (KRAKEN_VIEW (list_view)), list);
    } else if (kraken_file_is_in_search (file)) {
        gchar **column_array = string_array_from_string_glist (list);
        ...
        kraken_file_set_metadata_list (file,
                                     KRAKEN_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS,
                                     list);
                                ...
                                KrakenListView *list_view)
{
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        KrakenWindow *window = kraken_view_get_kraken_window (KRAKEN_VIEW (list_view));
        kraken_window_set_ignore_meta_visible_columns (window, NULL);
        kraken_window_set_ignore_meta_column_order (window, NULL);
    } else {
        kraken_file_set_metadata_list (file, KRAKEN_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
        kraken_file_set_metadata_list (file, KRAKEN_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
    }

    if (kraken_file_is_in_search (file)) {
        g_settings_reset (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
    }
                                 ...
                                 KrakenListView *list_view)
{
    ...
    kraken_list_model_set_temporarily_disable_sort (list_view->details->model, active);
}
                       ...
                       KrakenListView *list_view)
{
    ...
    gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
                                    kraken_list_model_get_temporarily_disable_sort (list_view->details->model));
...
static void
apply_columns_settings (KrakenListView *list_view,
			char **column_order,
			char **visible_columns)
{
	GList *all_columns;
	...
	all_columns = kraken_get_columns_for_file (file);
	all_columns = kraken_sort_columns (all_columns, column_order);
	...
	kraken_column_list_free (all_columns);
			 ...
			 KrakenListView        *view)
{
	...
	gtk_tree_model_get (model, iter,
                ...
                KRAKEN_LIST_MODEL_TEXT_WEIGHT_COLUMN, &weight,
			    -1);
...
focus_in_event_callback (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	...
	KrakenListView *list_view = KRAKEN_LIST_VIEW (user_data);

	/* make the corresponding slot (and the pane that contains it) active */
	slot = kraken_view_get_kraken_window_slot (KRAKEN_VIEW (list_view));
	kraken_window_slot_make_hosting_pane_active (slot);
...
static void
prioritize_visible_files (KrakenListView *view)
{
    KrakenFile *last_file;
    // GList *queue_list, *l;
    GdkRectangle vrect;
    GtkTreeIter iter;
    GtkTreePath *path;
    ...
    gtk_tree_view_get_visible_rect (view->details->tree_view,
                                    &vrect);
    icon_size = kraken_get_list_icon_size_for_zoom_level (kraken_list_view_get_zoom_level (KRAKEN_VIEW (view)));
    ...
    while (cy > start_y) {
        if (gtk_tree_view_get_path_at_pos (view->details->tree_view,
            ...
            gtk_tree_model_get (GTK_TREE_MODEL (view->details->model),
                                &iter,
                                KRAKEN_LIST_MODEL_ICON_SHOWN, &shown,
                                KRAKEN_LIST_MODEL_FILE_COLUMN, &file, -1);
            ...
            if (file != NULL && file != last_file) {
                ...
                if (kraken_file_is_thumbnailing (file)) {
...
static gboolean
update_visible_icons_cb (KrakenListView *view)
{
...
static void
queue_update_visible_icons(KrakenListView *view,
                           gint          delay)
{
...
static gint
get_icon_scale_callback (KrakenListModel *model,
                         KrakenListView  *view)
{
...
static void
update_date_fonts (KrakenListView *view)
{
    g_return_if_fail (KRAKEN_IS_LIST_VIEW (view));
    KrakenDateFontChoice mono_pref;
    gchar *font_name;
    ...
    mono_pref = g_settings_get_enum (kraken_preferences, KRAKEN_PREFERENCES_DATE_FONT_CHOICE);

    if (g_settings_get_enum (kraken_preferences, KRAKEN_PREFERENCES_DATE_FORMAT) == KRAKEN_DATE_FORMAT_INFORMAL ||
        mono_pref == KRAKEN_DATE_FONT_CHOICE_NONE ||
        g_strstr_len (font_name, -1, "Mono")) {
        date_name = g_strdup (font_name);
    } else {
        if (mono_pref == KRAKEN_DATE_FONT_CHOICE_AUTO) {
            PangoFontDescription *font_desc = pango_font_description_from_string (font_name);
            const gchar *current_font_family = pango_font_description_get_family (font_desc);

            if (current_font_family != NULL) {
                date_family = kraken_global_preferences_get_mono_font_family_match (current_font_family);
            } else {
            ...
            date_name = kraken_global_preferences_get_mono_system_font ();
        }
    }
...
static void
create_and_set_up_tree_view (KrakenListView *view)
{
	GtkCellRenderer *cell;
	GtkTreeViewColumn *column;
	GtkBindingSet *binding_set;
	AtkObject *atk_obj;
	GList *kraken_columns;
	GList *l;
    ...
    gtk_tree_view_set_show_expanders (view->details->tree_view,
                                      g_settings_get_boolean (kraken_list_view_preferences,
                                                              KRAKEN_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION));
    g_signal_connect_swapped (kraken_list_view_preferences,
                              "changed::" KRAKEN_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION,
	...
	view->details->drag_dest =
		kraken_tree_view_drag_dest_new (view->details->tree_view, TRUE);
	...
	view->details->model = g_object_new (KRAKEN_TYPE_LIST_MODEL, NULL);
	gtk_tree_view_set_model (view->details->tree_view, GTK_TREE_MODEL (view->details->model));
	/* Need the model for the dnd drop icon "accept" change */
	kraken_list_model_set_drag_view (KRAKEN_LIST_MODEL (view->details->model),
	...
	kraken_columns = kraken_get_all_columns ();

	for (l = kraken_columns; l != NULL; l = l->next) {
		KrakenColumn *kraken_column;
		int column_num;
		char *name;
		char *label;
		float xalign;
		...
		kraken_column = KRAKEN_COLUMN (l->data);

		g_object_get (kraken_column,
			      "name", &name,
			      "label", &label,
			      "xalign", &xalign,
		...
		column_num = kraken_list_model_add_column (view->details->model,
						       kraken_column);
		...
		if (!strcmp (name, "name")) {
			...
			gtk_tree_view_column_set_attributes (view->details->file_name_column,
							     cell,
							     "surface", KRAKEN_LIST_MODEL_SMALLEST_ICON_COLUMN,
							     NULL);
            ...
            gtk_tree_view_column_set_attributes (column, cell,
                                                 "text", column_num,
                                                 "weight", KRAKEN_LIST_MODEL_TEXT_WEIGHT_COLUMN,
                                                 NULL);
    ...
    g_signal_connect_swapped (kraken_preferences, "changed::" KRAKEN_PREFERENCES_DATE_FONT_CHOICE, G_CALLBACK (update_date_fonts), view);
    g_signal_connect_swapped (gnome_interface_preferences, "changed::" KRAKEN_PREFERENCES_MONO_FONT_NAME, G_CALLBACK (update_date_fonts), view);
	kraken_column_list_free (kraken_columns);

	default_visible_columns = g_settings_get_strv (kraken_list_view_preferences,
						       KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
	default_column_order = g_settings_get_strv (kraken_list_view_preferences,
						    KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER);
...
static void
kraken_list_view_add_file (KrakenView *view, KrakenFile *file, KrakenDirectory *directory)
{
	KrakenListModel *model;
	...
	model = KRAKEN_LIST_VIEW (view)->details->model;
	kraken_list_model_add_file (model, file, directory);
    queue_update_visible_icons (KRAKEN_LIST_VIEW (view), INITIAL_UPDATE_VISIBLE_DELAY);
}

static char **
get_default_visible_columns (KrakenListView *list_view)
{
    KrakenFile *file;
    ...
    if (kraken_file_is_in_recent (file)) {
    ...
    return g_settings_get_strv (kraken_list_view_preferences,
                                KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
}

static char **
get_visible_columns (KrakenListView *list_view)
{
	KrakenFile *file;
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        visible_columns = kraken_window_get_ignore_meta_visible_columns (kraken_view_get_kraken_window (KRAKEN_VIEW (list_view)));
    } else {
        if (kraken_file_is_in_search (file)) {
            gchar **modified_cols;

            modified_cols = g_settings_get_strv (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
            ...
            visible_columns = kraken_file_get_metadata_list (file, KRAKEN_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS);
        }
    }
...
static char **
get_default_column_order (KrakenListView *list_view)
{
    KrakenFile *file;
...
static char **
get_column_order (KrakenListView *list_view)
{
	KrakenFile *file;
	GList *column_order;
	char **ret;
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        column_order = kraken_window_get_ignore_meta_column_order (kraken_view_get_kraken_window (KRAKEN_VIEW (list_view)));
    } else {
        if (kraken_file_is_in_search (file)) {
            gchar **modified_cols;
            modified_cols = g_settings_get_strv (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
            ...
            column_order = kraken_file_get_metadata_list (file, KRAKEN_METADATA_KEY_LIST_VIEW_COLUMN_ORDER);
        }
    }
...
static void
set_columns_settings_from_metadata_and_preferences (KrakenListView *list_view)
{
	char **column_order;
	char **visible_columns;
...
static void
set_sort_order_from_metadata_and_preferences (KrakenListView *list_view)
{
	char *sort_attribute;
	int sort_column_id;
	KrakenFile *file;
	...
	file = kraken_view_get_directory_as_file (KRAKEN_VIEW (list_view));
        ...
        if (kraken_global_preferences_get_ignore_view_metadata ())
                sort_attribute = g_strdup (kraken_window_get_ignore_meta_sort_column (kraken_view_get_kraken_window (KRAKEN_VIEW (list_view))));
        else if (kraken_file_is_in_search (file)) {
            sort_attribute = g_settings_get_string (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_SORT_COLUMN);
        } else {
            sort_attribute = kraken_file_get_metadata (file,
                                                     KRAKEN_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
                                                     NULL);
        }
	sort_column_id = kraken_list_model_get_sort_column_id_from_attribute (list_view->details->model,
	...
	if (sort_column_id == -1) {
		sort_column_id =
			kraken_list_model_get_sort_column_id_from_attribute (list_view->details->model,
									 g_quark_from_string (default_sort_order));
	}

    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        gint dir = kraken_window_get_ignore_meta_sort_direction (kraken_view_get_kraken_window (KRAKEN_VIEW (list_view)));
        sort_reversed = dir > SORT_NULL ? dir == SORT_DESCENDING : default_sort_reversed;
    } else if (kraken_file_is_in_search (file)) {
        sort_reversed = g_settings_get_boolean (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_REVERSE_SORT);
    } else {
        sort_reversed = kraken_file_get_boolean_metadata (file,
                                                        KRAKEN_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
...
get_default_zoom_level (void) {
	...
	default_zoom_level = g_settings_get_enum (kraken_list_view_preferences,
						  KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_ZOOM_LEVEL);

	if (default_zoom_level <  KRAKEN_ZOOM_LEVEL_SMALLEST
	    || KRAKEN_ZOOM_LEVEL_LARGEST < default_zoom_level) {
		default_zoom_level = KRAKEN_ZOOM_LEVEL_SMALL;
	}
...
static void
set_zoom_level_from_metadata_and_preferences (KrakenListView *list_view)
{
	KrakenFile *file;
	int level;

	if (kraken_view_supports_zooming (KRAKEN_VIEW (list_view))) {
		file = kraken_view_get_directory_as_file (KRAKEN_VIEW (list_view));
        if (kraken_global_preferences_get_ignore_view_metadata ()) {
            gchar *uri;
            ...
            if (eel_uri_is_search (uri)) {
                ...
                gint ignore_level;
                ignore_level = kraken_window_get_ignore_meta_zoom_level (kraken_view_get_kraken_window (KRAKEN_VIEW (list_view)));
            ...
            level = kraken_file_get_integer_metadata (file,
							    KRAKEN_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL,
							    get_default_zoom_level ());
        }
		kraken_list_view_set_zoom_level (list_view, level, TRUE);
...
static void
kraken_list_view_begin_loading (KrakenView *view)
{
	KrakenListView *list_view;
    ...
    kraken_list_model_set_view_directory (list_view->details->model, kraken_view_get_model (view));

    AtkObject *atk = gtk_widget_get_accessible (GTK_WIDGET (KRAKEN_LIST_VIEW (view)->details->tree_view));
...
static void
stop_cell_editing (KrakenListView *list_view)
{
	GtkTreeViewColumn *column;
...
static void
kraken_list_view_clear (KrakenView *view)
{
	KrakenListView *list_view;
	...
	if (list_view->details->model != NULL) {
		...
		kraken_list_model_clear (list_view->details->model);
	}
...
static void
kraken_list_view_rename_callback (KrakenFile *file,
				    GFile *result_location,
				    GError *error,
				    gpointer callback_data)
{
	...
	if (view->details->renaming_file) {
		...
		if (error != NULL) {
			 ...
			 */
			kraken_file_unref (view->details->renaming_file);
...
static void
kraken_list_view_file_changed (KrakenView *view, KrakenFile *file, KrakenDirectory *directory)
{
	KrakenListView *listview;
	GtkTreeIter iter;
	GtkTreePath *file_path;

	listview = KRAKEN_LIST_VIEW (view);

	kraken_list_model_file_changed (listview->details->model, file, directory);
	...
	if (listview->details->renaming_file != NULL &&
		 ...
		 */
		if (kraken_list_model_get_tree_iter_from_file (listview->details->model, file, directory, &iter)) {
		...
		kraken_file_unref (listview->details->renaming_file);
...
static char *
kraken_list_view_get_backing_uri (KrakenView *view)
{
	KrakenListView *list_view;
	KrakenListModel *list_model;
	KrakenFile *file;
	GtkTreeView *tree_view;
	GtkTreeSelection *selection;
	GtkTreePath *path;
	...
	g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), NULL);
	...
	if (length == 1) {
		...
		file = kraken_list_model_file_for_path (list_model, path);
		if (file == NULL) {
	...
	if (uri == NULL && length > 0) {
		 ...
		 * is handled by KrakenView::get_backing_directory.) */
		...
		if (is_common && !is_root) {
			...
			file = kraken_list_model_file_for_path (list_model, path);
			g_assert (file != NULL);
			uri = kraken_file_get_parent_uri (file);
			kraken_file_unref (file);
	...
	return KRAKEN_VIEW_CLASS (kraken_list_view_parent_class)->get_backing_uri (view);
}

static void
kraken_list_view_get_selection_foreach_func (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
	GList **list;
	KrakenFile *file;
...
static GList *
kraken_list_view_get_selection (KrakenView *view)
{
	GList *list;

	list = NULL;

	gtk_tree_selection_selected_foreach (gtk_tree_view_get_selection (KRAKEN_LIST_VIEW (view)->details->tree_view),
					     kraken_list_view_get_selection_foreach_func, &list);

	return g_list_reverse (list);
}

static GList *
kraken_list_view_peek_selection (KrakenView *view)
{
    KrakenListView *list_view = KRAKEN_LIST_VIEW (view);

    if (list_view->details->current_selection_count == -1) {
        kraken_list_view_update_selection (KRAKEN_VIEW (list_view));
    }
...
static gint
kraken_list_view_get_selection_count (KrakenView *view)
{
    KrakenListView *list_view = KRAKEN_LIST_VIEW (view);
...
static void
kraken_list_view_update_selection (KrakenView *view)
{
    KrakenListView *list_view = KRAKEN_LIST_VIEW (view);
    ...
    list_view->details->current_selection = kraken_list_view_get_selection (view);
    list_view->details->current_selection_count = g_list_length (list_view->details->current_selection);
}

static void
kraken_list_view_get_selection_for_file_transfer_foreach_func (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
	KrakenFile *file;
	struct SelectionForeachData *selection_data;
...
static GList *
kraken_list_view_get_selection_for_file_transfer (KrakenView *view)
{
	struct SelectionForeachData selection_data;

	selection_data.list = NULL;
	selection_data.selection = gtk_tree_view_get_selection (KRAKEN_LIST_VIEW (view)->details->tree_view);

	gtk_tree_selection_selected_foreach (selection_data.selection,
					     kraken_list_view_get_selection_for_file_transfer_foreach_func, &selection_data);
...
static guint
kraken_list_view_get_item_count (KrakenView *view)
{
	g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), 0);

	return kraken_list_model_get_length (KRAKEN_LIST_VIEW (view)->details->model);
}

static gboolean
kraken_list_view_is_empty (KrakenView *view)
{
	return kraken_list_model_is_empty (KRAKEN_LIST_VIEW (view)->details->model);
}

static void
kraken_list_view_end_file_changes (KrakenView *view)
{
	KrakenListView *list_view;
...
static void
kraken_list_view_remove_file (KrakenView *view, KrakenFile *file, KrakenDirectory *directory)
{
	GtkTreePath *path;
	GtkTreePath *file_path;
	GtkTreeIter iter;
	...
	if (kraken_list_model_get_tree_iter_from_file (list_view->details->model, file, directory, &iter)) {
		...
		kraken_list_model_remove_file (list_view->details->model, file, directory);
...
static void
kraken_list_view_set_selection (KrakenView *view, GList *selection)
{
	KrakenListView *list_view;
	GtkTreeSelection *tree_selection;
	GList *node;
	...
	for (node = selection; node != NULL; node = node->next) {
		file = node->data;
		iters = kraken_list_model_get_all_iters_for_file (list_view->details->model, file);
...
static void
kraken_list_view_invert_selection (KrakenView *view)
{
	KrakenListView *list_view;
	GtkTreeSelection *tree_selection;
	GList *node;
	...
	gtk_tree_selection_selected_foreach (tree_selection,
					     kraken_list_view_get_selection_foreach_func, &selection);
...
static void
kraken_list_view_select_all (KrakenView *view)
{
	gtk_tree_selection_select_all (gtk_tree_view_get_selection (KRAKEN_LIST_VIEW (view)->details->tree_view));
}

static void
kraken_list_view_merge_menus (KrakenView *view)
{
  KrakenListView *list_view;
  ...
  KRAKEN_VIEW_CLASS (kraken_list_view_parent_class)->merge_menus (view);
  ...
  list_view->details->list_merge_id =
    gtk_ui_manager_add_ui_from_resource (ui_manager, "/org/kraken/kraken-list-view-ui.xml", NULL);

  list_view->details->menus_ready = TRUE;
}

static void
kraken_list_view_unmerge_menus (KrakenView *view)
{
  KrakenListView *list_view;
  GtkUIManager *ui_manager;
  ...
  KRAKEN_VIEW_CLASS (kraken_list_view_parent_class)->unmerge_menus (view);
...
static void
kraken_list_view_update_menus (KrakenView *view)
{
	KrakenListView *list_view;
	...
	KRAKEN_VIEW_CLASS (kraken_list_view_parent_class)->update_menus (view);
}

/* Reset sort criteria and zoom level to match defaults */
static void
kraken_list_view_reset_to_defaults (KrakenView *view)
{
	KrakenFile *file;

	file = kraken_view_get_directory_as_file (view);

    g_signal_handlers_block_by_func (KRAKEN_LIST_VIEW (view)->details->tree_view,
                                     columns_reordered_callback,
                                     KRAKEN_LIST_VIEW (view));

    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        KrakenWindow *window = kraken_view_get_kraken_window (KRAKEN_VIEW (view));
        kraken_window_set_ignore_meta_sort_column (window, NULL);
        kraken_window_set_ignore_meta_sort_direction (window, SORT_NULL);
        kraken_window_set_ignore_meta_zoom_level (window, KRAKEN_ZOOM_LEVEL_NULL);
        kraken_window_set_ignore_meta_column_order (window, NULL);
        kraken_window_set_ignore_meta_visible_columns (window, NULL);
    } else if (kraken_file_is_in_search (file)) {
        g_settings_reset (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
        g_settings_reset (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_SORT_COLUMN);
        g_settings_reset (kraken_search_preferences, KRAKEN_PREFERENCES_SEARCH_REVERSE_SORT);
    } else {
        kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL, NULL);
        kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_LIST_VIEW_SORT_REVERSED, NULL, NULL);
        kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL, NULL, NULL);
        kraken_file_set_metadata_list (file, KRAKEN_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
        kraken_file_set_metadata_list (file, KRAKEN_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
    }
    ...
    default_columns = get_default_visible_columns (KRAKEN_LIST_VIEW (view));
    default_order = get_default_column_order (KRAKEN_LIST_VIEW (view));
    apply_columns_settings (KRAKEN_LIST_VIEW (view), default_order, default_columns);

    g_signal_handlers_unblock_by_func (KRAKEN_LIST_VIEW (view)->details->tree_view,
                                       columns_reordered_callback,
                                       KRAKEN_LIST_VIEW (view));
}

static void
kraken_list_view_scale_font_size (KrakenListView *view,
				    KrakenZoomLevel new_level)
{
	GList *l;
	...
	if (first_time) {
		first_time = FALSE;
		medium = KRAKEN_ZOOM_LEVEL_SMALLER;
		pango_scale[medium] = PANGO_SCALE_MEDIUM;
		for (i = medium; i > KRAKEN_ZOOM_LEVEL_SMALLEST; i--) {
			pango_scale[i - 1] = (1 / 1.2) * pango_scale[i];
		}
		for (i = medium; i < KRAKEN_ZOOM_LEVEL_LARGEST; i++) {
...
static void
kraken_list_view_set_zoom_level (KrakenListView *view,
				   KrakenZoomLevel new_level,
				   gboolean always_emit)
{
	...
	if (view->details->zoom_level == new_level) {
		if (always_emit) {
			g_signal_emit_by_name (KRAKEN_VIEW(view), "zoom_level_changed");
		}
		return;
	}
    ...
    if (kraken_global_preferences_get_ignore_view_metadata ()) {
        ...
        kraken_file_set_integer_metadata (file,
                                        KRAKEN_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL,
                                        get_default_zoom_level (),
                                        new_level);
    }
	...
	column = kraken_list_model_get_column_id_from_zoom_level (new_level);
	gtk_tree_view_column_set_attributes (view->details->file_name_column,
	...
	/* Scale text. */
	kraken_list_view_scale_font_size (view, new_level);

	/* Make all rows the same size. */
	icon_size = kraken_get_list_icon_size_for_zoom_level (new_level);
...
static void
kraken_list_view_bump_zoom_level (KrakenView *view, int zoom_increment)
{
	KrakenListView *list_view;
	gint new_level;
	...
	if (new_level >= KRAKEN_ZOOM_LEVEL_SMALLEST &&
	    new_level <= KRAKEN_ZOOM_LEVEL_LARGEST) {
		kraken_list_view_set_zoom_level (list_view, new_level, FALSE);
	}
}

static KrakenZoomLevel
kraken_list_view_get_zoom_level (KrakenView *view)
{
	KrakenListView *list_view;

	g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), KRAKEN_ZOOM_LEVEL_STANDARD);
...
static void
kraken_list_view_zoom_to_level (KrakenView *view,
				  KrakenZoomLevel zoom_level)
{
	KrakenListView *list_view;
	...
	kraken_list_view_set_zoom_level (list_view, zoom_level, FALSE);
}

static void
kraken_list_view_restore_default_zoom_level (KrakenView *view)
{
	KrakenListView *list_view;
	...
	kraken_list_view_set_zoom_level (list_view, get_default_zoom_level (), FALSE);
}

static KrakenZoomLevel
kraken_list_view_get_default_zoom_level (KrakenView *view)
{
    g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), KRAKEN_ZOOM_LEVEL_NULL);

    return get_default_zoom_level();
}

static gboolean
kraken_list_view_can_zoom_in (KrakenView *view)
{
	g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), FALSE);

	return KRAKEN_LIST_VIEW (view)->details->zoom_level	< KRAKEN_ZOOM_LEVEL_LARGEST;
}

static gboolean
kraken_list_view_can_zoom_out (KrakenView *view)
{
	g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), FALSE);

	return KRAKEN_LIST_VIEW (view)->details->zoom_level > KRAKEN_ZOOM_LEVEL_SMALLEST;
}

static void
kraken_list_view_start_renaming_file (KrakenView *view,
					KrakenFile *file,
					gboolean select_all)
{
	KrakenListView *list_view;
	...
	KRAKEN_VIEW_CLASS (kraken_list_view_parent_class)->start_renaming_file (view, file, select_all);

	/* Freeze updates to the view to prevent losing rename focus when the tree view updates */
	kraken_view_freeze_updates (KRAKEN_VIEW (view));
...
static void
kraken_list_view_click_to_rename_mode_changed (KrakenView *directory_view)
{
    KrakenListView *view;

    g_assert (KRAKEN_IS_LIST_VIEW (directory_view));

    view = KRAKEN_LIST_VIEW (directory_view);

    view->details->click_to_rename = g_settings_get_boolean (kraken_preferences,
                                                                  KRAKEN_PREFERENCES_CLICK_TO_RENAME);
}

static void
kraken_list_view_click_policy_changed (KrakenView *directory_view)
{
	GdkWindow *win;
	GdkDisplay *display;
	KrakenListView *view;
    ...
    click_policy = g_settings_get_enum (kraken_preferences,
                                        KRAKEN_PREFERENCES_CLICK_POLICY);

	/* ensure that we unset the hand cursor and refresh underlined rows */
	if (click_policy == KRAKEN_CLICK_POLICY_DOUBLE) {
		if (view->details->hover_path != NULL) {
	...
	} else if (click_policy == KRAKEN_CLICK_POLICY_SINGLE) {
		if (hand_cursor == NULL) {
...
default_sort_order_changed_callback (gpointer callback_data)
{
	...
	list_view = KRAKEN_LIST_VIEW (callback_data);
...
static void
kraken_list_view_sort_directories_first_changed (KrakenView *view)
{
	KrakenListView *list_view;

	list_view = KRAKEN_LIST_VIEW (view);

	kraken_list_model_set_should_sort_directories_first (list_view->details->model,
							 kraken_view_should_sort_directories_first (view));
}

static void
kraken_list_view_sort_favorites_first_changed (KrakenView *view)
{
	KrakenListView *list_view;

	list_view = KRAKEN_LIST_VIEW (view);

	kraken_list_model_set_should_sort_favorites_first (list_view->details->model,
							 kraken_view_should_sort_favorites_first (view));
}

static int
kraken_list_view_compare_files (KrakenView *view, KrakenFile *file1, KrakenFile *file2)
{
	KrakenListView *list_view;

	list_view = KRAKEN_LIST_VIEW (view);
	return kraken_list_model_compare_func (list_view->details->model, file1, file2);
}

static gboolean
kraken_list_view_using_manual_layout (KrakenView *view)
{
	g_return_val_if_fail (KRAKEN_IS_LIST_VIEW (view), FALSE);
...
static void
kraken_list_view_dispose (GObject *object)
{
	KrakenListView *list_view;

	list_view = KRAKEN_LIST_VIEW (object);

    g_signal_handlers_disconnect_by_func (gtk_settings_get_default (), update_date_fonts, list_view);
    g_signal_handlers_disconnect_by_func (kraken_preferences, update_date_fonts, list_view);
    ...
    G_OBJECT_CLASS (kraken_list_view_parent_class)->dispose (object);
}

static void
kraken_list_view_finalize (GObject *object)
{
	KrakenListView *list_view;
	...
	g_signal_handlers_disconnect_by_func (kraken_preferences,
					      ...
					      list_view);
	g_signal_handlers_disconnect_by_func (kraken_list_view_preferences,
	...
	G_OBJECT_CLASS (kraken_list_view_parent_class)->finalize (object);
}

static char *
kraken_list_view_get_first_visible_file (KrakenView *view)
{
	KrakenFile *file;
	GtkTreePath *path;
	GtkTreeIter iter;
...
static void
kraken_list_view_scroll_to_file (KrakenListView *view,
				   KrakenFile *file)
{
	GtkTreePath *path;
	GtkTreeIter iter;

	if (!kraken_list_model_get_first_iter_for_file (view->details->model, file, &iter)) {
		return;
	}
...
static void
list_view_scroll_to_file (KrakenView *view,
			  const char *uri)
{
	KrakenFile *file;
	...
	if (uri != NULL) {
		...
		if (file != NULL) {
			kraken_list_view_scroll_to_file (KRAKEN_LIST_VIEW (view), file);
			kraken_file_unref (file);
		}
	}
}
...
static void
list_view_notify_clipboard_info (KrakenClipboardMonitor *monitor,
                                 ...
                                 KrakenListView *view)
{
	...
	if (info != NULL && info->cut) {
		kraken_list_model_set_highlight_for_files (view->details->model, info->files);
	} else {
		kraken_list_model_set_highlight_for_files (view->details->model, NULL);
	}
}

static void
kraken_list_view_end_loading (KrakenView *view,
				gboolean all_files_seen)
{
	KrakenClipboardMonitor *monitor;
	KrakenClipboardInfo *info;

    set_ok_to_load_deferred_attrs (KRAKEN_LIST_VIEW (view), TRUE);

	monitor = kraken_clipboard_monitor_get ();
	info = kraken_clipboard_monitor_get_clipboard_info (monitor);

	list_view_notify_clipboard_info (monitor, info, KRAKEN_LIST_VIEW (view));
}

static const char *
kraken_list_view_get_id (KrakenView *view)
{
	return KRAKEN_LIST_VIEW_ID;
}

static void
kraken_list_view_class_init (KrakenListViewClass *class)
{
	KrakenViewClass *kraken_view_class;

	kraken_view_class = KRAKEN_VIEW_CLASS (class);

	G_OBJECT_CLASS (class)->dispose = kraken_list_view_dispose;
	G_OBJECT_CLASS (class)->finalize = kraken_list_view_finalize;

	kraken_view_class->add_file = kraken_list_view_add_file;
	kraken_view_class->begin_loading = kraken_list_view_begin_loading;
	kraken_view_class->end_loading = kraken_list_view_end_loading;
	kraken_view_class->bump_zoom_level = kraken_list_view_bump_zoom_level;
	kraken_view_class->can_zoom_in = kraken_list_view_can_zoom_in;
	kraken_view_class->can_zoom_out = kraken_list_view_can_zoom_out;
        kraken_view_class->click_policy_changed = kraken_list_view_click_policy_changed;
	kraken_view_class->clear = kraken_list_view_clear;
	kraken_view_class->file_changed = kraken_list_view_file_changed;
	kraken_view_class->get_backing_uri = kraken_list_view_get_backing_uri;
	kraken_view_class->get_selection = kraken_list_view_get_selection;
    kraken_view_class->peek_selection = kraken_list_view_peek_selection;
    kraken_view_class->get_selection_count = kraken_list_view_get_selection_count;
	kraken_view_class->get_selection_for_file_transfer = kraken_list_view_get_selection_for_file_transfer;
	kraken_view_class->get_item_count = kraken_list_view_get_item_count;
	kraken_view_class->is_empty = kraken_list_view_is_empty;
	kraken_view_class->remove_file = kraken_list_view_remove_file;
    kraken_view_class->merge_menus = kraken_list_view_merge_menus;
    kraken_view_class->unmerge_menus = kraken_list_view_unmerge_menus;
	kraken_view_class->update_menus = kraken_list_view_update_menus;
	kraken_view_class->reset_to_defaults = kraken_list_view_reset_to_defaults;
	kraken_view_class->restore_default_zoom_level = kraken_list_view_restore_default_zoom_level;
    kraken_view_class->get_default_zoom_level = kraken_list_view_get_default_zoom_level;
	kraken_view_class->reveal_selection = kraken_list_view_reveal_selection;
	kraken_view_class->select_all = kraken_list_view_select_all;
	kraken_view_class->set_selection = kraken_list_view_set_selection;
	kraken_view_class->invert_selection = kraken_list_view_invert_selection;
	kraken_view_class->compare_files = kraken_list_view_compare_files;
	kraken_view_class->sort_directories_first_changed = kraken_list_view_sort_directories_first_changed;
	kraken_view_class->sort_favorites_first_changed = kraken_list_view_sort_favorites_first_changed;
	kraken_view_class->start_renaming_file = kraken_list_view_start_renaming_file;
	kraken_view_class->get_zoom_level = kraken_list_view_get_zoom_level;
	kraken_view_class->zoom_to_level = kraken_list_view_zoom_to_level;
	kraken_view_class->end_file_changes = kraken_list_view_end_file_changes;
	kraken_view_class->using_manual_layout = kraken_list_view_using_manual_layout;
	kraken_view_class->get_view_id = kraken_list_view_get_id;
	kraken_view_class->get_first_visible_file = kraken_list_view_get_first_visible_file;
	kraken_view_class->scroll_to_file = list_view_scroll_to_file;
    kraken_view_class->click_to_rename_mode_changed = kraken_list_view_click_to_rename_mode_changed;
}

static void
kraken_list_view_init (KrakenListView *list_view)
{
	list_view->details = g_new0 (KrakenListViewDetails, 1);
	...
	g_signal_connect_swapped (kraken_preferences,
				  ...
				  list_view);
	g_signal_connect_swapped (kraken_list_view_preferences,
				  "changed::" KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_ZOOM_LEVEL,
				  G_CALLBACK (default_zoom_level_changed_callback),
				  list_view);
	g_signal_connect_swapped (kraken_list_view_preferences,
				  "changed::" KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS,
				  G_CALLBACK (default_visible_columns_changed_callback),
				  list_view);
	g_signal_connect_swapped (kraken_list_view_preferences,
				  "changed::" KRAKEN_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER,
				  G_CALLBACK (default_column_order_changed_callback),
				  list_view);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_TOOLTIPS_LIST_VIEW,
	...
	kraken_list_view_click_policy_changed (KRAKEN_VIEW (list_view));
    kraken_list_view_click_to_rename_mode_changed (KRAKEN_VIEW (list_view));

	kraken_list_view_sort_directories_first_changed (KRAKEN_VIEW (list_view));
	kraken_list_view_sort_favorites_first_changed (KRAKEN_VIEW (list_view));

    list_view->details->current_selection_count = -1;

	/* ensure that the zoom level is always set in begin_loading */
	list_view->details->zoom_level = KRAKEN_ZOOM_LEVEL_SMALLEST - 1;
...
static KrakenView *
kraken_list_view_create (KrakenWindowSlot *slot)
{
	KrakenListView *view;

	view = g_object_new (KRAKEN_TYPE_LIST_VIEW,
			     "window-slot", slot,
			     NULL);
...
static gboolean
kraken_list_view_supports_uri (const char *uri,
				 GFileType file_type,
...
static KrakenViewInfo kraken_list_view = {
	(char *)KRAKEN_LIST_VIEW_ID,
	...
	kraken_list_view_create,
	kraken_list_view_supports_uri
};

void
kraken_list_view_register (void)
{
	kraken_list_view.view_combo_label = _(kraken_list_view.view_combo_label);
	kraken_list_view.view_menu_label_with_mnemonic = _(kraken_list_view.view_menu_label_with_mnemonic);
	kraken_list_view.error_label = _(kraken_list_view.error_label);
	kraken_list_view.startup_error_label = _(kraken_list_view.startup_error_label);
	kraken_list_view.display_location_label = _(kraken_list_view.display_location_label);

	kraken_view_factory_register (&kraken_list_view);
}

GtkTreeView*
kraken_list_view_get_tree_view (KrakenListView *list_view)
{
