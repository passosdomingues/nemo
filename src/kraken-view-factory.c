/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   kraken-view-factory.c: register and create KrakenViews
...
void
kraken_view_factory_register (KrakenViewInfo *view_info)
{
	g_return_if_fail (view_info != NULL);
	g_return_if_fail (view_info->id != NULL);
	g_return_if_fail (kraken_view_factory_lookup (view_info->id) == NULL);
	
	registered_views = g_list_append (registered_views, view_info);
}

const KrakenViewInfo *
kraken_view_factory_lookup (const char *id)
{
	GList *l;
	KrakenViewInfo *view_info;
...
KrakenView *
kraken_view_factory_create (const char *id,
			      KrakenWindowSlot *slot)
{
	const KrakenViewInfo *view_info;
	KrakenView *view;

	view_info = kraken_view_factory_lookup (id);
	if (view_info == NULL) {
		return NULL;
	}
...
gboolean
kraken_view_factory_view_supports_uri (const char *id,
					 GFile *location,
					 GFileType file_type,
...
GList *
kraken_view_factory_get_views_for_uri (const char *uri,
					 GFileType file_type,
					 const char *mime_type)
{
	GList *l, *res;
