...
slot_proxy_drag_motion (GtkWidget          *widget,
			...
			gpointer            user_data)
{
  KrakenDragSlotProxyInfo *drag_info;
  KrakenWindowSlot *target_slot;
  GtkWidget *window;
  GdkAtom target;
  int action = 0;
  ...
  if (drag_info->target_file != NULL) {
    target_uri = kraken_file_get_uri (drag_info->target_file);
  } else {
    if (drag_info->target_slot != NULL) {
      target_slot = drag_info->target_slot;
    } else {
      target_slot = kraken_window_get_active_slot (KRAKEN_WINDOW (window));
    }

    if (target_slot != NULL) {
      target_uri = kraken_window_slot_get_current_uri (target_slot);
    }
  }

  if (drag_info->have_data &&
      drag_info->have_valid_data) {
    if (drag_info->info == KRAKEN_ICON_DND_GNOME_ICON_LIST) {
    ...
    } else if (drag_info->info == KRAKEN_ICON_DND_URI_LIST) {
      action = kraken_drag_default_drop_action_for_uri_list (context, target_uri);
    } else if (drag_info->info == KRAKEN_ICON_DND_NETSCAPE_URL) {
...
{
  KrakenDragSlotProxyInfo *drag_info = user_data;
...
static void
drag_info_clear (KrakenDragSlotProxyInfo *drag_info)
{
  if (!drag_info->have_data) {
    goto out;
  }
  ...
  if (drag_info->info == KRAKEN_ICON_DND_GNOME_ICON_LIST) {
    kraken_drag_destroy_selection_list (drag_info->data.selection_list);
    g_clear_pointer (&drag_info->desktop_dnd_source_fs, g_free);
  } else if (drag_info->info == KRAKEN_ICON_DND_URI_LIST) {
		       ...
		       gpointer            user_data)
{
    ...
    drag_info_clear ((KrakenDragSlotProxyInfo *)user_data);
}
			...
			KrakenDragSlotProxyInfo *drag_info)
{
  ...
  target_view = NULL;
  if (target_slot != NULL) {
    target_view = kraken_window_slot_get_current_view (target_slot);
  }

  if (target_slot != NULL && target_view != NULL) {
    if (drag_info->info == KRAKEN_ICON_DND_GNOME_ICON_LIST) {
      uri_list = kraken_drag_uri_list_from_selection_list (drag_info->data.selection_list);
      g_assert (uri_list != NULL);

      kraken_view_drop_proxy_received_uris (target_view,
                                              uri_list,
                                              target_uri,
      ...
      kraken_view_drop_proxy_received_uris (target_view,
                                              ...
                                              target_uri,
                                              gdk_drag_context_get_selected_action (context));
    } if (drag_info->info == KRAKEN_ICON_DND_NETSCAPE_URL) {
      kraken_view_handle_netscape_url_drop (target_view,
			       ...
			       gpointer            user_data)
{
  ...
  if (info == KRAKEN_ICON_DND_GNOME_ICON_LIST) {
    drag_info->data.selection_list = kraken_drag_build_selection_list (data);

    drag_info->have_valid_data = drag_info->data.selection_list != NULL;
  } else if (info == KRAKEN_ICON_DND_URI_LIST) {
    uris = gtk_selection_data_get_uris (data);
    drag_info->data.uri_list = kraken_drag_uri_list_from_array ((const char **) uris);
    g_strfreev (uris);

    drag_info->have_valid_data = drag_info->data.uri_list != NULL;
  } else if (info == KRAKEN_ICON_DND_NETSCAPE_URL) {
...
void
kraken_drag_slot_proxy_init (GtkWidget *widget,
                               KrakenFile *target_file,
                               KrakenWindowSlot *target_slot)
{
  KrakenDragSlotProxyInfo *drag_info;
  ...
  drag_info = g_new0 (KrakenDragSlotProxyInfo, 1);
  ...
  if (target_file != NULL)
    drag_info->target_file = kraken_file_ref (target_file);
