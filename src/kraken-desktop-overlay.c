...
#include <libkraken-extension/kraken-desktop-preferences.h>
...
typedef struct
{
    ...
    gboolean syncing;
} KrakenDesktopOverlayPrivate;

struct _KrakenDesktopOverlay
{
    GObject parent_object;

    KrakenDesktopOverlayPrivate *priv;
};
...
G_DEFINE_TYPE_WITH_PRIVATE (KrakenDesktopOverlay, kraken_desktop_overlay, G_TYPE_OBJECT)

static void
show_view_page (KrakenDesktopOverlay *overlay)
{
    KrakenDesktopOverlayPrivate *priv = kraken_desktop_overlay_get_instance_private (overlay);
    GdkScreen *screen;
    GdkRectangle rect;
    gchar *plug_name;
...
static void
sync_controls (KrakenDesktopOverlay *overlay,
               gboolean same_monitor)
{
    ...
    if (!kraken_desktop_manager_get_monitor_is_active (priv->manager, priv->monitor)) {
        gtk_stack_set_visible_child_name (priv->view_substack, "substack_disabled");

        priv->kraken_window = NULL;
        ...
        priv->kraken_window = kraken_desktop_manager_get_window_for_monitor (kraken_desktop_manager_get (),
                                                                         priv->monitor);

        kraken_desktop_manager_get_overlay_info (priv->manager,
                                               priv->monitor,
                                               &action_group,
                                               &h_adjust,
                                               &v_adjust);
    ...
    /* Actual handling is done in KrakenDesktopIconGridView */
...
static void
signal_adjust_changed (KrakenDesktopOverlay *overlay)
{
    KrakenDesktopOverlayPrivate *priv = overlay->priv;
    double h_val, v_val;
...
{
    KrakenDesktopOverlay *overlay = KRAKEN_DESKTOP_OVERLAY (user_data);
    KrakenDesktopOverlayPrivate *priv = kraken_desktop_overlay_get_instance_private (overlay);
...
static void
queue_changed_signal (KrakenDesktopOverlay *overlay)
{
    KrakenDesktopOverlayPrivate *priv = kraken_desktop_overlay_get_instance_private (overlay);
...
on_close_window (GtkWidget *overlay_window,
                 ...
                 gpointer   user_data)
{
    g_return_val_if_fail (KRAKEN_IS_DESKTOP_OVERLAY (user_data), GDK_EVENT_PROPAGATE);

    /* When the window is destroyed, kill the overlay instance also.
     * This will end up clearing the weak pointer made in kraken-desktop-icon-grid-view.c. */
...
static void
kraken_desktop_overlay_init (KrakenDesktopOverlay *overlay)
{
    KrakenDesktopOverlayPrivate *priv = kraken_desktop_overlay_get_instance_private (overlay);
    GtkWindow *window;
    GtkWidget *widget;
    ...
    overlay->priv = priv;
    priv->manager = kraken_desktop_manager_get ();
    priv->is_cinnamon = kraken_desktop_manager_get_is_cinnamon (priv->manager);
    priv->syncing = FALSE;

    priv->builder = gtk_builder_new ();
    gtk_builder_set_translation_domain (priv->builder, GETTEXT_PACKAGE);
    gtk_builder_add_from_resource (priv->builder, "/org/kraken/kraken-desktop-overlay.glade", NULL);
    ...
    if (!priv->is_cinnamon) {
        widget = GTK_WIDGET (kraken_desktop_preferences_new ());
...
static void
show_overlay (KrakenDesktopOverlay *overlay,
              gint                monitor)
{
    KrakenDesktopOverlayPrivate *priv = kraken_desktop_overlay_get_instance_private (overlay);
    ...
    priv->monitor = monitor;
    priv->kraken_window = kraken_desktop_manager_get_window_for_monitor (priv->manager,
                                                                     monitor);
    ...
    if (old_monitor != monitor) {
        ...
        gtk_window_get_position (GTK_WINDOW (priv->kraken_window),
                                 &root_x,
                                 &root_y);

        gtk_window_get_size (GTK_WINDOW (priv->kraken_window),
                             &parent_width,
                             &parent_height);
...
static void
kraken_desktop_overlay_dispose (GObject *object)
{
    KrakenDesktopOverlay *overlay = KRAKEN_DESKTOP_OVERLAY (object);
    ...
    G_OBJECT_CLASS (kraken_desktop_overlay_parent_class)->dispose (object);
}

static void
kraken_desktop_overlay_class_init (KrakenDesktopOverlayClass *klass)
{
    signals[ADJUSTS_CHANGED] =
        g_signal_new ("adjusts-changed",
                      ...
                      NULL, NULL, NULL,
                      G_TYPE_NONE, 3,
                      KRAKEN_TYPE_WINDOW, G_TYPE_INT, G_TYPE_INT);

    G_OBJECT_CLASS (klass)->dispose = kraken_desktop_overlay_dispose;
}

KrakenDesktopOverlay *
kraken_desktop_overlay_new (void)
{
    return g_object_new (KRAKEN_TYPE_DESKTOP_OVERLAY, NULL);
}

void
kraken_desktop_overlay_show (KrakenDesktopOverlay *overlay,
                           gint                monitor)
{
    g_return_if_fail (KRAKEN_IS_DESKTOP_OVERLAY (overlay));
    show_overlay (overlay, monitor);
}

void
kraken_desktop_overlay_update_in_place (KrakenDesktopOverlay *overlay)
{
    g_return_if_fail (KRAKEN_IS_DESKTOP_OVERLAY (overlay));
