...
/* kraken-error-reporting.h - implementation of file manager functions that report
 	                        errors to the user.
...
#define NEW_NAME_TAG "Kraken: new name"
#define MAXIMUM_DISPLAYED_FILE_NAME_LENGTH	50

static void finish_rename (KrakenFile *file, gboolean stop_timer, GError *error);

void
kraken_report_error_loading_directory (KrakenFile *file,
					 GError *error,
...
void
kraken_report_error_setting_group (KrakenFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
	...
	if (message == NULL) {
		...
		g_warning ("Hit unhandled case %s:%d in kraken_report_error_setting_group", 
...
void
kraken_report_error_setting_owner (KrakenFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
...
void
kraken_report_error_setting_permissions (KrakenFile *file,
					   GError *error,
					   GtkWindow *parent_window)
{
	char *file_name;
...
typedef struct _KrakenRenameData {
	char *name;
...
void
kraken_report_error_renaming_file (KrakenFile *file,
				     const char *new_name,
				     GError *error,
				     GtkWindow *parent_window)
{
	 ...
	 */
	original_name = kraken_file_get_display_name (file);
	...
	if (message == NULL) {
		/* We should invent decent error messages for every case we actually experience. */
		g_warning ("Hit unhandled case %s:%d in kraken_report_error_renaming_file", 
			   g_quark_to_string (error->domain), error->code);
		/* fall through */
...
static void
kraken_rename_data_free (KrakenRenameData *data)
{
	g_free (data->name);
	g_free (data);
}
...
static void
rename_callback (KrakenFile *file, GFile *result_location,
		 GError *error, gpointer callback_data)
{
	KrakenRenameData *data;
	...
	if (error &&
		...
		kraken_report_error_renaming_file (file, data->name, error, NULL);
	}
...
cancel_rename_callback (gpointer callback_data)
{
	...
	finish_rename (KRAKEN_FILE (callback_data), FALSE, error);
	g_error_free (error);
}

static void
finish_rename (KrakenFile *file, gboolean stop_timer, GError *error)
{
	KrakenRenameData *data;
	...
	kraken_file_cancel (file, rename_callback, NULL);
	if (stop_timer) {
...
void
kraken_rename_file (KrakenFile *file,
		      const char *new_name,
		      KrakenFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	data = g_new0 (KrakenRenameData, 1);
	...
	g_object_set_data_full (G_OBJECT (file),
				NEW_NAME_TAG,
				data, (GDestroyNotify)kraken_rename_data_free);
