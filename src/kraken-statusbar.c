...
G_DEFINE_TYPE (KrakenStatusBar, kraken_status_bar, GTK_TYPE_BOX);

static void
kraken_status_bar_init (KrakenStatusBar *bar)
{
    bar->window = NULL;
}

static void
kraken_status_bar_set_property (GObject        *object,
                              guint           arg_id,
                              const GValue   *value,
                              GParamSpec     *pspec)
{
    KrakenStatusBar *self = KRAKEN_STATUS_BAR (object);
...
static void
kraken_status_bar_get_property (GObject      *object,
                              guint         arg_id,
                              GValue       *value,
                              GParamSpec   *pspec)
{
...
static void
kraken_status_bar_dispose (GObject *object)
{
    KrakenStatusBar *bar = KRAKEN_STATUS_BAR (object);

    bar->window = NULL;

    G_OBJECT_CLASS (kraken_status_bar_parent_class)->dispose (object);
}

static void
action_places_toggle_callback (GtkButton *button, KrakenStatusBar *bar)
{
    kraken_window_set_sidebar_id (KRAKEN_WINDOW (bar->window), KRAKEN_WINDOW_SIDEBAR_PLACES);

    kraken_status_bar_sync_button_states (bar);
}

static void
action_treeview_toggle_callback (GtkButton *button, KrakenStatusBar *bar)
{
    kraken_window_set_sidebar_id (KRAKEN_WINDOW (bar->window), KRAKEN_WINDOW_SIDEBAR_TREE);

    kraken_status_bar_sync_button_states (bar);
}

static void
action_show_sidebar_callback (GtkButton *button, KrakenStatusBar *bar)
{
    kraken_window_show_sidebar (bar->window);
}

static void
action_hide_sidebar_callback (GtkButton *button, KrakenStatusBar *bar)
{
    kraken_window_hide_sidebar (bar->window);
}

static void
sidebar_state_changed_cb (gpointer pointer, gboolean state, gpointer user_data)
{
    kraken_status_bar_sync_button_states (KRAKEN_STATUS_BAR (user_data));
}
...
{
    KrakenStatusBar *bar = KRAKEN_STATUS_BAR (user_data);
    gdouble val = gtk_range_get_value (GTK_RANGE (zoom_slider));

    KrakenWindowSlot *slot = kraken_window_get_active_slot (bar->window);

    if (!KRAKEN_IS_WINDOW_SLOT (slot))
        return;

    KrakenView *view = slot->content_view;

    if (!KRAKEN_IS_VIEW (view))
        return;

    kraken_view_zoom_to_level (view, (int) val);
}

#define SLIDER_WIDTH 100

static void
kraken_status_bar_constructed (GObject *object)
{
    KrakenStatusBar *bar = KRAKEN_STATUS_BAR (object);
    G_OBJECT_CLASS (kraken_status_bar_parent_class)->constructed (object);
    ...
    GtkIconSize size = gtk_icon_size_from_name (KRAKEN_STATUSBAR_ICON_SIZE_NAME);
    ...
    icon = gtk_image_new_from_icon_name ("kraken-sidebar-places-symbolic", size);
    ...
    button = gtk_toggle_button_new ();
    icon = gtk_image_new_from_icon_name ("kraken-sidebar-tree-symbolic", size);
    gtk_button_set_image (GTK_BUTTON (button), icon);
    ...
    icon = gtk_image_new_from_icon_name ("kraken-sidebar-hide-symbolic", size);
    gtk_button_set_image (GTK_BUTTON (button), icon);
    ...
    button = gtk_button_new ();
    icon = gtk_image_new_from_icon_name ("kraken-sidebar-show-symbolic", size);
    gtk_button_set_image (GTK_BUTTON (button), icon);
    ...
    GtkWidget *zoom_slider = gtk_scale_new_with_range (GTK_ORIENTATION_HORIZONTAL,
                                                       (gdouble) KRAKEN_ZOOM_LEVEL_SMALLEST,
                                                       (gdouble) KRAKEN_ZOOM_LEVEL_LARGEST,
                                                       1.0);
    ...
    g_signal_connect_object (KRAKEN_WINDOW (bar->window), "notify::show-sidebar",
                             G_CALLBACK (sidebar_state_changed_cb), bar, G_CONNECT_AFTER);

    g_signal_connect_object (KRAKEN_WINDOW (bar->window), "notify::sidebar-view-id",
...
static void
kraken_status_bar_class_init (KrakenStatusBarClass *status_bar_class)
{
    GObjectClass *oclass;

    oclass = G_OBJECT_CLASS (status_bar_class);

    oclass->set_property = kraken_status_bar_set_property;
    oclass->get_property = kraken_status_bar_get_property;

    oclass->dispose = kraken_status_bar_dispose;
    oclass->constructed = kraken_status_bar_constructed;
...
GtkWidget *
kraken_status_bar_new (KrakenWindow *window)
{
    return g_object_new (KRAKEN_TYPE_STATUS_BAR,
...
GtkWidget *
kraken_status_bar_get_real_statusbar (KrakenStatusBar *bar)
{
    return bar->real_statusbar;
}

void
kraken_status_bar_sync_button_states (KrakenStatusBar *bar)
{
    const gchar *sidebar_id = kraken_window_get_sidebar_id (KRAKEN_WINDOW (bar->window));

    gboolean sidebar_visible = kraken_window_get_show_sidebar (KRAKEN_WINDOW (bar->window));
    ...
    if (g_strcmp0 (sidebar_id, KRAKEN_WINDOW_SIDEBAR_PLACES) == 0) {
...
void
kraken_status_bar_sync_zoom_widgets (KrakenStatusBar *bar)
{
    ...
    KrakenZoomLevel zoom_level = kraken_view_get_zoom_level (KRAKEN_VIEW (view));
