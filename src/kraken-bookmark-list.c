...
/* kraken-bookmark-list.c - implementation of centralized list of bookmarks.
 */
...
static KrakenBookmarkList *singleton = NULL;

/* forward declarations */

static void        kraken_bookmark_list_load_file     (KrakenBookmarkList *bookmarks);
static void        kraken_bookmark_list_save_file     (KrakenBookmarkList *bookmarks);

G_DEFINE_TYPE(KrakenBookmarkList, kraken_bookmark_list, G_TYPE_OBJECT);
...
static KrakenBookmark *
new_bookmark_from_uri (const char *uri, const char *label, KrakenBookmarkMetadata *md)
{
	...
	if (location) {
		new_bookmark = kraken_bookmark_new (location, label, NULL, md);
...
static GFile *
kraken_bookmark_list_get_legacy_file (void)
{
	char *filename;
	GFile *file;
...
static GFile *
kraken_bookmark_list_get_file (void)
{
    char *filename;
    GFile *file;
...
static void
bookmark_in_list_changed_callback (KrakenBookmark     *bookmark,
				   KrakenBookmarkList *bookmarks)
{
	g_assert (KRAKEN_IS_BOOKMARK (bookmark));
	g_assert (KRAKEN_IS_BOOKMARK_LIST (bookmarks));
	/* save changes to the list */
	kraken_bookmark_list_save_file (bookmarks);
}

static gboolean
idle_notify (KrakenBookmarkList *bookmarks)
{
...
static gboolean
bookmark_location_mounted_callback (KrakenBookmark *bookmark,
                                    GFile *location,
                                    KrakenBookmarkList *bookmarks)
{
    gboolean ret = FALSE;
...
static void
stop_monitoring_bookmark (KrakenBookmarkList *bookmarks,
...
{
	g_assert (KRAKEN_IS_BOOKMARK (data));
	g_assert (KRAKEN_IS_BOOKMARK_LIST (user_data));

	stop_monitoring_bookmark (KRAKEN_BOOKMARK_LIST (user_data), 
				  KRAKEN_BOOKMARK (data));
}

static void
clear_bookmarks (KrakenBookmarkList *bookmarks, GList *list)
{
...
{
	KrakenBookmarkList *self = KRAKEN_BOOKMARK_LIST (object);
	...
	G_OBJECT_CLASS (kraken_bookmark_list_parent_class)->finalize (object);
}
                ...
                GObjectConstructParam *construct_params)
{
	...
	retval = G_OBJECT_CLASS (kraken_bookmark_list_parent_class)->constructor
		(type, n_construct_params, construct_params);

	singleton = KRAKEN_BOOKMARK_LIST (retval);
...
{
    G_OBJECT_CLASS (kraken_bookmark_list_parent_class)->constructed (object);

    kraken_bookmark_list_load_file (KRAKEN_BOOKMARK_LIST (object));
}

static void
kraken_bookmark_list_class_init (KrakenBookmarkListClass *class)
{
	...
	signals[CHANGED] =
		g_signal_new ("changed",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (KrakenBookmarkListClass, 
					       changed),
		              NULL, NULL,
...
bookmark_monitor_changed_cb (GFileMonitor      *monitor,
			     ...
			     gpointer           user_data)
{
	KrakenBookmarkList *self = KRAKEN_BOOKMARK_LIST (user_data);

	if (eflags == G_FILE_MONITOR_EVENT_CHANGED ||
	    eflags == G_FILE_MONITOR_EVENT_CREATED) {
		g_return_if_fail (KRAKEN_IS_BOOKMARK_LIST (self));
		kraken_bookmark_list_load_file (self);
	}
}
...
{
    KrakenBookmarkList *bookmarks = KRAKEN_BOOKMARK_LIST (user_data);

    kraken_bookmark_list_load_file (bookmarks);
}

static void
kraken_bookmark_list_init (KrakenBookmarkList *bookmarks)
{
	GFile *file;
	...
	file = kraken_bookmark_list_get_file ();
...
static void
connect_bookmark_signals (KrakenBookmark     *bookmark,
                          KrakenBookmarkList *bookmarks)
{
    ...
    kraken_bookmark_connect (bookmark);
}

static void
insert_bookmark_internal (KrakenBookmarkList *bookmarks,
			  KrakenBookmark     *bookmark,
			  int                   index)
{
...
/**
 * kraken_bookmark_list_append:
 *
 * Append a bookmark to a bookmark list.
 * @bookmarks: KrakenBookmarkList to append to.
 * @bookmark: Bookmark to append a copy of.
 **/
void
kraken_bookmark_list_append (KrakenBookmarkList *bookmarks, 
			       KrakenBookmark     *bookmark)
{
	g_return_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks));
	g_return_if_fail (KRAKEN_IS_BOOKMARK (bookmark));

	insert_bookmark_internal (bookmarks, 
				  kraken_bookmark_copy (bookmark), 
				  -1);
...
/**
 * kraken_bookmark_list_contains:
 *
 * Check whether a bookmark with matching name and url is already in the list.
 * @bookmarks: KrakenBookmarkList to check contents of.
 * @bookmark: KrakenBookmark to match against.
 * 
 * Return value: TRUE if matching bookmark is in list, FALSE otherwise
 **/
gboolean
kraken_bookmark_list_contains (KrakenBookmarkList *bookmarks, 
				 KrakenBookmark     *bookmark)
{
	g_return_val_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks), FALSE);
...
/**
 * kraken_bookmark_list_delete_item_at:
 * 
 ...
 **/
void
kraken_bookmark_list_delete_item_at (KrakenBookmarkList *bookmarks, 
				       guint                 index)
{
	GList *doomed;
	...
	g_assert (KRAKEN_IS_BOOKMARK (doomed->data));
	stop_monitoring_bookmark (bookmarks, KRAKEN_BOOKMARK (doomed->data));
...
/**
 * kraken_bookmark_list_move_item:
 *
 ...
 **/
void
kraken_bookmark_list_move_item (KrakenBookmarkList *bookmarks,
				  guint index,
				  guint destination)
{
	GList *bookmark_item;
...
static gint
kraken_bookmark_list_compare_func (gconstpointer a, gconstpointer b)
{
    g_assert (KRAKEN_IS_BOOKMARK (a));
    g_assert (KRAKEN_IS_BOOKMARK (b));

    return g_utf8_collate (kraken_bookmark_get_name (KRAKEN_BOOKMARK (a)),
                           kraken_bookmark_get_name (KRAKEN_BOOKMARK (b)));
}

/**
 * kraken_bookmark_list_sort_ascending:
 *
 * Sort bookmarks in ascending order.
 * @bookmarks: the list of bookmarks.
 **/
void
kraken_bookmark_list_sort_ascending (KrakenBookmarkList *bookmarks)
{
    g_assert (KRAKEN_IS_BOOKMARK_LIST (bookmarks));

    bookmarks->list = g_list_sort (
        bookmarks->list, (GCompareFunc)kraken_bookmark_list_compare_func);
...
/**
 * kraken_bookmark_list_delete_items_with_uri:
 * 
 ...
 **/
void
kraken_bookmark_list_delete_items_with_uri (KrakenBookmarkList *bookmarks, 
				      	      const char           *uri)
{
	GList *node, *next;
	...
	for (node = bookmarks->list; node != NULL;  node = next) {
		next = node->next;
		KrakenBookmark *bookmark = KRAKEN_BOOKMARK (node->data);

		bookmark_uri = kraken_bookmark_get_uri (bookmark);
...
/**
 * kraken_bookmark_list_get_window_geometry:
 * 
 * Get a string representing the bookmark_list's window's geometry.
 * This is the value set earlier by kraken_bookmark_list_set_window_geometry.
 ...
 **/
const char *
kraken_bookmark_list_get_window_geometry (KrakenBookmarkList *bookmarks)
{
	return window_geometry;
}

/**
 * kraken_bookmark_list_insert_item:
 * 
 ...
 **/
void
kraken_bookmark_list_insert_item (KrakenBookmarkList *bookmarks,
				    KrakenBookmark     *new_bookmark,
				    guint                 index)
{
	g_return_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks));
	...
	insert_bookmark_internal (bookmarks,
				  kraken_bookmark_copy (new_bookmark), 
				  index);
...
GList *
kraken_bookmark_list_get_for_uri (KrakenBookmarkList   *bookmarks,
                                      const char   *uri)
{
    g_return_val_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks), NULL);
    ...
    for (iter = bookmarks->list; iter != NULL; iter = iter->next) {
        bookmark = iter->data;
        gchar *bm_uri = kraken_bookmark_get_uri (bookmark);
 ...
 **/
KrakenBookmark *
kraken_bookmark_list_item_at (KrakenBookmarkList *bookmarks, guint index)
{
	g_return_val_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks), NULL);
	g_return_val_if_fail (index < g_list_length (bookmarks->list), NULL);

	return KRAKEN_BOOKMARK (g_list_nth_data (bookmarks->list, index));
}

/**
 * kraken_bookmark_list_length:
 * 
 ...
 **/
guint
kraken_bookmark_list_length (KrakenBookmarkList *bookmarks)
{
	g_return_val_if_fail (KRAKEN_IS_BOOKMARK_LIST(bookmarks), 0);

	return g_list_length (bookmarks->list);
}

static GList *
load_bookmark_metadata_file (KrakenBookmarkList *list)
{   /* Part of load_files thread */
    ...
    if (g_key_file_load_from_file (kfile,
        ...
        for (i = 0; i < g_strv_length (items); i++) {
            KrakenBookmarkMetadata *meta = kraken_bookmark_metadata_new ();
...
static void
load_files_finish (KrakenBookmarkList *bookmarks,
                   GObject          *source,
                   GAsyncResult     *res)
{
    GError *error = NULL;
...
find_meta_func (gconstpointer a,
                gconstpointer b)
{
    return g_strcmp0 (((KrakenBookmarkMetadata *) a)->bookmark_name, (gchar *) b);
}
...
{
    KrakenBookmarkList *list = KRAKEN_BOOKMARK_LIST (source_object);
    ...
    if (!g_file_query_exists (file, NULL)) {
        g_object_unref (file);
        file = kraken_bookmark_list_get_legacy_file ();
    }
    ...
    if (g_file_load_contents (file,
        ...
        for (i = 0; lines[i]; i++) {
            ...
            if (lines[i][0] != '\0' && lines[i][0] != ' ') {
                ...
                KrakenBookmarkMetadata *md = NULL;

                if (meta_ptr) {
                    metadata = g_list_remove_link (metadata, meta_ptr);
                    md = (KrakenBookmarkMetadata *) meta_ptr->data;
        ...
        g_list_free_full (metadata, (GDestroyNotify) kraken_bookmark_metadata_free);
...
static void
load_files_async (KrakenBookmarkList    *self,
...
static void
save_bookmark_metadata_file (KrakenBookmarkList *list)
{   /* Part of save_files thread */
    ...
    for (ptr = list->list; ptr != NULL; ptr = ptr->next) {
        KrakenBookmark *bookmark = KRAKEN_BOOKMARK (ptr->data);
...
static void
save_files_finish (KrakenBookmarkList *bookmarks,
                   GObject          *source,
                   GAsyncResult     *res)
{
    GError *error = NULL;
    ...
    GFile *file = kraken_bookmark_list_get_file ();
...
save_files_thread (GTask        *task,
                   ...
                   GCancellable *cancellable)
{
    KrakenBookmarkList *bookmarks = KRAKEN_BOOKMARK_LIST (source_object);
    ...
    for (l = bookmarks->list; l; l = l->next) {
        ...
        const char *label;
        char *uri;
        label = kraken_bookmark_get_name (bookmark);
        uri = kraken_bookmark_get_uri (bookmark);
...
static void
save_files_async (KrakenBookmarkList    *self,
                  GAsyncReadyCallback  callback)
{
    GTask *task;
...
static void
process_next_op (KrakenBookmarkList *bookmarks);
...
op_processed_cb (GObject      *source,
                 ...
                 gpointer      user_data)
{
	KrakenBookmarkList *self = user_data;
	int op;
...
/**
 * kraken_bookmark_list_save_file:
 * 
 * Save bookmarks to disk.
 * @bookmarks: the list of bookmarks to save.
 **/
static void
kraken_bookmark_list_save_file (KrakenBookmarkList *bookmarks)
{
	g_signal_emit (bookmarks, signals[CHANGED], 0);
...
static KrakenBookmarkList *list = NULL;

/**
 * kraken_bookmark_list_get_default:
 * 
 ...
 **/
KrakenBookmarkList *
kraken_bookmark_list_get_default (void)
{
    if (list == NULL) {
        list = KRAKEN_BOOKMARK_LIST (g_object_new (KRAKEN_TYPE_BOOKMARK_LIST, NULL));
    }
...
/**
 * kraken_bookmark_list_set_window_geometry:
 * 
 ...
 **/
void
kraken_bookmark_list_set_window_geometry (KrakenBookmarkList *bookmarks,
					    const char           *geometry)
{
	g_return_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks));
