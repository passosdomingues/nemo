...
G_DEFINE_TYPE (KrakenNavigationAction, kraken_navigation_action, GTK_TYPE_ACTION);
#define KRAKEN_NAVIGATION_ACTION_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), KRAKEN_TYPE_NAVIGATION_ACTION, KrakenNavigationActionPrivate))

struct KrakenNavigationActionPrivate
{
	KrakenWindow *window;
	KrakenNavigationDirection direction;
	char *arrow_tooltip;
				    ...
				    gboolean back)
{
	...
	kraken_window_back_or_forward (window, back, index, kraken_event_get_window_open_flags ());
}
...
static void
activate_forward_menu_item_callback (GtkMenuItem *menu_item, KrakenWindow *window)
{
	activate_back_or_forward_menu_item (menu_item, window, FALSE);
}

static void
fill_menu (KrakenWindow *window,
	   GtkWidget *menu,
	   gboolean back)
{
	...
	index = 0;
	while (list != NULL) {
		menu_item = kraken_bookmark_menu_item_new (KRAKEN_BOOKMARK (list->data));
...
static void
show_menu (KrakenNavigationAction *self,
           guint button,
           guint32 event_time)
{
	KrakenWindow *window;
	...
	case KRAKEN_NAVIGATION_DIRECTION_FORWARD:
		fill_menu (window, menu, FALSE);
		break;
	case KRAKEN_NAVIGATION_DIRECTION_BACK:
		fill_menu (window, menu, TRUE);
		break;
 	case KRAKEN_NAVIGATION_DIRECTION_UP:
 		return;
 	case KRAKEN_NAVIGATION_DIRECTION_RELOAD:
 		return;
 	case KRAKEN_NAVIGATION_DIRECTION_HOME:
 		return;
 	case KRAKEN_NAVIGATION_DIRECTION_COMPUTER:
 		return;
 	case KRAKEN_NAVIGATION_DIRECTION_EDIT:
 		return;
	default:
...
{
        KrakenNavigationAction *self = data;
...
static void
unschedule_menu_popup_timeout (KrakenNavigationAction *self)
{
...
static void
schedule_menu_popup_timeout (KrakenNavigationAction *self)
{
...
static gboolean
new_tab_action (KrakenNavigationAction *self)
{
	KrakenWindow *window;
	...
	switch (self->priv->direction) {
	case KRAKEN_NAVIGATION_DIRECTION_FORWARD:
		kraken_window_back_or_forward (window, FALSE, 0,
					     KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB);
		break;
	case KRAKEN_NAVIGATION_DIRECTION_BACK:
		kraken_window_back_or_forward (window, TRUE, 0,
					     KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB);
		break;
	case KRAKEN_NAVIGATION_DIRECTION_UP:
		kraken_window_slot_go_up (kraken_window_get_active_slot (window),
					KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB);
		break;
...
tool_button_press_cb (GtkButton *button,
                      ...
                      gpointer user_data)
{
        KrakenNavigationAction *self = user_data;
               ...
               GtkWidget *proxy)
{
	...
	(* GTK_ACTION_CLASS (kraken_navigation_action_parent_class)->connect_proxy) (action, proxy);
}
                  ...
                  GtkWidget *proxy)
{
	if (GTK_IS_BUTTON (proxy)) {
        ...
        unschedule_menu_popup_timeout (KRAKEN_NAVIGATION_ACTION (action));
	...
	(* GTK_ACTION_CLASS (kraken_navigation_action_parent_class)->disconnect_proxy) (action, proxy);
}

static void
kraken_navigation_action_finalize (GObject *object)
{
	KrakenNavigationAction *action = KRAKEN_NAVIGATION_ACTION (object);
	...
	(* G_OBJECT_CLASS (kraken_navigation_action_parent_class)->finalize) (object);
}

static void
kraken_navigation_action_set_property (GObject *object,
					 guint prop_id,
					 const GValue *value,
					 GParamSpec *pspec)
{
	KrakenNavigationAction *nav;

	nav = KRAKEN_NAVIGATION_ACTION (object);
...
static void
kraken_navigation_action_get_property (GObject *object,
					 guint prop_id,
					 GValue *value,
					 GParamSpec *pspec)
{
...
static void
kraken_navigation_action_class_init (KrakenNavigationActionClass *class)
{
	GObjectClass *object_class = G_OBJECT_CLASS (class);
	GtkActionClass *action_class = GTK_ACTION_CLASS (class);

	object_class->finalize = kraken_navigation_action_finalize;
	object_class->set_property = kraken_navigation_action_set_property;
	object_class->get_property = kraken_navigation_action_get_property;
	...
	g_type_class_add_private (object_class, sizeof(KrakenNavigationActionPrivate));
}

static void
kraken_navigation_action_init (KrakenNavigationAction *action)
{
        action->priv = KRAKEN_NAVIGATION_ACTION_GET_PRIVATE (action);
}
