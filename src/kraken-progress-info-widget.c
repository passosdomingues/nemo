...
G_DEFINE_TYPE (KrakenProgressInfoWidget, kraken_progress_info_widget,
               GTK_TYPE_BOX);

static void
update_data (KrakenProgressInfoWidget *self)
{
	char *status, *details;
	char *markup;

	status = kraken_progress_info_get_status (self->priv->info);
	gtk_label_set_text (GTK_LABEL (self->priv->status), status);
	g_free (status);

	details = kraken_progress_info_get_details (self->priv->info);
...
static void
update_progress (KrakenProgressInfoWidget *self)
{
	double progress;

	progress = kraken_progress_info_get_progress (self->priv->info);
	if (progress < 0) {
...
static void
on_info_started (KrakenProgressInfoWidget *self) {
    gtk_stack_set_visible_child_name (GTK_STACK (self->priv->stack), "running");
}

static void
on_info_finished (KrakenProgressInfoWidget *self)
{
...
cancel_clicked (GtkWidget *button,
		KrakenProgressInfoWidget *self)
{
	kraken_progress_info_cancel (self->priv->info);

    KrakenJobQueue *queue = kraken_job_queue_get ();
    kraken_job_queue_start_job_by_info (queue, self->priv->info);
               ...
               KrakenProgressInfoWidget *self)
{
    ...
    if (kraken_progress_info_get_is_paused (self->priv->info)) {
        kraken_progress_info_resume (self->priv->info);
        gtk_image_set_from_icon_name (GTK_IMAGE (gtk_button_get_image (GTK_BUTTON (button))), PAUSE_ICON, GTK_ICON_SIZE_BUTTON);
    } else
    if (kraken_progress_info_get_is_started (self->priv->info)) {
        kraken_progress_info_pause (self->priv->info);
...
static void
kraken_progress_info_widget_constructed (GObject *obj)
{
	GtkWidget *label, *progress_bar, *hbox, *vbox, *button, *view, *bb, *main_box;
	KrakenProgressInfoWidget *self = KRAKEN_PROGRESS_INFO_WIDGET (obj);
    KrakenProgressInfoWidgetPriv *priv = KRAKEN_PROGRESS_INFO_WIDGET (obj)->priv;

	G_OBJECT_CLASS (kraken_progress_info_widget_parent_class)->constructed (obj);
    ...
    gchar *initial_details = kraken_progress_info_get_initial_details (self->priv->info);
    ...
    gboolean started = kraken_progress_info_get_is_started (self->priv->info);
...
static void
kraken_progress_info_widget_dispose (GObject *obj)
{
	KrakenProgressInfoWidget *self = KRAKEN_PROGRESS_INFO_WIDGET (obj);

	g_clear_object (&self->priv->info);

	G_OBJECT_CLASS (kraken_progress_info_widget_parent_class)->dispose (obj);
}

static void
kraken_progress_info_widget_set_property (GObject *object,
					    guint property_id,
					    const GValue *value,
					    GParamSpec *pspec)
{
	KrakenProgressInfoWidget *self = KRAKEN_PROGRESS_INFO_WIDGET (object);
...
static void
kraken_progress_info_widget_init (KrakenProgressInfoWidget *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, KRAKEN_TYPE_PROGRESS_INFO_WIDGET,
						  KrakenProgressInfoWidgetPriv);

	
}

static void
kraken_progress_info_widget_class_init (KrakenProgressInfoWidgetClass *klass)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (klass);
	oclass->set_property = kraken_progress_info_widget_set_property;
	oclass->constructed = kraken_progress_info_widget_constructed;
	oclass->dispose = kraken_progress_info_widget_dispose;

	properties[PROP_INFO] =
		g_param_spec_object ("info",
				     "KrakenProgressInfo",
				     "The KrakenProgressInfo associated with this widget",
	...
	g_type_class_add_private (klass, sizeof (KrakenProgressInfoWidgetPriv));
}

GtkWidget *
kraken_progress_info_widget_new (KrakenProgressInfo *info)
{
	return g_object_new (KRAKEN_TYPE_PROGRESS_INFO_WIDGET,
			     "info", info,
