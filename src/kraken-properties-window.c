...
#include "kraken-desktop-item-properties.h"
...
#include <libkraken-private/kraken-undo-signal-handlers.h>
...
struct KrakenPropertiesWindowDetails {
	...
	KrakenFile *group_change_file;
	char         *group_change_group;
	unsigned int  group_change_timeout;
	KrakenFile *owner_change_file;
...
#define KRAKEN_RESPONSE_REVERT 10
...
static gboolean is_computer_directory             (KrakenFile *file);
static gboolean is_network_directory              (KrakenFile *file);
static gboolean is_burn_directory                 (KrakenFile *file);
static void directory_contents_value_field_update (KrakenPropertiesWindow *window);
static void file_changed_callback                 (KrakenFile       *file,
						   gpointer            user_data);
static void permission_button_update              (KrakenPropertiesWindow *window,
						   GtkToggleButton    *button);
static void permission_combo_update               (KrakenPropertiesWindow *window,
						   GtkComboBox        *combo);
static void value_field_update                    (KrakenPropertiesWindow *window,
						   GtkLabel           *field);
static void properties_window_update              (KrakenPropertiesWindow *window,
						   GList              *files);
static void size_field_update                     (KrakenPropertiesWindow *window);
static void is_directory_ready_callback           (KrakenFile       *file,
						   gpointer            data);
static void cancel_group_change_callback          (KrakenPropertiesWindow *window);
static void cancel_owner_change_callback          (KrakenPropertiesWindow *window);
static void parent_widget_destroyed_callback      (GtkWidget          *widget,
						   gpointer            callback_data);
static void select_image_button_callback          (GtkWidget          *widget,
						   KrakenPropertiesWindow *properties_window);
...
static void append_extension_pages                (KrakenPropertiesWindow *window);

static gboolean name_field_focus_out              (KrakenEntry *name_field,
						   GdkEventFocus *event,
						   gpointer callback_data);
static void name_field_activate                   (KrakenEntry *name_field,
						   gpointer callback_data);
...
static GtkWidget* create_pie_widget 		  (KrakenPropertiesWindow *window);

G_DEFINE_TYPE (KrakenPropertiesWindow, kraken_properties_window, GTK_TYPE_DIALOG);

static gboolean
is_multi_file_window (KrakenPropertiesWindow *window)
{
	GList *l;
	guint count;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		if (!kraken_file_is_gone (KRAKEN_FILE (l->data))) {
			count++;
			if (count > 1) {
				return TRUE;
			}
...
static int
get_not_gone_original_file_count (KrakenPropertiesWindow *window)
{
	GList *l;
	int count;
...
static KrakenFile *
get_original_file (KrakenPropertiesWindow *window)
{
	...
	return KRAKEN_FILE (window->details->original_files->data);
}

static KrakenFile *
get_target_file_for_original_file (KrakenFile *file)
{
	...
	if (KRAKEN_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			...
			if (location) {
				target_file = kraken_file_get (location);
		...
		uri_to_display = kraken_file_get_activation_uri (file);
		if (uri_to_display != NULL) {
			target_file = kraken_file_get_by_uri (uri_to_display);
...
static KrakenFile *
get_target_file (KrakenPropertiesWindow *window)
{
	return KRAKEN_FILE (window->details->target_files->data);
}
...
static void
get_image_for_properties_window (KrakenPropertiesWindow *window,
				 char **icon_name,
				 GdkPixbuf **icon_pixbuf)
{
	KrakenIconInfo *icon, *new_icon;
	GList *l;
	gint icon_scale;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		...
		if (!icon) {
			icon = kraken_file_get_icon (file, KRAKEN_ICON_SIZE_STANDARD, 0, icon_scale,
                                       KRAKEN_FILE_ICON_FLAGS_USE_THUMBNAILS |
                                       KRAKEN_FILE_ICON_FLAGS_IGNORE_VISITING);
		} else {
			new_icon = kraken_file_get_icon (file, KRAKEN_ICON_SIZE_STANDARD, 0, icon_scale,
                                           KRAKEN_FILE_ICON_FLAGS_USE_THUMBNAILS |
                                           KRAKEN_FILE_ICON_FLAGS_IGNORE_VISITING);
			if (!new_icon || new_icon != icon) {
				kraken_icon_info_unref (icon);
				kraken_icon_info_unref (new_icon);
				icon = NULL;
				break;
			}
			kraken_icon_info_unref (new_icon);
	...
	if (!icon) {
		icon = kraken_icon_info_lookup_from_name ("text-x-generic", KRAKEN_ICON_SIZE_STANDARD, icon_scale);
	}

	if (icon_name != NULL) {
		*icon_name = g_strdup (kraken_icon_info_get_used_name (icon));
	}

	if (icon_pixbuf != NULL) {
		*icon_pixbuf = kraken_icon_info_get_pixbuf_at_size (icon, KRAKEN_ICON_SIZE_STANDARD);
	}
...
static void
update_properties_window_icon (KrakenPropertiesWindow *window)
{
	GdkPixbuf *pixbuf;
...
static void
reset_icon (KrakenPropertiesWindow *properties_window)
{
	GList *l;
	...
	for (l = properties_window->details->original_files; l != NULL; l = l->next) {
		...
		kraken_file_set_metadata (file,
					    KRAKEN_METADATA_KEY_ICON_SCALE,
					    NULL, NULL);
		kraken_file_set_metadata (file,
					    KRAKEN_METADATA_KEY_CUSTOM_ICON,
					    NULL, NULL);
        kraken_file_set_metadata (file,
                        KRAKEN_METADATA_KEY_CUSTOM_ICON_NAME,
                        NULL, NULL);
	}
}
...
static void
kraken_properties_window_drag_data_received (GtkWidget *widget, GdkDragContext *context,
					       int x, int y,
					       ...
					       guint info, guint time)
{
	...
	if (!exactly_one) {
		...
		if (uri_is_local_image (uris[0])) {
			set_icon (uris[0], KRAKEN_PROPERTIES_WINDOW (window));
		} else {
			GFile *f;
...
static GtkWidget *
create_image_widget (KrakenPropertiesWindow *window,
		     gboolean is_customizable)
{
	...
	if (is_customizable) {
		...
		g_signal_connect (image, "drag_data_received",
				  G_CALLBACK (kraken_properties_window_drag_data_received), NULL);
...
static void
set_name_field (KrakenPropertiesWindow *window,
		...
		const gchar *name)
{
	 ...
	 */
	use_label = is_multi_file_window (window) || !kraken_file_can_rename (get_original_file (window));
	new_widget = !window->details->name_field || (use_label ? KRAKEN_IS_ENTRY (window->details->name_field) : GTK_IS_LABEL (window->details->name_field));
	...
	if (new_widget) {
		...
		if (use_label) {
			...
			window->details->name_field = kraken_entry_new ();
			 ...
			 * we've removed Undo from Kraken for now, I'm just ifdeffing out this
			...
			kraken_undo_set_up_kraken_entry_for_undo ( KRAKEN_ENTRY (window->details->name_field));
			kraken_undo_editable_set_undo_key (GTK_EDITABLE (window->details->name_field), TRUE);
#endif
...
static void
update_name_field (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	...
	if (is_multi_file_window (window)) {
		...
		if (file == NULL || kraken_file_is_gone (file)) {
			...
			current_name = kraken_file_get_display_name (file);
		}
...
static void
name_field_restore_original_name (KrakenEntry *name_field)
{
	...
	kraken_entry_select_all (name_field);

	g_free (displayed_name);
}

static void
rename_callback (KrakenFile *file, GFile *res_loc, GError *error, gpointer callback_data)
{
	KrakenPropertiesWindow *window;

	window = KRAKEN_PROPERTIES_WINDOW (callback_data);

	/* Complain to user if rename failed. */
	if (error != NULL) {
		kraken_report_error_renaming_file (file,
						     window->details->pending_name,
						     error,
						     GTK_WINDOW (window));
		if (window->details->name_field != NULL) {
			name_field_restore_original_name (KRAKEN_ENTRY (window->details->name_field));
		}
	}
...
static void
set_pending_name (KrakenPropertiesWindow *window, const char *name)
{
...
static void
name_field_done_editing (KrakenEntry *name_field, KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *new_name;
	const char *original_name;

	g_return_if_fail (KRAKEN_IS_ENTRY (name_field));
	...
	if (strlen (new_name) == 0) {
		name_field_restore_original_name (KRAKEN_ENTRY (name_field));
	} else {
		...
		if (strcmp (new_name, original_name) != 0) {
			...
			g_object_ref (window);
			kraken_file_rename (file, new_name,
...
static gboolean
name_field_focus_out (KrakenEntry *name_field,
		      GdkEventFocus *event,
		      gpointer callback_data)
{
	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (callback_data));

	if (gtk_widget_get_sensitive (GTK_WIDGET (name_field))) {
		name_field_done_editing (name_field, KRAKEN_PROPERTIES_WINDOW (callback_data));
	}
...
static void
name_field_activate (KrakenEntry *name_field, gpointer callback_data)
{
	g_assert (KRAKEN_IS_ENTRY (name_field));
	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (callback_data));

	/* Accept changes. */
	name_field_done_editing (name_field, KRAKEN_PROPERTIES_WINDOW (callback_data));

	kraken_entry_select_all_at_idle (name_field);
}

static void
update_properties_window_title (KrakenPropertiesWindow *window)
{
	char *name, *title;
	KrakenFile *file;
...
static void
clear_extension_pages (KrakenPropertiesWindow *window)
{
...
static void
refresh_extension_pages (KrakenPropertiesWindow *window)
{
...
static void
remove_from_dialog (KrakenPropertiesWindow *window,
		    KrakenFile *file)
{
	int index;
	GList *original_link;
	...
	original_file = KRAKEN_FILE (original_link->data);
	target_file = KRAKEN_FILE (target_link->data);
	...
	kraken_file_monitor_remove (original_file, &window->details->original_files);
	kraken_file_monitor_remove (target_file, &window->details->target_files);
...
static GList *
get_mime_list (KrakenPropertiesWindow *window)
{
	GList *ret;
	GList *l;
	...
	ret = NULL;
	for (l = window->details->target_files; l != NULL; l = l->next) {
		ret = g_list_append (ret, kraken_file_get_mime_type (KRAKEN_FILE (l->data)));
	}
...
static void
properties_window_update (KrakenPropertiesWindow *window,
			  GList *files)
{
	GList *l;
	GList *mime_list;
	GList *tmp;
	KrakenFile *changed_file;
	...
	for (tmp = files; tmp != NULL; tmp = tmp->next) {
		changed_file = KRAKEN_FILE (tmp->data);

		if (changed_file && kraken_file_is_gone (changed_file)) {
...
update_files_callback (gpointer data)
{
 	...
 	window = KRAKEN_PROPERTIES_WINDOW (data);
	...
	if (window->details->original_files == NULL) {
		...
		kraken_file_list_free (window->details->changed_files);
...
static void
schedule_files_update (KrakenPropertiesWindow *window)
 {
 	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (window));
...
file_list_attributes_identical (GList *file_list, const char *attribute_name)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (first_attr == NULL) {
			first_attr = kraken_file_get_string_attribute_with_default (file, attribute_name);
		} else {
			char *attr;
			attr = kraken_file_get_string_attribute_with_default (file, attribute_name);
				...
				const char *inconsistent_value)
{
	if (file_list_attributes_identical (file_list, attribute_name)) {
		...
		for (l = file_list; l != NULL; l = l->next) {
			...
			file = KRAKEN_FILE (l->data);
			if (!kraken_file_is_gone (file)) {
				return kraken_file_get_string_attribute_with_default
					(file,
					 attribute_name);
			}
		}
...
file_list_all_directories (GList *file_list)
{
	GList *l;
	for (l = file_list; l != NULL; l = l->next) {
		if (!kraken_file_is_directory (KRAKEN_FILE (l->data))) {
			return FALSE;
		}
	}
	return TRUE;
...
static void
value_field_update (KrakenPropertiesWindow *window, GtkLabel *label)
{
...
static GtkWidget*
attach_value_field_ellipsizing (KrakenPropertiesWindow *window,
				GtkGrid *grid,
				GtkWidget *sibling,
...
static GtkWidget*
attach_value_field (KrakenPropertiesWindow *window,
		    GtkGrid *grid,
		    GtkWidget *sibling,
		    const char *file_attribute_name,
		    const char *inconsistent_string,
...
static void
group_change_callback (KrakenFile *file,
		       GFile *res_loc,
		       GError *error,
		       KrakenPropertiesWindow *window)
{
	...
	kraken_report_error_setting_group (file, error, GTK_WINDOW (window));
...
static void
cancel_group_change_callback (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *group;
	...
	kraken_file_cancel (file, (KrakenFileOperationCallback) group_change_callback, window);
...
static gboolean
schedule_group_change_timeout (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *group;
	...
	kraken_file_set_group
		(file,  group,
		 (KrakenFileOperationCallback) group_change_callback, window);
...
static void
schedule_group_change (KrakenPropertiesWindow *window,
		       KrakenFile       *file,
		       const char         *group)
{
	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (window));
	...
	window->details->group_change_file = kraken_file_ref (file);
...
static void
unschedule_or_cancel_group_change (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *group;
...
static void
changed_group_callback (GtkComboBox *combo_box, KrakenFile *file)
{
	KrakenPropertiesWindow *window;
	char *group;
	char *cur_group;
	...
	cur_group = kraken_file_get_group_name (file);

	if (group != NULL && strcmp (group, cur_group) != 0) {
		/* Try to change file group. If this fails, complain to user. */
		window = KRAKEN_PROPERTIES_WINDOW (gtk_widget_get_ancestor (GTK_WIDGET (combo_box), GTK_TYPE_WINDOW));
...
static void
synch_groups_combo_box (GtkComboBox *combo_box, KrakenFile *file)
{
	GList *groups;
	GList *node;
	GtkTreeModel *model;
	...
	groups = kraken_file_get_settable_group_names (file);
	...
	current_group_name = kraken_file_get_group_name (file);
		        ...
		        KrakenFile *file)
{
	...
	g_signal_connect_data (combo_box, "changed",
			       ...
			       kraken_file_ref (file),
			       (GClosureNotify)kraken_file_unref, 0);

	return combo_box;
}

static void
owner_change_callback (KrakenFile *file,
		       ...
		       KrakenPropertiesWindow *window)
{
	...
	kraken_report_error_setting_owner (file, error, GTK_WINDOW (window));
...
static void
cancel_owner_change_callback (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *owner;
	...
	kraken_file_cancel (file, (KrakenFileOperationCallback) owner_change_callback, window);
...
static gboolean
schedule_owner_change_timeout (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *owner;
	...
	kraken_file_set_owner
		(file,  owner,
		 (KrakenFileOperationCallback) owner_change_callback, window);
...
static void
schedule_owner_change (KrakenPropertiesWindow *window,
		       KrakenFile       *file,
		       const char         *owner)
{
	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (window));
	...
	window->details->owner_change_file = kraken_file_ref (file);
...
static void
unschedule_or_cancel_owner_change (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	char *owner;
...
static void
changed_owner_callback (GtkComboBox *combo_box, KrakenFile* file)
{
	KrakenPropertiesWindow *window;
	char *owner_text;
	char **name_array;
	...
	g_free (owner_text);
	cur_owner = kraken_file_get_owner_name (file);
...
static void
synch_user_menu (GtkComboBox *combo_box, KrakenFile *file)
{
	GList *users;
	GList *node;
	GtkTreeModel *model;
	...
	owner_name = kraken_file_get_string_attribute (file, "owner");
...
static gboolean
file_has_prefix (KrakenFile *file,
		 GList *prefix_candidates)
{
	...
	for (p = prefix_candidates; p != NULL; p = p->next) {
		...
		candidate_location = kraken_file_get_location (KRAKEN_FILE (p->data));
...
file_size_for_display (goffset size)
{
	...
	size_prefix = kraken_global_preferences_get_size_prefix_preference ();
...
static void
directory_contents_value_field_update (KrakenPropertiesWindow *window)
{
	KrakenRequestStatus file_status, status;
	gchar *text, *temp;
	...
	for (l = window->details->target_files; l; l = l->next) {
		...
		if (kraken_file_is_directory (file)) {
			file_status = kraken_file_get_deep_counts (file,
					 &directory_count,
					 &file_count,
					 &file_unreadable,
                     &hidden_count,
			...
			if (file_status != KRAKEN_REQUEST_DONE) {
				status = file_status;
			}
		} else {
			++total_count;
			total_size += kraken_file_get_size (file);
		}
	}
...
static void
schedule_directory_contents_update (KrakenPropertiesWindow *window)
{
	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (window));
...
static GtkLabel *
attach_directory_contents_value_field (KrakenPropertiesWindow *window,
				       GtkGrid *grid,
				       GtkWidget *sibling)
{
	GtkLabel *value_field;
	...
	for (l = window->details->target_files; l; l = l->next) {
		file = KRAKEN_FILE (l->data);
		kraken_file_recompute_deep_counts (file);
...
static void
append_title_value_pair (KrakenPropertiesWindow *window,
			 GtkGrid *grid,
			 const char *title,
 			 const char *file_attribute_name,
...
static void
append_title_and_ellipsizing_value (KrakenPropertiesWindow *window,
				    GtkGrid *grid,
				    const char *title,
				    const char *file_attribute_name,
				    const char *inconsistent_state,
...
static void
append_directory_contents_fields (KrakenPropertiesWindow *window,
				  GtkGrid *grid)
{
 ...
 */
static void
append_size_field (KrakenPropertiesWindow *window,
		   GtkGrid *grid)
{
	GtkLabel *title_field, *value_field;
...
static gboolean
is_computer_directory (KrakenFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_network_directory (KrakenFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_burn_directory (KrakenFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
should_show_custom_icon_buttons (KrakenPropertiesWindow *window)
{
	if (is_multi_file_window (window)) {
		return FALSE;
	}
...
static gboolean
should_show_file_type (KrakenPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && (kraken_file_is_in_trash (get_target_file (window)) ||
...
static gboolean
should_show_location_info (KrakenPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && (kraken_file_is_in_trash (get_target_file (window)) ||
		is_computer_directory (get_target_file (window)) ||
...
static gboolean
should_show_accessed_date (KrakenPropertiesWindow *window)
{
...
static gboolean
should_show_modified_date (KrakenPropertiesWindow *window)
{
    return !is_multi_file_window (window);
}

static gboolean
should_show_link_target (KrakenPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && kraken_file_is_symbolic_link (get_target_file (window))) {
		return TRUE;
	}
...
static gboolean
location_show_original (KrakenPropertiesWindow *window)
{
   KrakenFile *file;

   /* there is no way a recent item will be mixed with
      other items so just pick the first file to check */
   file = KRAKEN_FILE (g_list_nth_data (window->details->original_files, 0));
   return (file != NULL && !kraken_file_is_in_recent (file));
}

static gboolean
should_show_free_space (KrakenPropertiesWindow *window)
{
	...
	if (!is_multi_file_window (window)
		...
		is_computer_directory (get_target_file (window)) ||
		is_network_directory (get_target_file (window)) ||
        kraken_file_is_in_recent (get_target_file (window)) ||
		is_burn_directory (get_target_file (window)))) {
		return FALSE;
	}
...
static gboolean
should_show_volume_usage (KrakenPropertiesWindow *window)
{
	KrakenFile 		*file;
...
static GtkWidget*
create_pie_widget (KrakenPropertiesWindow *window)
{
	KrakenFile		*file;
	GtkGrid                 *grid;
...
static GtkWidget*
create_volume_usage_widget (KrakenPropertiesWindow *window)
{
	GtkWidget *piewidget;
	gchar *uri;
	KrakenFile *file;
...
static void
create_basic_page (KrakenPropertiesWindow *window)
{
	GtkGrid *grid;
	...
	hbox = create_page_with_hbox (window->details->stack, "basic", _("Basic"),
				      "help:gnome-help/kraken-file-properties-basic");
	...
	if (KRAKEN_IS_ENTRY (window->details->name_field)) {
		kraken_entry_select_all (KRAKEN_ENTRY (window->details->name_field));
		gtk_widget_grab_focus (GTK_WIDGET (window->details->name_field));
	}

	if (kraken_desktop_item_properties_should_show (window->details->target_files)) {
		...
		gtk_size_group_add_widget (label_size_group,
					   GTK_WIDGET (window->details->name_label));
		box = kraken_desktop_item_properties_make_box (label_size_group,
								 window->details->target_files);
	...
	if (is_multi_file_window (window) ||
	    kraken_file_is_directory (get_target_file (window))) {
...
static gboolean
files_has_directory (KrakenPropertiesWindow *window)
{
	GList *l;
...
static gboolean
files_has_changable_permissions_directory (KrakenPropertiesWindow *window)
{
	GList *l;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (kraken_file_is_directory (file) &&
		    ...
		    kraken_file_can_set_permissions (file)) {
			return TRUE;
		}
...
static gboolean
files_has_file (KrakenPropertiesWindow *window)
{
	GList *l;
...
static void
start_long_operation (KrakenPropertiesWindow *window)
{
...
static void
end_long_operation (KrakenPropertiesWindow *window)
{
...
static void
size_field_update (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
	goffset size;
	 ...
	 */
	if (is_multi_file_window (window) || kraken_file_is_directory (file)) {
		return;
	}

	size = kraken_file_get_size (file);
...
static void
permission_change_callback (KrakenFile *file,
			    GFile *res_loc,
			    GError *error,
			    gpointer callback_data)
{
	...
	/* Report the error if it's an error. */
	kraken_report_error_setting_permissions (file, error, NULL);

	g_object_unref (window);
}

static void
update_permissions (KrakenPropertiesWindow *window,
		    ...
		    gboolean use_original)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (!apply_to_both_folder_and_dir &&
		    ((kraken_file_is_directory (file) && !is_folder) ||
		     (!kraken_file_is_directory (file) && is_folder))) {
			continue;
		}

		permissions = kraken_file_get_permissions (file);
		if (use_original) {
			gpointer ptr;
...
static gboolean
initial_permission_state_consistent (KrakenPropertiesWindow *window,
				     guint32 mask,
				     gboolean is_folder,
...
static void
permission_button_update (KrakenPropertiesWindow *window,
			  GtkToggleButton *button)
{
	GList *l;
	gboolean all_set;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		file_permissions = kraken_file_get_permissions (file);
...
static void
set_up_permissions_checkbox (KrakenPropertiesWindow *window,
...
static GtkWidget *
add_permissions_checkbox_with_label (KrakenPropertiesWindow *window,
				     GtkGrid *grid,
				     GtkWidget *sibling,
				     const char *label,
...
static GtkWidget *
add_permissions_checkbox (KrakenPropertiesWindow *window,
			  GtkGrid *grid,
			  GtkWidget *sibling,
			  CheckboxType type,
			  guint32 permission_to_check,
...
static void
permission_combo_changed (GtkWidget *combo, KrakenPropertiesWindow *window)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
permission_combo_update (KrakenPropertiesWindow *window,
			 GtkComboBox *combo)
{
	PermissionType type;
...
static void
add_permissions_combo_box (KrakenPropertiesWindow *window, GtkGrid *grid,
...
static GtkWidget *
append_special_execution_checkbox (KrakenPropertiesWindow *window,
				   GtkGrid *grid,
				   GtkWidget *sibling,
...
static void
append_special_execution_flags (KrakenPropertiesWindow *window, GtkGrid *grid)
{
	GtkWidget *title;
...
static void
create_simple_permissions (KrakenPropertiesWindow *window, GtkGrid *page_grid)
{
	...
	if (!is_multi_file_window (window) && kraken_file_can_set_owner (get_target_file (window))) {
	...
	if (!is_multi_file_window (window) && kraken_file_can_set_group (get_target_file (window))) {
...
static void
create_permission_checkboxes (KrakenPropertiesWindow *window,
			      GtkGrid *page_grid,
			      gboolean is_folder)
{
...
static void
create_advanced_permissions (KrakenPropertiesWindow *window, GtkGrid *page_grid)
{
	GtkLabel *group_label;
	GtkLabel *owner_label;
	GtkComboBox *group_combo_box;
			 ...
			 KrakenPropertiesWindow *window)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (kraken_file_is_directory (file) &&
			...
			start_long_operation (window);
			g_object_ref (window);
			kraken_file_set_permissions_recursive (uri,
								 file_permission,
...
static void
create_permissions_page (KrakenPropertiesWindow *window)
{
	...
	vbox = create_page_with_vbox (window->details->stack, "permissions",
				      _("Permissions"),
				      "help:gnome-help/kraken-file-properties-permissions");
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_SHOW_ADVANCED_PERMISSIONS)) {
		...
		if (!is_multi_file_window (window)) {
			file_name = kraken_file_get_display_name (get_target_file (window));
...
static void
append_extension_pages (KrakenPropertiesWindow *window)
{
	GList *providers;
	GList *p;

 	providers = kraken_module_get_extensions_for_type (KRAKEN_TYPE_PROPERTY_PAGE_PROVIDER);

	for (p = providers; p != NULL; p = p->next) {
		KrakenPropertyPageProvider *provider;
		GList *pages;
		GList *l;

		provider = KRAKEN_PROPERTY_PAGE_PROVIDER (p->data);

		pages = kraken_property_page_provider_get_pages
		...
		for (l = pages; l != NULL; l = l->next) {
			...
			page = KRAKEN_PROPERTY_PAGE (l->data);
...
static gboolean
should_show_permissions (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
...
get_pending_key (GList *file_list)
{
	...
	uris = NULL;
	for (l = file_list; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, kraken_file_get_uri (KRAKEN_FILE (l->data)));
	}
		  ...
		  const char *startup_id)
{
	...
	data->original_files = kraken_file_list_copy (original_files);
	data->target_files = kraken_file_list_copy (target_files);
...
{
	kraken_file_list_free (data->original_files);
	kraken_file_list_free (data->target_files);
...
static void
file_changed_callback (KrakenFile *file, gpointer user_data)
{
	KrakenPropertiesWindow *window = KRAKEN_PROPERTIES_WINDOW (user_data);
...
static gboolean
is_a_special_file (KrakenFile *file)
{
	if (file == NULL ||
...
static gboolean
should_show_open_with (KrakenPropertiesWindow *window)
{
	KrakenFile *file;
...
static void
create_open_with_page (KrakenPropertiesWindow *window)
{
	GtkWidget *vbox;
	char *mime_type;
	char *uri = NULL;
	...
	mime_type = kraken_file_get_mime_type (get_target_file (window));

	if (!is_multi_file_window (window)) {
		uri = kraken_file_get_uri (get_target_file (window));
		if (uri == NULL) {
			return;
		}
	} else {
	...
	vbox = kraken_mime_application_chooser_new (uri, uris, mime_type, NULL);
    g_free (uri);
...
static KrakenPropertiesWindow *
create_properties_window (StartupData *startup_data)
{
	KrakenPropertiesWindow *window;
	GList *l;

	window = KRAKEN_PROPERTIES_WINDOW (gtk_widget_new (KRAKEN_TYPE_PROPERTIES_WINDOW, NULL));

	window->details->original_files = kraken_file_list_copy (startup_data->original_files);

	window->details->target_files = kraken_file_list_copy (startup_data->target_files);

	gtk_window_set_wmclass (GTK_WINDOW (window), "file_properties", "Kraken");
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		attributes = 0;
		if (kraken_file_is_directory (file)) {
			attributes |= KRAKEN_FILE_ATTRIBUTE_DEEP_COUNTS;
		}

		attributes |= KRAKEN_FILE_ATTRIBUTE_INFO;
		kraken_file_monitor_add (file, &window->details->target_files, attributes);
	}

	for (l = window->details->target_files; l != NULL; l = l->next) {
		g_signal_connect_object (KRAKEN_FILE (l->data),
					 "changed",
					 G_CALLBACK (file_changed_callback),
					 G_OBJECT (window),
					 0);
...
get_target_file_list (GList *original_files)
{
	...
	for (l = original_files; l != NULL; l = l->next) {
		...
		target = get_target_file_for_original_file (KRAKEN_FILE (l->data));
...
static void
add_window (KrakenPropertiesWindow *window)
{
	if (!is_multi_file_window (window)) {
...
static void
remove_window (KrakenPropertiesWindow *window)
{
	gpointer key;
...
static void
is_directory_ready_callback (KrakenFile *file,
			     gpointer data)
{
	...
	if (g_hash_table_size (startup_data->pending_files) == 0) {
		KrakenPropertiesWindow *new_window;
...
void
kraken_properties_window_present (GList       *original_files,
				    GtkWidget   *parent_widget,
				    const gchar *startup_id)
{
	...
	kraken_file_list_free (target_files);
	g_free(pending_key);
...
{
	GError *error = NULL;
	KrakenPropertiesWindow *window = KRAKEN_PROPERTIES_WINDOW (dialog);
	GtkWidget *curpage;
	const char *helpuri;
...
real_destroy (GtkWidget *object)
{
	...
	window = KRAKEN_PROPERTIES_WINDOW (object);

	remove_window (window);

	for (l = window->details->original_files; l != NULL; l = l->next) {
		kraken_file_monitor_remove (KRAKEN_FILE (l->data), &window->details->original_files);
	}
	kraken_file_list_free (window->details->original_files);
	window->details->original_files = NULL;

	for (l = window->details->target_files; l != NULL; l = l->next) {
		kraken_file_monitor_remove (KRAKEN_FILE (l->data), &window->details->target_files);
	}
	kraken_file_list_free (window->details->target_files);
	...
	GTK_WIDGET_CLASS (kraken_properties_window_parent_class)->destroy (object);
}
...
real_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (kraken_properties_window_parent_class)->finalize (object);
}
...
static void
set_icon (const char* icon_string, KrakenPropertiesWindow *properties_window)
{
    KrakenFile *file;
    char *file_uri;
    GList *l;
	...
	g_assert (KRAKEN_IS_PROPERTIES_WINDOW (properties_window));
    ...
    if (g_path_is_absolute (icon_string)) {
		...
		for (l = properties_window->details->original_files; l != NULL; l = l->next) {
			...
			if (kraken_file_is_mime_type (file, "application/x-desktop")) {
				if (kraken_link_local_set_icon (file_uri, icon_path)) {
                ...
                kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_CUSTOM_ICON_NAME, NULL, NULL);
				kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_CUSTOM_ICON, NULL, real_icon_uri);
				kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_ICON_SCALE, NULL, NULL);
        ...
        for (l = properties_window->details->original_files; l != NULL; l = l->next) {
            ...
            if (kraken_file_is_mime_type (file, "application/x-desktop")) {
                if (kraken_link_local_set_icon (file_uri, icon_string)) {
                    kraken_file_invalidate_attributes (file,
                                         KRAKEN_FILE_ATTRIBUTE_INFO |
                                         KRAKEN_FILE_ATTRIBUTE_LINK_INFO);
                }
            } else {
                kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_CUSTOM_ICON, NULL, NULL);
                kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_CUSTOM_ICON_NAME, NULL, icon_string);
                kraken_file_set_metadata (file, KRAKEN_METADATA_KEY_ICON_SCALE, NULL, NULL);
            }
			      ...
			      KrakenPropertiesWindow *window)
{
	...
	if (dialog == NULL) {
        ...
        for (l = window->details->original_files; l != NULL; l = l->next) {
            ...
            image_uri = kraken_file_get_metadata (file, KRAKEN_METADATA_KEY_CUSTOM_ICON, NULL);
            icon_name = kraken_file_get_metadata (file, KRAKEN_METADATA_KEY_CUSTOM_ICON_NAME, NULL);
...
static void
kraken_properties_window_class_init (KrakenPropertiesWindowClass *class)
{
	...
	g_type_class_add_private (class, sizeof (KrakenPropertiesWindowDetails));
}

static void
kraken_properties_window_init (KrakenPropertiesWindow *window)
{
	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, KRAKEN_TYPE_PROPERTIES_WINDOW,
						       KrakenPropertiesWindowDetails);
}
