...
#include <libkraken-private/kraken-action-symbols.h>
#include "kraken-global-preferences.h"

G_DEFINE_TYPE (KrakenActionConfigWidget, kraken_action_config_widget, KRAKEN_TYPE_CONFIG_BASE_WIDGET);


typedef struct {
    KrakenActionConfigWidget *widget;
...
on_check_toggled(GtkWidget *button, ActionProxy *proxy)
{
    ...
    gchar **blacklist = g_settings_get_strv (kraken_plugin_preferences, KRAKEN_PLUGIN_PREFERENCES_DISABLED_ACTIONS);
    ...
    g_settings_set_strv (kraken_plugin_preferences,
    		             KRAKEN_PLUGIN_PREFERENCES_DISABLED_ACTIONS,
...
static void
populate_from_directory (KrakenActionConfigWidget *widget, const gchar *path)
{
    GDir *dir;
...
static void
refresh_widget (KrakenActionConfigWidget *widget)
{
    gchar **data_dirs;
    gchar *path;
    guint i;
    ...
    kraken_config_base_widget_set_default_buttons_sensitive (KRAKEN_CONFIG_BASE_WIDGET (widget), widget->actions != NULL);
}

static void
on_settings_changed (GSettings *settings, gchar *key, gpointer user_data)
{
    KrakenActionConfigWidget *w = KRAKEN_ACTION_CONFIG_WIDGET (user_data);

    refresh_widget (w);
}

static void
on_enable_clicked (GtkWidget *button, KrakenActionConfigWidget *widget)
{
    g_settings_set_strv (kraken_plugin_preferences,
    		             KRAKEN_PLUGIN_PREFERENCES_DISABLED_ACTIONS,
						 NULL);
...
static void
on_disable_clicked (GtkWidget *button, KrakenActionConfigWidget *widget)
{
    GPtrArray *new_list = g_ptr_array_new ();
...
static void
on_open_folder_clicked (GtkWidget *button, KrakenActionConfigWidget *widget)
{
    gchar *path = NULL;
    path = g_build_filename (g_get_user_data_dir (), "kraken", "actions", NULL);
...
static void
on_layout_editor_clicked (GtkWidget *button, KrakenActionConfigWidget *widget)
{
    g_spawn_command_line_async ("kraken-action-layout-editor", NULL);
}
...
on_dir_changed (GFileMonitor     *monitor,
                ...
                gpointer          user_data)
{
    KrakenActionConfigWidget *widget = KRAKEN_ACTION_CONFIG_WIDGET (user_data);

    refresh_widget (widget);
}

static void
try_monitor_path (KrakenActionConfigWidget *widget, const gchar *path)
{
...
static void setup_dir_monitors (KrakenActionConfigWidget *widget)
{
    ...
    guint i;
    for (i = 0; i < g_strv_length (data_dirs); i++) {
        gchar *path = kraken_action_manager_get_system_directory_path (data_dirs[i]);
        try_monitor_path (widget, path);
        g_free (path);
    }

    gchar *path = kraken_action_manager_get_user_directory_path ();
    try_monitor_path (widget, path);
    g_free (path);
}

static void
kraken_action_config_widget_finalize (GObject *object)
{
    KrakenActionConfigWidget *widget = KRAKEN_ACTION_CONFIG_WIDGET (object);
    ...
    G_OBJECT_CLASS (kraken_action_config_widget_parent_class)->finalize (object);
}

static void
kraken_action_config_widget_class_init (KrakenActionConfigWidgetClass *klass)
{
    GObjectClass *oclass;
    oclass = G_OBJECT_CLASS (klass);

    oclass->finalize = kraken_action_config_widget_finalize;
}

static void
kraken_action_config_widget_init (KrakenActionConfigWidget *self)
{
    self->actions = NULL;
...
GtkWidget *
kraken_action_config_widget_new (void)
{
  return g_object_new (KRAKEN_TYPE_ACTION_CONFIG_WIDGET, NULL);
}
