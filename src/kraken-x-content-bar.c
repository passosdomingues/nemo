...
#define KRAKEN_X_CONTENT_BAR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), KRAKEN_TYPE_X_CONTENT_BAR, KrakenXContentBarPrivate))

struct KrakenXContentBarPrivate
{
	GtkWidget *label;
	GtkWidget *button;
...
G_DEFINE_TYPE (KrakenXContentBar, kraken_x_content_bar, GTK_TYPE_INFO_BAR)
			 ...
			 gpointer user_data)
{
	...
	if (default_app != NULL) {
		kraken_launch_application_for_mount (default_app, bar->priv->mount,
...
static void
kraken_x_content_bar_set_x_content_type (KrakenXContentBar *bar, const char *x_content_type)
{					  
	char *message;
	char *description;
	GAppInfo *default_app;
...
static void
kraken_x_content_bar_set_mount (KrakenXContentBar *bar, GMount *mount)
{
...
static void
kraken_x_content_bar_set_property (GObject      *object,
				     guint         prop_id,
				     const GValue *value,
				     GParamSpec   *pspec)
{
	...
	bar = KRAKEN_X_CONTENT_BAR (object);

	switch (prop_id) {
	case PROP_MOUNT:
		kraken_x_content_bar_set_mount (bar, G_MOUNT (g_value_get_object (value)));
		break;
	case PROP_X_CONTENT_TYPE:
		kraken_x_content_bar_set_x_content_type (bar, g_value_get_string (value));
		break;
	default:
...
static void
kraken_x_content_bar_get_property (GObject    *object,
				     guint       prop_id,
				     GValue     *value,
				     GParamSpec *pspec)
{
	KrakenXContentBar *bar = KRAKEN_X_CONTENT_BAR (object);
...
static void
kraken_x_content_bar_finalize (GObject *object)
{
	KrakenXContentBar *bar = KRAKEN_X_CONTENT_BAR (object);
	...
	if (bar->priv->mount != NULL)
        ...
        G_OBJECT_CLASS (kraken_x_content_bar_parent_class)->finalize (object);
}

static void
kraken_x_content_bar_class_init (KrakenXContentBarClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);
	object_class->get_property = kraken_x_content_bar_get_property;
	object_class->set_property = kraken_x_content_bar_set_property;
	object_class->finalize = kraken_x_content_bar_finalize;

	g_type_class_add_private (klass, sizeof (KrakenXContentBarPrivate));
...
static void
kraken_x_content_bar_init (KrakenXContentBar *bar)
{
	GtkWidget *content_area;

	bar->priv = KRAKEN_X_CONTENT_BAR_GET_PRIVATE (bar);
...
GtkWidget *
kraken_x_content_bar_new (GMount *mount, 
			    const char *x_content_type)
{
	return g_object_new (KRAKEN_TYPE_X_CONTENT_BAR, 
			     "mount", mount,
