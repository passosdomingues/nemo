/* kraken-template-config-widget.h */
...
G_DEFINE_TYPE (KrakenTemplateConfigWidget, kraken_template_config_widget, KRAKEN_TYPE_CONFIG_BASE_WIDGET);

typedef struct {
    KrakenTemplateConfigWidget *widget;
...
static void refresh_widget (KrakenTemplateConfigWidget *widget);
static void start_renaming_selected_row (KrakenTemplateConfigWidget *widget, GtkWidget *row);
...
static void
cancel_file_monitors (KrakenTemplateConfigWidget *widget)
{
    GList *l;
...
on_dir_changed (GFileMonitor     *monitor,
                ...
                gpointer          user_data)
{
    KrakenTemplateConfigWidget *widget = KRAKEN_TEMPLATE_CONFIG_WIDGET (user_data);

    refresh_widget (widget);
}

static void
monitor_path (KrakenTemplateConfigWidget *widget, const gchar *path)
{
...
static void
populate_from_directory (KrakenTemplateConfigWidget *widget, const gchar *path)
{
    GDir *dir;
...
static void
refresh_widget (KrakenTemplateConfigWidget *widget)
{
    ...
    if (widget->templates == NULL) {
        ...
        gtk_widget_set_sensitive (GTK_WIDGET (KRAKEN_CONFIG_BASE_WIDGET (widget)->listbox), TRUE);
    }

    kraken_config_base_widget_set_default_buttons_sensitive (KRAKEN_CONFIG_BASE_WIDGET (widget), widget->templates != NULL);
}
...
{
    KrakenTemplateConfigWidget *widget = KRAKEN_TEMPLATE_CONFIG_WIDGET (user_data);
    GtkListBoxRow *row = gtk_list_box_get_selected_row (GTK_LIST_BOX (KRAKEN_CONFIG_BASE_WIDGET (widget)->listbox));
...
static void
start_renaming_selected_row (KrakenTemplateConfigWidget *widget, GtkWidget *row)
{
    TemplateInfo *info = g_object_get_data (G_OBJECT (row), "template-info");
...
static void
import_template (KrakenTemplateConfigWidget *widget,
                 const gchar              *filename)
{
    ...
    template_dir = kraken_get_templates_directory ();
...
{
    KrakenTemplateConfigWidget *widget = KRAKEN_TEMPLATE_CONFIG_WIDGET (user_data);
    GtkWidget *row = GTK_WIDGET (gtk_list_box_get_selected_row (GTK_LIST_BOX (KRAKEN_CONFIG_BASE_WIDGET (widget)->listbox)));
...
static void
on_open_folder_clicked (GtkWidget *button, KrakenTemplateConfigWidget *widget)
{
    gchar *path = NULL;
    kraken_ensure_valid_templates_directory ();
    path = kraken_get_templates_directory ();
    ...
    kraken_application_open_location (kraken_application_get_singleton (),
                                    location,
                                    NULL,
                                    "kraken",
                                    FALSE);
...
on_drag_data_received(GtkWidget *widget,
                      ...
                      gpointer user_data)
{
    KrakenTemplateConfigWidget *self = KRAKEN_TEMPLATE_CONFIG_WIDGET (user_data);
...
static void
kraken_template_config_widget_finalize (GObject *object)
{
    KrakenTemplateConfigWidget *widget = KRAKEN_TEMPLATE_CONFIG_WIDGET (object);
    ...
    G_OBJECT_CLASS (kraken_template_config_widget_parent_class)->finalize (object);
}

static void
kraken_template_config_widget_class_init (KrakenTemplateConfigWidgetClass *klass)
{
    GObjectClass *oclass;
    oclass = G_OBJECT_CLASS (klass);

    oclass->finalize = kraken_template_config_widget_finalize;
}

static void
kraken_template_config_widget_init (KrakenTemplateConfigWidget *self)
{
    self->templates = NULL;

    GtkWidget *label = kraken_config_base_widget_get_label (KRAKEN_CONFIG_BASE_WIDGET (self));
    gtk_list_box_set_selection_mode (GTK_LIST_BOX (KRAKEN_CONFIG_BASE_WIDGET (self)->listbox), GTK_SELECTION_SINGLE);
    gtk_list_box_set_activate_on_single_click (GTK_LIST_BOX (KRAKEN_CONFIG_BASE_WIDGET (self)->listbox), FALSE);
    ...
    bb = KRAKEN_CONFIG_BASE_WIDGET (self)->lbuttonbox;
    ...
    gtk_widget_hide (kraken_config_base_widget_get_enable_button (KRAKEN_CONFIG_BASE_WIDGET (self)));
    gtk_widget_hide (kraken_config_base_widget_get_disable_button (KRAKEN_CONFIG_BASE_WIDGET (self)));

    g_signal_connect (KRAKEN_CONFIG_BASE_WIDGET (self)->listbox, "row-selected", G_CALLBACK (on_row_selected), self);
    g_signal_connect (KRAKEN_CONFIG_BASE_WIDGET (self)->listbox, "row-activated", G_CALLBACK (on_row_activated), self);

    gtk_drag_dest_set(KRAKEN_CONFIG_BASE_WIDGET (self)->listbox,
                      GTK_DEST_DEFAULT_ALL,
                      drop_targets,
                      G_N_ELEMENTS(drop_targets),
                      GDK_ACTION_COPY);

    g_signal_connect(KRAKEN_CONFIG_BASE_WIDGET (self)->listbox, "drag-data-received",
                    G_CALLBACK(on_drag_data_received), self);
    g_signal_connect(KRAKEN_CONFIG_BASE_WIDGET (self)->listbox, "drag-motion",
                    G_CALLBACK(on_drag_motion), NULL);

    refresh_widget (self);
    on_row_selected (KRAKEN_CONFIG_BASE_WIDGET (self)->listbox, NULL, self);
}

GtkWidget *
kraken_template_config_widget_new (void)
{
  return g_object_new (KRAKEN_TYPE_TEMPLATE_CONFIG_WIDGET, NULL);
}
