...
/* kraken-mime-actions.c - uri-specific versions of mime action functions
...
#define DEBUG_FLAG KRAKEN_DEBUG_MIME
...
typedef struct {
	...
	KrakenFileListHandle *files_handle;
...
{
	kraken_file_unref (location->file);
	g_free (location->uri);
	g_free (location);
}
...
launch_location_from_file (KrakenFile *file)
{
	LaunchLocation *location;
	location = g_new (LaunchLocation, 1);
	location->file = kraken_file_ref (file);
	location->uri = kraken_file_get_uri (file);
				 ...
				 const char *uri)
{
	kraken_file_unref (location->file);
	g_free (location->uri);
	location->file = kraken_file_get_by_uri (uri);
...
static GList*
filter_kraken_handler (GList *apps)
{
	GList *l, *next;
	GAppInfo *application;
	const char *id;
...
static gboolean
kraken_mime_actions_check_if_required_attributes_ready (KrakenFile *file)
{
	KrakenFileAttributes attributes;
	gboolean ready;

	attributes = kraken_mime_actions_get_required_file_attributes ();
	ready = kraken_file_check_if_ready (file, attributes);

	return ready;
}

KrakenFileAttributes
kraken_mime_actions_get_required_file_attributes (void)
{
...
static gboolean
file_has_local_path (KrakenFile *file)
{
	GFile *location;
	char *path;
	gboolean res;
...
GAppInfo *
kraken_mime_get_default_application_for_file (KrakenFile *file)
{
	GAppInfo *app = NULL;
	char *mime_type;
	char *uri_scheme;

	if (!kraken_mime_actions_check_if_required_attributes_ready (file)) {
        if (file_has_local_path (file)) {
            return NULL;
        }
	}

    uri_scheme = kraken_file_get_uri_scheme (file);
...
static int
file_compare_by_mime_type (KrakenFile *file_a,
			   KrakenFile *file_b)
{
	char *mime_type_a, *mime_type_b;
	int ret;

	mime_type_a = kraken_file_get_mime_type (file_a);
	mime_type_b = kraken_file_get_mime_type (file_b);
...
static int
file_compare_by_parent_uri (KrakenFile *file_a,
			    KrakenFile *file_b) {
	char *parent_uri_a, *parent_uri_b;
	int ret;

	parent_uri_a = kraken_file_get_parent_uri (file_a);
	parent_uri_b = kraken_file_get_parent_uri (file_b);
...
GList *
kraken_mime_get_applications_for_file (KrakenFile *file)
{
	char *mime_type;
	char *uri_scheme;
	GList *result;
	...
	return filter_kraken_handler (result);
}

GAppInfo *
kraken_mime_get_default_application_for_files (GList *files)
{
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_app = kraken_mime_get_default_application_for_file (file);
...
GList *
kraken_mime_get_applications_for_files (GList *files)
{
	GList *l, *sorted_files;
	KrakenFile *file;
	GList *one_ret, *ret;
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_ret = kraken_mime_get_applications_for_file (file);
...
static void
report_broken_symbolic_link (GtkWindow *parent_window, KrakenFile *file)
{
	char *target_path;
	char *display_name;
	char *prompt;
	...
	target_path = kraken_file_get_symbolic_link_target_path (file);
	if (target_path == NULL) {
...
get_executable_text_file_action (GtkWindow *parent_window, KrakenFile *file)
{
	GtkDialog *dialog;
	char *file_name;
	char *prompt;
	...
	g_assert (kraken_file_contains_text (file));

	preferences_value = g_settings_get_enum	(kraken_preferences,
						 KRAKEN_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION);
	switch (preferences_value) {
	case KRAKEN_EXECUTABLE_TEXT_LAUNCH:
		return ACTIVATION_ACTION_LAUNCH;
	case KRAKEN_EXECUTABLE_TEXT_DISPLAY:
		return ACTIVATION_ACTION_OPEN_IN_APPLICATION;
	case KRAKEN_EXECUTABLE_TEXT_ASK:
		break;
	default:
		/* Complain non-fatally, since preference data can't be trusted */
		g_warning ("Unknown value %d for KRAKEN_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION",
			   preferences_value);

	}


	file_name = kraken_file_get_display_name (file);
...
gboolean
kraken_mime_file_opens_in_view (KrakenFile *file)
{
...
static gboolean
kraken_mime_file_is_image (KrakenFile *file) {
	return kraken_file_is_mime_type(file, "image/*");
}

static ActivationAction
get_activation_action (KrakenFile *file, gboolean is_desktop)
{
	...
	if (activation_uri == NULL) {
		activation_uri = kraken_file_get_uri (file);
	}
	...
	if (kraken_file_is_launchable (file)) {
		...
		if (!executable_path) {
			action = ACTIVATION_ACTION_DO_NOTHING;
		} else if (kraken_file_contains_text (file)) {
	...
	if (action == ACTIVATION_ACTION_DO_NOTHING) {
		if (kraken_mime_file_opens_in_view (file) && !is_desktop) {
...
gboolean
kraken_mime_file_opens_in_external_app (KrakenFile *file)
{
 ...
 *
 * Construct a list of ApplicationLaunchParameters from a list of KrakenFiles,
			    ...
			    GList **unhandled_uris)
{
	...
	*unhandled_uris = NULL;
	...
	for (l = uris; l != NULL; l = l->next) {
		...
		app = kraken_mime_get_default_application_for_file (file);
		if (app != NULL) {
			app_uris = NULL;
...
static gboolean
file_was_cancelled (KrakenFile *file)
{
	GError *error;
...
static gboolean
file_was_not_mounted (KrakenFile *file)
{
	GError *error;
...
activation_parameters_free (ActivateParameters *parameters)
{
	...
	kraken_file_list_free (parameters->mountables);
	kraken_file_list_free (parameters->start_mountables);
	kraken_file_list_free (parameters->not_mounted);
...
cancel_activate_callback (gpointer callback_data)
{
	...
	if (parameters->files_handle) {
		kraken_file_list_cancel_call_when_ready (parameters->files_handle);
...
activate_parameters_special_free (ActivateParametersSpecial *parameters_special)
{
    ...
    kraken_file_unref (parameters_special->file);
    g_free (parameters_special);
}

static void
make_exec_callback (KrakenFile *file,
                       GFile *res_loc,
                      GError *error,
                     gpointer callback_data)
{
    ...
    if (error == NULL) {
        gchar *path = kraken_file_get_path (params->file);
        kraken_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (params->parent_window)),
                                              path, FALSE, NULL);
        g_free (path);
    }
...
run_open_with_dialog (ActivateParametersSpecial *params)
{
    ...
    mime_type = kraken_file_get_mime_type (params->file);
    uri = kraken_file_get_uri (params->file);
                                 ...
                                 ActivateParametersSpecial *parameters)
{
    ...
    case RESPONSE_RUN:
        ;
        guint32 existing = kraken_file_get_permissions (parameters->file);
        kraken_file_set_permissions (parameters->file,
                                   existing | (UNIX_PERM_USER_EXEC|UNIX_PERM_GROUP_EXEC|UNIX_PERM_OTHER_EXEC),
                                   (KrakenFileOperationCallback) make_exec_callback,
                                   parameters);
        break;
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
    ...
    KrakenFile *file = kraken_file_get_existing_by_uri (uri);
    ...
    parameters_special->file = kraken_file_ref (file);

    enable_exec_button = g_settings_get_boolean (kraken_preferences, KRAKEN_PREFERENCES_SHOW_MIME_MAKE_EXECUTABLE);
    ...
    param_uri = kraken_file_get_uri (parameters_special->file);
    if (param_uri != NULL) {
        gtk_dialog_add_button (GTK_DIALOG (dialog),
                   _("Choose a program"), RESPONSE_OPEN_WITH);
    }

    if (!kraken_file_can_set_permissions (file) && enable_exec_button) {
				      ...
				      ActivateParametersSpecial *parameters)
{
	...
	case RESPONSE_RUN:
		...
		uri = kraken_file_get_uri (parameters->file);
		DEBUG ("Launching untrusted launcher %s", uri);
		kraken_launch_desktop_file (screen, uri, NULL,
					      parameters->parent_window);
		g_free (uri);
		break;
	case RESPONSE_MARK_TRUSTED:
		file = kraken_file_get_location (parameters->file);
		kraken_file_mark_desktop_file_trusted (file,
							 parameters->parent_window,
							 TRUE,
							 NULL, NULL);
		g_object_unref (file);
		       ...
		       KrakenFile *file)
{
	...
	if (!kraken_file_is_trusted_link (file)) {
							 ...
							 gpointer callback_data)
{
	...
	return KRAKEN_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->compare_files (view, file1, file2);
}
...
add_sorted_view_uris (GList *uris,
					  ...
					  ActivateParameters *parameters)
{
	KrakenDirectory *current_directory;
	KrakenFile *file;
	...
	current_directory = kraken_view_get_model (parameters->slot->content_view);
	files = kraken_directory_get_file_list (current_directory);
	files = g_list_sort_with_data (files, compare_files_by_view_order, parameters->slot->content_view);

	for (l = files; l != NULL; l = l->next) {
		file = l->data;

		if (kraken_mime_file_is_image (file)) {
			mime_type = kraken_file_get_mime_type (file);
...
// allows an external sort order to be defined, adds all supported images in current kraken view to @uris.
static void
launch_application (GAppInfo *application,
				GList *uris,
				ActivateParameters *parameters)
{
	KrakenFile *single_file;
	...
	if (uris != NULL && uris->next == NULL && accepts_uri_list) {
		sortable_image_viewers = g_settings_get_strv (kraken_preferences,
													  KRAKEN_PREFERENCES_IMAGE_VIEWERS_WITH_EXTERNAL_SORT);
		...
		if (sortable_image_viewers != NULL &&
			...
			single_file = kraken_file_get_by_uri (uris->data);
			if (kraken_mime_file_is_image (single_file)) {
				add_sorted_view_uris (uris, application, parameters);
			}
			kraken_file_unref (single_file);
		}
		g_strfreev (sortable_image_viewers);
	}

	kraken_launch_application_by_uri (application,
					uris,
					parameters->parent_window);
}
...
activate_files (ActivateParameters *parameters)
{
    ...
    if (parameters->slot != NULL) {
        window = kraken_window_slot_get_window (parameters->slot);

        launch_location_is_desktop = KRAKEN_IS_DESKTOP_WINDOW (window);
    }
	...
	for (l = launch_files; l != NULL; l = l->next) {
		...
		kraken_launch_application_from_command (screen, quoted_path, FALSE, NULL);
		g_free (quoted_path);
		g_free (executable_path);
		g_free (uri);
	...
	for (l = launch_in_terminal_files; l != NULL; l = l->next) {
		...
		kraken_launch_application_from_command (screen, quoted_path, TRUE, NULL);
	...
	if (count > 1) {
		if ((parameters->flags & KRAKEN_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0 &&
	...
	if (parameters->slot != NULL &&
		...
		if ((flags & KRAKEN_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
		    g_settings_get_enum (kraken_preferences, KRAKEN_PREFERENCES_NEW_TAB_POSITION) ==
		...
		for (l = open_in_view_files; l != NULL; l = l->next) {
            ...
            if (g_file_is_native (initial_location) &&
                (kraken_file_is_in_admin (file) ||
                 !kraken_file_can_read (file) ||
            ...
            kraken_window_slot_open_location (parameters->slot, final_location, flags);
	...
	if (open_in_app_parameters != NULL ||
	    unhandled_open_in_app_uris != NULL) {
		if ((parameters->flags & KRAKEN_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0 &&
		    window != NULL) {
...
activation_mount_not_mounted (ActivateParameters *parameters)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		loc = l->data;
		next = l->next;
		kraken_file_invalidate_all_attributes (loc->file);
	}

	files = get_file_list_for_launch_locations (parameters->locations);
	kraken_file_list_call_when_ready
		(files,
		 kraken_mime_actions_get_required_file_attributes () | KRAKEN_FILE_ATTRIBUTE_LINK_INFO,
					 ...
					 gpointer callback_data)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		...
		if (kraken_file_is_broken_symbolic_link (file) && !kraken_file_is_in_favorites (file)) {
			launch_location_free (location);
			parameters->locations = g_list_delete_link (parameters->locations, l);
			pause_activation_timed_cancel (parameters);
		...
		if (kraken_file_get_file_type (file) == G_FILE_TYPE_MOUNTABLE &&
		    !kraken_file_has_activation_uri (file)) {
	...
	for (l = parameters->locations; l != NULL; l = l->next) {
		 ...
		 */
		uri = kraken_file_get_activation_uri (location->file);
		if (uri != NULL) {
...
static void
activation_mountable_mounted (KrakenFile  *file,
			      ...
			      gpointer       callback_data)
{
	...
	if (error == NULL) {
		...
		target_file = kraken_file_get (result_location);
...
static void
activation_mountable_started (KrakenFile  *file,
			      GFile         *gfile_of_file,
			      GError        *error,
			      gpointer       callback_data)
{
 ...
 * @files: A GList of KrakenFiles to activate.
 *
 **/
void
kraken_mime_activate_files (GtkWindow *parent_window,
			      KrakenWindowSlot *slot,
			      GList *files,
			      ...
			      gboolean user_confirmation)
{
	...
	if (file_count == 1) {
		file_name = kraken_file_get_display_name (files->data);
 ...
 * @view: FMDirectoryView in question.
 * @file: A KrakenFile representing the file in this view to activate.
 * @use_new_window: Should this item be opened in a new window?
 *
 **/

void
kraken_mime_activate_file (GtkWindow *parent_window,
			     KrakenWindowSlot *slot,
			     KrakenFile *file,
			     const char *launch_directory,
			     KrakenWindowOpenFlags flags)
{
	...
	kraken_mime_activate_files (parent_window, slot, files, launch_directory, flags, FALSE);
	g_list_free (files);
}

void
kraken_mime_launch_fm_and_select_file (GFile *file)
{
    GAppInfo *info;
    GList *list;
