...
/* kraken-bookmarks-window.c - implementation of bookmark-editing window.
 */
...
static KrakenBookmarkList *bookmarks = NULL;
static GtkTreeView	    *bookmark_list_widget = NULL; /* awkward name to distinguish from KrakenBookmarkList */
...
static void     name_or_uri_field_activate                  (KrakenEntry        *entry);
static void     kraken_bookmarks_window_restore_geometry  (GtkWidget            *window);
static void     on_bookmark_list_changed                    (KrakenBookmarkList *list,
							     gpointer              user_data);
...
/* We store a pointer to the bookmark in a column so when an item is moved
   ...
   kraken_bookmark_list_insert_item() makes a copy of the bookmark, so we
...
static void
kraken_bookmarks_window_response_callback (GtkDialog *dialog,
					     int response_id,
...
bookmarks_set_empty (gboolean empty)
{
	...
	if (empty) {
		...
		if (kraken_bookmark_list_length (bookmarks) > 0 &&
 ...
 * @list: The KrakenBookmarkList that this window will edit.
 *
 * Return value: A pointer to the new window.
 **/
GtkWindow *
create_bookmarks_window (KrakenBookmarkList *list, GObject *undo_manager_source)
{
	GtkWidget         *window;
	...
	if (!gtk_builder_add_from_resource (builder,
					    "/org/kraken/kraken-bookmarks-window.glade",
					    NULL)) {
		return NULL;
	}
	...
	kraken_undo_share_undo_manager (G_OBJECT (window), undo_manager_source);

	gtk_window_set_wmclass (GTK_WINDOW (window), "bookmarks", "Kraken");
	kraken_bookmarks_window_restore_geometry (window);
	...
	g_signal_connect (window, "response",
			  G_CALLBACK (kraken_bookmarks_window_response_callback), NULL);
...
static void
kraken_bookmarks_window_restore_geometry (GtkWidget *window)
{
	...
	window_geometry = kraken_bookmark_list_get_window_geometry (bookmarks);
...
/**
 * kraken_bookmarks_window_save_geometry:
 * 
 * Save window size & position to disk.
 * @window: The bookmarks window whose geometry should be saved.
 **/
void
kraken_bookmarks_window_save_geometry (GtkWindow *window)
{
	g_return_if_fail (GTK_IS_WINDOW (window));
	g_return_if_fail (KRAKEN_IS_BOOKMARK_LIST (bookmarks));
	...
	if (gtk_widget_get_visible (GTK_WIDGET (window))) {
		...
		kraken_bookmark_list_set_window_geometry (bookmarks, geometry_string);
		g_free (geometry_string);
	}
}

static void
on_bookmark_list_changed (KrakenBookmarkList *bmarks, gpointer data)
{
	g_return_if_fail (KRAKEN_IS_BOOKMARK_LIST (bmarks));
...
open_selected_bookmark (gpointer user_data, GdkScreen *screen)
{
	...
	location = kraken_bookmark_get_location (selected);
	if (location == NULL) { 
		return;
	}
                        ...
                        gpointer   user_data)
{
    ...
    kraken_bookmark_list_sort_ascending (bookmarks);
}
...
bookmarks_delete_bookmark (void)
{
    ...
    for (i = 0; i < kraken_bookmark_list_length (bookmarks); i++) {
        KrakenBookmark *bookmark_in_list = kraken_bookmark_list_item_at (bookmarks, i);

        if (bookmark_in_list == bookmark) {
            kraken_bookmark_list_delete_item_at (bookmarks, i);
            break;
        }
    }
    ...
    if (gtk_tree_model_get_iter_first (GTK_TREE_MODEL (store), &row_iter)) {
        do {
            ...
            if (bookmark != NULL) {
                ...
                for (i = 0; i < kraken_bookmark_list_length (bookmarks); i++) {
                    KrakenBookmark *old_bm = kraken_bookmark_list_item_at (bookmarks, i);

                    if (old_bm == bookmark) {
                        kraken_bookmark_list_move_item (bookmarks, i, new_index++);
                    }
                }
            } else {
                g_settings_set_int (kraken_window_state, KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT, row);
            }
		      ...
		      gpointer user_data)
{
	...
	if (selected) {
		name = kraken_bookmark_get_name (selected);
		location = kraken_bookmark_get_location (selected);
	...
	kraken_entry_set_text (KRAKEN_ENTRY (name_field),
				 name ? name : "");
	...
	kraken_entry_set_text (KRAKEN_ENTRY (uri_field),
...
{
	if (text_changed) {
		KrakenBookmark *bookmark, *bookmark_in_list;
		const char *name;
		gchar *icon_name;
		guint selected_row;
		GtkTreeIter iter;
		...
		bookmark = kraken_bookmark_new (location,
        ...
        if (selected_row > g_settings_get_int (kraken_window_state, KRAKEN_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT)) {
            selected_row--;
        }
		...
		g_signal_handler_block (bookmarks, 
					bookmark_list_changed_signal_id);
		kraken_bookmark_list_delete_item_at (bookmarks, selected_row);
		kraken_bookmark_list_insert_item (bookmarks, bookmark, selected_row);
		g_signal_handler_unblock (bookmarks, 
					  bookmark_list_changed_signal_id);
		...
		bookmark_in_list = kraken_bookmark_list_item_at (bookmarks,
								   selected_row);

		name = kraken_bookmark_get_name (bookmark_in_list);
		icon_name = kraken_bookmark_get_icon_name (bookmark_in_list);
...
on_text_field_focus_out_event (GtkWidget *widget,
			       ...
			       gpointer user_data)
{
	g_assert (KRAKEN_IS_ENTRY (widget));
...
static void
name_or_uri_field_activate (KrakenEntry *entry)
{
	g_assert (KRAKEN_IS_ENTRY (entry));

	update_bookmark_from_text ();
	kraken_entry_select_all_at_idle (entry);
}
...
restore_geometry (gpointer data)
{
	...
	kraken_bookmarks_window_restore_geometry (GTK_WIDGET (data));
...
on_window_hide_event (GtkWidget *widget,
		      gpointer user_data)
{
	kraken_bookmarks_window_save_geometry (GTK_WINDOW (widget));

	/* Disable undo for entry widgets */
	kraken_undo_unregister (G_OBJECT (name_field));
	kraken_undo_unregister (G_OBJECT (uri_field));
...
repopulate_now (void)
{
    ...
    bookmarks_length = kraken_bookmark_list_length (bookmarks);
    ...
    while (index < bookmarks_length) {
        ...
        if (index == breakpoint && !breakpoint_added) {
            ...
            bookmark = kraken_bookmark_list_item_at (bookmarks, index);
            bookmark_name = kraken_bookmark_get_name (bookmark);
            bookmark_icon = kraken_bookmark_get_icon_name (bookmark);
