...
#include "kraken-blank-desktop-window.h"
#include "kraken-desktop-window.h"
#include "kraken-application.h"
#include "kraken-cinnamon-dbus.h"
...
static gboolean layout_changed (KrakenDesktopManager *manager);
...
typedef struct {
    KrakenCinnamon *proxy;
    KrakenActionManager *action_manager;
    KrakenDesktopOverlay *overlay;
...
} KrakenDesktopManagerPrivate;

struct _KrakenDesktopManager
{
    GtkWindow parent_object;

    KrakenDesktopManagerPrivate *priv;
};

G_DEFINE_TYPE_WITH_PRIVATE (KrakenDesktopManager, kraken_desktop_manager, G_TYPE_OBJECT);

#define FETCH_PRIV(m) KrakenDesktopManagerPrivate *priv = KRAKEN_DESKTOP_MANAGER (m)->priv;
...
static RunState
get_run_state (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gint ret;
    GError *error;
    ...
    if (!kraken_cinnamon_call_get_run_state_sync (priv->proxy,
                                                &ret,
                                                NULL,
                                                &error)) {
...
static gint
get_n_monitors (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gsize n_monitors, i;
    ...
    if (priv->current_run_state == RUN_STATE_FALLBACK) {
        ...
        n_monitors = kraken_desktop_utils_get_num_monitors ();
    ...
    if (!kraken_cinnamon_call_get_monitors_sync (priv->proxy,
                                               &monitors,
                                               NULL,
                                               &error)) {
...
static void
get_window_rect_for_monitor (KrakenDesktopManager *manager,
                             gint                monitor,
                             GdkRectangle       *rect)
{
    FETCH_PRIV (manager);
    ...
    if (priv->current_run_state == RUN_STATE_FALLBACK) {
        ...
        kraken_desktop_utils_get_monitor_geometry (monitor, &out_rect);

        goto out;
    }

    if (!kraken_cinnamon_call_get_monitor_work_rect_sync (priv->proxy,
                                                        monitor,
                                                        &out_rect_var,
                                                        NULL,
                                                        &error)) {
    ...
    scale_factor = kraken_desktop_utils_get_scale_factor ();
...
static void
close_all_windows (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
static void
queue_update_layout (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
static void
global_scale_changed (KrakenDesktopManager *manager)
{
    g_return_if_fail (KRAKEN_IS_DESKTOP_MANAGER (manager));
...
static void
create_new_desktop_window (KrakenDesktopManager *manager,
                                         gint  monitor,
                                     gboolean  primary,
                                     gboolean  show_desktop)
{
    ...
    if (show_desktop) {
        window = GTK_WIDGET (kraken_desktop_window_new (monitor));
    } else {
        window = GTK_WIDGET (kraken_blank_desktop_window_new (monitor));
    }
    ...
    gtk_application_add_window (GTK_APPLICATION (kraken_application_get_singleton ()),
                                GTK_WINDOW (window));
...
static gboolean
update_overlay_in_idle (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);

    if (manager->priv->overlay) {
        kraken_desktop_overlay_update_in_place (priv->overlay);
    }
...
static gboolean
layout_changed (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gint n_monitors = 0;
    gint x_primary = 0;
    ...
    gchar *pref = g_settings_get_string (kraken_desktop_preferences, KRAKEN_PREFERENCES_DESKTOP_LAYOUT);

    if (g_strcmp0 (pref, "") == 0) {
        g_settings_set_string (kraken_desktop_preferences, KRAKEN_PREFERENCES_DESKTOP_LAYOUT, DESKTOPS_DEFAULT);
        g_free (pref);
    ...
    for (i = 0; i < n_monitors; i++) {
        ...
        } else if (!kraken_desktop_utils_get_monitor_cloned (i, x_primary)) {
...
static void
on_bus_name_owner_changed (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gchar *name_owner;
...
static void
on_run_state_changed (KrakenDesktopManager *manager)
{
    g_return_if_fail (KRAKEN_IS_DESKTOP_MANAGER (manager));
    ...
    if (new_state == RUN_STATE_RUNNING || new_state == RUN_STATE_FALLBACK) {
        priv->current_run_state = new_state;
        layout_changed (manager);
        g_application_release (G_APPLICATION (kraken_application_get_singleton ()));
    }
}

static void
on_monitors_changed (KrakenDesktopManager *manager)
{
    g_return_if_fail (KRAKEN_IS_DESKTOP_MANAGER (manager));
    ...
    for (l = priv->desktops; l != NULL; l = l->next) {
        ...
        if (KRAKEN_IS_DESKTOP_WINDOW (info->window)) {
            kraken_desktop_window_update_geometry (KRAKEN_DESKTOP_WINDOW (info->window));
        }
        else
        if (KRAKEN_IS_BLANK_DESKTOP_WINDOW (info->window)) {
            kraken_blank_desktop_window_update_geometry (KRAKEN_BLANK_DESKTOP_WINDOW (info->window));
        }
    }
}

static void
on_overlay_adjustments_changed (KrakenDesktopOverlay      *overlay,
                                KrakenWindow              *window,
                                gint                     h_percent,
                                gint                     v_percent,
                                KrakenDesktopManager      *manager)
{
    g_return_if_fail (KRAKEN_IS_DESKTOP_WINDOW (window));

    kraken_desktop_window_set_grid_adjusts (KRAKEN_DESKTOP_WINDOW (window),
                                          h_percent,
                                          v_percent);
}
                 ...
                 gpointer   *user_data)
{
    if (g_strcmp0 (signal_name, "RunStateChanged") == 0) {
        on_run_state_changed (KRAKEN_DESKTOP_MANAGER (user_data));
    } 
    else
    if (g_strcmp0 (signal_name, "MonitorsChanged") == 0) {
        on_monitors_changed (KRAKEN_DESKTOP_MANAGER (user_data));
    }
}

static gboolean
on_failsafe_timeout (KrakenDesktopManager *manager)
{
    g_return_val_if_fail (KRAKEN_IS_DESKTOP_MANAGER (manager), FALSE);
    ...
    g_warning ("kraken-desktop: Desktop failsafe timeout reached, applying fallback behavior");
...
static void
connect_fallback_signals (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
on_proxy_created (GObject      *source,
                  ...
                  gpointer      user_data)
{
    KrakenDesktopManager *manager = KRAKEN_DESKTOP_MANAGER (user_data);
    FETCH_PRIV (manager);

    priv->proxy = kraken_cinnamon_proxy_new_for_bus_finish (res, NULL);
...
static gboolean
fallback_startup_idle_cb (KrakenDesktopManager *manager)
{
    on_run_state_changed (manager);
...
static void
kraken_desktop_manager_dispose (GObject *object)
{
    KrakenDesktopManager *manager = KRAKEN_DESKTOP_MANAGER (object);
    FETCH_PRIV (manager);

    DEBUG ("Disposing KrakenDesktopManager");

    close_all_windows (manager);

    g_clear_object (&priv->overlay);

    g_signal_handlers_disconnect_by_func (kraken_desktop_preferences, queue_update_layout, manager);
    g_signal_handlers_disconnect_by_func (kraken_preferences, queue_update_layout, manager);
    ...
    G_OBJECT_CLASS (kraken_desktop_manager_parent_class)->dispose (object);
}

static void
kraken_desktop_manager_finalize (GObject *object)
{
    FETCH_PRIV (object);
    ...
    DEBUG ("Finalizing KrakenDesktopManager");

    G_OBJECT_CLASS (kraken_desktop_manager_parent_class)->finalize (object);
}

static void
kraken_desktop_manager_class_init (KrakenDesktopManagerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = kraken_desktop_manager_finalize;
  object_class->dispose = kraken_desktop_manager_dispose;
}

static void
kraken_desktop_manager_init (KrakenDesktopManager *manager)
{
    KrakenDesktopManagerPrivate *priv;

    manager->priv = G_TYPE_INSTANCE_GET_PRIVATE (manager, KRAKEN_TYPE_DESKTOP_MANAGER, KrakenDesktopManagerPrivate);
    ...
    g_signal_connect_swapped (kraken_desktop_preferences, 
                              "changed::" KRAKEN_PREFERENCES_SHOW_DESKTOP,
                              G_CALLBACK (queue_update_layout),
                              manager);

    g_signal_connect_swapped (kraken_desktop_preferences,
                              "changed::" KRAKEN_PREFERENCES_DESKTOP_LAYOUT,
                              G_CALLBACK (queue_update_layout),
                              manager);

    g_signal_connect_swapped (kraken_desktop_preferences,
                              "changed::" KRAKEN_PREFERENCES_USE_DESKTOP_GRID,
                              G_CALLBACK (queue_update_layout),
                              manager);
    ...
    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_DESKTOP_IS_HOME_DIR,
                              G_CALLBACK (queue_update_layout),
                              manager);

    g_signal_connect_swapped (kraken_preferences,
                              "changed::" KRAKEN_PREFERENCES_SHOW_ORPHANED_DESKTOP_ICONS,
                              G_CALLBACK (queue_update_layout),
                              manager);
    ...
    g_application_hold (G_APPLICATION (kraken_application_get_singleton ()));

    if (is_cinnamon_desktop ()) {
         g_message ("kraken-desktop: session is cinnamon, establishing proxy");

        kraken_cinnamon_proxy_new_for_bus (G_BUS_TYPE_SESSION,
        ...
        g_message ("kraken-desktop: session is not cinnamon (checked XDG_SESSION_DESKTOP,"
...
static KrakenDesktopManager *_manager = NULL;

KrakenDesktopManager*
kraken_desktop_manager_get (void)
{
    if (_manager == NULL) {
        _manager = g_object_new (KRAKEN_TYPE_DESKTOP_MANAGER, NULL);
    }
...
gboolean
kraken_desktop_manager_has_desktop_windows (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
gboolean
kraken_desktop_manager_get_monitor_is_active (KrakenDesktopManager *manager,
                                                          gint  monitor)
{
    FETCH_PRIV (manager);
    GList *iter;
...
gboolean
kraken_desktop_manager_get_monitor_is_primary (KrakenDesktopManager *manager,
                                                           gint  monitor)
{
    FETCH_PRIV (manager);
    GList *iter;
...
gboolean
kraken_desktop_manager_get_primary_only (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
KrakenActionManager *
kraken_desktop_manager_get_action_manager (void)
{
    g_return_val_if_fail (_manager != NULL, NULL);
    FETCH_PRIV (_manager);

    if (priv->action_manager == NULL) {
        priv->action_manager = kraken_action_manager_new ();
    }
...
void
kraken_desktop_manager_get_window_rect_for_monitor (KrakenDesktopManager *manager,
                                                  gint                monitor,
                                                  GdkRectangle       *rect)
{
...
void
kraken_desktop_manager_get_margins (KrakenDesktopManager *manager,
                                  gint                monitor,
                                  gint               *left,
                                  gint               *right,
                                  gint               *top,
                                  gint               *bottom)
{
    ...
    if (monitor != kraken_desktop_utils_get_primary_monitor ()) {
        *left = *right = *top = *bottom = 25;

        return;
    }

    kraken_desktop_utils_get_monitor_geometry (monitor, &geometry);
    kraken_desktop_utils_get_monitor_work_rect (monitor, &work_rect);
...
GtkWindow *
kraken_desktop_manager_get_window_for_monitor (KrakenDesktopManager *manager,
                                             gint                monitor)
{
    GtkWindow *window;
    GList *iter;
...
void
kraken_desktop_manager_get_overlay_info (KrakenDesktopManager *manager,
                                       gint                monitor,
                                       ...
                                       gint               *v_adjust)
{
    ...
    window = kraken_desktop_manager_get_window_for_monitor (manager, monitor);

    if (KRAKEN_IS_DESKTOP_WINDOW (window) &&
        kraken_desktop_window_get_grid_adjusts (KRAKEN_DESKTOP_WINDOW (window),
                                              h_adjust,
                                              v_adjust)) {

        *action_group = kraken_desktop_window_get_action_group (KRAKEN_DESKTOP_WINDOW (window));
    } else {
        *action_group = NULL;
    }
}
...
void
kraken_desktop_manager_show_desktop_overlay (KrakenDesktopManager *manager,
                                           gint                monitor)
{
    FETCH_PRIV (manager);

    if (priv->overlay == NULL) {
        priv->overlay = kraken_desktop_overlay_new ();
    ...
    kraken_desktop_overlay_show (priv->overlay, monitor);
}

gboolean
kraken_desktop_manager_get_is_cinnamon         (KrakenDesktopManager *manager)
{
    FETCH_PRIV (manager);
