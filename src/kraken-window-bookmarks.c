...
static void refresh_bookmarks_menu (KrakenWindow *window);
...
remove_bookmarks_for_uri_if_yes (GtkDialog *dialog, int response, gpointer callback_data)
{
	...
	if (response == GTK_RESPONSE_YES) {
		...
		kraken_bookmark_list_delete_items_with_uri (window->details->bookmark_list, uri);
	}
...
static void
show_bogus_bookmark_window (KrakenWindow *window,
...
static GtkWindow *
get_or_create_bookmarks_window (KrakenWindow *window)
{
 ...
 **/
void
kraken_bookmarks_exiting (void)
{
	if (bookmarks_window != NULL) {
		kraken_bookmarks_window_save_geometry (bookmarks_window);
 ...
 */
void
kraken_window_add_bookmark_for_current_location (KrakenWindow *window)
{
	KrakenBookmark *bookmark;
	KrakenWindowSlot *slot;
	KrakenBookmarkList *list;

	slot = kraken_window_get_active_slot (window);
	bookmark = slot->current_location_bookmark;
	list = window->details->bookmark_list;

	if (!kraken_bookmark_list_contains (list, bookmark)) {
		kraken_bookmark_list_append (list, bookmark); 
	}
}

void
kraken_window_edit_bookmarks (KrakenWindow *window)
{
	GtkWindow *dialog;
...
static void
remove_bookmarks_menu_items (KrakenWindow *window)
{
	GtkUIManager *ui_manager;
	
	ui_manager = kraken_window_get_ui_manager (window);
...
bookmark_holder_new (KrakenBookmark *bookmark, 
		     KrakenWindow *window,
		     GCallback refresh_callback,
		     KrakenBookmarkFailedCallback failed_callback)
{
...
activate_bookmark_in_menu_item (GtkAction *action, gpointer user_data)
{
    ...
    location = kraken_bookmark_get_location (holder->bookmark);
    slot = kraken_window_get_active_slot (holder->window);
    kraken_window_slot_open_location (slot, location, kraken_event_get_window_open_flags ());
    g_object_unref (location);
}

void
kraken_menus_append_bookmark_to_menu (KrakenWindow *window, 
					KrakenBookmark *bookmark, 
...
static void
update_bookmarks (KrakenWindow *window)
{
	...
	if (window->details->bookmark_list == NULL) {
		window->details->bookmark_list = kraken_bookmark_list_get_default ();
	}

	bookmarks = window->details->bookmark_list;

	ui_manager = kraken_window_get_ui_manager (KRAKEN_WINDOW (window));
	...
	bookmark_count = kraken_bookmark_list_length (bookmarks);

	for (index = 0; index < bookmark_count; ++index) {
		bookmark = kraken_bookmark_list_item_at (bookmarks, index);

		kraken_menus_append_bookmark_to_menu
			(KRAKEN_WINDOW (window),
			 bookmark,
...
static void
refresh_bookmarks_menu (KrakenWindow *window)
{
	g_assert (KRAKEN_IS_WINDOW (window));
